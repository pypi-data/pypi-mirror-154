...
def addPathPlugin(pluginPath:str) -> None:
    try:
        ...
        logging.warning("User plugin \"%s\" can not be imported", pluginPath)
...
def logAllPlugins() -> None:
    for plugin in plugin_manager.get_plugins():
        logging.info("Using Plugin: \"%s\" with canonical name: \"%s\"", str(plugin_manager.get_name(plugin)), str(plugin_manager.get_canocical_name(plugin)))
