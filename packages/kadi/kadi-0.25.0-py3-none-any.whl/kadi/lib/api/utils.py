...
def get_access_token_scopes():
    ...
    """
    explicit_scopes = current_app.config["API_SCOPES"]
    api_scopes = {}

    for object_name in set(
        list(db.metadata.tables.keys()) + list(explicit_scopes.keys())
    ):
        actions = []
        ...
        for action in explicit_scopes.get(object_name, []):
            if action not in actions:
                actions.append(action)

        if actions:
            api_scopes[object_name] = actions
