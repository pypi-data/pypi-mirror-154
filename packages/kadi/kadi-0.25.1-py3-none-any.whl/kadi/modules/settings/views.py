...
def edit_profile(qparams):
    ...
    if request.method == "POST":
        if qparams["action"] == "confirm_email":
            ...
            danger(_("Error updating profile."))
...
def change_password():
    ...
    if request.method == "POST":
        ...
        danger(_("Error changing password."))
...
def manage_tokens():
    ...
    if request.method == "POST":
        if form.validate():
            ...
            db.session.commit()
            success(_("Access token created successfully."))
            ...
            danger(_("Error creating access token."))
...
def manage_services(qparams):
    ...
    if request.method == "POST":
        ...
        success(_("Service disconnected successfully."))
...
def oauth2_authorize(provider):
    ...
    try:
        ...
        danger(_("Error connecting service."))
        return redirect(url_for("settings.manage_services"))
    ...
    success(_("Service connected successfully."))
    return redirect(url_for("settings.manage_services"))
