...
class UserState(StringEnum):
    ...
    __values__ = [const.MODEL_STATE_ACTIVE, "inactive", "deleted"]
...
class User(SimpleReprMixin, SimpleTimestampMixin, UserMixin, db.Model):
    ...
    legals_accepted = db.Column(db.Boolean, default=False, nullable=False)
    """Flag indicating whether a user accepted the legal notices, if configured."""
    ...
    def needs_legals_acceptance(self):
        ...
        This is the case if accepting the legal notices is enforced and the user did not
        accept them yet.
        """
        return not self.legals_accepted and legals_acceptance_required()
