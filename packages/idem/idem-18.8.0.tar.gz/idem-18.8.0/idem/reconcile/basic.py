...
async def loop(
    ...
    pending_tags = []
    if hub.reconcile.basic.has_passed(pending_plugin, last_run, pending_tags):
    ...
    while count_wo_change < MAX_RERUNS_WO_CHANGE:
        ...
        if hub.reconcile.basic.has_passed(pending_plugin, current_run, pending_tags):
...
def has_passed(hub, pending_plugin, runs, pending_tags):
    # invoke pending plugin and keep only pending state in runs
    # save passed runs in passed_map
    for tag in runs:
...
def update_changes(hub, last_run, tag_to_old_state_map):
    ...
    for tag in last_run:
        ...
        if orig_old_state and orig_old_state != last_old_state:
            hub.log.debug(
        ...
        last_run[tag]["changes"] = differ.deep_diff(
            last_run[tag].get("old_state", dict()),
