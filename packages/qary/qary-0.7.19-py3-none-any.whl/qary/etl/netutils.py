...
from qary.config import DATA_DIR, HOME_DIR, BASE_DIR
from qary.services import SPACES_ACCESS_KEY, SPACES_SECRET_KEY
from qary.etl.fileutils import LARGE_FILES
from qary.re_patterns import cre_url
...
constants
    ...
    if not SPACES_ACCESS_KEY or not SPACES_SECRET_KEY:
        log.error(f"SPACES_ACCESS_KEY: {SPACES_ACCESS_KEY} or SPACES_SECRET_KEY: {SPACES_SECRET_KEY} not set.")
        return
    try:
        client = session.client('s3',
                                region_name='SF02',
