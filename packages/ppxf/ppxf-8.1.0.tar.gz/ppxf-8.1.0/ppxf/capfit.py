...
class lsq_lin:
    ...
    NOTE: The initialization assumes the matrix defined by the
        free inequality constraints, and any additional equality ones,
        is full rank, but lsq_lin will stop if this is not the case.
        If that happens one should use lsq_lin_cvxopt instead.
...
class capfit:
    ...
    def __init__(self, func, p0, A_eq=None, A_ineq=None, abs_step=None,
                 ...
                 diff_step=1e-4, fixed=None, ftol=1e-4, linear_method='cvxopt',
        ...
        self.bounds_only = all(v is None for v in [A_ineq, A_eq])
        if not self.bounds_only:
            ...
            if A_ineq is not None:
                ...
                assert np.all(A_ineq @ p0 <= b_ineq), \
    ...
    def optimize(self, call, p1, A_eq, b_eq, A_ineq, b_ineq, bounds):
        ...
        while True:
            ...
            if ratio > 0.01:  # Successful step: move on
                J2 = self.fdjac(call, p2, f2, bounds)
                p1, f1, J1 = p2, f2, J2
