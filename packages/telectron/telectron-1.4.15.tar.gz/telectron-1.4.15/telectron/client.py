...
class Client(Methods, Scaffold):
    ...
    Parameters:
        ...
        s3_config (``dict``, *optional*):
            Config to connect to the S3 repository if you want to upload files to it
    """
    ...
    def __init__(
        ...
        self.s3 = None
        self.s3_bucket = s3_config.pop('bucket') if s3_config else None
    ...
    async def handle_download(self, packet):
        ...
        try:
            ...
            if self.s3:
                file_name = utils.randstr(length=6) + file_name
            file_path = re.sub("\\\\", "/", os.path.join(directory, file_name))
    ...
    def load_config(self):
        ...
        if not self.api_id and self.api_hash and parser.has_section("telectron"):
            self.api_id = parser.getint("telectron", "api_id")
            self.api_hash = parser.get("telectron", "api_hash")
    ...
    async def handle_progress(self, offset: int, file_size: int, progress: callable, progress_args: tuple = ()):
    ...
    async def get_file(
        ...
        if self.s3:
            ...
            if file_size > part_size:
                upload_id = (await self.s3.create_multipart_upload(Bucket=self.s3_bucket, Key=file_path, ACL='public-read'))['UploadId']
                parts = []
                i = 1
                async for chunk in self.iter_file(file_id):
                    buffer += chunk
                    ...
                    if progress:
                        offset += chunk_size
                        await self.handle_progress(offset, file_size, progress, progress_args)
                parts.append(
                    {"PartNumber": i,
                     "ETag": (await self.s3.upload_part(
                         Bucket=self.s3_bucket,
            ...
            if not os.path.isabs(file_path):
                file_path = self.PARENT_DIR / file_path
            file_name = ''
            try:
