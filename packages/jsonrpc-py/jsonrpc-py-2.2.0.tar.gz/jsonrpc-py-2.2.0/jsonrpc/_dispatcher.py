...
_D = TypeVar("_D", bound="BaseDispatcher")
...
class BaseDispatcher(MutableMapping[str, _AnyCallable]):
    ...
    def __hash__(self) -> int:
        return hash(tuple(objects := self.items())) ^ len(objects)
    ...
    def __or__(self: type[_D], obj: MutableMapping[str, _AnyCallable], /) -> _D:
        if not isinstance(obj, MutableMapping):
            return NotImplemented

        dispatcher: Final[_D] = self.__class__()
        dispatcher.update(self)
        dispatcher.update(obj)
        return dispatcher

    def __ror__(self: type[_D], obj: MutableMapping[str, _AnyCallable], /) -> _D:
        if not isinstance(obj, MutableMapping):
            return NotImplemented
    ...
    def __ior__(self: type[_D], obj: MutableMapping[str, _AnyCallable], /) -> _D:
        if not isinstance(obj, MutableMapping):
            return NotImplemented
