...
class CHPControlMultiEnergy(G2PControlMultiEnergy):
    ...
    def __init__(self, multinet, 
        ...
        heat_net = multinet[NETS_ACCESS][self.__name_heat_net]
    ...
    def get_gas_consume(self, multinet):
        try:
            gas_sink = multinet[NETS_ACCESS][self.name_net_gas].sink.at[self.elm_idx_gas,
                                                                'mdot_kg_per_s']
        except (ValueError, TypeError):
            gas_sink = multinet[NETS_ACCESS][self.name_net_gas].sink.loc[self.elm_idx_gas,
                                                                'mdot_kg_per_s'].values[:]
        return gas_sink

    def get_power_production(self, multinet):
        try:
            power_gen = multinet[NETS_ACCESS][self.name_net_power][self.elm_type_power].at[
                self.elm_idx_power, 'p_mw']
        except (ValueError, TypeError):
            power_gen = multinet[NETS_ACCESS][self.name_net_power][self.elm_type_power].loc[
                            self.elm_idx_power, 'p_mw'].values[:]
        return power_gen

    def get_heat_production(self, multinet):
        try:
            return multinet[NETS_ACCESS][self.__name_heat_net].heat_exchanger.at[self.__heat_exchanger_id, 'qext_w']
        except (ValueError, TypeError):
            return multinet[NETS_ACCESS][self.__name_heat_net].heat_exchanger.loc[self.__heat_exchanger_id, 'qext_w'].values[:]
    ...
    def time_step(self, multinet, time):
        ...
        if self.el_power_led:
            ...
            self.gas_cons = (self.power_gen / (self.conversion_factor_kgps_to_mw() * self.efficiency))

        else:
            self.gas_cons = self.ref_gas_consume * self.__load_regulation_multiplier
            self.power_gen = self.gas_cons * self.conversion_factor_kgps_to_mw() * self.efficiency
    ...
    def calc_heat(self, gas_flow):
        return (gas_flow * self.conversion_factor_kgps_to_mw() * (0.000028 * self.__ambient_temperature_in_k \
    ...
    def write_to_net(self, multinet):
        try:
            multinet[NETS_ACCESS][self.name_net_gas].sink.at[self.elm_idx_gas,
                                                        'mdot_kg_per_s'] = self.gas_cons
        except (ValueError, TypeError):
            multinet[NETS_ACCESS][self.name_net_gas].sink.loc[self.elm_idx_gas,
                                                            'mdot_kg_per_s'] = self.gas_cons
        try:
        ...
        try:
            multinet[NETS_ACCESS][self.__name_heat_net].heat_exchanger.at[self.__heat_exchanger_id, 'qext_w'] = -self.heat_energy
        except (ValueError, TypeError):
            multinet[NETS_ACCESS][self.__name_heat_net].heat_exchanger.loc[self.__heat_exchanger_id, 'qext_w'].values[:] = -self.heat_energy
    ...
    @property
    def heat_cap(self):
        return self.calc_heat(self.gas_cap)
...
def create_chp(mn, 
    ...
    chp_sink = ppipes.create_sink(gas_net, junction=junction_gas, mdot_kg_per_s=gas_consume)
    chp_heat_feed_in = ppipes.create_heat_exchanger(heat_net, from_junction=junction_heat_from, to_junction=junction_heat_to, 
                                                    diameter_m=heat_diameter_m, qext_w=-10000)
    return CHPControlMultiEnergy(mn, chp_gen, chp_sink, chp_heat_feed_in, efficiency, name, ambient_temperature=ambient_temperature,
                        use_storage=use_storage, storage_cap=storage_cap, storage_charge_eff=storage_charge_eff, 
...
def create_g2p(mn, 
    ...
    g2p_sink = ppipes.create_sink(gas_net, junction=junction_gas, mdot_kg_per_s=gas_consume)
...
class RegulatedP2GControlMultiEnergy(P2GControlMultiEnergy):
    ...
    def time_step(self, multinet, _):
        ...
        self.mdot_kg_per_s = self.power_load * self.conversion_factor_mw_to_kgps()[0] \
            * self.efficiency
        self.write_to_net(multinet)
    ...
    @property
    def gas_cap(self):
        return self.ref_power_load * self.conversion_factor_mw_to_kgps()[0] \
            * self.efficiency
...
def create_p2g(mn, 
    ...
    return RegulatedP2GControlMultiEnergy(mn, p2g_id_el, p2g_id_gas, efficiency = efficiency, name=name)
