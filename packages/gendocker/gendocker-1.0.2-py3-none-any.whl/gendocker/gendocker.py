...
def gendocker(mode, tag, expected_port, container_port, path):
    ...
    with open(f'./auto_generated/docker_publish.sh', "w+") as writer:
        writer.write('docker buildx create --name armbuilder\n')
        writer.write('docker buildx use armbuilder\n')
        writer.write(f'docker buildx build --platform linux/amd64 -t jaytrairat/{project_name}:{tag} . --push\n')
    ...
    with open(f'./auto_generated/docker-compose.yaml', "w+") as writer:
        ...
        writer.write(f'      - \"{expected_port}:{container_port}\"\n')
        writer.write(f'    deploy:\n')
        writer.write(f'      resources:\n')
        writer.write(f'        limits:\n')
        writer.write(f'          memory: "{"32" if mode == "client" else "512"}M"\n')
        writer.write(f'        reservations:\n')
        writer.write(f'          memory: "{"32" if mode == "client" else "512"}M"\n')

    with open(f'./auto_generated/reverse_proxy.txt', "w+") as writer:
        writer.write(f'location {path} {{\n')
        writer.write(f'    proxy_pass http://localhost:{expected_port}/;\n')
        writer.write(f'}}\n\n')
    ...
    with open('./auto_generated/web.config', 'w+') as writer:
        ...
        writer.write(f'          <match url="^{path[1:]}(.*)" />\n')
        writer.write(f'          <action type="Rewrite" url="http://127.0.0.1:{expected_port}/{{R:1}}" />\n')
        writer.write(f'        </rule>\n')
        writer.write(f'      </rules>\n')
        writer.write(f'    </rewrite>\n')
        writer.write(f'  </system.webServer>\n')
...
if __name__ == '__main__':
    received_params = sys.argv
    if len(received_params) == 6:
        [_, mode, tag, expected_port, container_port, path] = sys.argv
        gendocker(mode, tag, expected_port, container_port, path)
    elif len(received_params) == 2:
        [_, flag] = received_params
        if flag == '-h':
            print('gendocker "mode" "tag" "expected_port" "container_port" "path"')
    else:
        print(datetime.now(), ':: this function require 5 parameters.')
