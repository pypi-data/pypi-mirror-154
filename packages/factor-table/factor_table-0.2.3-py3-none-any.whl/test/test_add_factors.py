...
class MyTestCaseAddFactors(unittest.TestCase):
    def test_100(self):
        ...
        conn2 = BaseSingleFactorTableNode(conn)
        ft = FactorTable(cik_dt='datetime', cik_id='code', strict_cik=False)

        ft.add_factor('EDGAR_LOG.edgar_log', conn2, factor_names=['size'], cik_dt='date', cik_id='ip')
    ...
    def test_add_factor_from_db_table(self):
        from ClickSQL import BaseSingleFactorTableNode
        conn2 = BaseSingleFactorTableNode(conn)
        ft = FactorTable(cik_dt='datetime', cik_iid='code', strict_cik=False)
        ft.add_factor('EDGAR_LOG.edgar_log',conn2, factor_names=['size'], cik_dt='date', cik_iid='ip')
    ...
    def test_add_factor_from_sql(self):
        ...
        conn2 = BaseSingleFactorTableNode(conn)
        ft = FactorTable( cik_dt='datetime', cik_iid='code', strict_cik=False)
        ft.add_factor('select * from EDGAR_LOG.edgar_log',conn2, factor_names=['size'], cik_dt='date', cik_iid='ip')
