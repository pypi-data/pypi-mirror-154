# pylint: disable=invalid-name,redefined-builtin,line-too-long
# pylint: disable=missing-class-docstring,missing-function-docstring
"""Auto generated. Do not touch."""
from raf._lib import _APIS
# Defined in ./src/distributed/cuda/mpi_communicator.cc
mpi = _APIS.get("raf.distributed.communicator._make.mpi", None)
# Defined in ./src/distributed/cuda/nccl_communicator.cc
nccl = _APIS.get("raf.distributed.communicator._make.nccl", None)
# Defined in ./src/distributed/common/communicator.cc
void = _APIS.get("raf.distributed.communicator._make.void", None)
