...
suite('rb/views/CommentIssueBarView', function () {
  ...
  var $reopenButton;
  var $fixedButton;
  var $verifyFixedButton;
  var $verifyDroppedButton;
  beforeEach(function () {
  ...
  describe('Actions', function () {
    ...
    beforeEach(function () {
      ...
      spyOn(comment, 'ready').and.callFake(function (options) {
    ...
    it('Resolving as fixed', function () {
      ...
      expect(commentIssueManager.setCommentState).toHaveBeenCalledWith(1, 2, 'diff_comments', 'resolved');
    });
    it('Dropping', function () {
      $dropButton.click();
      expect(view._$buttons.prop('disabled')).toBe(true);
      expect(commentIssueManager.setCommentState).toHaveBeenCalledWith(1, 2, 'diff_comments', 'dropped');
    });
    it('Re-opening', function () {
      ...
      $reopenButton.click();
      expect(view._$buttons.prop('disabled')).toBe(true);
      expect(commentIssueManager.setCommentState).toHaveBeenCalledWith(1, 2, 'diff_comments', 'open');
    });
    it('Resolving with verification', function () {
      comment.get('extraData').require_verification = true;
      $fixedButton.click();
      expect(view._$buttons.prop('disabled')).toBe(true);
      expect(commentIssueManager.setCommentState).toHaveBeenCalledWith(1, 2, 'diff_comments', 'verifying-resolved');
    });
    it('Dropping with verification', function () {
      comment.get('extraData').require_verification = true;
      $dropButton.click();
      expect(view._$buttons.prop('disabled')).toBe(true);
      expect(commentIssueManager.setCommentState).toHaveBeenCalledWith(1, 2, 'diff_comments', 'verifying-dropped');
    });
  });
