...
RB.ChunkHighlighterView = Backbone.View.extend({
  ...
  render: function render() {
    ...
    this._$window.on('resize.' + this.cid, function () {
      var windowWidth = _this._$window.width();

      if (windowWidth !== _this._prevWindowWidth) {
        _this._prevWindowWidth = windowWidth;
   ...
   */
  highlight: function highlight($chunk) {
  ...
  updateLayout: function updateLayout() {
    ...
    var top = Math.floor(chunkEl.offsetTop + this._chunkContainerEl.offsetTop + 1);
    var height = chunkEl.clientHeight + 1;
    ...
    if (changed) {
       ...
       */
      var padding = this._$pageContainer.getExtents('p', 'l');
