...
suite('rb/newReviewRequest/views/BranchesView', function () {
  ...
  describe('Rendering', function () {
    it('With items', function () {
      view.render();
      expect(view.$el.html()).toBe('<option selected="selected">master</option>' + '<option>release-1.7.x</option>' + '<option>release-1.6.x</option>');
    });
  });
  describe('Selected event', function () {
    it('When clicked', function () {
      ...
      view.$el.change();
      expect(view.$el.html()).toBe('<option>master</option>' + '<option selected="selected">release-1.7.x</option>' + '<option>release-1.6.x</option>');
    });
  });
});

//# sourceMappingURL=branchesViewTests.js.map