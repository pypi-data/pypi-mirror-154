...
RB.DiffViewerPageView = RB.ReviewablePageView.extend({
  ...
  initialize: function initialize(options) {
    ...
    this.router.route(/^(\d+(?:-\d+)?)\/?(\?[^#]*)?/, 'revision', function (revision, queryStr) {
      ...
      var interdiffRevision = revisionRange.length === 2 ? parseInt(revisionRange[1], 10) : null;
      var baseCommitID = null;
      var tipCommitID = null;
    ...
    Backbone.history.start({
      pushState: true,
      hashChange: false,
      root: "".concat(this.model.get('reviewRequest').get('reviewURL'), "diff/"),
      silent: true
    });
  ...
  remove: function remove() {
    ...
    this._$window.off("resize.".concat(this.cid));
  ...
  render: function render() {
    ...
    if (!this.model.commits.isEmpty()) {
      var commitListModel = new RB.DiffCommitList({
      ...
      this.listenTo(this.model.revision, 'change:baseCommitID change:tipCommitID', function (model) {
        return commitListModel.set({
    ...
    this._$window.on("resize.".concat(this.cid), _.throttleLayout(this._onWindowResize.bind(this)));
    /*
  ...
  queueLoadDiff: function queueLoadDiff(diffReviewable) {
    ...
    $.funcQueue('diff_files').add(function () {
      ...
      if (!options.showDeleted && $("#file".concat(fileDiffID)).length === 1) {
        /*
  ...
  _renderFileDiff: function _renderFileDiff(diffReviewable) {
    ...
    this.listenTo(diffReviewableView, 'moveFlagClicked', function (line) {
      _this4.selectAnchor(_this4.$("a[target=".concat(line, "]")));
    });
    ...
    if (this._startAtAnchorName) {
      ...
      if ($anchor.length === 0 && urlSplit.length === 2 && elementName === urlSplit[0]) {
        ...
        diffReviewableView.$el.find("tr[line='".concat(urlSplit[1], "']")).addClass('highlight-anchor').append($anchor);
      }
    ...
    this.listenTo(diffReviewableView, 'showDeletedClicked', function () {
      _this4.queueLoadDiff(diffReviewable, {
        showDeleted: true
      });
  ...
  _navigate: function _navigate(options) {
    ...
    if (interdiffRevision) {
      baseURL += "-".concat(interdiffRevision);
    }
   ...
   */
  _onCommitIntervalChanged: function _onCommitIntervalChanged(model) {
    this._navigate({
      baseCommitID: model.get('baseCommitID'),
      tipCommitID: model.get('tipCommitID')
    });
