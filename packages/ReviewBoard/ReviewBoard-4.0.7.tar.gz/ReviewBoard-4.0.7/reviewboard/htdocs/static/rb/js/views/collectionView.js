...
RB.CollectionView = Backbone.View.extend({
   ...
   */
  _addCollectionViews: function _addCollectionViews() {
    var _this = this;

    this.views.forEach(function (view) {
      return _this.$container.append(view.render().el);
    });
  },
  ...
  _onRemoved: function _onRemoved(item) {
    var toRemove = this._viewsByModelID[item.cid];
  ...
  _onSorted: function _onSorted() {
    ...
    this.views = this.collection.map(function (model) {
      return _this2._viewsByModelID[model.cid];
    });
