...
RB.ReviewRequestPage.ReviewReplyEditor = Backbone.Model.extend({
  ...
  save: function save() {
    ...
    if (contextType === 'body_top') {
      ...
      if (!obj) {
        var ReplyClass = this.replyClasses[contextType];
    ...
    obj.ready({
      ready: function ready() {
        var text = _this.get('text');

        if (text) {
          obj.set(valueAttr, text);
          obj.set(richTextAttr, _this.get('richText'));
          obj.set({
            forceTextType: 'html',
   ...
   */
  resetStateIfEmpty: function resetStateIfEmpty() {
    ...
    var replyObject = this.get('replyObject');
   ...
   */
  _setupReviewReply: function _setupReviewReply() {
    var _this2 = this;

    var reviewReply = this.get('reviewReply');
    var oldReviewReply = this.previous('reviewReply');
    ...
    this.listenTo(reviewReply, 'destroyed', function () {
      _this2.trigger('discarded');

      _this2._resetState();
    });
    this.listenTo(reviewReply, 'published', function () {
      _this2.trigger('published');

      _this2._resetState(false);
    });
  },
   ...
   *         comments.
   */
  _resetState: function _resetState(shouldDiscardIfEmpty) {
    var _this3 = this;
    ...
    if (shouldDiscardIfEmpty === false) {
      ...
      this.get('reviewReply').discardIfEmpty({
        success: function success() {
          return _this3.trigger('resetState');
        }
      });
    }
  }
