...
RB.FormView = Backbone.View.extend({
   ...
   */
  setupFormWidgets: function setupFormWidgets($el) {
    if ($el === undefined) {
      $el = this.$el;
    }
    /*
    ...
    if (window.SelectFilter) {
      $el.find('.selectfilter').each(function (i, el) {
        var parts = el.name.split('-');
        SelectFilter.init(el.id, parts[parts.length - 1], false);
      });
      $el.find('.selectfilterstacked').each(function (i, el) {
        var parts = el.name.split('-');
   ...
   */
  setSubformVisibility: function setSubformVisibility(options) {
    console.assert(_.isObject(options), 'An options object must be provided.');
    var group = options.group;
    var subformID = options.subformID;
    var visible = options.visible;
    console.assert(group, 'Missing option "group"');
    var subformIDs = this._subformsByGroup[group];
    console.assert(subformIDs, "Invalid subform group ".concat(group));

    if (options.hideOthers || !subformID) {
      _.each(subformIDs, function ($subform, id) {
        var isHidden = subformID === undefined ? !visible : id !== subformID;
        $subform.prop({
          disabled: isHidden,
          hidden: isHidden
        });
      ...
      var $subform = subformIDs[subformID];
      console.assert($subform, "Invalid subform ID ".concat(subformID));
      $subform.prop({
        disabled: !visible,
        hidden: !visible
      });
   ...
   */
  _setupSubforms: function _setupSubforms() {
    var _this = this;

    var configuredControllers = {};

    this._$subforms.each(function (i, subformEl) {
      var $subform = $(subformEl);
      var controllerID = $subform.data('subform-controller');
      var subformID = $subform.data('subform-id');
      var group = $subform.data('subform-group');
      var $controller;
      ...
      if (!group && !controllerID) {
        console.error('Subform %o is missing either data-subform-group= ' + 'or data-subform-controller=', subformEl);
        return;
      }
      /*
      ...
      if (controllerID) {
        $controller = _this.$("#".concat(controllerID));
        console.assert($controller.length === 1, "Missing controller #".concat(controllerID));
        var controllerGroup = $controller.data('subform-group');
        /*
      ...
      if (!_this._subformsByGroup.hasOwnProperty(group)) {
        _this._subformsByGroup[group] = {};
      }

      _this._subformsByGroup[group][subformID] = $subform;
      /*
      ...
      if ($controller) {
        ...
        if (!configuredControllers[controllerID]) {
          ...
          $controller.on('change', function () {
            return _this.setSubformVisibility({
              group: group,
   ...
   */
  _onToggleFieldSetClicked: function _onToggleFieldSetClicked(e) {
    e.preventDefault();
    e.stopPropagation();
    var $toggle = $(e.target);
    var $fieldset = $toggle.closest('.rb-c-form-fieldset');
