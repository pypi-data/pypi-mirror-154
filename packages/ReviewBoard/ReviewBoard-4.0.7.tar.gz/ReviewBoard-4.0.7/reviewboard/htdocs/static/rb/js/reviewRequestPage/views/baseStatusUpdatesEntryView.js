...
(function () {
  var ParentView = RB.ReviewRequestPage.EntryView;
  /**
  ...
  RB.ReviewRequestPage.BaseStatusUpdatesEntryView = ParentView.extend({
     ...
     */
    beforeApplyUpdate: function beforeApplyUpdate() {
      /*
      ...
      var diffFragmentQueue = RB.PageManager.getPage().diffFragmentQueue;
      var diffCommentsData = this.model.get('diffCommentsData') || [];
    ...
    render: function render() {
      ...
      this._reviewViews = this.model.get('reviews').map(function (review) {
        var $reviewEl = _this.$("#review".concat(review.id));

        var view = new RB.ReviewRequestPage.ReviewView({
          el: $reviewEl,
          model: review,
          entryModel: _this.model,
     ...
     */
    _onRequestRunClicked: function _onRequestRunClicked(e) {
      var _this2 = this;

      var $target = $(e.target);
      var updateId = $target.data('statusUpdateId');
      var reviewRequestId = this.model.get('reviewRequestId');
      RB.apiCall({
        type: 'PUT',
        prefix: this.model.get('localSitePrefix') || '',
        path: "/review-requests/".concat(reviewRequestId, "/status-updates/").concat(updateId, "/"),
        buttons: $target,
        data: {
          state: 'request-run'
        },
        success: function success() {
           ...
           * pending state.
           */
          _this2.model.stopWatchingUpdates();

          _this2.model.watchUpdates(0);
        }
      });
    }
  });
