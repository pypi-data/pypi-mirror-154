...
(function () {
  ...
  var CommentsListView = Backbone.View.extend({
     ...
     */
    setComments: function setComments(comments, replyType) {
      var _this = this;
      ...
      var commentIssueManager = this.options.commentIssueManager;
      var interactive = this.options.issuesInteractive;
      var odd = true;
      var $items = $();

      _.each(comments, function (serializedComment) {
        var commentID = serializedComment.comment_id;
        var $item = $(_this.itemTemplate({
        ...
        if (serializedComment.issue_opened) {
          var commentIssueBar = new RB.CommentIssueBarView({
          ...
          _this.listenTo(commentIssueManager, 'issueStatusUpdated', function (comment) {
  ...
  RB.CommentDialogView = Backbone.View.extend({
    ...
    render: function render() {
      ...
      this.listenTo(this._textEditor, 'change', function () {
        return _this2.model.set('text', _this2._textEditor.getText());
      });
      ...
      this.listenTo(this.model, 'change:text', function () {
        return _this2._textEditor.setText(_this2.model.get('text'));
      });
      this.listenTo(this.model, 'change:richText', this._handleResize);
      this.$el.css('position', 'absolute').mousedown(function (evt) {
        /*
      ...
      this.listenTo(this.model, 'change:publishedComments', function () {
        return _this2._onPublishedCommentsChanged();
      });
      ...
      RB.CommentDialogHook.each(function (hook) {
        ...
        var hookView = new HookViewType({
          extension: hook.get('extension'),
          commentDialog: _this2,
          commentEditor: _this2.model,
          el: _this2.el
        });
        hookView.render();
      });
    ...
    open: function open() {
      ...
      if (this.options.animate) {
        this.$el.animate({
          top: "+=".concat(this.SLIDE_DISTANCE, "px"),
          opacity: 1
    ...
    close: function close() {
      var onClosed = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : undefined;
     ...
     */
    positionBeside: function positionBeside($el, options) {
    ...
    _handleResize: function _handleResize() {
      ...
      if (this._$commentsPane.is(':visible')) {
        ...
        var $commentsList = this.commentsList.$el;
      ...
      var $textField = this._textEditor.$el;
     ...
     */
    _onCancelClicked: function _onCancelClicked() {
      var shouldExit = true;
     ...
     *         The keydown event.
     */
    _onTextKeyDown: function _onTextKeyDown(e) {
      e.stopPropagation();
    ...
    create: function create() {
      ...
      var reviewRequestEditor = options.reviewRequestEditor || RB.PageManager.getPage().model.reviewRequestEditor;
      var dlg = new RB.CommentDialogView({
      ...
      dlg.on('closed', function () {
        return RB.CommentDialogView._instance = null;
      });
      var instance = RB.CommentDialogView._instance;

      var showCommentDlg = function showCommentDlg() {
        try {
          dlg.open();
        } catch (e) {
          dlg.close();
