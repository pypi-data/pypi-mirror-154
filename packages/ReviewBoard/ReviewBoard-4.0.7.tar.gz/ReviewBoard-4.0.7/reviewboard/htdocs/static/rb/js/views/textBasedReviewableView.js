...
RB.TextBasedReviewableView = RB.FileAttachmentReviewableView.extend({
  ...
  initialize: function initialize(options) {
    ...
    this.listenTo(this.router, 'route:viewMode', function (viewMode, lineNum) {
      /*
      ...
      if (viewMode) {
        _this.model.set('viewMode', viewMode);
      }

      if (lineNum) {
        _this._scrollToLine(lineNum);
      }
    });
  },
  ...
  renderContent: function renderContent() {
    ...
    var $fileHeader = this.$('.review-ui-header');
    ...
    var reviewURL = this.model.get('reviewRequest').get('reviewURL');
    var attachmentID = this.model.get('fileAttachmentID');
    Backbone.history.start({
      root: "".concat(reviewURL, "file/").concat(attachmentID, "/")
    });
  },
   ...
   */
  _scrollToLine: function _scrollToLine(lineNum) {
    ...
    var $row = $($table[0].tBodies[0].rows[lineNum]);
   ...
   */
  _getTableForViewMode: function _getTableForViewMode(viewMode) {
    if (viewMode === 'source') {
   ...
   *     The row selector.
   */
  _getRowSelectorForViewMode: function _getRowSelectorForViewMode(viewMode) {
    if (viewMode === 'source') {
   ...
   */
  _placeCommentBlockView: function _placeCommentBlockView(commentBlockView) {
    var commentBlock = commentBlockView.model;
    var beginLineNum = commentBlock.get('beginLineNum');
    var endLineNum = commentBlock.get('endLineNum');

    if (beginLineNum && endLineNum) {
      var viewMode = commentBlock.get('viewMode');

      var rowSelector = this._getRowSelectorForViewMode(viewMode);
      ...
      if (this.model.get('diffRevision')) {
         ...
         */
        var rows = rowSelector.el.tBodies[0].rows;
  ...
  _onViewChanged: function _onViewChanged() {
    ...
    this._$viewTabs.removeClass('active').filter("[data-view-mode=".concat(viewMode, "]")).addClass('active');
