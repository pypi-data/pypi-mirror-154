...
RB.ReviewRequestPage.ReviewReplyDraftBannerView = RB.FloatingBannerView.extend({
  ...
  render: function render() {
    ...
    this.listenTo(this.model, 'saving destroying', function () {
      return _this.$('input').prop('disabled', true);
    });
    this.listenTo(this.model, 'saved', function () {
      return _this.$('input').prop('disabled', false);
    });
    this.listenTo(this.model, 'publishError', function (errorText) {
