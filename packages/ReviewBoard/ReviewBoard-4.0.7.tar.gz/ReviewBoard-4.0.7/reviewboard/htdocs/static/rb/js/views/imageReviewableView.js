...
(function () {
   ...
   */
  var scalingFactors = new Map([[0.33, '33%'], [0.5, '50%'], [1.0, '100%'], [2.0, '200%']]);
  /**
  ...
  var BaseImageView = Backbone.View.extend({
    template: null,
    mode: null,
    name: null,
    ...
    initialize: function initialize() {
      ...
      this.listenTo(this.model, 'change:scale', function (model, scale) {
        return _this._onScaleChanged(scale);
      });
    },
    ...
    className: function className() {
      return "image-diff-".concat(this.mode);
    },
      ...
      $images.each(function (ix, image) {
        ...
        if ($image.data('initial-width') === undefined) {
          image.onload = function () {
            ...
            if (loadsRemaining === 0) {
              _this2.onImagesLoaded();

              _this2.trigger('regionChanged');
            }
          };
        } else {
          loadsRemaining--;
    ...
    onImagesLoaded: function onImagesLoaded() {
      ...
      if (scale === null) {
        var _this$getInitialSize = this.getInitialSize(),
            width = _this$getInitialSize.width;

        var maxWidth = this.$el.closest('.image-content').width();
        var scales = Array.from(scalingFactors.keys()).filter(function (f) {
          return f <= 1;
        });
     ...
     *         etc).
     */
    _onScaleChanged: function _onScaleChanged(scale) {
      this._$images.each(function (index, el) {
        var $image = $(el);
        $image.css({
     ...
     */
    getInitialSize: function getInitialSize() {
  ...
  var ImageAttachmentView = BaseImageView.extend({
    mode: 'attachment',
    tagName: 'img',
  ...
  var ImageDifferenceDiffView = BaseImageView.extend({
    mode: 'difference',
    ...
    onImagesLoaded: function onImagesLoaded() {
      ...
      var modifiedImage = this._modifiedImage;
      var scale = this.model.get('scale');
      ...
      var $modifiedCanvas = $('<canvas/>').attr({
        width: this._maxWidth,
        height: this._maxHeight
      });

      var origContext = this._$canvas[0].getContext('2d');

      origContext.drawImage(origImage, 0, 0);
      var modifiedContext = $modifiedCanvas[0].getContext('2d');
      modifiedContext.drawImage(modifiedImage, 0, 0);
      var origImageData = origContext.getImageData(0, 0, this._maxWidth, this._maxHeight);
      var origPixels = origImageData.data;
      var modifiedPixels = modifiedContext.getImageData(0, 0, this._maxWidth, this._maxHeight).data;

      for (var i = 0; i < origPixels.length; i += 4) {
  ...
  var ImageOnionDiffView = BaseImageView.extend({
    mode: 'onion',
    ...
    render: function render() {
      ...
      this.$('.image-slider').slider({
        ...
        slide: function slide(e, ui) {
          return _this3.setOpacity(ui.value / 100.0);
        }
      });
     ...
     */
    setOpacity: function setOpacity(percentage) {
    ...
    _resize: function _resize() {
      var scale = this.model.get('scale');
      var origW = this._$origImage.data('initial-width') * scale;
      var origH = this._$origImage.data('initial-height') * scale;
      var newW = this._$modifiedImage.data('initial-width') * scale;
      var newH = this._$modifiedImage.data('initial-height') * scale;
  ...
  var ImageSplitDiffView = BaseImageView.extend({
    mode: 'split',
    name: gettext("Split"),
    ...
    render: function render() {
      ...
      this._$slider = this.$('.image-slider').slider({
        ...
        slide: function slide(e, ui) {
          return _this4.setSplitPercentage(ui.value / 100.0);
        }
      });
      this.loadImages(this.$('img'));
      return this;
     ...
     */
    setSplitPercentage: function setSplitPercentage(percentage) {
    ...
    _resize: function _resize() {
      ...
      var newW = this._$modifiedImage.data('initial-width') * scale;
      var newH = this._$modifiedImage.data('initial-height') * scale;
      var maxH = Math.max(origH, newH);
      var maxOuterH = maxH + $origImageContainer.getExtents('b', 'tb');
  ...
  var ImageTwoUpDiffView = BaseImageView.extend({
    mode: 'two-up',
    name: gettext("Two-Up"),
  ...
  RB.ImageReviewableView = RB.FileAttachmentReviewableView.extend({
    ...
    initialize: function initialize() {
      ...
      this.on('commentBlockViewAdded', function (commentBlockView) {
        commentBlockView.setSelectionRegionSizeFunc(function () {
          return _.pick(_this5._imageView.getSelectionRegion(), 'width', 'height');
        });
        commentBlockView.setScale(_this5.model.get('scale'));

        _this5._$selectionArea.append(commentBlockView.$el);

        _this5._commentBlockViews.push(commentBlockView);

        _this5.listenTo(commentBlockView, 'removing', function () {
          _this5._commentBlockViews = _.without(_this5._commentBlockViews, commentBlockView);
        });
      });
      this.listenTo(this.model, 'change:scale', function (model, scale) {
        _this5._commentBlockViews.forEach(function (view) {
          return view.setScale(scale);
        });

        _this5.$('.image-resolution-menu-current').text(scalingFactors.get(scale));
        /*
    ...
    renderContent: function renderContent() {
      ...
      var hasDiff = !!this.model.get('diffAgainstFileAttachmentID');
       ...
       */
      .hover(function () {
        _this6._$selectionArea.show();

        _this6._adjustPos();
      }, function () {
        if (_this6._$selectionRect.is(':hidden') && !_this6.commentDlg) {
          _this6._$selectionArea.hide();
        }
      });
      var $wrapper = $('<div class="image-content" />').append(this._$selectionArea);
      ...
      if (this.model.get('numRevisions') > 1) {
        ...
        if (!this.renderedInline) {
          if (hasDiff) {
            var captionItems = [this.captionItemTemplate({
            ...
            var $captionBar = $('<div class="image-single-revision">').appendTo($header);
      ...
      var $resolutionMenu = $(['<li class="image-resolution-menu has-menu">', ' <a href="#" class="menu-header">', '  <span class="fa fa-search-plus"></span>', '  <span class="image-resolution-menu-current">100%</span>', '  <span class="rb-icon rb-icon-dropdown-arrow">', ' </a>', ' <ul class="menu" />', '</li>'].join(''));
      var $menu = $resolutionMenu.find('.menu');
      scalingFactors.forEach(function (text, scale) {
        $("<li class=\"menu-item\" data-image-scale=\"".concat(scale, "\" />")).text(text).appendTo($menu);
      });
     ...
     */
    _addDiffMode: function _addDiffMode(ViewClass) {
      ...
      view.render();
      var $selector = $(this.modeItemTemplate({
        mode: mode,
        name: view.name
      }));
      /*
     ...
     *         The new diff mode.
     */
    _setDiffMode: function _setDiffMode(mode) {
      var _this7 = this;
      ...
      if (this._imageView) {
        ...
        this._imageView.$el.fadeOut(this.ANIM_SPEED_MS, function () {
          return _this7._showDiffMode(newView, true);
        });
      } else {
     ...
     */
    _showDiffMode: function _showDiffMode(newView, animate) {
      if (this._imageView) {
     ...
     */
    _onImageModeClicked: function _onImageModeClicked(e) {
      e.preventDefault();
      e.stopPropagation();
     ...
     *         The event which triggered the callback.
     */
    _onImageZoomLevelClicked: function _onImageZoomLevelClicked(e) {
      e.preventDefault();
      e.stopPropagation();
    ...
    _onMouseUp: function _onMouseUp(e) {
      if (!this.commentDlg && this._$selectionRect.is(':visible')) {
        ...
        var position = this._$selectionRect.data();
    ...
    _onMouseMove: function _onMouseMove(e) {
      if (!this.commentDlg && this._$selectionRect.is(':visible')) {
        ...
        var x = e.pageX - Math.floor(offset.left) - 1;
        var y = e.pageY - Math.floor(offset.top) - 1;
        var updateData = {};
     ...
     */
    _adjustPos: function _adjustPos() {
