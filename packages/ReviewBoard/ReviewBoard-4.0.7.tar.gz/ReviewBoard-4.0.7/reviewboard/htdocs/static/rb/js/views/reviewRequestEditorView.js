...
(function () {
    ...
    render: function render() {
      ...
      if (this.DescriptionFieldViewType) {
        this.field = new this.DescriptionFieldViewType({
          el: this.$("#field_".concat(this.descriptionFieldID)),
      ...
      this.reviewRequestEditor.on('saving destroying', function () {
        return _this.$buttons.prop('disabled', true);
      });
      this.reviewRequestEditor.on('saved saveFailed destroyed', function () {
        return _this.$buttons.prop('disabled', false);
      });
      return this;
    }
  });
  ...
  var ClosedBannerView = BannerView.extend({
  ...
  var DiscardedBannerView = ClosedBannerView.extend({
    id: 'discard-banner',
  ...
  var SubmittedBannerView = ClosedBannerView.extend({
    id: 'submitted-banner',
  ...
  var DraftBannerView = BannerView.extend({
    id: 'draft-banner',
     ...
     *     False, always.
     */
    _onPublishDraftClicked: function _onPublishDraftClicked() {
     ...
     *     False, always.
     */
    _onDiscardDraftClicked: function _onDiscardDraftClicked() {
     ...
     *     False, always.
     */
    _onCloseDiscardedClicked: function _onCloseDiscardedClicked() {
    ...
    render: function render() {
      ...
      var draft = this.reviewRequest.draft;
      ...
      BannerView.prototype.render.apply(this, arguments);
      var interdiffLink = draft.get('interdiffLink');
  ...
  RB.ReviewRequestEditorView = Backbone.View.extend({
     ...
     */
    addFieldView: function addFieldView(view) {
      var _this2 = this;

      this._fieldViews[view.fieldID] = view;
      view.reviewRequestEditorView = this;
      this.listenTo(view, 'resize', this._scheduleResizeLayout);
      this.listenTo(view, 'fieldError', function (err) {
        _this2._$warning.delay(6000).fadeOut(400, function () {
          return _this2._$warning.hide();
        }).html(err.errorText).show();
      });
     ...
     *     The view which handles editing for the field.
     */
    getFieldView: function getFieldView(fieldID) {
    ...
    render: function render() {
      ...
      var fileAttachments = this.model.get('fileAttachments');
      ...
      fileAttachments.each(function (fileAttachment) {
        return _this3.buildFileAttachmentThumbnail(fileAttachment, fileAttachments, {
          noAnimation: true
        });
      });
      ...
      this.listenTo(fileAttachments, 'remove', function (model) {
        var index = _this3._fileAttachmentThumbnailViews.findIndex(function (view) {
          return view.model === model;
        });

        _this3._fileAttachmentThumbnailViews[index].remove();

        _this3._fileAttachmentThumbnailViews.splice(index, 1);
      });
      this.listenTo(fileAttachments, 'destroy', function () {
        if (fileAttachments.length === 0) {
          _this3._$attachmentsContainer.hide();
        }
      });
      /*
      ...
      for (var fieldID in this._fieldViews) {
      ...
      this.model.on('publishError', function (errorText) {
        ...
        _this3.$('#btn-draft-publish').enable();

        _this3.$('#btn-draft-discard').enable();
      });
     ...
     *
     */
    _onBeforeUnload: function _onBeforeUnload(evt) {
    ...
    showBanner: function showBanner() {
      ...
      var reviewRequest = this.model.get('reviewRequest');
      var state = reviewRequest.get('state');
      var BannerClass;
      ...
      var $existingBanner = this._$bannersContainer.children();
     ...
     */
    publishDraft: function publishDraft(options) {
      // Save all the fields if we need to.
      var fields = Object.values(this._fieldViews).filter(function (view) {
        return view.needsSave();
      });
      this.model.set({
        publishing: true,
    ...
    _setupActions: function _setupActions() {
      ...
      var $closeSubmitted = this.$('#submit-review-request-action');
      var $deletePermanently = this.$('#delete-review-request-action');
      var $updateDiff = this.$('#upload-diff-action');
      var $uploadFile = this.$('#upload-file-action');
      /*
     ...
     */
    buildFileAttachmentThumbnail: function buildFileAttachmentThumbnail(fileAttachment, collection) {
      var _this5 = this;

      var options = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};
      var fileAttachmentComments = this.model.get('fileAttachmentComments');
      var $thumbnail = options.$el;
      var view = new RB.FileAttachmentThumbnail({
        el: $thumbnail,
      ...
      if (!$thumbnail) {
        ...
        var fileAttachments = this.model.get('fileAttachments');
        var index = fileAttachments.indexOf(fileAttachment);
      ...
      this.listenTo(view, 'hoverIn', function ($thumbnail) {
        _this5._$attachments.find('.file').not($thumbnail.find('.file')[0]).addClass('faded');
      });
      this.listenTo(view, 'hoverOut', function () {
        return _this5._$attachments.find('.file').removeClass('faded');
      });
      view.on('beginEdit', function () {
        return _this5.model.incr('editCount');
      });
      view.on('endEdit', function () {
        return _this5.model.decr('editCount');
      });
      view.on('commentSaved', function () {
        return RB.DraftReviewBannerView.instance.show();
      });
    },
     ...
     */
    _importFileAttachmentThumbnail: function _importFileAttachmentThumbnail(thumbnailEl) {
      var $thumbnail = $(thumbnailEl);
      var id = $thumbnail.data('file-id');
     ...
     *         The existing DOM element to import.
     */
    _importScreenshotThumbnail: function _importScreenshotThumbnail(thumbnailEl) {
      var _this6 = this;

      var $thumbnail = $(thumbnailEl);
      var id = $thumbnail.data('screenshot-id');
      var reviewRequest = this.model.get('reviewRequest');
      var screenshot = reviewRequest.createScreenshot(id);
      var view = new RB.ScreenshotThumbnail({
        el: $thumbnail,
        model: screenshot
      });
      view.render();
      ...
      view.on('beginEdit', function () {
        return _this6.model.incr('editCount');
      });
      view.on('endEdit', function () {
        return _this6.model.decr('editCount');
      });
    },
     ...
     * a layout to resize.
     */
    _checkResizeLayout: function _checkResizeLayout() {
      /*
     ...
     */
    _resizeLayout: function _resizeLayout() {
      var $lastContent = this._$main.children('.review-request-section:last-child');

      var $lastFieldContainer = $lastContent.children('.field-container');
      var $lastField = $lastFieldContainer.children('.editable');

      var lastFieldView = this._fieldViews[$lastField.data('field-id')];

      var lastContentTop = Math.ceil($lastContent.position().top);
      var editor = lastFieldView.inlineEditorView.textEditor;
      var detailsWidth = 300; // Defined as @details-width in reviews.less

      var detailsPadding = 10;
      var $detailsBody = $('#review-request-details tbody');
      var $detailsLabels = $detailsBody.find('th:first-child');
      var $detailsValues = $detailsBody.find('span');
      ...
      if ($lastContent.outerHeight() + lastContentTop < height) {
        ...
        var contentHeight = $lastContent.height() - Math.ceil($lastFieldContainer.position().top);
        /*
        ...
        if (lastFieldView.inlineEditorView.editing() && editor) {
           ...
           * height.
           */
          var newEditableHeight = contentHeight + $lastField.getExtents('m', 'tb');
    ...
    _onCloseDiscardedClicked: function _onCloseDiscardedClicked() {
      ...
      var confirmText = gettext("Are you sure you want to discard this review request?");

      if (confirm(confirmText)) {
        this.model.get('reviewRequest').close({
          type: RB.ReviewRequest.CLOSE_DISCARDED,
          error: function error(model, xhr) {
            return _this7.model.trigger('closeError', xhr.errorText);
          }
        });
      }
     ...
     *     False, always.
     */
    _onCloseSubmittedClicked: function _onCloseSubmittedClicked() {
      var _this8 = this;
      ...
      if (submit) {
        this.model.get('reviewRequest').close({
          ...
          error: function error(model, xhr) {
            return _this8.model.trigger('closeError', xhr.errorText);
          }
        });
      }
     ...
     *     False, always.
     */
    _onDeleteReviewRequestClicked: function _onDeleteReviewRequestClicked() {
      var _this9 = this;

      var $dlg = $('<p>').text(gettext("This deletion cannot be undone. All diffs and reviews will be deleted as well.")).modalBox({
        title: gettext("Are you sure you want to delete this review request?"),
        buttons: [$("<input type=\"button\" value=\"".concat(gettext("Cancel"), "\"/>")), $("<input type=\"button\" value=\"".concat(gettext("Delete"), "\"/>")).click(function () {
          _this9.model.get('reviewRequest').destroy({
     ...
     *     False, always.
     */
    _onUpdateDiffClicked: function _onUpdateDiffClicked() {
      var reviewRequest = this.model.get('reviewRequest');
      var updateDiffView = new RB.UpdateDiffView({
     ...
     */
    _onUploadFileClicked: function _onUploadFileClicked(e) {
      e.stopPropagation();
      e.preventDefault();
     ...
     *     False, always.
     */
    _onArchiveClicked: function _onArchiveClicked() {
     ...
     *     False, always.
     */
    _onUnarchiveClicked: function _onUnarchiveClicked() {
     ...
     *     False, always.
     */
    _onMuteClicked: function _onMuteClicked() {
     ...
     *     False, always.
     */
    _onUnmuteClicked: function _onUnmuteClicked() {
     ...
     *     False, always.
     */
    _updateArchiveState: function _updateArchiveState(collection, add, newState) {
      var reviewRequest = this.model.get('reviewRequest');
      var options = {
        success: function success() {
          return reviewRequest.set('visibility', newState);
        }
      };
     ...
     */
    _updateArchiveVisibility: function _updateArchiveVisibility() {
      ...
      var visible = visibility === RB.ReviewRequest.VISIBILITY_VISIBLE;
      var iconClass = visible ? 'rb-icon-archive-off' : 'rb-icon-archive-on';
      var iconTitle = visible ? gettext("Archive review request") : gettext("Unarchive review request");
      var iconId = visible ? 'toggle-archived' : 'toggle-unarchived';
      this.$('#hide-review-request-link').html("<span class=\"rb-icon ".concat(iconClass, "\" id=\"").concat(iconId, "\" title=\"").concat(iconTitle, "\"></span>"));
