...
RB.ScrollManagerView = Backbone.View.extend({
   ...
   */
  scrollToPosition: function scrollToPosition(scrollY) {
   ...
   */
  markForUpdate: function markForUpdate($el) {
    console.assert($el.length === 1);
    var oldOffset = $el.offset();
   ...
   */
  markUpdated: function markUpdated($el) {
    console.assert($el.length === 1);
    var el = $el[0];

    var elInfo = this._pendingElements.get(el);
   ...
   */
  _scheduleUpdateScrollPos: function _scheduleUpdateScrollPos() {
  ...
  _updateScrollPos: function _updateScrollPos() {
    ...
    this._elements.forEach(function (elInfo, el) {
      /*
    ...
    if (elInfos.length > 0) {
      ...
      elInfos.sort(function (a, b) {
        return a.newOffset.top - b.newOffset.top;
      });

      for (var i = 0; i < elInfos.length; i++) {
