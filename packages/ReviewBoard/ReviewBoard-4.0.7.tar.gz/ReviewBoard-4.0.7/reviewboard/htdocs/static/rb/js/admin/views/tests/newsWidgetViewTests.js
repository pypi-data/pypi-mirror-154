  ...
  describe('Rendering', function () {
    describe('Actions', function () {
      it('More News', function () {
        view.render();
        var $actions = view.$footerActions.children();
        expect($actions.length).toBe(1);
        var $action = $actions.eq(0);
        expect($action.hasClass('js-action-more-news')).toBeTrue();
        expect($action.children('a').attr('href')).toBe('http://example.com/news/');
      });
      it('Subscribe', function () {
        ...
        var $action = $actions.eq(1);
        expect($action.hasClass('js-action-subscribe')).toBeTrue();
        expect($action.children('a').attr('href')).toBe('http://example.com/news/subscribe/');
      });
      describe('Reload', function () {
        it('On loadingNews event', function () {
          view.render();
          var $action = view.$headerActions.children('.js-action-reload');
    ...
    it('News items', function () {
      ...
      var $items = view.$content.children();
      expect($items.length).toBe(3);
      var $item = $items.eq(0);
      expect($item.attr('href')).toBe('http://example.com/news/post6');
      expect($item.find('.rb-c-admin-news-widget__item-date').text().strip()).toBe('Oct 20');
      expect($item.find('.rb-c-admin-news-widget__item-title').text().strip()).toBe('Headline 6');
      $item = $items.eq(1);
      expect($item.attr('href')).toBe('http://example.com/news/post5');
      expect($item.find('.rb-c-admin-news-widget__item-date').text().strip()).toBe('Oct 01');
      expect($item.find('.rb-c-admin-news-widget__item-title').text().strip()).toBe('Headline 5');
      $item = $items.eq(2);
      expect($item.attr('href')).toBe('http://example.com/news/post4');
      expect($item.find('.rb-c-admin-news-widget__item-date').text().strip()).toBe('Sep 26');
      expect($item.find('.rb-c-admin-news-widget__item-title').text().strip()).toBe('Headline 4');
    });
    it('Load errors', function () {
      view.render();
      model.set('newsItems', []);
      var $els = view.$content.children();
      expect($els.length).toBe(1);
      var $error = $els.eq(0);
      expect($error.hasClass('rb-c-admin-news-widget__error')).toBeTrue();
      expect($error.html().strip()).toBe('There was an error loading the news. Please ' + '<a href="http://example.com/news/">visit the news page</a> ' + 'directly, or try again later.');
    });
  });
});

//# sourceMappingURL=newsWidgetViewTests.js.map