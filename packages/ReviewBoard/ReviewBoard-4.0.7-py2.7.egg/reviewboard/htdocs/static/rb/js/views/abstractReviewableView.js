...
RB.AbstractReviewableView = Backbone.View.extend({
   ...
   */
  createAndEditCommentBlock: function createAndEditCommentBlock(options) {
    var _this = this;
    ...
    var defaultCommentBlockFields = _.result(this.model, 'defaultCommentBlockFields');
    ...
    this.once('commentBlockViewAdded', function (commentBlockView) {
      return _this.showCommentDlg(commentBlockView);
    });
   ...
   */
  showCommentDlg: function showCommentDlg(commentBlockView) {
    var _this2 = this;
    ...
    this.commentDlg = RB.CommentDialogView.create({
      ...
      position: function position(dlg) {
        return commentBlockView.positionCommentDlg(dlg);
      }
    });
    this._activeCommentBlock = commentBlock;
    this.listenTo(this.commentDlg, 'closed', function () {
      _this2.commentDlg = null;
      _this2._activeCommentBlock = null;
    });
  },
   ...
   */
  _addCommentBlockView: function _addCommentBlockView(commentBlock) {
    var _this3 = this;
    ...
    commentBlockView.on('clicked', function () {
      return _this3.showCommentDlg(commentBlockView);
    });
