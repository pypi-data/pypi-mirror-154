...
suite('rb/newReviewRequest/views/PostCommitView', function () {
  ...
  beforeEach(function () {
    ...
    spyOn(repository.branches, 'sync').and.callFake(function (method, collection, options) {
      options.success({
        stat: 'ok',
        branches: [{
          name: 'master',
    ...
    spyOn(repository, 'getCommits').and.callFake(function (options) {
      commits = new RB.RepositoryCommits([], {
        urlBase: _.result(_this, 'url') + 'commits/',
        start: options.start,
        branch: options.branch
      });
      spyOn(commits, 'sync').and.callFake(function (method, collection, options) {
        options.success({
          stat: 'ok',
          commits: [{
            authorName: 'Author 1',
  ...
  it('Create', function () {
    ...
    var call = RB.ReviewRequest.prototype.save.calls.mostRecent();
  ...
  describe('Error handling', function () {
    describe('Branches', function () {
      ...
      var returnError;
      beforeEach(function () {
        spyOn(repository.branches, 'fetch').and.callFake(function (options, context) {
          if (returnError) {
            options.error.call(context, repository.branches, xhr);
          } else {
      ...
      it('UI state', function () {
        ...
        expect(view._showLoadError).toHaveBeenCalledWith('branches', xhr);
      ...
      it('Reloading', function () {
        ...
        var $reload = view._$error.find('#reload_branches');
    ...
    describe('Commits', function () {
      ...
      beforeEach(function () {
        view.render();
        spyOn(RB.RepositoryCommits.prototype, 'fetch').and.callFake(function (options, context) {
          if (returnError) {
            options.error.call(context, repository.commits, xhr);
          } else {
            options.success.call(context);
          }
        });
      ...
      it('UI state', function () {
        ...
        expect(view._showLoadError).toHaveBeenCalledWith('commits', xhr);
      ...
      it('Reloading', function () {
        ...
        var $reload = view._$error.find('#reload_commits');
        ...
        var display = view._commitsView.$el.css('display');
