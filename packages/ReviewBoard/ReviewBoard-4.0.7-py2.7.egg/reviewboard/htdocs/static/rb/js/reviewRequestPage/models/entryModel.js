...
RB.ReviewRequestPage.Entry = Backbone.Model.extend({
  ...
  parse: function parse(attrs) {
    return {
      ...
      updatedTimestamp: _.isDate(attrs.updatedTimestamp) ? attrs.updatedTimestamp : moment.utc(attrs.updatedTimestamp).toDate(),
      typeID: attrs.typeID,
   ...
   */
  isUpdated: function isUpdated(metadata) {
    var newTimestamp = moment.utc(metadata.updatedTimestamp).toDate();
    /* Normalize these to null, if undefined or empty. */

    var newETag = metadata.etag || null;
    var entryETag = this.get('etag') || null;
    return newTimestamp > this.get('updatedTimestamp') || newETag !== entryETag;
  },
   ...
   */
  watchUpdates: function watchUpdates(periodMS) {
   ...
   */
  stopWatchingUpdates: function stopWatchingUpdates() {
