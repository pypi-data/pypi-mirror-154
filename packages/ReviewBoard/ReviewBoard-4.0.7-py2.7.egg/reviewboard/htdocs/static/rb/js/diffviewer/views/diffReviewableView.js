...
RB.DiffReviewableView = RB.AbstractReviewableView.extend({
  ...
  render: function render() {
    ...
    _.each(this.$el.children('tbody.binary'), function (thumbnailEl) {
      ...
      var reviewRequest = _this.model.get('reviewRequest');
   ...
   */
  toggleWhitespaceOnlyChunks: function toggleWhitespaceOnlyChunks() {
    var _this2 = this;
    ...
    _.each(this.$el.children('tbody.whitespace-chunk'), function (chunk) {
      var $chunk = $(chunk);
      var dimming = $chunk.hasClass('replace');
      $chunk.toggleClass('replace');
      var $children = $chunk.children();
      $children.first().toggleClass('first');
      $children.last().toggleClass('last');
      var chunkID = chunk.id.split('chunk')[1];

      if (dimming) {
        _this2.trigger('chunkDimmed', chunkID);
      } else {
        _this2.trigger('chunkUndimmed', chunkID);
      }
    });
    /*
   ...
   *         commented upon.
   */
  createComment: function createComment(beginLineNum, endLineNum, beginNode, endNode) {
   ...
   *         searched.
   */
  _placeCommentBlockView: function _placeCommentBlockView(commentBlockView, prevBeginRowIndex) {
    var commentBlock = commentBlockView.model;

    var rowEls = this._selector.getRowsForRange(commentBlock.get('beginLineNum'), commentBlock.get('endLineNum'), prevBeginRowIndex);

    if (rowEls !== null) {
      var beginRowEl = rowEls[0];
      var endRowEl = rowEls[1];
      /*
   ...
   */
  _placeHiddenCommentBlockViews: function _placeHiddenCommentBlockViews() {
    var hiddenCommentBlockViews = this._hiddenCommentBlockViews;
    this._hiddenCommentBlockViews = [];
    var prevBeginRowIndex;

    for (var i = 0; i < hiddenCommentBlockViews.length; i++) {
   ...
   */
  _hideRemovedCommentBlockViews: function _hideRemovedCommentBlockViews() {
    var visibleCommentBlockViews = this._visibleCommentBlockViews;
    this._visibleCommentBlockViews = [];

    for (var i = 0; i < visibleCommentBlockViews.length; i++) {
      var commentBlockView = visibleCommentBlockViews[i];
   ...
   * chunk.
   */
  _updateCollapseButtonPos: function _updateCollapseButtonPos() {
   ...
   */
  _expandOrCollapse: function _expandOrCollapse($btn, expanding) {
    var _this3 = this;

    var chunkIndex = $btn.data('chunk-index');
    var linesOfContext = $btn.data('lines-of-context');
    this.model.getRenderedDiffFragment({
      ...
      success: function success(html) {
        ...
        var scrollOffsetTop = $scrollAnchor.offset().top - _this3._$window.scrollTop();
        /*
        ...
        if (expanding) {
          _this3._placeHiddenCommentBlockViews();
        } else {
          _this3._hideRemovedCommentBlockViews();
        }
        /*
        ...
        if (tbodyID !== undefined) {
          var newEl = document.getElementById(tbodyID);

          if (newEl !== null) {
            $scrollAnchor = $(newEl);

            _this3._$window.scrollTop($scrollAnchor.offset().top - scrollOffsetTop);
          }
        }
        /* Recompute the set of buttons for later use. */


        _this3._centered.setElements(new Map(Array.prototype.map.call(_this3.$('.diff-collapse-btn'), function (el) {
          return [el, {
            $top: $(el).closest('tbody')
          }];
        })));

        _this3._updateCollapseButtonPos();
        /*
        ...
        _this3._precalculateContentWidths();

        _this3._updateColumnSizes();

        _this3.trigger('chunkExpansionChanged');
      }
    });
  },
   ...
   */
  _precalculateContentWidths: function _precalculateContentWidths() {
    var cellPadding = 0;

    if (!this.$el.hasClass('diff-error') && this._$revisionRow.length > 0) {
      var containerExtents = this.$el.getExtents('p', 'lr');
      /* Calculate the widths and state of the diff columns. */

      var $cells = $(this._$revisionRow[0].cells);
   ...
   */
  _updateColumnSizes: function _updateColumnSizes() {
    ...
    var contentWidth = fullWidth - this._colReservedWidths;
    ...
    var filenameWidth = fullWidth - this._filenameReservedWidths;
   ...
   */
  _onDownloadLinkClicked: function _onDownloadLinkClicked(e) {
    e.stopPropagation();
  },
   ...
   *         The ``click`` event that triggered this handler.
   */
  _onFileHeaderClicked: function _onFileHeaderClicked(e) {
    e.preventDefault();
    e.stopPropagation();
   ...
   *         The ``click`` event that triggered this handler.
   */
  _onMovedLineClicked: function _onMovedLineClicked(e) {
    e.preventDefault();
    e.stopPropagation();
  ...
  _onMouseUp: function _onMouseUp(e) {
    ...
    if ($tbody.length > 0 && ($tbody.hasClass('delete') || $tbody.hasClass('insert') || $tbody.hasClass('replace'))) {
      var anchor = $tbody[0].querySelector('a');
   ...
   *         The ``click`` event that triggered this handler.
   */
  _onExpandChunkClicked: function _onExpandChunkClicked(e) {
    e.preventDefault();
    var $target = $(e.target);
   ...
   *         The ``click`` event that triggered this handler.
   */
  _onCollapseChunkClicked: function _onCollapseChunkClicked(e) {
    e.preventDefault();
    var $target = $(e.target);
   ...
   *         The ``click`` event that triggered this handler.
   */
  _onShowDeletedClicked: function _onShowDeletedClicked(e) {
    e.preventDefault();
    e.stopPropagation();
    /*
