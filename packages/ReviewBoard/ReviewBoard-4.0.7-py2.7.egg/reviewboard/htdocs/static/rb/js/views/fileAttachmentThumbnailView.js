...
RB.FileAttachmentThumbnail = Backbone.View.extend({
  ...
  render: function render() {
    ...
    if (this.options.canEdit !== false) {
      ...
      this.listenTo(this._captionEditorView, 'beginEditPreShow', function () {
        _this.$el.addClass('editing');

        _this._stopAnimating();
      });
      this.listenTo(this._captionEditorView, 'beginEdit', function () {
        if (_this._$caption.hasClass('empty-caption')) {
          _this._captionEditorView.$field.val('');
        }

        _this.trigger('beginEdit');
      });
      this.listenTo(this._captionEditorView, 'cancel', function () {
        _this.$el.removeClass('editing');

        _this.trigger('endEdit');
      });
      this.listenTo(this._captionEditorView, 'complete', function (value) {
        _this.$el.removeClass('editing');
        /*
   ...
   */
  showCommentDlg: function showCommentDlg() {
  ...
  _processComments: function _processComments() {
    ...
    comments.forEach(function (comment) {
      if (comment.localdraft) {
        _this2._createDraftComment(comment.comment_id, comment.text);
      } else {
        _this2._comments.push(comment);
      }
    });
   ...
   *         The comment text.
   */
  _createDraftComment: function _createDraftComment(commentID, text) {
    var _this3 = this;
    ...
    var review = this.options.reviewRequest.createReview();
    ...
    this.listenTo(this._draftComment, 'saved', function () {
      return _this3.trigger('commentSaved', _this3._draftComment);
    });
  },
  ...
  _renderContents: function _renderContents() {
    ...
    var captionText = caption ? caption : gettext("No caption");
    var captionClass = caption ? 'edit' : 'edit empty-caption';
  ...
  _renderThumbnail: function _renderThumbnail() {
    ...
    this._$thumbnailContainer.find('a').each(function (i, el) {
      el.tabIndex = -1;
    });
  },
   ...
   */
  _onLoadedChanged: function _onLoadedChanged() {
   ...
   */
  _onCaptionChanged: function _onCaptionChanged() {
    var caption = this.model.get('caption');
   ...
   *         The event that triggered the action.
   */
  _onUpdateClicked: function _onUpdateClicked(e) {
    e.preventDefault();
    e.stopPropagation();
    var updateDlg = new RB.UploadAttachmentView({
   ...
   */
  _onHoverIn: function _onHoverIn() {
    var _this4 = this;

    var $thumbnail = this.$('.file-thumbnail').children();

    var actionsWidth = this._$actionsContainer.outerWidth();

    var actionsRight = this._$file.offset().left + this._$file.outerWidth() + actionsWidth;
    ...
    if (!this.$el.hasClass('editing') && $thumbnail.length === 1) {
      ...
      if (thumbnailEl.tagName === 'VIDEO') {
        ...
        var thumbnailHeight = $thumbnail.height() || 0;

        if (thumbnailHeight > elHeight) {
          var distance = elHeight - thumbnailHeight;
          var duration = Math.abs(distance) / 200 * 1000; // 200 pixels/s
          ...
          $thumbnail.delay(1000).animate({
            ...
            complete: function complete() {
              _this4._scrollingThumbnail = false;
            }
          });
        }
      }
   ...
   */
  _onHoverOut: function _onHoverOut() {
   ...
   */
  _stopAnimating: function _stopAnimating() {
