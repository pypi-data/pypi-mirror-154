...
suite('rb/ui/views/InfoboxManagerView', function () {
  var DummyInfoboxView = RB.BaseInfoboxView.extend({
    infoboxID: 'dummy-infobox'
  });
  var infoboxManagerView;
  beforeEach(function () {
    infoboxManagerView = RB.InfoboxManagerView.getInstance();
    spyOn(infoboxManagerView, '_fetchInfoboxContents').and.callFake(function (url, onDone) {
  ...
  describe('getOrCreateInfobox', function () {
    it('Caches infobox views', function () {
      var infoboxView1 = infoboxManagerView.getOrCreateInfobox(DummyInfoboxView);
      var infoboxView2 = infoboxManagerView.getOrCreateInfobox(DummyInfoboxView);
      expect(infoboxView1.cid).toBe(infoboxView2.cid);
    });
    it('Starts infobox hidden', function () {
      var infoboxView = infoboxManagerView.getOrCreateInfobox(DummyInfoboxView);
  ...
  describe('Target Events', function () {
    var $el;
    beforeEach(function () {
      spyOn(window, 'setTimeout').and.callFake(function (cb) {
        return cb();
      });
      $el = $('<a href="/foo/" />');
