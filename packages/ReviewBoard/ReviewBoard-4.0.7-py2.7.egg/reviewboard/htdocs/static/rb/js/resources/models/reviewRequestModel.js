...
RB.ReviewRequest = RB.BaseResource.extend({
   ...
   */
  url: function url() {
    var url = SITE_ROOT + (this.get('localSitePrefix') || '') + 'api/review-requests/';
    return this.isNew() ? url : "".concat(url).concat(this.id, "/");
  },
   ...
   */
  createFromCommit: function createFromCommit(options, context) {
   ...
   */
  createReview: function createReview(reviewID) {
   ...
   */
  createScreenshot: function createScreenshot(screenshotID) {
   ...
   *         Context to bind when calling callbacks.
   */
  setStarred: function setStarred(starred, options, context) {
   ...
   *         Context to bind when calling callbacks.
   */
  close: function close(options, context) {
    var _this = this;
    ...
    var changingState = options.type !== this.get('state');
    ...
    var saveOptions = _.defaults({
      ...
      success: function success() {
        ...
        _this.markUpdated(_this.get('lastUpdated'));
  ...
  reopen: function reopen() {
    ...
    this.save(_.defaults({
      data: {
        status: 'pending'
      },
      success: function success() {
        _this2.trigger('reopened');

        _this2.markUpdated(_this2.get('lastUpdated'));
   ...
   */
  markUpdated: function markUpdated(timestamp) {
   ...
   */
  beginCheckForUpdates: function beginCheckForUpdates(type, lastUpdateTimestamp) {
    var _this3 = this;
    ...
    this.ready({
      ready: function ready() {
        return setTimeout(_this3._checkForUpdates.bind(_this3), RB.ReviewRequest.CHECK_UPDATES_MSECS);
      }
    });
  },
   ...
   */
  _checkForUpdates: function _checkForUpdates() {
    var _this4 = this;
    ...
    RB.apiCall({
      ...
      success: function success(rsp) {
        ...
        if ((!_this4._checkUpdatesType || _this4._checkUpdatesType === lastUpdate.type) && _this4._lastUpdateTimestamp !== lastUpdate.timestamp) {
          _this4.trigger('updated', lastUpdate);
        }

        _this4._lastUpdateTimestamp = lastUpdate.timestamp;
        setTimeout(_this4._checkForUpdates.bind(_this4), RB.ReviewRequest.CHECK_UPDATES_MSECS);
      }
    });
  },
