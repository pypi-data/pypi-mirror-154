...
RB.InlineEditorView = Backbone.View.extend({
  ...
  render: function render() {
    ...
    this._$form = $('<form>').addClass("inline-editor-form ".concat(this.options.formClass)).css('display', 'inline').hide().insertBefore(this.$el);
    ...
    if (this.options.showEditIcon) {
      ...
      if (this.options.editIconPath) {
        this._$editIcon.append("<img src=\"".concat(this.options.editIconPath, "\">"));
      } else if (this.options.editIconClass) {
        this._$editIcon.append("<div class=\"".concat(this.options.editIconClass, "\" aria-hidden=\"true\"></div>"));
      }
      ...
      if (this.options.multiline && this.$el[0].id) {
        $("label[for=\"".concat(this.$el[0].id, "\"]")).append(this._$editIcon);
      } else {
...
RB.RichTextInlineEditorView = RB.InlineEditorView.extend({
  ...
  createField: function createField() {
    ...
    this.once('beginEdit', function () {
      ...
      var $markdownRef = $('<a class="markdown-info" target="_blank">').attr('href', "".concat(MANUAL_URL, "users/markdown/")).text(gettext("Markdown Reference")).setVisible(_this5.textEditor.richText).appendTo(_this5.$buttons);
