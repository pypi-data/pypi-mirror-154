...
(function () {
   ...
   */
  var FilesOnlyPreCommitModel = Backbone.Model.extend({
    defaults: _.defaults({
      repository: null
    })
  });
  ...
  var FilesOnlyPreCommitView = Backbone.View.extend({
     ...
     *         The click event.
     */
    _onCreateClicked: function _onCreateClicked(ev) {
      ev.preventDefault();
      ev.stopPropagation();
      ...
      reviewRequest.save({
        ...
        error: function error() {// TODO: handle errors
        }
      });
    }
  });
  ...
  RB.NewReviewRequestView = RB.PageView.extend({
    ...
    renderPage: function renderPage() {
      ...
      this.listenTo(this._repositorySelectionView, 'selected', function (repository) {
        return _this.model.set('repository', repository);
      });
     ...
     */
    onMobileModeChanged: function onMobileModeChanged(inMobileMode) {
     ...
     */
    _onRepositoryChanged: function _onRepositoryChanged() {
      var repository = this.model.get('repository');
     ...
     * them to be rebuilt.
     */
    _removeCommitViews: function _removeCommitViews() {
      if (this._preCommitView) {
     ...
     */
    _onShowRepositoriesClicked: function _onShowRepositoriesClicked() {
