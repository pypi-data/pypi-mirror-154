...
RB.Config.ResourceListItem = Djblets.Config.ListItem.extend({
  ...
  initialize: function initialize() {
    ...
    this.listenTo(resource, 'request', function () {
      ...
      return _this.trigger.apply(_this, ['request'].concat(args));
    });
    this.listenTo(resource, 'sync', function () {
      ...
      return _this.trigger.apply(_this, ['sync'].concat(args));
    });
    ...
    this.syncAttrs.forEach(function (attr) {
      return _this.listenTo(resource, "change:".concat(attr), function (model, value) {
        return _this.set(attr, value);
      });
    });
  },
