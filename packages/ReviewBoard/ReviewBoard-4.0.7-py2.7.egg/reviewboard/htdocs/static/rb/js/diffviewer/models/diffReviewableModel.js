...
RB.DiffReviewable = RB.AbstractReviewable.extend({
   ...
   */
  loadSerializedCommentBlock: function loadSerializedCommentBlock(serializedCommentBlock) {
   ...
   */
  getRenderedDiff: function getRenderedDiff(callbacks, context) {
   ...
   */
  getRenderedDiffFragment: function getRenderedDiffFragment(options, callbacks, context) {
   ...
   *         The context passed to each callback function.
   */
  _fetchFragment: function _fetchFragment(options, callbacks, context) {
    RB.apiCall(_.defaults({
      type: 'GET',
      dataType: 'html'
    }, options, _.bindCallbacks(callbacks, context)));
  },
   ...
   */
  _buildRenderedDiffURL: function _buildRenderedDiffURL() {
    var options = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
    var reviewURL = this.get('reviewRequest').get('reviewURL');
    var interdiffRevision = this.get('interdiffRevision');
    var fileDiffID = this.get('fileDiffID');
    var interFileDiffID = this.get('interFileDiffID');
    var baseFileDiffID = this.get('baseFileDiffID');
    var revision = this.get('revision');
    var revisionPart = interdiffRevision ? "".concat(revision, "-").concat(interdiffRevision) : revision;
    var fileDiffPart = interFileDiffID ? "".concat(fileDiffID, "-").concat(interFileDiffID) : fileDiffID;
    var url = "".concat(reviewURL, "diff/").concat(revisionPart, "/fragment/").concat(fileDiffPart, "/");

    if (options.chunkIndex !== undefined) {
      url += "chunk/".concat(options.chunkIndex, "/");
    }
    ...
    if (baseFileDiffID) {
      queryParts.push("base-filediff-id=".concat(baseFileDiffID));
    }

    if (options.index !== undefined) {
      queryParts.push("index=".concat(options.index));
    }

    if (options.linesOfContext !== undefined) {
      queryParts.push("lines-of-context=".concat(options.linesOfContext));
    }
    ...
    queryParts.push("_=".concat(TEMPLATE_SERIAL));
