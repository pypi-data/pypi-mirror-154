...
RB.DiffViewerPage = RB.ReviewablePage.extend({
   ...
   */
  loadDiffRevision: function loadDiffRevision() {
    var _this = this;

    var options = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
    var reviewRequestURL = this.get('reviewRequest').url();
    var queryData = [];
    ...
    var url = Djblets.buildURL({
      baseURL: "".concat(reviewRequestURL, "diff-context/"),
      queryData: queryData
    });
    $.ajax(url).done(function (rsp) {
      return _this.set(_this._parseDiffContext(rsp.diff_context));
    });
  },
   ...
   *     The returned attributes.
   */
  _parseDiffContext: function _parseDiffContext(rsp) {
