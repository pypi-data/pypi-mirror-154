...
RB.UploadDiffModel = Backbone.Model.extend({
  ...
  _updateState: function _updateState() {
    ...
    var changeNumber = this.get('changeNumber');
    var diff = this.get('diffFile');
    var diffValid = this.get('diffValid');
    var parentDiff = this.get('parentDiffFile');
    var repository = this.get('repository');
    var requiresBasedir = repository.get('requiresBasedir');
    var requiresChangeNumber = repository.get('requiresChangeNumber');
   ...
   */
  _tryValidate: function _tryValidate() {
    this.set('diffValid', false);
    var diff = this.get('diffFile');
    console.assert(diff);
    var parentDiff = this.get('parentDiffFile');
    var repository = this.get('repository');
    var uploader = new RB.ValidateDiffModel();
    uploader.set({
   ...
   */
  _onValidateSuccess: function _onValidateSuccess() {
   ...
   */
  _onValidateError: function _onValidateError(model, xhr) {
    var rsp = $.parseJSON(xhr.responseText);
    var newState = this.State.ERROR;
    var error;
   ...
   * ValidateDiffModel.
   */
  _createReviewRequest: function _createReviewRequest() {
    var _this = this;
    ...
    reviewRequest.save({
      success: function success() {
        _this.set('reviewRequest', reviewRequest);
   ...
   */
  _createDiff: function _createDiff() {
