...
RB.RevisionSelectorView = Backbone.View.extend({
   ...
   */
  render: function render(revisionLabels) {
    var _this = this;
    ...
    _.each(revisionLabels, function (label, i) {
      $('<div/>').addClass('revision-selector-tick').css('left', _this._positions[i] + 'px').appendTo(_this._$ticks);
      var $label = $('<div/>').text(label).addClass('revision-selector-label').appendTo(_this._$labels);
      $label.css('left', _this._positions[i] - $label.width() / 2 + 'px');

      if (_this._firstLabelActive || i > 0) {
  ...
  _updateHandles: function _updateHandles() {
    ...
    var values = this._mouseActive ? this._activeValues : this._values;

    this._$handles.children().each(function (i, el) {
   ...
   *         The mousedown event.
   */
  _onHandleMouseDown: function _onHandleMouseDown(ev) {
    ev.preventDefault();
    ev.stopPropagation();
    this._activeValues = [];
   ...
   *         The mouseup event.
   */
  _onHandleMouseUp: function _onHandleMouseUp(ev) {
   ...
   *         The mousemove event.
   */
  _onHandleMouseMove: function _onHandleMouseMove(ev) {
    console.assert(this._mouseActive);

    var mouseX = window.pageXOffset + ev.clientX - this._$trough.offset().left;

    var closestPos;
    var closestDist;

    for (var i = 0; i < this._positions.length; i++) {
      var dist = Math.abs(this._positions[i] - mouseX);
