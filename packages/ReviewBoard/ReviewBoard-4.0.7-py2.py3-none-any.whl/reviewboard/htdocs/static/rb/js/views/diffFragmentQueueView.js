...
RB.DiffFragmentQueueView = Backbone.View.extend({
  ...
  initialize: function initialize(options) {
    ...
    this._fragmentsBasePath = "".concat(options.reviewRequestPath, "_fragments/diff-comments/");
  ...
  _getCommentContainer: function _getCommentContainer(commentID) {
    return $("#".concat(this._containerPrefix, "_").concat(commentID));
  },
  ...
  _loadDiff: function _loadDiff(commentIDs) {
    ...
    if (options.linesOfContext !== undefined) {
      queryArgs.push("lines_of_context=".concat(options.linesOfContext));
    }
    ...
    RB.apiCall({
      url: "".concat(this._fragmentsBasePath).concat(commentIDs, "/"),
      data: queryArgs.join('&'),
      ...
      success: function success(arrayBuffer) {
        ...
        var onFragmentLoaded = function onFragmentLoaded(commentID, html) {
          ...
          var containerID = "#".concat(containerPrefix, "_").concat(commentID);
          ...
          if ($container.length === 0) {
            ...
            var view = _this2._renderFragment($("#".concat(containerPrefix, "_").concat(commentID)), commentID, html);
