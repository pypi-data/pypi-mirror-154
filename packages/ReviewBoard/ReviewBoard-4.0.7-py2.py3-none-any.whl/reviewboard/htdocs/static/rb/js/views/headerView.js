...
RB.HeaderView = Backbone.View.extend({
  ...
  render: function render() {
    ...
    this._$window.on('resize.rbHeaderView', _.throttle(this._recalcMobileMode.bind(this), 100));
   ...
   *         The click event.
   */
  _handleNavClick: function _handleNavClick(e) {
    e.stopPropagation();
    e.preventDefault();
   ...
   */
  _recalcMobileMode: function _recalcMobileMode() {
    var inMobileMode = this._$navToggle.is(':visible');
   ...
   * event.
   */
  _closeMobileMenu: function _closeMobileMenu() {
    this._setMobileMenuOpened(false);
    ...
    this._$search = $('#search_field').rbautocomplete({
      formatItem: function formatItem(data) {
        ...
        if (data.username) {
          ...
          if (data.fullname) {
            s += " <span>(".concat(_.escape(data.fullname), ")</span>");
          }
        } else if (data.name) {
          // For the format of groups:
          var displayName = _.escape(data.display_name);

          s = "".concat(data.name, " <span>(").concat(displayName, ")</span>");
        } else if (data.summary) {
          // For the format of review requests:
          if (data.summary.length < _this.SEARCH_SUMMARY_TRIM_LEN) {
            s = data.summary;
          } else {
            s = data.summary.substring(0, _this.SEARCH_SUMMARY_TRIM_LEN);
          }

          s += " <span>(".concat(_.escape(data.id), ")</span>");
        }
      ...
      parse: function parse(data) {
        ...
        var keys = ['username', 'name', 'summary'];
        var parsed = [];

        for (var j = 0; j < objects.length; j++) {
          var object = objects[j];
          var items = data.search[object];
      ...
      url: "".concat(SITE_ROOT, "api/search/")
    });
  },
   ...
   */
  _setMobileMenuOpened: function _setMobileMenuOpened(opened) {
    var _this2 = this;
    ...
    if (opened) {
      ...
      _.defer(function () {
        return _this2._$body.addClass('js-mobile-menu-open');
      });
    } else {
      this._$body.removeClass('js-mobile-menu-open');

      _.delay(function () {
        return _this2._$mobileMenuMask.hide();
      }, 300);
    }
