...
RB.MenuButtonView = Backbone.View.extend({
  ...
  render: function render() {
    ...
    var labelID = _.uniqueId('__rb-menubuttonview__label');

    var hasPrimaryButton = this._hasPrimaryButton;
    ...
    var menu = new RB.MenuView({
      ariaLabelledBy: labelID,
      $controller: this._$dropDownButton,
      type: this._menuType
    });
    menu.render();
    this.listenTo(menu, 'opening', function () {
      _this._$dropDownButton.addClass('js-hover');

      _this._updateMenuPosition();
    });
    this.listenTo(menu, 'closing', function () {
      _this._$dropDownButton.removeClass('js-hover');
    });
   ...
   */
  _updateMenuPosition: function _updateMenuPosition() {
    var $button = this._$dropDownButton;
    var buttonY1 = $button.offset().top;
    var buttonY2 = buttonY1 + $button.innerHeight();
    var pageY1 = window.pageYOffset;
    var pageY2 = window.pageYOffset + window.innerHeight;
    var direction;
    ...
    if (pageY1 >= buttonY1) {
      ...
      var menuHeight = this.menu.$el.outerHeight();
      /*
   ...
   */
  _openMenu: function _openMenu(options) {
   ...
   */
  _closeMenu: function _closeMenu(options) {
   ...
   *         The focus-in event.
   */
  _onFocusOut: function _onFocusOut(evt) {
    evt.stopPropagation();
    /*
   ...
   */
  _onToggleButtonKeyDown: function _onToggleButtonKeyDown(evt) {
    switch (evt.which) {
