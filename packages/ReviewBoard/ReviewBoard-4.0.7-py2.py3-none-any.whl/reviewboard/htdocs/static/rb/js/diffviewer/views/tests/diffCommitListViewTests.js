...
suite('rb/diffviewer/views/DiffCommitListView', function () {
  function testRows($rows, options) {
    ...
    var linkIndex = (options.haveHistory ? 1 : 0) + (options.haveInterCommitDiffControls ? 2 : 0);
    var valueStartIndex = linkIndex + (options.haveExpandCollapse ? 1 : 0);

    for (var i = 0; i < $rows.length; i++) {
      var $row = $rows.eq(i).find('td');
      var rowOptions = options.rowOptions[i];
      var values = rowOptions.values;
      ...
      if (options.haveInterCommitDiffControls) {
        var $baseSelector = $row.eq(0).find('input');
        ...
        var $tipSelector = $row.eq(1).find('input');
      ...
      if (options.haveExpandCollapse) {
        var $link = $row.eq(linkIndex).find('a');
  ...
  describe('Render', function () {
    var model;
    beforeEach(function () {
      var commits = new RB.DiffCommitCollection([{
        id: 1,
        commit_id: 'r1',
        parent_id: 'r0',
  ...
  describe('Event handlers', function () {
    ...
    it('Expand/collapse', function () {
      ...
      var $links = $table.find('a'); // Expand first row.
    ...
    it('Select base/tip', function () {
      ...
      view.render();
      var $baseSelectors = $container.find('.base-commit-selector');
      var $tipSelectors = $container.find('.tip-commit-selector');
