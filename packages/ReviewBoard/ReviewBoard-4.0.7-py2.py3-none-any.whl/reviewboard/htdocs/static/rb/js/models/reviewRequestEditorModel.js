...
RB.ReviewRequestEditor = Backbone.Model.extend({
  ...
  initialize: function initialize() {
    ...
    var reviewRequest = this.get('reviewRequest'); // Set up file attachments.
  ...
  createFileAttachment: function createFileAttachment() {
    ...
    if (attributes.attachmentHistoryID) {
      var oldAttachment = fileAttachments.findWhere({
        attachmentHistoryID: attributes.attachmentHistoryID
      });
      var index = fileAttachments.indexOf(oldAttachment);
  ...
  setDraftField: function setDraftField(fieldName, value) {
    ...
    console.assert(jsonFieldName, "jsonFieldName must be set when setting draft " + "field \"".concat(fieldName, "\"."));

    if (options.useExtraData) {
      jsonFieldName = "extra_data.".concat(jsonFieldName);
    }
    ...
    if (options.allowMarkdown) {
      ...
      if (options.useExtraData) {
        jsonTextTypeFieldName = "extra_data.".concat(jsonTextTypeFieldName);
      }
