...
RB.DiffFragmentView = Backbone.View.extend({
  ...
  render: function render() {
    ...
    if (this._collapsible) {
       ...
       */
      _.defer(function () {
        return _this.$el.addClass('allow-transitions');
      });
    }
   ...
   *         ``true``.
   */
  hideControls: function hideControls(animate) {
    var _this2 = this;
    ...
    var $firstDiffHeader = this._$diffHeaders.eq(0);
    ...
    if ($firstDiffHeader.hasClass('diff-header-above')) {
       ...
       */
      var translateY = $firstDiffHeader.height() - this.COLLAPSED_HEADERS_HEIGHT;

      this._$thead.css('transform', "translateY(".concat(translateY, "px)"));
    }
    /*
    ...
    _.each(this._$diffHeaders, function (diffHeaderEl) {
      var $diffHeader = $(diffHeaderEl);
      var scale = _this2.COLLAPSED_HEADERS_HEIGHT / $diffHeader.height();
      $diffHeader.css('transform', "scaleY(".concat(scale, ")"));
    });

    if (animate === false) {
      _.defer(function () {
        return _this2.$el.addClass('allow-transitions');
      });
    }
  },
   ...
   *         function call.
   */
  _expandOrCollapse: function _expandOrCollapse($btn) {
    this._loadDiff({
   ...
   */
  _tryShowControlsDelayed: function _tryShowControlsDelayed() {
    var _this3 = this;

    if (this._collapsible) {
      _.delay(function () {
        if (_this3.$el.is(':hover')) {
          _this3.showControls();
        }
   ...
   * Attempt to hide the controls in the given container after a delay.
   */
  _tryHideControlsDelayed: function _tryHideControlsDelayed() {
    var _this4 = this;

    if (this._collapsible) {
      _.delay(function () {
        if (!_this4.$el.is(':hover')) {
          _this4.hideControls();
        }
      }, this._controlsHoverTimeout);
    }
   ...
   * re-render the view.
   */
  _onExpandOrCollapseFinished: function _onExpandOrCollapseFinished() {
    ...
    var $collapseButtons = this.$('.diff-collapse-btn');
    /*
    ...
    if ($collapseButtons.length > 0) {
      this._centeredMgr = new RB.CenteredElementManager({
        elements: new Map(Array.prototype.map.call($collapseButtons, function (el) {
          var $chunks = $(el).closest('.sidebyside').children('tbody').not('.diff-header');
          return [el, {
            $top: $chunks.eq(0),
            $bottom: $chunks.eq(-1)
          }];
   ...
   *         The click event.
   */
  _onExpandButtonClicked: function _onExpandButtonClicked(e) {
    e.preventDefault();
    e.stopPropagation();
   ...
   *         The click event.
   */
  _onCollapseButtonClicked: function _onCollapseButtonClicked(e) {
    e.preventDefault();
    e.stopPropagation();
