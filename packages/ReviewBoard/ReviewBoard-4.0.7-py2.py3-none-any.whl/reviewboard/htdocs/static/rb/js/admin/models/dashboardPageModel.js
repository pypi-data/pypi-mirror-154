...
RB.Admin.DashboardPage = RB.Page.extend({
   ...
   */
  loadWidgets: function loadWidgets(onWidgetLoaded) {
    var classTypes = {};
    var widgetsData = this.get('widgetsData');
    this.widgets.reset();

    for (var i = 0; i < widgetsData.length; i++) {
      var widgetInfo = widgetsData[i];

      try {
        var ModelType = classTypes[widgetInfo.modelClass];
        var ViewType = classTypes[widgetInfo.viewClass];
        ...
        var widgetModel = new ModelType(_.defaults({
          id: widgetInfo.id
