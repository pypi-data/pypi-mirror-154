...
RB.UploadDiffView = Backbone.View.extend({
  ...
  render: function render() {
    ...
    this._$fileInput = $('<input type="file">').hide().appendTo(this.$el).change(function () {
      return _this.model.handleFiles(_this._$fileInput.get(0).files);
    });
   ...
   */
  _canDragDrop: function _canDragDrop() {
   ...
   */
  _onErrorChanged: function _onErrorChanged(model, error) {
    var errorHTML = "<div class=\"rb-icon rb-icon-warning\"></div> ".concat(error);

    this._$errorContents.html(errorHTML);

    this._$diffRevisionError.html(errorHTML);

    var innerHeight = this._$errorContents.height();

    var outerHeight = this._$error.height();
   ...
   */
  _onStateChanged: function _onStateChanged(model, state) {
   ...
   */
  _onBasedirSubmit: function _onBasedirSubmit(event) {
    event.stopPropagation();
    event.preventDefault();
   ...
   */
  _onChangenumSubmit: function _onChangenumSubmit() {
   ...
   */
  _onStartOverClicked: function _onStartOverClicked() {
   ...
   */
  _onSelectFileClicked: function _onSelectFileClicked() {
