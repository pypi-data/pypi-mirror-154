...
RB.StarManagerView = Backbone.View.extend({
  ...
  initialize: function initialize() {
    ...
    this.$('div.star').on('click', this._toggleStar.bind(this)).each(function (idx, el) {
      var $el = $(el);
      var objType = $el.attr('data-object-type');
      var objID = $el.attr('data-object-id');
      var objStarred = parseInt($el.attr('data-starred'), 10) === 1;
      var obj;
    ...
    if (this._datagridMode) {
      this.$el.on('datagridDisplayModeChanged', function ($grid, options) {
        if (options.mode === 'desktop') {
          for (var objID in _this._toUpdate) {
            if (_this._toUpdate.hasOwnProperty(objID)) {
              _this._updateStarColumn(objID);
            }
          }

          _this._watchUpdates = false;
          _this._toUpdate = {};
        } else if (options.mode === 'mobile') {
          _this._watchUpdates = true;
        }
      });
   ...
   */
  _updateStarColumn: function _updateStarColumn(objID) {
    var $el = this.$(".star[data-object-id=\"".concat(objID, "\"]"));
    var starred = this.model.get('starred')[objID];
   ...
   *         The click event.
   */
  _toggleStar: function _toggleStar(e) {
    var $target = $(e.target);
    var objID = $target.attr('data-object-id');
    var obj = this.model.get('objects')[objID];
    var starred = this.model.get('starred');
    var objStarred = !starred[objID];
    e.preventDefault();
    e.stopPropagation();
