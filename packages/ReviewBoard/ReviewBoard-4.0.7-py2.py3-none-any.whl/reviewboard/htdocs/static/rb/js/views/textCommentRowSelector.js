...
RB.TextCommentRowSelector = Backbone.View.extend({
   ...
   */
  getRowsForRange: function getRowsForRange(beginLineNum, endLineNum, minRowIndex) {
    var beginRowEl = this.findLineNumRow(beginLineNum, minRowIndex);

    if (beginRowEl) {
      var rowIndex = beginRowEl.rowIndex;
      var endRowEl = endLineNum === beginLineNum ? beginRowEl : this.findLineNumRow(endLineNum, rowIndex, rowIndex + endLineNum - beginLineNum);
   ...
   */
  findLineNumRow: function findLineNumRow(lineNum, startRow, endRow) {
    var table = this.el;
    var rowOffset = 1; // Get past the headers.
    ...
    for (var i = Math.round((low + high) / 2); low < high - 1;) {
      ...
      var guessRowNum = lineNum - value + rowOffset + i;
  ...
  _addRow: function _addRow($row) {
    ...
    var min = Math.min(this._lastSeenIndex, $row[0].rowIndex);
    var max = Math.max(this._lastSeenIndex, $row[0].rowIndex);
   ...
   */
  _highlightRow: function _highlightRow($row) {
   ...
   */
  _removeOldRows: function _removeOldRows($row) {
   ...
   */
  _removeSelectionClasses: function _removeSelectionClasses(startRowIndex, endRowIndex) {
   ...
   *         The cell to inspect.
   */
  _isLineNumCell: function _isLineNumCell(cell) {
   ...
   *     The row.
   */
  _getActualLineNumCell: function _getActualLineNumCell($node) {
   ...
   */
  _findPreTags: function _findPreTags(result, parentEl, tdClass, excludeTBodyClass) {
   ...
   */
  _copySelectionToClipboard: function _copySelectionToClipboard(clipboardData) {
    var excludeTBodyClass;
    var tdClass;
  ...
  _onMouseDown: function _onMouseDown(e) {
    ...
    var node = this._$ghostCommentFlagCell ? this._$ghostCommentFlagCell[0] : e.target;

    if (this._isLineNumCell(node)) {
      this._begin($(node.parentNode));
    } else {
      var $node = node.tagName === 'TD' ? $(node) : $(node).parentsUntil('tr', 'td');
  ...
  _onTouchStart: function _onTouchStart(e) {
    ...
    var $node = this._getActualLineNumCell($(firstTouch.target));
  ...
  _onTouchEnd: function _onTouchEnd(e) {
    ...
    var target = document.elementFromPoint(firstTouch.clientX, firstTouch.clientY);

    var $node = this._getActualLineNumCell($(target));
   ...
   *     The line number.
   */
  getLineNum: function getLineNum(row) {
