...
RB.FloatingBannerView = Backbone.View.extend({
  ...
  _updateSize: function _updateSize() {
    if (this._$floatSpacer !== null) {
      if (this.$el.hasClass('floating')) {
        var rect = this._$floatSpacer.parent()[0].getBoundingClientRect();
   ...
   * container.
   */
  _updateFloatPosition: function _updateFloatPosition() {
    ...
    var $container = this.options.$floatContainer;
    var containerTop = $container.offset().top;
    var containerHeight = $container.outerHeight();
    var containerBottom = containerTop + containerHeight;
    var windowTop = $(window).scrollTop();
    var topOffset = this._$floatSpacer.offset().top - windowTop;
    var outerHeight = this.$el.outerHeight(true);
    var wasFloating = this.$el.hasClass('floating');
