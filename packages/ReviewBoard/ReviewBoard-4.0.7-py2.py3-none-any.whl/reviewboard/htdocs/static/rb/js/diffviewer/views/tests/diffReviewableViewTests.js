...
suite('rb/diffviewer/views/DiffReviewableView', function () {
  ...
  describe('CommentRowSelector', function () {
    ...
    describe('Selecting range', function () {
      ...
      describe('Removing rows from selection', function () {
        it('Above', function () {
          var $prevRow = $rows.eq(3);
          var prevCell = $prevRow[0].cells[0];
        ...
        it('Below', function () {
          var $nextRow = $rows.eq(5);
          var nextCell = $nextRow[0].cells[0];
    ...
    describe('Expanding', function () {
      ...
      describe('Fetching fragment', function () {
        ...
        it('Full chunk', function () {
          ...
          var options = model.getRenderedDiffFragment.calls.argsFor(0)[0];
      ...
      describe('Injecting HTML', function () {
        it('Whole chunk', function () {
          spyOn(model, 'getRenderedDiffFragment').and.callFake(function (options, callbacks, context) {
            callbacks.success.call(context, ['<tbody class="equal tests-new-chunk">', ' <tr line="6">', '  <th></th>', '  <td>', '   <div class="collapse-floater">', '    <img class="diff-collapse-btn"', '         data-chunk-index="1"', '         data-lines-of-context="0" />', '   </div>', '  </td>', '  <th></th>', '  <td></td>', ' </tr>', '</tbody>'].join(''));
          });
          spyOn(view, 'trigger').and.callThrough();
          view.$('.tests-expand-chunk').click();
          expect(model.getRenderedDiffFragment).toHaveBeenCalled();
          var $tbodies = view.$('tbody');
          ...
          expect(view.trigger).toHaveBeenCalledWith('chunkExpansionChanged');
        });
    ...
    describe('Collapsing', function () {
      ...
      describe('Injecting HTML', function () {
        it('Single expanded chunk', function () {
          spyOn(model, 'getRenderedDiffFragment').and.callFake(function (options, callbacks, context) {
            callbacks.success.call(context, ['<tbody class="equal tests-new-chunk">', ' <tr line="6">', '  <th></th>', '  <td></td>', '  <th></th>', '  <td></td>', ' </tr>', '</tbody>'].join(''));
          });
          spyOn(view, 'trigger').and.callThrough();
          $collapseButton.click();
          expect(model.getRenderedDiffFragment).toHaveBeenCalled();
  ...
  describe('Comment flags', function () {
    describe('Placing visible comments', function () {
      var expandedDiffFragmentHTML = "<tbody class=\"equal tests-new-chunk\">\n <tr line=\"11\">\n  <th></th>\n  <td>\n   <div class=\"collapse-floater\">\n    <img class=\"diff-collapse-btn\"\n         data-chunk-index=\"1\"\n         data-lines-of-context=\"0\" />\n   </div>\n  </td>\n  <th></th>\n  <td></td>\n </tr>\n</tbody>";
      var $commentFlag;
      var $commentFlags;
      var $rows;
      var diffFragmentHTML;
      beforeEach(function () {
        view = new RB.DiffReviewableView({
          model: new RB.DiffReviewable({
            reviewRequest: reviewRequest,
        ...
        spyOn(view.model, 'getRenderedDiffFragment').and.callFake(function (options, callbacks, context) {
      ...
      it('On chunk re-expand (after collapsing)', function () {
        var collapsedDiffFragmentHTML = ['<tbody class="diff-header">', $(view.$('tbody')[1]).html(), '</tbody>'].join('');
        expect($commentFlags.length).toBe(2);
        view.$('.tests-expand-chunk').click();
    ...
    describe('toggleWhitespaceOnlyChunks', function () {
      ...
      describe('Toggle on', function () {
        ...
        it('Whitespace-only file classes', function () {
          ...
          var $whitespaceChunk = $('<tbody/>').addClass('whitespace-file').hide().html('<tr><td></td></tr>').appendTo(view.$el);
      ...
      describe('Toggle off', function () {
        ...
        it('Whitespace-only file classes', function () {
          var $tbodies = view.$el.children('tbody');
          var $whitespaceChunk = $('<tbody/>').addClass('whitespace-file').html('<tr><td></td></tr>').hide().appendTo(view.$el);
          expect($whitespaceChunk.is(':visible')).toBe(false);
          expect($tbodies.is(':visible')).toBe(true);
          view.toggleWhitespaceOnlyChunks();
          view.toggleWhitespaceOnlyChunks();
