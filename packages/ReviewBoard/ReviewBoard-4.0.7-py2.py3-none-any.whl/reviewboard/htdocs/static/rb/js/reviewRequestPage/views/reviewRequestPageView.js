...
(function () {
  ...
  RB.ReviewRequestPage.ReviewRequestPageView = RB.ReviewablePageView.extend({
    ...
    initialize: function initialize() {
      ...
      this.listenTo(this.model, 'updatesProcessed', function () {
        return _this.diffFragmentQueue.loadFragments();
      });
      /*
      ...
      this.listenTo(this.model, 'applyingUpdate:entry', function (metadata, html) {
        var entryID = metadata.entryID;
        var entryView = _this._entryViewsByID[entryID];
        var collapsed = entryView.isCollapsed();

        _this._onApplyingUpdate(entryView, metadata);

        _this.listenToOnce(_this.model, "appliedModelUpdate:entry:".concat(entryID), function (metadata, html) {
          return _this._reloadView(entryView, html);
        });

        _this.listenToOnce(_this.model, "appliedUpdate:entry:".concat(entryID), function (metadata) {
          _this._onAppliedUpdate(entryView, metadata);
    ...
    render: function render() {
      ...
      this._entryViews.forEach(function (entryView) {
      ...
      this.listenTo(this.model, 'appliedUpdate:issue-summary-table', function (metadata, html) {
        _this2._reloadView(_this2._issueSummaryTableView, html);
      });
      this._rendered = true;
      return this;
    },
     ...
     */
    addEntryView: function addEntryView(entryView) {
      var entry = entryView.model;
     ...
     */
    queueLoadDiff: function queueLoadDiff(commentID, key, onFragmentRendered) {
     ...
     */
    openCommentEditor: function openCommentEditor(contextType, contextID) {
      for (var i = 0; i < this._entryViews.length; i++) {
        var entryView = this._entryViews[i];
        var reviewReplyEditorView = _.isFunction(entryView.getReviewReplyEditorView) ? entryView.getReviewReplyEditorView(contextType, contextID) : null;
     ...
     *         The new HTML to set.
     */
    _reloadView: function _reloadView(view, html) {
      var $oldEl = view.$el;
      var $newEl = $(html);
     ...
     */
    _onApplyingUpdate: function _onApplyingUpdate(view, metadata) {
     ...
     *         The metadata set in the update.
     */
    _onAppliedUpdate: function _onAppliedUpdate(view, metadata) {
     ...
     * is shown on-screen.
     */
    _onHashChanged: function _onHashChanged() {
      var hash = RB.getLocationHash();
      var selector = null;

      if (hash !== '') {
        if (hash.includes('comment')) {
          selector = "a[name=".concat(hash, "]");
        } else {
          selector = "#".concat(hash);
        }
      }
     ...
     */
    _onCollapseAllClicked: function _onCollapseAllClicked(e) {
      e.preventDefault();
      e.stopPropagation();
     ...
     *         The event which triggered the action.
     */
    _onExpandAllClicked: function _onExpandAllClicked(e) {
      e.preventDefault();
      e.stopPropagation();

      this._entryViews.forEach(function (entryView) {
        return entryView.expand();
      });
    },
     ...
     */
    _onIssueClicked: function _onIssueClicked(params) {
      var prefix = commentTypeToIDPrefix[params.commentType];
      var selector = "#".concat(prefix, "comment").concat(params.commentID);
