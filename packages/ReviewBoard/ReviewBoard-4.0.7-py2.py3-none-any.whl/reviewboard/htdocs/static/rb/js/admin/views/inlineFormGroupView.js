...
RB.Admin.InlineFormGroupView = Backbone.View.extend({
  ...
  render: function render() {
    ...
    var $inlines = this.$el.children('.rb-c-admin-form-inline-group__inlines');
    console.assert($inlines.length === 1);
    var $actions = this.$el.children('.rb-c-admin-form-inline-group__actions');
    console.assert($actions.length === 1);
    var $addButton = $actions.children('.rb-c-admin-form-inline-group__add-action');
    ...
    this._$inlineTemplate = $inlines.children('.-is-template').detach().removeClass('-is-template');
    ...
    var $initialForms = $inlines.children("#id_".concat(prefix, "-INITIAL_FORMS"));
    var $maxNumForms = $inlines.children("#id_".concat(prefix, "-MAX_NUM_FORMS"));
    var $minNumForms = $inlines.children("#id_".concat(prefix, "-MIN_NUM_FORMS"));
    var $totalForms = $inlines.children("#id_".concat(prefix, "-TOTAL_FORMS"));
    ...
    var maxInlines = $maxNumForms.val();
    model.set({
    ...
    this.listenTo(model.inlines, 'update', function () {
    ...
    $inlines.children('.rb-c-admin-form-inline').each(function (index, el) {
      _this._setupInlineForm(el, {
        index: index,
        isInitial: true
      });
    });
    console.assert(parseInt($totalForms.val(), 10) === model.inlines.length);
    return this;
  },
   ...
   */
  addInlineForm: function addInlineForm() {
    var newIndex = this.model.inlines.length;

    var $inline = this._$inlineTemplate.clone();

    var view = this._setupInlineForm($inline[0]);
   ...
   */
  _setupInlineForm: function _setupInlineForm(el, attrs) {
    var model = this.model;
    var inline = new RB.Admin.InlineForm(_.extend({
      prefix: model.get('prefix')
    }, attrs));
    var inlineView = new RB.Admin.InlineFormView({
      el: el,
      model: inline
    });
    inlineView.render();
   ...
   */
  _onInlineRemoved: function _onInlineRemoved(inline) {
    var index = inline.get('index');
    var inlineView = this._inlineViews[index];
    ...
    this._inlineViews.forEach(function (view, i) {
      return view.model.set('index', i);
    });
   ...
   *         The click event.
   */
  _onAddClicked: function _onAddClicked(e) {
    e.preventDefault();
    e.stopPropagation();
    this.addInlineForm();
  }
...
//# sourceMappingURL=inlineFormGroupView.js.map