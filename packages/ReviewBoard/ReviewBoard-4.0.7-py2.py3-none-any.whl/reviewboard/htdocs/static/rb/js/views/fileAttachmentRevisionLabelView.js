...
RB.FileAttachmentRevisionLabelView = Backbone.View.extend({
  ...
  render: function render() {
    ...
    var diffAgainstRevision = this.model.get('diffRevision');
    var latestRevision = this.model.get('numRevisions');
    var header = '';
    var detail = null;
   ...
   */
  _onSelectLatest: function _onSelectLatest(ev) {
    ev.stopPropagation();
    ev.preventDefault();
   ...
   *         The event that triggered the action.
   */
  _onSelectChanged: function _onSelectChanged(ev) {
    ev.stopPropagation();
    ev.preventDefault();
    this.trigger('revisionSelected', [this.model.get('fileRevision'), this.model.get('numRevisions')]);
  }
});

//# sourceMappingURL=fileAttachmentRevisionLabelView.js.map