...
RB.MenuView = Backbone.View.extend({
  ...
  initialize: function initialize() {
    ...
    if (options.type === undefined || options.type === RB.MenuView.TYPE_STANDARD_MENU) {
  ...
  addItem: function addItem() {
    ...
    $el.attr({
      role: 'menuitem',
      tabindex: '-1'
    }).on('mousemove', this._onMenuItemMouseMove.bind(this)).appendTo(this.el);
    return $el;
  },
  ...
  focusLastItem: function focusLastItem() {
    var numChildren = this.el.children.length;
   ...
   *         to ``true``.
   */
  _setOpened: function _setOpened(opened) {
    var _this = this;
    ...
    if (options.animate === false) {
      ...
      _.defer(function () {
        return _this.$el.removeClass('js-no-animation');
      });
    }

    this.isOpen = opened;
    var triggerEvents = options.triggerEvents !== false;
   ...
   */
  _focusItem: function _focusItem(index) {
   ...
   */
  _focusPreviousItem: function _focusPreviousItem() {
    if (this._activeItemIndex === null) {
      this.focusFirstItem();
    } else {
      var index = this._activeItemIndex - 1;
   ...
   */
  _focusNextItem: function _focusNextItem() {
    if (this._activeItemIndex === null) {
      this.focusFirstItem();
    } else {
      var index = this._activeItemIndex + 1;
   ...
   *         The mousemove event.
   */
  _onMenuItemMouseMove: function _onMenuItemMouseMove(evt) {
    ...
    var menuItems = this.el.children;
