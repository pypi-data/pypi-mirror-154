...
__all__ = ('Craft', 'Rudder', 'Prober', 'Nozzle',
           ...
           'Reshape', 'Permute', 'Expand', 'Squash', 'Zoom', 'SubPix', 'SurPix', 'MaxPool', 'AvgPool',
           'EMA', 'Retroact',
           'Sqrt',
           'Concat', 'Stack',
           'D2S', 'S2D', 'SpDot',
...
class Zoom(Craft):
    def __init__(self, size: tuple, scale: tuple, interp=LINEAR, aligned=True, scope='ZOOM'):
        ...
        assert len(size)==0 or len(scale)==0, 'NEBULAE ERROR ⨷ either size or scale must be an empty tuple.'
        if len(size)==0:
