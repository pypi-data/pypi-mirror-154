...
class ToArrowFetchOrCompute(FetchOrCompute[AsyncIterator[pa.RecordBatch]]):
    ...
    async def __read_from_cache__(
        ...
        if table.schema.names == [
            "data",
            "sarus_is_public",
            "sarus_protected_entity",
            "sarus_weights",
        ]:
            batches = map(
                lambda x: pa.RecordBatch.from_struct_array(x.column("data")),
                batches,
            )
...
class LocalSDKManager(BaseAsyncManager):
    ...
    def __init__(
        ...
        self.parquet_dir = os.path.join(
            "/", "tmp", os.getlogin(), "sarus_dataspec"
        )
        ...
        if remote:
            ...
            for dataspec in computation_graph["dataspecs"]:
                ...
                if not status:
                    ...
                    code = status.protobuf().stage.WhichOneof("stage")
