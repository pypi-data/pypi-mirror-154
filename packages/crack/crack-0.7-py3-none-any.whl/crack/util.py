...
def hash_code(str_):
    n = len(str_)
    return sum((ord(s) * 31 ** (n - i) for i, s in enumerate(str_, 1)))
...
class Aes:
    ...
    def padding(self, b_str: bytes, is_src=False) -> bytes:
        padding_length = (AES.block_size - len(b_str) % AES.block_size)
    ...
    def _encrypt(self, b_str: bytes, *, key=None, iv=None, mode=None):
        ...
        return aes_.encrypt(self.padding(b_str, is_src=True))
