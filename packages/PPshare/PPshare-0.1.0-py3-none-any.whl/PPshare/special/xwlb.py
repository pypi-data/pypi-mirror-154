...
if __name__ == "__main__":
    yearmonthday = pd.date_range("2021-01-01","2021-12-31",freq="D").strftime("%Y%m%d").to_list()
    path = './new_cctv'
    if not os.path.exists(path):
        os.mkdir(path)
    for num in range(len(yearmonthday)):
        date = yearmonthday[num]
        news_cctv_df = news_cctv(date=date)
        news_cctv_df.to_csv('{}/{}.txt'.format(path, date),sep='\\')
        print('{}已完成！({}/{})'.format(date,num+1,len(yearmonthday)))