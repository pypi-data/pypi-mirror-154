...
class Search():
    ...
    def __init__(self, df, varnames, df_test=None, dist=None, code_name="search",
                 avail=None, test_av=None, weights=None,
                 choice_set=None,
        ...
        """
        if dist is None:
            logger.info("Dist not specifying. Allowing all random distributions.")
            dist = ['n', 'ln', 'tn', 'u', 't', 'f']
        ...
        if df_test is None and multi_objective:
            ...
            #         test_av = df_test['AV']
            #     except Exception:
            #         logger.info("Did not find avail column. Using avail=None.")
            ...
            #     weights = df['weight']
            # except KeyError:
            #     logger.info("Did not find weight column. Using weight=None.")
            ...
            #     except Exception:
            #         logger.info("Did not find a column for choice id.  .")
    ...
    def df_coeff_col(self, names_asvars,
                     choiceset, var_alt):
        """
        ...
        """
        random_matrix = self.random_state.randint(0, 2, len(names_asvars))
        asvars_new = []
    ...
    def generate_sol(self, asvars_avail, isvars_avail, rvars_avail,
        ...
        asvars_new = self.remove_redundant_asvars(asvars_1, transasvars,
                                                  asvarnames)
        # TODO! TESTING REMOVAL
        # asvars = self.df_coeff_col(data, test_df, asvars_new,
        #                            choice_set, alt_var)
        asvars = asvars_new
        ...
        if intercept_ps is None:
            asc_ind = bool(self.random_state.randint(2, size=1))
        else:
        ...
        if self.latent_class:
            ...
            for i in range(self.num_classes):
                ...
                for var in class_vars:
                    ...
                    if self.random_state.uniform() < 0.8:  # 0.5 prob of accepting asvar  TODO?
    ...
    def fit_mnl(self, dat, test_df, as_vars, is_vars, bcvars, choice, alt,
        ...
        try:
            ...
            model.fit(X, y, varnames=all_vars, isvars=is_vars, alts=alt_var,
                    ...
                    weights=weights, base_alt=self.base_alt, random_state=seed)
            ...
            if self.multi_objective:
                ...
                model.fit(X_test, y_test, varnames=all_vars, isvars=is_vars,
                          alts=alt_var, ids=self.test_choice_id,
    ...
    def fit_mxl(self, dat, test_df, as_vars, is_vars, rand_vars, bcvars, corvars,
                ...
                asc_ind, avail=None,
                weights=None, iterations=200, tol=1e-2):
        """
        ...
        return(bic, val, as_vars, is_vars, rand_vars, bcvars, corvars, conv, pvals, coef_names)

    def fit_lccm(self, dat, test_df, as_vars, is_vars, class_params_spec,
                 member_params_spec, num_classes, bcvars, choice, alt,
                 id_choice, test_choice_var, asc_ind):
        """Estimates multinomial model for the generated solution.
        Inputs:
        (1) dat in csv
        (2) as_vars: list of alternative-specific variables
        ...
        transvars = [var for var in bcvars if var in class_params_spec]
        if transvars != bcvars:
    ...
    def check_latent_validity(self, class_params_spec, member_params_spec):
        check_latent = True
        if class_params_spec is None or member_params_spec is None:
            return False
        if self.latent_class:
    ...
    def evaluate_objective_function(self, df, as_vars,
        ...
        conv = False  # TODO? why conv AND convergene?
        sig = None
        sig_member = None
        coefs = []
        ...
        if all_vars:
            ...
            if bool(rand_vars):
                if self.latent_class and check_latent:
                    ...
                    bic, val, asvars, isvars, randvars, bcvars, corvars, \
                        conv, sig, sig_member, coefs = self.fit_lccmm(df,
                                                          self.df_test,
                                                          as_vars, is_vars,
                                                          class_params_spec,
                                                          member_params_spec,
                    ...
                    bic, val, asvars, isvars, randvars, bcvars, corvars, \
                        conv, sig, coefs = self.fit_mxl(df, self.df_test,
                                                        as_vars, is_vars,
                                                        rand_vars, bc_vars,
                                                        cor_vars,
                                                        choice, alts,
                ...
                if self.latent_class:
                    if check_latent:
                        bic, val, asvars, isvars, randvars, bcvars, corvars, \
                            conv, sig, sig_member, coefs = self.fit_lccm(df, self.df_test,
                                                            as_vars, is_vars,
                                                            class_params_spec,
                                                            member_params_spec,
                                                            2,
                    ...
                    bic, val, asvars, isvars, randvars, bcvars, corvars, \
                        conv, sig, coefs = self.fit_mnl(df, self.df_test,
                                                        as_vars, is_vars,
                                                        bc_vars, choice, alts,
                                                        id_choice,
                                                        self.test_choice_var,
            ...
            if conv:
                ...
                if self.multi_objective:
                    sol = Solution(bic=bic, MAE=val, asvars=asvars, isvars=isvars,
                                   randvars=randvars, bcvars=bcvars,
                                   corvars=corvars, asc_ind=asc_ind,
                                   class_params_spec=class_params_spec,
                                   member_params_spec=member_params_spec)
                else:
                    sol = Solution(bic=bic, loglik=val, asvars=asvars, isvars=isvars,
                                   randvars=randvars, bcvars=bcvars,
                                   corvars=corvars, asc_ind=asc_ind,
                                   class_params_spec=class_params_spec,
                                   member_params_spec=member_params_spec)
                ...
                if all(v for v in sig <= self.p_val):
                    ...
                    while any([v for v in sig if self.p_val]):
                        ...
                        if rem_alvars:
                            ...
                            if conv:
                                if self.multi_objective:
                                    sol = Solution(bic=bic,
                                                   MAE=val,
                                                   asvars=asvars,
                                                   isvars=isvars,
                                                   randvars=randvars,
                                                   bcvars=bcvars, corvars=corvars,
                                                   asc_ind=new_asc_ind,
                                                   class_params_spec=class_params_spec,
                                                   member_params_spec=member_params_spec
                                                   )
                                ...
                                sol['bcvars'] = [var for var in sol['bcvars'] if var in sol['asvars'] and var not in ps_corvars]
                                if sol['cor_vars']:
    ...
    def initialize_memory(self, choice_data, HMS, asvars_avail, isvars_avail,
        ...
        while dummy_iter < 30000:
            ...
            asconstant = \
                self.generate_sol(asvars_avail,
            ...
            sol, conv = \
                self.evaluate_objective_function(choice_data, asvars,
            ...
            # ad_var = [x for x in self.varnames if x not in sol['asvars']]
            op_asvars, op_isvars, op_rvars, op_bcvars, op_corvars, \
                op_bctrans, op_cor, op_class_params_spec, op_member_params_spec, op_asconstant = \
                    self.generate_sol(asvars_avail, isvars_avail,
            ...
            opp_sol, opp_conv = \
                self.evaluate_objective_function(choice_data, op_asvars,
                                                 op_isvars, op_rvars,
                                                 op_bcvars, op_corvars,
    ...
    def harmony_consideration(self, har_mem, HMCR_itr, itr, HMS, df,
        ...
        if self.random_state.choice([0, 1], p=[1-HMCR_itr, HMCR_itr]) <= HMCR_itr:
            ...
            # select_new_isvars = list(self.random_state.choice(har_mem[m_pos][2],int((len(har_mem[m_pos][2]))*HMCR_itr),replace = False, p=[1-HMCR_itr, HMCR_itr]))
            ...
            asvars, isvars, randvars, bcvars, corvars, bctrans, cor, class_params_spec, member_params_spec, asconstant = \
                self.generate_sol(avail_asvars,
                                  ...
                                  alt_var)
            new_sol = Solution(asvars=asvars, isvars=isvars, randvars=randvars,
                               bcvars=bcvars, corvars=corvars,
    ...
    def add_new_asfeature(self, solution):
        ...
        if new_asvar:
            ...
            logger.info("new sol: {}".format(str(solution['asvars'])))
    ...
    def add_new_class_paramfeature(self, solution):
        ...
        if len(new_params) > 0:
            # new_params = self.random_state.choice(new_params, 1)
            new_param = np.array([])
    ...
    def remove_corfeature(self, solution):
        ...
        if solution['cor_vars']:
            ...
            solution['cor_vars'] = [var for var in solution['cor_vars'] if var
                           in solution['randvars'].keys() and var not in rem_nps_corvar]
            if len(solution['cor_vars']) < 2:
                solution['cor_vars'] = []
        return(solution)
    ...
    def remove_class_paramfeature(self, solution):
        ...
        if solution['class_params_spec'] is not None:
            ...
            logger.info("DEBUG class_params_spec - {}".format(str(class_params_spec)))
            ii = self.random_state.randint(0, len(class_params_spec))
            logger.info("DEBUG ii - {}".format(str(ii)))
            class_i = class_params_spec[ii]
            logger.info("DEBUG class_i- {}".format(str(class_i)))
            if len(class_i) > 0:
                rem_asvar = list(self.random_state.choice(class_i, 1))
                logger.info("DEBUG rem_asvar- {}".format(str(rem_asvar)))
                tmp_class_i = [var for var in class_i if not rem_asvar]
                logger.info("DEBUG tmp_class_i-  {}".format(str(tmp_class_i)))
    ...
    def assess_sol(self, solution, har_mem):
        ...
        check_latent = self.check_latent_validity(solution['class_params_spec'], solution['member_params_spec'])
        asc_ind = solution['asc_ind']
        # TODO! Check ... removing _inter from class params if no intercept
        if not asc_ind and self.latent_class and check_latent:
            rem_class_params_spec = solution['class_params_spec']
            rem_member_params_spec = solution['member_params_spec']
            for ii, class_params in enumerate(rem_class_params_spec):
                for jj, var in enumerate(class_params):
                    if '_inter' in var:
                        if len(class_params) > 1:
            ...
            solution['class_params_spec'] = rem_class_params_spec
            solution['member_params_spec'] = rem_member_params_spec

        improved_sol, conv = \
            self.evaluate_objective_function(data,
                                             solution['asvars'], solution['isvars'],
                                             solution['randvars'], solution['bcvars'],
                                             solution['cor_vars'],
                                             solution['class_params_spec'], solution['member_params_spec'],
                                             self.choice_var,
                                             self.test_choice_var, self.alt_var,
                                             self.choice_id, self.ind_id,
                                             solution['asc_ind'], self.ps_asvars,
    ...
    def pitch_adjustment(self, sol, har_mem, PAR_itr, itr, HMS):
        ...
        if self.random_state.choice([0, 1], p=[1-PAR_itr, PAR_itr]) <= PAR_itr:
            if self.random_state.randint(2):
                ...
                if pa_sol['class_params_spec'] is not None:  # todo? confirm
                    logger.info("pitch adjustment adding class param variables")
                    pa_sol = self.add_new_class_paramfeature(pa_sol)
                    improved_harmony, current_sol = self.assess_sol(pa_sol,
                                                                    har_mem)

                if pa_sol['member_params_spec'] is not None:  # todo? confirm
    ...
    def improvise_harmony(self, HCR_max, HCR_min, PR_max, PR_min, har_mem,
        ...
        while itr < max_itr:
            ...
            hmc_sol = \
                self.harmony_consideration(har_mem, HMCR_itr, itr, self.HMS,
            ...
            if itr == max_itr:  # Plot on final iteration
                val_key = 'MAE' if self.multi_objective else 'loglik'
                valid_idx = [ii for ii, har_mem_ii in enumerate(har_mem) if np.abs(har_mem_ii['bic']) < 1e+7
                ...
                logger.info('best_bic_points: {}'.format(best_bic_points))
                logger.info('best_val_points: {}'.format(best_val_points))
                ...
                if self.multi_objective:  # Plot for MOOF
                    ...
                    # TODO: REMOVE 100000?
                    # current_bic_points = [har_mem_ii['bic'] for ii, har_mem_ii in enumerate(har_mem) if np.abs(har_mem_ii['bic']) != 1000000.0]
    ...
    def _prep_inputs(self, asvarnames=[], isvarnames=[]):
        ...
        ps_bctrans = None  # (True or False or None)
        ...
        ps_cor = None  # (True or False or None)
    ...
    def run_search(self, HMS=10, HMCR_min=0.6, HMCR_max=0.9, PAR_max=0.85,
        ...
        logger.info("{} GPU device(s) available. searchlogit will " + gpu_txt + "use GPU processing".format(n_gpus))
        ...
        asvars_new = self.df_coeff_col(avail_asvars,
                                       self.choice_set,
                                       self.alt_var)
        self.remove_redundant_asvars(asvars_new, self.trans_asvars,
                                     self.asvarnames)
        self.generate_sol(avail_asvars, avail_isvars,
                          avail_rvars, self.trans_asvars, avail_bcvars,
                          avail_corvars, self.ps_asvars, self.ps_isvars, self.ps_rvars,
                          self.ps_bcvars, self.ps_corvars, self.ps_bctrans, self.ps_cor,
                          self.ps_intercept, self.asvarnames, self.choice_set,
                          self.alt_var)
        Init_HM = self.initialize_memory(self.df, HMS, avail_asvars,
                                         avail_isvars, avail_rvars,
                                         avail_bcvars, avail_corvars,
                                         self.ps_asvars, self.ps_isvars, self.ps_rvars,
                                         self.ps_bcvars, self.ps_corvars, self.ps_bctrans,
                                         self.ps_cor, self.ps_intercept,
                                         self.trans_asvars, self.asvarnames,
                                         self.choice_set, self.alt_var,
                                         self.choice_var, self.choice_id,
                                         self.ind_id, self.ps_asvars,
                                         self.isvarnames, self.ps_isvars,
                                         self.ps_intercept, self.ps_rvars, self.ps_corvars)
        ...
        new_HM, best_BICs, current_BICs = \
            ...
            threshold, v,
            self.ps_corvars, self.ps_bctrans, self.ps_cor, self.ps_intercept, self.asvarnames,
        ...
        for v in Fronts.values():
            for l in v:
                if v[0] == l or v[-1] == l:
                    ...
                    dis = abs(q_dis.get(l) + ((HM[v[v.index(l)+1]][val_key] -
                                               HM[l][val_key])
