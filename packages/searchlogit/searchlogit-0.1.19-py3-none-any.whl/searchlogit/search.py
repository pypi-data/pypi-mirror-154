...
class Search():
    ...
    def evaluate_objective_function(self, new_df, test_df, seed, as_vars,
        ...
        if all_vars:
            ...
            if conv:
                ...
                sol = Solution(bic=bic, val_key=val, asvars=asvars, isvars=isvars,
                ...
                if all(v for v in sig <= self.p_val):
                    ...
                    while any([v for v in sig if self.p_val]):
                        ...
                        if rem_alvars:
                            ...
                            if conv:
                                val_key = 'MAE' if self.multi_objective else 'loglik'
                                sol = Solution(bic=bic, val_key=val, asvars=asvars,
                                               isvars=isvars,
                                               randvars=randvars,
    ...
    def find_bestsol(self, HM):
        ...
        max_obj2 = max(HM[x][val_key] for x in range(len(HM)))
        min_obj1 = min(HM[x]['bic'] for x in range(len(HM)))
        min_obj2 = min(HM[x][val_key] for x in range(len(HM)))
        weights_obj1 = [(HM[x]['bic'])-min_obj1/(max_obj1-min_obj1) for x in range(len(HM))]
        weights_obj2 = [(HM[x][val_key])-min_obj2/(max_obj2-min_obj2) for x in range(len(HM))]
