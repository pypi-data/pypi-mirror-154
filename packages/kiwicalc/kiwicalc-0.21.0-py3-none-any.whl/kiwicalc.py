...
from math import sin, cos, tan, asin, acos, atan, sinh, cosh, tanh, asinh, acosh, atanh, sqrt, e, pi, floor, ceil, log, \
    log10, log2, exp, erf, erfc, gamma, lgamma, tau, degrees, radians
...
if version_info[0] < 3.8:
    # earlier version than 3.8
    comb = comb1
else:
...
variable_pattern = r"(\d+[.,]?\d*|[a-zA-Z])"  # TODO: fix this
...
def solve_poly_system(
    ...
    for i in range(nmax):
        ...
        jacobian_inverse = assigned_jacobian.inverseWithNumpy()
        if jacobian_inverse is None:
            warnings.warn("Couldn't find a solution!")
            return None
...
def scatter_dots(x_values, y_values, title: str = "", ymin: float = -10, ymax: float = 10, color=None,
    ...
    x_length, y_length = len(x_values), len(y_values)
    if x_length != y_length:
        raise ValueError(f"You must enter an equal number of x and y values. Got {x_length} x values and "
                         f"{y_length} y values.")
    if None in (fig, ax):
        fig, ax = create_grid()
    if show_axis:
...
class IExpression(ABC):
    ...
    def scatter(self, start: float = -10, stop: float = 10,
                step: float = 0.1, ymin: float = -10, ymax: float = 10, title=None, show_axis=True, show=True,
        ...
        if num_of_variables == 0:  # TODO: plot this in a number axis
            ...
            if step < 0.1:
                step = 0.1
                warnings.warn("step parameter altered to 0.1 to avoid lag")
...
class ExpressionSum(IExpression, IPlottable, IScatterable):
    ...
    def __init__(self, expressions: Iterable[IExpression] = None, copy=True):
        ...
        self._expressions = [expression for index, expression in enumerate(self._expressions) if
                             index not in indices_to_delete]
...
class Poly(IExpression, IPlottable):
    ...
    def try_evaluate(self) -> Optional[Union[int, float, complex]]:
        if not self._expressions:
            return 0
        length = len(self._expressions)
        if self.is_number() and length > 0:
            if length > 1:
                self.simplify()
        ...
        if isinstance(other, PolyFraction):
            if self._denominator == other._denominator:
                if operation == '+':
                    ...
                    self._numerator -= other._numerator
                return self
            division_result = self._denominator.__truediv__(other._denominator, get_remainder=True)
            if division_result[1] == 0:  # My denominator is bigger
                if operation == '+':
            ...
            division_result = other._denominator.__truediv__(self._denominator, get_remainder=True)
            if division_result[1] == 0:  # My denominator is smaller
...
class TrigoExpr(IExpression, IPlottable, IScatterable):
    ...
    def assign(self, **kwargs):
        ...
        for index, [method, inside, degree] in enumerate(self._expressions):
            ...
            new_inside = new_inside.try_evaluate()
            if new_inside is not None:  # if the inside expression can be evaluated, then simplify
        ...
        if method1 is method2 and power1 == power2:
            ...
            if method1 is TrigoMethods.SIN:  # sin()
                ...
                if expression_sum == pi:
                    if coef1 == -coef2:
                        return True
        first_condition = (method1, method2) in (
            (TrigoMethods.SIN, TrigoMethods.COS), (TrigoMethods.COS, TrigoMethods.SIN))
        second_condition = (method1, method2) in (
            (TrigoMethods.CSC, TrigoMethods.SEC), (TrigoMethods.SEC, TrigoMethods.CSC))
...
class Factorial(IExpression, IPlottable, IScatterable):
    ...
    def __init__(self, expression: Optional[Union[IExpression, int, float, str]],
        ...
        if isinstance(expression, (int, float)):
            ...
            self._expression = copy_expression(expression)
    ...
    def random_worksheet(path=None, title="Quadratic Equations Worksheet", num_of_equations=20,
        ...
        if get_solutions:
            ...
            for i in range(num_of_equations):
                ...
                solutions.append(f"{i + 1}. {', '.join([str(s) for s in sol])}")
            create_pages(path=path, num_of_pages=2, titles=['Exercises', 'Solutions'], lines=[equations, solutions])
        else:
            lines = []
            equations = []
            for i in range(num_of_equations):
...
class CubicEquation(Equation):
    ...
    def coefficients(self):
        return [self._variables_dict['x'][0], self._variables_dict['x'][1], self._variables_dict['x'][2]]
    ...
    def toIExpression(self):
        ...
        try:
            ...
            poly_types = [self.Classification.linear, self.Classification.quadratic, self.Classification.polynomial
                , self.Classification.linear_several_parameters, self.Classification.non_linear_several_parameters]
            # nonlinear isn't supposed to be here, change it only to poly several parameters for now
...
class FunctionCollection(IPlottable, IScatterable):
    def __init__(self, *functions, gen_copies=False):
        ...
        for func in functions:
            if isinstance(func, Function):
            ...
            elif isinstance(func, str) or is_lambda(func):  # lambda expressions might not work here
                ...
                if all(isinstance(f, Function) for f in func):
                    ...
                    pass  # TODO Recursive Algorithm to break down anything, or raise an Error
    ...
    def plot(self, start: float = -10, stop: float = 10,
        ...
        if num_of_variables == 1:
            ...
            if step < 0.1:
                step = 0.2
                warnings.warn("Step altered to 0.1 in order to prevent lag.")
            if abs(stop - start) > 5:
                warnings.warn("Reducing axes range in order to prevent lag.")
                while abs(stop - start) > 5:
                    start /= 1.41
                    stop /= 1.41

            plot_functions_3d(self._functions, start, stop, step)
...
class Circle(IPlottable):
    ...
    def plot(self, fig=None, ax=None, show=True, color='r', with_legend=True):
        radius_eval = self._radius.try_evaluate()
        center_x_eval = self.center_x.try_evaluate()
        center_y_eval = self.center_y.try_evaluate()
        ...
        circle1 = plt.Circle((center_x_eval, center_y_eval), radius_eval, color=color, fill=False)
        if None in (fig, ax):
            fig, ax = plt.subplots()  # note we must use plt.subplots, not plt.subplot
        patch = ax.add_patch(circle1)
        ax.set_aspect('equal', adjustable='datalim')
        ax.plot()  # Causes an autoscale update.
        if with_legend:
            ax.legend([patch], [f'{self.equation}'])
        if show:
            plt.show()
        return patch  # return the patch
    ...
    def equation(self) -> str:
        ...
        radius_part = f"{self._radius}^2" if radius_eval is None or (
                radius_eval is not None and radius_eval > 100) else f"{round_decimal(radius_eval ** 2)}"
        ...
        if isinstance(other, Circle):
            if self.has_parameters() or other.has_parameters():
                ...
                if not isinstance(initial_x, (int, float)):
                    initial_x = initial_x.try_evaluate()
                    if initial_x is None:
                        initial_x = 2
                        warnings.warn("Chose a random initial x value due to an unexpected error.")
                if not isinstance(initial_y, (int, float)):
                    initial_y = initial_y.try_evaluate()
                    if initial_y is None:
                        initial_y = 2
                        warnings.warn("Chose a random initial y value due to an unexpected error.")
                intersections = solve_poly_system([self._expression(), other._expression()],
                                                  initial_vals={'x': initial_x, 'y': initial_y})
                return intersections

    def plot_intersection(self, other: "Circle"):
        fig, ax = plt.subplots(figsize=(10, 8))
        patch1 = self.plot(fig=fig, ax=ax, color='r', show=False, with_legend=False)
        patch2 = other.plot(fig=fig, ax=ax, color='b', show=False, with_legend=False)
        ax.legend([patch1, patch2], [f'{self.equation}', f'{other.equation}'])
        plt.show()
...
class PointCollection:
    ...
    def longest_distance(self, get_points=False):
        ...
        p1, p2 = max(pairs, key=lambda pair: sum(
            (coord1 - coord2) ** 2 for (coord1, coord2) in zip(pair[0].coordinates, pair[1].coordinates)))
    ...
    def shortest_distance(self, get_points=False):
        ...
        p1, p2 = min(pairs, key=lambda pair: sum(
            (coord1 - coord2) ** 2 for (coord1, coord2) in zip(pair[0].coordinates, pair[1].coordinates)))
...
class Point2DCollection(PointCollection):
    ...
    def scatter_with_regression(self, show=True):
        ...
        linear_function = self.linear_regression()  # a lambda expression is returned
        min_x, max_x = self.min_coord_at(0) - 5, self.max_coord_at(0) + 5
        ax.plot((min_x, max_x), (linear_function(min_x), linear_function(max_x)))
        if show:
            plt.show()

    @staticmethod
    def random(num_of_points: int, coordinates_range: Tuple[float, float], ints_only=False):
        start, end = coordinates_range
        random_generator = random.randint if ints_only else random.uniform
        return Point2DCollection(
            [[random_generator(start, end), random_generator(start, end)] for _ in range(num_of_points)])
...
class Vector:
    ...
    def intersection(self, other: "Union[Vector, VectorCollection, Surface]", get_points=False):
        if self._direction_vector == other._direction_vector:
            # The vectors have the same directions, unhandled case for now
            return NotImplementedError
...
def surface_from_str(input_string: str, get_coefficients=False):
    ...
    for first_index, second_value in zip(range(len(first_coefficients)), second_coefficients):
        first_value = first_coefficients[first_index]
        first_coefficients[first_index] = float(first_value) - float(second_value)
    if get_coefficients:
        return first_coefficients
        ...
        if matrix is not None:
            if isinstance(matrix, str):
                ...
                if isinstance(matrix[0], Iterable):  # Find a better type hint
                    ...
                    self._matrix = [copy_expression(item) for item in matrix]  # TODO: should i allow this?
                    self._num_of_rows, self._num_of_columns = 1, len(self._matrix)
