...
from offline_model_builder.user_profile.constants import \
    ...
    CUSTOMER_MODIFIED_ON, GENDER, LABEL, CENTROID, PROPERTIES, IS_PAYTV, IS_PAY_TV
from ingestor.common.constants import DURATION, CONTENT_DURATION
...
class CentroidGenerator(InterCentroidDistance):
    ...
    def compute_centroids(
        ...
        for method in NON_FEATURES:
            ...
            ConnectS3().write_csv_to_s3(bucket_name=s3_bucket_name,
                                        object_name=s3_object_name + status + method + "_centroids_feature.csv",
                                        df_to_upload=centroids_df.drop([method], axis=1),
                                        resource=resource)

            ConnectS3().write_csv_to_s3(bucket_name=s3_bucket_name,
                                        object_name=s3_object_name + status + method + "_users_clustering_feature.csv",
