...
class LocaleQuery:
    ...
    async def try_update(self, newval, dt = datetime.now()):
        _dt = (dt.day, dt.hour)
    ...
    async def _update_peaks(self):
        ...
        _LOGGER.info(f"peaks are now: observed:{self.observed_peak}, charged:{self.charged_peak}")

    async def reset_values(self, newval, dt):
        self._peaks.p.clear()
        await self.try_update(newval, dt)
