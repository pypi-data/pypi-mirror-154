from PAMI.geoReferencedPeriodicPattern import abstract as _ab


class GPPMiner(_ab._geoReferencedPeriodicPatterns):
    """
    ...
    ------------------------------
        Format:
            python3 GPPMiner.py <inputFile> <outputFile> <neighbourFile> <maxDist> <maxIAT> <minPS> <cutIAT>
        Examples:
            python3 GPPMiner.py sampleTDB.txt output.txt sampleN.txt 0.5 0.3 (minSup & maxPer will be considered in percentage of database transactions)

            python3 GPPMiner.py sampleTDB.txt output.txt sampleN.txt 5 3 (minSup & maxPer will be considered in support count or frequency)
                                                                (it considers "\t" as separator)

            python3 GPPMiner.py sampleTDB.txt output.txt sampleN.txt 3 ',' (it will consider "," as a separator)

    Sample run of importing the code :
    -------------------------------

        import PAMI.geoReferencedPeriodicpattern.GPPMiner as alg

        obj = alg.GPPMiner("sampleTDB.txt", "sampleN.txt", 5, 3, 4, 8)

        obj.startMine()

        geoReferencedPeriodicPatterns = obj.getPatterns()

        print("Total number of Periodic Spatial Frequent Patterns:", len(geoReferencedPeriodicPatterns))
