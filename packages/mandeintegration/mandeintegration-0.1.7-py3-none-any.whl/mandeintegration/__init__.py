...
class MandEIntegration:
    ...
    def format_project_data(self, project_data: Any) -> Project:
        ...
        def format_date(date: str) -> str:
            return datetime.strftime(datetime.strptime(date, '%m/%d/%Y'), '%Y-%m-%d') if date else ""
        
        def format_status(status: str) -> str:
            return status.upper().replace(" ", "") if status else ""
        
        def format_name(name: str) -> str:
            return name.lower().strip() if name else ""
        
        project = Project(code, format_name(name), format_date(start_date), format_date(end_date), format_amount(budget_total_amount), format_status(status), grant_type, format_country(country))
