...
def construct_performance_df(df, seconds_gran, location_slots, time_slots, distance, height_satellite, 
                             hours_ahead, weather, generate_weather, api):
    max_cloud_cover = 50
    ...
    def cloud_gen(lat, long, parameter, alpha = 3, beta =4.5):
        ...
        #cloud_stoc2=cloud
        cloud_stoc1 = cloud + parameter*np.random.randint(-10,10) #to generate bias in both ends
        cloud_stoc2 = max(min(cloud_stoc1 + np.random.randint(-5,5),100),0)
        return(cloud_stoc2)
    ...
    if (weather == False and generate_weather == True):
        ...
        for i in range(0,len(performance_df)):
            ...
            w_i = max(min(unique_weather[which_unique_loc] + np.random.randint(-5,5), 100),0)
            error_time = np.random.randint(-10,10)*performance_df['uncertainty'][i]
    ...
    if weather == True:
        ...
        if weather_type == "forecast":
            import requests
            if api == None:
                print("API KEY MISSING FOR OWM")
            else:
                API_key = api     
            
            unique_locs = np.unique(performance_df["request location"])
            ##for more API keys - the free will only suppy max of 50 cloud coverage requests
            ...
            for i in range(0,len(performance_df)):
                performance_df["cloud cover real"][i] = max(min(performance_df["cloud cover estimate"][i]+(n/len(location_slots[0]))*np.random.randint(-20,20), 100),0)
