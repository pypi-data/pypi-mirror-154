...
class PyocsConfig:
    ...
    def __init__(self):
        ...
        self.client = ApolloClient(app_id="NAOt2HhY", cluster="default", config_server_url=config_server_url,
                              cache_file_path=os.environ['HOME'])
        self.client.start()
    @staticmethod
    def get_config():
        config = dict()
        ...
        config['FAAS_URL'] = client.get_value(key="FAAS_URL", default_val="DefaultValue",
                                                       namespace="platform.pyocs")
        PyocsConfig.delete_cache()

        return config

    @staticmethod
    def delete_cache():
        if os.path.exists(os.environ['HOME'] + "/NAOt2HhY_configuration_application.txt"):
            os.remove(os.environ['HOME'] + "/NAOt2HhY_configuration_application.txt")
        if os.path.exists(os.environ['HOME'] + "/NAOt2HhY_configuration_platform.pyocs.txt"):
            os.remove(os.environ['HOME'] + "/NAOt2HhY_configuration_platform.pyocs.txt")
