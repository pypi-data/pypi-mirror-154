...
global_dict = {'db':None,'passwd_v2':None}
...
class yxsFile:
    ...
    def __init__(self,filename,workdir=None,passwd=None,passwd_v2=None):
        if passwd is None:
            passwd = encrypt.get_default_passwd()
        self.filename = Path(str(filename))
        self.title_size = 2048
        self.passwd = passwd
        if passwd_v2 is None:
            passwd_v2 = global_dict['passwd_v2']
        self.passwd_v2 = passwd_v2
    ...
    def to_pureFile(self,to_dir = None):
        if not self.is_pureFile:
            ...
            if tts[0:2] == b'/{' or tts[0] == b'\0':
                jds = tts.replace(bytes([0]),b'').decode()[1:]
                info = json.loads(jds)
                title = info['name']
                version = info['ver']
                decodef = yxs_encrypt(self.passwd_v2)
                ipos = 0
            else:
                title = tts.replace(bytes([0]),b'').decode()
                version = 1
    ...
    def to_yxsFile(self,to_dir=None):
        if self.is_pureFile:
            ...
            if yxs_filenme.is_file():
                os.remove(yxs_filenme)
            ver = 1
            passwd_v2 = self.passwd_v2
            if passwd_v2:
                encodef = yxs_encrypt(passwd_v2)
                ipos = 0
                ver = 2 
            tempfn = yxs_filenme.with_suffix('.temp_YXS')
            fpxs = open(tempfn,'wb')
            fp   = open(self.filename,'rb')
            info = {'name':self.filename.name,'ver':ver}
            jds = b'/'+json.dumps(info,ensure_ascii=False).encode()
            tbytes = bytes([0]*(self.title_size-len(jds)))
            t = jds+tbytes
            while t != b'':
                fpxs.write(t)
                t = fp.read(1024*1024*32)
                if passwd_v2:
                    t = encodef.encode(t,ipos)
                    ipos += len(t)
            fpxs.close()
...
@click.option('--passwd_v2',default='',help="设置密码")
def main(args=None,filename=None,pure=False,xfile=False,find=None,decode=False,encode=False,update_db=False,db=None,delete=False,transfer='',
        suffix='*',encode_dir=False,passwd_v2=None):
    suffix_list = suffix
    key_option = 0
    global_dict['db'] = db
    global_dict['passwd_v2'] = passwd_v2
    if update_db:
        md = file_database()
