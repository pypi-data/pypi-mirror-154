...
class Convener:
    ...
    def __job(self) -> pd.DataFrame:
        ...
        queue = Queue()
        tasks = [self.__experiment.copy(self.__seed, True if i != 0 else self.__experiment._quiet) for i in range(n_parallel)]
        ...
        if len(rows) == 0:
            raise ValueError("Experiment processes produced no outputs. There may have been errors in the subprocesses that caused a crash.")
