...
class StandardTests:
    ...
    def __init__(self, atlas, work_dir, n_parallel_set=None, skip_download=True):
        self.atlas = atlas
    ...
    def _reader_test(self, reader, n, x_max, name=None):
        ...
        e = Experiment(lambda x: reader().select('*').limit(x), [1, x_max], skip_download=self.skip_download)
    ...
    def _file_read_test(self, reader, test_data_path, n, x_max, name=None):
        ...
        e = Experiment(fn, [1, x_max], skip_download=self.skip_download)
        name = reader.get_name() if name is None else name
        conveners = []
    ...
    def _select_test(self, reader, n, x_max, m, name=None):
        ...
        e = Experiment(lambda x: r.select(*cols).limit(x), [1, x_max], skip_download=self.skip_download)
        conveners = []
    ...
    def _join_test(self, reader, n, x_max, m, name=None):
        ...
        test_data = r.select(*r.get_columns()[:m]).limit(x_max).to_table_var('test_data')

        tv = test_data.select('*').limit(x_max).to_table_var('x')
        ...
        convener_pairs = []
        e = Experiment(build_fn, [1, x_max], skip_download=self.skip_download)
        e_base = Experiment(baseline_fn, [1, x_max], skip_download=self.skip_download)

        for n_p in self.n_parallel_set:
            c = Convener(e, work_dir, name + f'_NP{n_p}', n, n_parallel=n_p)
            c_base = Convener(e_base, work_dir, name + f'_NP{n_p}_baseline', n, n_parallel=n_p)
            convener_pairs.append((c, c_base))

        return convener_pairs

    def _writer_test(self, writer, test_data_path, n, x_max, name=None):
        work_dir = self.work_dir + '/writers'
        csv = self.atlas.drive_csv(file=test_data_path, apply_limit=x_max)
        ...
        def writer_fn(x):
            tv2 = csv.select('*').limit(x).to_table_var()
            return writer(to_write=tv2).select('*')

        baseline_ex = Experiment(baseline_fn, [1, x_max], skip_download=self.skip_download)
        writer_ex = Experiment(writer_fn, [1, x_max], skip_download=self.skip_download)
        convener_pairs = []
    ...
    def _file_write_test(self, writer, file_type, n, x_max, name=None):
        ...
        tv = t.select(*cols).limit(x_max).to_table_var()
        ...
        def writer_fn(x):
            ...
            return writer(tv2, type=file_type, path='/honeycomb/testing/', file_names='luminesceTest').select('*')
