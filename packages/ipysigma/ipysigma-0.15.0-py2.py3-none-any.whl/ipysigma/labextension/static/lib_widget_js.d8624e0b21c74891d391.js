...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.VisualVariableScalesBuilder = exports.CategorySummary = exports.AttributeHierarchicalCategories = exports.AttributeCategories = exports.AttributeExtents = exports.Extent = exports.HierarchicalMultiSet = void 0;
const multi_set_1 = __importDefault(__webpack_require__(/*! mnemonist/multi-set */ "./node_modules/mnemonist/multi-set.js"));
const default_map_1 = __importDefault(__webpack_require__(/*! mnemonist/default-map */ "./node_modules/mnemonist/default-map.js"));
 ...
 * Helper classes.
 */
class HierarchicalMultiSet {
    constructor() {
        this.container = new default_map_1.default(() => new multi_set_1.default());
    }
    add(value) {
        this.container.get(value[0]).add(value[1]);
    }
    firstLevel() {
        const values = new multi_set_1.default();
...
exports.HierarchicalMultiSet = HierarchicalMultiSet;
class Extent {
    constructor() {
        this.min = Infinity;
        this.max = -Infinity;
...
class AttributeHierarchicalCategories {
    constructor(names) {
        this.attributes = {};
        // NOTE: this naturally deduplicates names
        names.forEach((name) => (this.attributes[name] = new HierarchicalMultiSet()));
    }
    add(attributes) {
        for (const name in this.attributes) {
            const value = attributes[name];
...
exports.AttributeHierarchicalCategories = AttributeHierarchicalCategories;
class CategorySummary {
    ...
    static fromTopValues(name, frequencies, defaultColor) {
...
class VisualVariableScalesBuilder {
    constructor(visualVariables) {
        ...
        this.nodeHierarchicalCategories = new AttributeHierarchicalCategories(nodeHierarchicalCategoryAttributes);
    }
    readGraph(graph) {
        ...
        for (const variableName in this.variables) {
            ...
            // Category variables
                ...
                const summary = variable.palette
                    ...
                    : CategorySummary.fromTopValues(variable.attribute, categories.attributes[variable.attribute], variable.default || '#ccc');
                const palette = summary.palette;
                // TODO: this is ungodly
                scale = (attr) => palette.get(Array.isArray(attr[variable.attribute])
            ...
            // Hierarchical category
                ...
                const hierarchy = this.nodeHierarchicalCategories.attributes[variable.attribute];
                const summary = CategorySummary.fromTopValues(variable.attribute, hierarchy.firstLevel(), variable.default || '#ccc');
                const firstLevelPalette = summary.palette;
                const secondLevelPalettes = new Map();
                firstLevelPalette.forEach((color, firstLevelValue) => {
                    ...
                    const values = Array.from(secondLevel.top(count).map((item) => item[0]));
                    const lightColor = (0, chroma_js_1.default)(color).mix('#fff', 0.7).hex();
                    const subScale = (0, d3_scale_1.scaleLinear)()
                        .domain([0, values.length - 1])
                        .range([color, count > 1 ? lightColor : color]);
                    const colors = values.map((_, i) => subScale(i));
                    const mapping = new Map();
                    values.forEach((value, i) => {
                        mapping.set(value, colors[i]);
                    });
                    const secondLevelPalette = palette_1.default.fromMapping(firstLevelValue, mapping, '#999');
    ...
    render() {
        ...
        for (const metric in nodeMetrics) {
            ...
            if (metric === 'louvain' || metric === 'hlouvain') {
                const details = graphology_communities_louvain_1.default.detailed(graph, {
                ...
                const penultimateLayer = details.dendrogram.length >= 3
        ...
        this.displayed.then(() => {
            ...
            const scaleBuilder = new visual_variables_1.VisualVariableScalesBuilder(visualVariables);
