...
class adbClass():
    ...
    def myiperf_client(self, host, port=None, count=None, otherpara="-w 2M -P 3 -t 3"):
        _info = {}
        ...
        ret = self.sendshell("%s -c %s%s%s -f m %s" % (cmdfile, host, portstr, countstr, otherpara),
                             addtimeout=set_time + 10)
        if ("-P" in otherpara):
        ...
        try:
            ...
            return last_data, max(sec_data) if sec_data else last_data, sec_data, _info
    ...
    def boot_ready(self, name="com.android.tv.settings", bootdelay=10):
        ...
        logger.debug("[Check]%s waitting %s.." % (self.portpath, name))
        return False
    ...
    def checkready(self):
        ...
        for b in range(20):  # 20秒内的启动检测
            _boot = self.boot_ready(bootdelay=6)  # setting 启动后6秒再开始通信
            if _boot:
                break
            else:
                time.sleep(1)
