...
class BaseEnvironnement(BaseEstimator):
    ...
    def fit(
        ...
        **Attention pour les TPU** :
        ...
        if control.device.type != "tpu" and hasattr(self, 'post_move_to_tup'):
            self.post_move_to_tup()
        state_in_loss = list(
            filter(
                lambda x: 
                    (
                ...
                (
                    loss_fn.__code__.co_varnames
                    if isinstance(loss_fn, FunctionType)
                    else loss_fn.__class__.forward.__code__.co_varnames
                )
            )
        )
        state_in_loss_kargs = list(
            filter(
                lambda x: x == 'kargs' or x == 'kwargs',
                (
                    loss_fn.__code__.co_varnames
                    if isinstance(loss_fn, FunctionType)
                    else loss_fn.__class__.forward.__code__.co_varnames
        ...
        try:
            ...
            if isinstance(e, KeyboardInterrupt):
                warn(
                    "\n"
                    "-------------------------------------\n"
                    "Fin du trining par KeyboardInterrupt.\n"
                    "-------------------------------------\n"
                )
            else:
                raise e
