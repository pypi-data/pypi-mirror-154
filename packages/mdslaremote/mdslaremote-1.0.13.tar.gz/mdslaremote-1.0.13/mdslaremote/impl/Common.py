...
class Common(object):
    ...
    # return url
    def getUrl(self, path, useUserID = False):

        path = '%s%s' % (self.address, path)
        if (useUserID): path = '%s?userid=%s' % (path, self.userId)

        return path

    def requestUrl(self, path, data = None, method = 'GET', queryParams = None, useUserID = True):
        dataEncode = None
        html = None
        if queryParams != None:
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=Common.TIMEOUT) as response:
                html = response.read()

            if Common.VERBOSE:
                print('HTML response:', type(html), html)
