...
class Test(Parent):
    ...
    def getresultlist(self, testID, resulttype, eventnumber):
        try:
            jsonResponse = self.requestUrl('/tests/%s/results' % (testID), queryParams={'resultType' :resulttype})
        except Exception as ex:
            if Parent.VERBOSE:
                print('Exception', ex)
            return '%s' % ex
    ...
    def getcurrenttestlist(self):
        ret = ''
        try:
            jsonResponse = self.requestUrl('/tests', queryParams= {'onlycurrent': True})
        except Exception as ex:
            if Parent.VERBOSE:
                print('Exception', ex)
            return '%s' % ex
    ...
    def getfuturetestlist(self):
        ret = ''
        try:
            jsonResponse = self.requestUrl('/tests', queryParams={'onlyfuture': True})
        except Exception as ex:
            if Parent.VERBOSE:
                print('Exception', ex)
            return '%s' % ex
    ...
    def pause(self, testID, restart):
        ...
        try:
            jsonResponse = self.requestUrl('/tests/%s/pause' % (testID), queryParams= {'dorestart' :restart})
        except Exception as ex:
            if Parent.VERBOSE:
                print('Exception', ex)
            return '%s' % ex
