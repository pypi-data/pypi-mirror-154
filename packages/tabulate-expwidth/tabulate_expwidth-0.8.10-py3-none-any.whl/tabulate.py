...
def _moin_row_with_attrs(celltag, cell_values, colwidths, colaligns, header=""):
    ...
    values_with_attrs = [
        "{0}{1} {2} ".format(celltag, alignment.get(
            a, ""), header + c + header)
        for c, a in zip(cell_values, colaligns)
    ]
...
def _format_table(fmt, headers, rows, colwidths, colaligns, is_multiline):
    ...
    if padded_rows and fmt.linebetweenrows and "linebetweenrows" not in hidden:
        ...
        append_row(lines, padded_rows[-1],
                   padded_widths, colaligns, fmt.datarow)
    else:
...
def _pprint_file(fobject, headers, tablefmt, sep, floatfmt, file, colalign):
    ...
    print(
        tabulate(table, headers, tablefmt,
                 floatfmt=floatfmt, colalign=colalign),
        file=file,
    )
