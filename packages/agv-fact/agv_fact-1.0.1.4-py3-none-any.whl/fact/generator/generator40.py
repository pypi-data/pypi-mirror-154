...
class GeneratorXmlFourDotZero(AbstractGeneratorXml):
    ...
    def generate_xml(self):
        ...
        if self.tipo_objeto == INGRESO or self.tipo_objeto == EGRESO:
        ...
        try:
            ...
            if len(self.impuestos_trasladados) > 0:
                ...
                for traslado in self.impuestos_trasladados:
                    total_traslados += float(traslado.get('importe'))
                    if traslado.get('tipo_factor') != 'Exento':
                        diferente_exento += 1
            ...
            if len(self.impuestos_retenidos) > 0:
                ...
                for retencion in self.impuestos_retenidos:
                    total_retenciones += float(retencion.get('importe'))
    ...
    def get_conceptos(self, conceptos_list):
        ...
        for concepto in conceptos_list:
            try:
                ...
                cfdi_concepto.setAttribute('ObjetoImp', concepto.get('objeto_impuesto'))  # Nuevo
                if self.tipo_objeto != TRASLADO and concepto.get('objeto_impuesto') == '02':
    ...
    def get_traslados_concepto(self, traslado_data):
        ...
        traslado.setAttribute('Impuesto', traslado_data.get('impuesto', ''))
        traslado.setAttribute('TipoFactor', traslado_data.get('tipo_factor', 'Tasa'))  # Tipo factor Tasa ó Cuota
        if traslado_data.get('tipo_factor') != 'Exento':
            traslado.setAttribute('TasaOCuota', str(traslado_data.get('tasa_cuota')))  # valor del impuesto que se envia
            traslado.setAttribute('Importe', str(traslado_data.get('importe')))
        return traslado
