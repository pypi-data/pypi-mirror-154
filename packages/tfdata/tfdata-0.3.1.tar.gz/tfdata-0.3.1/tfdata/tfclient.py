...
class TFclient():
    def __init__ (self,access_key='',slient=False,offline_path=None,auto_login=True):
        if access_key=='':
    ...
    def _get_hf(self,code,hf_type,date,timeout=200):
        ...
        arg = {'akey':self.__akey,'uid':self.__uid,'rt':hf_type,'code':code,'date':date,'at':code_type}  
    ...
    def get_1d(self,att,timeout=200,get_doc=False):
        ...
        arg = {'uid':self.__uid,'akey':self.__akey,'rt':'1d','att':att}
        re = tryrq(url = self.__ds_url+'/datarequest',retry=1,timeout=timeout,wait=1,request_type='post',json=arg)
        if isinstance(re,str):
            return f'服务器不接受数据请求:{re}'
        else:
    ...
    def _get_tstable(self,code,att,start_date=None,end_date=None,sample_size=None,timeout=200):
        ...
        if pd.Timestamp(start_date)>pd.Timestamp(end_date):
            return '结束日早于起始日'
        arg = {'code':code,'uid':self.__uid,'akey':self.__akey,'rt':'tstable','start_date':start_date,'end_date':end_date,'sample_size':sample_size}
