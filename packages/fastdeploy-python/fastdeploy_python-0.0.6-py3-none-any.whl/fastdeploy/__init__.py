...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        type=_text_type,
        default=None,
        help='Define soc for the platform, supports x86/x86-nvidia-gpu/arm/jetson.'
    )
    parser.add_argument(
        '--save_dir',
        type=_text_type,
...
def read_sources():
    ...
    if os.path.exists(os.path.join(user_dir, "fastdeploy_newest_sources.cfg")):
        os.remove(os.path.join(user_dir, "fastdeploy_newest_sources.cfg"))
...
def main():
    ...
    if args.list_models:
        print("Currently, FastDeploy supports {} models:".format(
            len(all_models)), all_models)
        return
    ...
    download_and_decompress(all_sources[args.model][args.platform][args.soc],
                            new_save_dir)
    os.remove(
        os.path.join(new_save_dir,
                     os.path.split(all_sources[args.model][args.platform][
                         args.soc])[-1]))
