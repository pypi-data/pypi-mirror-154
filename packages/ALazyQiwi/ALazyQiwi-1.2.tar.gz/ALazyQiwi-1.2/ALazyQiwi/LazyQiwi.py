...
class LazyQiwi(object):
    ...
    def __init__(self, token: str, phone: str = "", proxies: dict = None, provide_id: str = '99'):
    ...
    async def get_commission(self, amount: float, reciver: str) -> float or int:

        """Calculate commission to transfer money
        ...
        Arguments
        ---------
            amount : float // Transfer summ
            reciver : str // Reciver lol
            ...
            match self.proxies:
                ...
                case None:
                    async with httpx.AsyncClient(headers=self.headers) as client:
                        r = await client.post(f'https://edge.qiwi.com/sinap/providers/{self.provide_id}/onlineCommission', json=request_data)

                        if r.status_code != 200:
                            raise InvalidTokenError(r.json())
