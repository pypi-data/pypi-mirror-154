...
class Finder(ABC):
    ...
    @abstractmethod
    def find_element(self, locator_tuple: tuple, timeout: str, explicit_timeout: int = None, condition: object = None) -> WebElement:
        pass

    @abstractmethod
    def find_elements(self, locator_tuple: tuple, timeout: str, explicit_timeout: int = None, condition: object = None) -> List[WebElement]:
        pass
...
class FluentFinder(Finder):
    ...
    def find_element(self, locator_tuple: tuple, timeout: str, explicit_timeout: int = None) -> WebElement:
        try:
            t = self.timeouts[timeout.lower()] if timeout is not None else self.default_timeout
        except KeyError:
            raise ValueError(f'Invalid timeout variant: "{timeout}", use {self.timeouts.keys()}')
    ...
    def find_elements(self, locator_tuple: tuple, timeout: str, explicit_timeout: int = None) -> WebElement:
        try:
            t = self.timeouts[timeout.lower()] if timeout is not None else self.default_timeout
        except KeyError:
            raise ValueError(f'Invalid timeout variant: "{timeout}", use {self.timeouts.keys()}')
