...
class feature_combine:
    def __init__(self,a_features, b_features,seqs, 
                ara2vec_file=f"{os.environ['HOME']}/.local/zzd/lib/ara_node2vec_feature.txt",
                pdb2dpc_file=f"{os.environ['HOME']}/.local/zzd/lib/pdb2dpc.pkl",
                esm_mean_file=f"{os.environ['HOME']}/.local/zzd/lib/esm_mean.pkl",
                prottrans_file=f"{os.environ['HOME']}/.local/zzd/lib/prottrans_embs.pkl"):
        ...
        self.features = {
                ...
                "ct":ct,
                "cksaap":cksaap,
                "ara2vec":ara_node2vec(ara2vec_file),
                "pdb2dpc":pdb2dpc_dict(pdb2dpc_file),
                "esm_mean":esm_mean(esm_mean_file),
                "prottrans":prottrans(prottrans_file)
                }
        self.feature_shape = {
                "ac":210,
                "dpc":400,
