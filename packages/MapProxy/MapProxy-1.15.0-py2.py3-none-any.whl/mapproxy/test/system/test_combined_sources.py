...
class TestCoverageWMS(SysTest):
    ...
    # def test_combined_layers(self, app):
    #     common_params = (
    #         r"?SERVICE=WMS&FORMAT=image%2Fpng"
    #         "&REQUEST=GetMap&HEIGHT=200&SRS=EPSG%3A4326&styles="
    #         "&VERSION=1.1.1&BBOX=9.0,50.0,10.0,51.0"
    ...
    # def test_combined_same_fwd_req_params(self, app):
    ...
    #             (
    #                 {
    #                     "path": "/service_a?SERVICE=WMS&FORMAT=image%2Fpng"
    #                     "&REQUEST=GetMap&HEIGHT=200&SRS=EPSG%3A4326&styles="
    #                     "&VERSION=1.1.1&BBOX=9.0,50.0,10.0,51.0"
    ...
    # def test_combined_no_fwd_req_params(self, app):
    ...
    # def test_combined_mixed_fwd_req_params(self, app):
    #     # not merged to one request because fwd_req_params are different
    #     common_params = (
    #         r"/service_a?SERVICE=WMS&FORMAT=image%2Fpng"
    #         "&REQUEST=GetMap&HEIGHT=200&SRS=EPSG%3A4326&styles="
    #         "&VERSION=1.1.1&BBOX=9.0,50.0,10.0,51.0"
    #         "&WIDTH=200&transparent=True"
    #     )
    ...
    def test_combined_cached(self, app):
        common_params = (
            r"?SERVICE=WMS&FORMAT=image%2Fpng"
            "&REQUEST=GetMap&HEIGHT=256&SRS=EPSG%3A900913&styles="
            "&VERSION=1.1.1"
            "&WIDTH=256&transparent=True&layers=combined_cached"
        )

        with tmp_image((200, 200), color=(225,10,50,128), format="png", mode="RGBA") as img:
            # img = img.read()
            img = 1
            expected_req = [
                (
                    {"path": "/service_a" + common_params + "&BBOX=1095801.2374962866,6418264.391049679,1252344.2714243275,6574807.424977721&layers=a_one"},
                    {"body": img, "headers": {"content-type": "image/png"}},
                ),
                (
                    {"path": "/service_a" + common_params + "&BBOX=1095801.2374962866,6418264.391049679,1252344.2714243275,6574807.424977721&layers=a_two,a_three"},
                    {"body": img, "headers": {"content-type": "image/png"}},
                ),
                (
                    {"path": "/service_b" + common_params + "&BBOX=1095801.2374962866,6418264.391049679,1252344.2714243275,6574807.424977721&layers=b_one"},
                    {"body": img, "headers": {"content-type": "image/png"}},
                ),
                (
                    {"path": "/service_b" + common_params + "&BBOX=939258.2035682462,6574807.42497772,1095801.237496287,6731350.458905761&layers=b_one"},
                    {"body": img, "headers": {"content-type": "image/png"}},
                ),
                (
                    {"path": "/service_a" + common_params + "&BBOX=939258.2035682462,6574807.42497772,1095801.237496287,6731350.458905761&layers=a_two,a_three"},
                    {"body": img, "headers": {"content-type": "image/png"}},
                ),
                (
                    {"path": "/service_a" + common_params + "&BBOX=939258.2035682462,6574807.42497772,1095801.237496287,6731350.458905761&layers=a_one"},
                    {"body": img, "headers": {"content-type": "image/png"}},
                ),
                (
                    {"path": "/service_a" + common_params + "&BBOX=939258.2035682462,6574807.42497772,1095801.237496287,6731350.458905761&layers=a_four"},
                    {"body": img, "headers": {"content-type": "image/png"}},
                ),
                (
                    {"path": "/service_a" + common_params + "&BBOX=1095801.2374962866,6574807.42497772,1252344.2714243275,6731350.458905761&layers=a_four"},
                    {"body": img, "headers": {"content-type": "image/png"}},
                ),
                (
                    {"path": "/service_a" + common_params + "&BBOX=939258.2035682462,6418264.391049679,1095801.237496287,6574807.424977721&layers=a_four"},
                    {"body": img, "headers": {"content-type": "image/png"}},
                ),
                (
                    {"path": "/service_a" + common_params + "&BBOX=1095801.2374962866,6418264.391049679,1252344.2714243275,6574807.424977721&layers=a_four"},
                    {"body": img, "headers": {"content-type": "image/png"}},
                ),
            ]

            with mock_httpd(("localhost", 42423), expected_req, True, False):
                    self.common_map_req.params.layers = "combined_cached"
                    self.common_map_req.params.transparent = True
                    resp = app.get(self.common_map_req)
                    assert resp.content_type == "image/png"
                    data = BytesIO(resp.body)
