...
class DSC_NSD_TEST(torch.nn.Module):
    def __init__(self, seg_path, gt_path, save_path, save_name):
        super().__init__()
    ...
    def forward(self):
        filenames = os.listdir(self.seg_path)
        ...
        for name in filenames:
            ...
            gt_nii = nb.load(join(self.gt_path, name))
            case_spacing = gt_nii.header.get_zooms()
            gt_data = np.uint8(gt_nii.get_fdata())
            seg_data = nb.load(join(self.seg_path, name)).get_fdata()
        ...
        dataframe.to_csv(join(self.save_path, self.save_name), index=False)
