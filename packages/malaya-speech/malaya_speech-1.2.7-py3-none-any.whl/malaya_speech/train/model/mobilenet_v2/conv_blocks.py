...
def _v1_compatible_scope_naming(scope):
    ...
    if scope is None:  # Create uniqified separable blocks.
        ...
        ) as s, tf.name_scope(s.original_name_scope):
            yield ''
    else:
...
def split_conv(
    input_tensor, num_outputs, num_ways, scope, divisible_by=8, **kwargs
):
  ...
  """
    ...
    if (
        num_ways == 1
        or min(b // num_ways, num_outputs // num_ways) < divisible_by
    ):
        # Don't do any splitting if we end up with less than 8 filters
        # on either side.
        return slim.conv2d(
            input_tensor, num_outputs, [1, 1], scope=scope, **kwargs
        )
    ...
    inputs = tf.split(
        input_tensor, input_splits, axis=3, name='split_' + scope
    )
    base = scope
...
def expanded_conv(
  ...
  """
    ...
    with tf.variable_scope(
    ...
    ) as s, tf.name_scope(s.original_name_scope), slim.arg_scope(
        (slim.conv2d,), **conv_defaults
    ), slim.arg_scope(
        ...
        if depthwise_location not in [None, 'input', 'output', 'expansion']:
            raise TypeError(
                '%r is unknown value for depthwise_location'
                % depthwise_location
            )
        if use_explicit_padding:
            if padding != 'SAME':
    ...
    with tf.variable_scope('squeeze_excite'):
        ...
        if pool is None:
            pooled = tf.reduce_mean(
                squeeze_input_tensor, axis=[1, 2], keepdims=True
            )
        else:
            pooled = tf.nn.avg_pool(
                squeeze_input_tensor,
        ...
        if pool is not None:
            ...
            excite = tf.image.resize_images(
                excite, input_size, align_corners=True
            )
