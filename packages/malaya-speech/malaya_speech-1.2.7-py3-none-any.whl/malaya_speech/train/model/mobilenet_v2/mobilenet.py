...
def mobilenet_base(  # pylint: disable=invalid-name
  ...
  """
    ...
    if use_explicit_padding:
        ...
        conv_defs_overrides[(slim.conv2d, slim.separable_conv2d)] = {
            'padding': 'VALID'
        }
    ...
    with _scope_all(scope, default_scope='Mobilenet'), safe_arg_scope(
        ...
        for i, opdef in enumerate(conv_defs['spec']):
            ...
            try:
                ...
                print(
                    'Failed to create op %i: %r params: %r' % (i, opdef, params)
                )
                raise
...
def mobilenet(
  ...
  """
    ...
    with tf.variable_scope(scope, 'Mobilenet', reuse=reuse) as scope:
        ...
        net, end_points = mobilenet_base(
            inputs, scope=scope, **mobilenet_args
        )
        if base_only:
            return net, end_points
        ...
        with tf.variable_scope('Logits'):
            ...
            if not num_classes:
                return net, end_points
            net = slim.dropout(
                net, scope='Dropout', is_training=is_training
            )
            # 1 x 1 x num_classes
