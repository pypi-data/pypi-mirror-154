...
def get_base_serializer(base_model, base_fields='__all__', auto_generate_annotate_fields=None):
    ...
    if not pure.convert_query_parameter_to_bool(auto_generate_annotate_fields):
        ...
        elif isinstance(auto_generate_annotate_fields, str):
            auto_generate_annotate_fields = [auto_generate_annotate_fields]

        field_names = [dc.name for dc in base_model._meta.fields]

        # --- 这里要把queryset里有, 但model.fields里没有的字段在serializers时自动加上
        new_dc_ls = []
        new_func_ls = []
        for field in base_fields:
            if field not in field_names and field in auto_generate_annotate_fields:
