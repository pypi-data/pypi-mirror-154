...
async def get(hub, ctx, name: str, resource_id: str):
    ...
    if resource_id is None or name is None:
        result["result"] = False
        result["comment"] = (
            "name and resource_id cannot be None for aws.ec2.subnet get()",
        )
        return result
...
async def search(
    ...
    Args:
        ...
        tags(Lict, optional): The list of tags to filter by. For example, to find all resources that have a tag with the key
    ...
    if not ret["ret"]["Subnets"]:
        result["comment"].append(f"Unable to find aws.ec2.subnet '{name}'")
        return result
