...
async def present(
    ...
    **Autogenerated function**
    ...
    if not before:
        ...
        new_res = before["ret"]
    update_ret = await hub.exec.aws.ec2.subnet.update_subnet_attributes(
        ctx,
        before=new_res,
...
async def search(
    ...
    """
    hub.log.warning(
        f"aws.ec2.subnet.search '{name}' state has been deprecated. Please use exec.run state instead."
    )
    result = dict(comment=[], old_state=None, new_state=None, name=name, result=True)
    ret = await hub.exec.aws.ec2.subnet.search(
        ctx=ctx,
        name=name,
        resource_id=resource_id,
        availability_zone=availability_zone,
