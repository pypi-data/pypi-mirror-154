...
#
#  This script is part of HectorP 0.0.3
#
...
def create_ctl_file(fname,station,noisemodels):
...
def main():
    ...
    print("\n***************************************")
    print("    findoffsets, version 0.0.3")
    print("***************************************")
    ...
    create_ctl_file(fname,stations[0],noisemodels)
    ...
    while True:
        ...
        for station in stations:
            ...
            create_ctl_file(fname,station,noisemodels)
            control = Control(fname)
            observations = Observations()
            for mjd in new_offsets:
                observations.add_offset(mjd) # add new offsets to class
            ...
            if verbose==True:
                ...
                mjd       = observations.data.index[index] 
                print('==> {0:s} - best offset at {1:9.2f} : {2:9.3f}'.\
						format(station,mjd,max_value))
