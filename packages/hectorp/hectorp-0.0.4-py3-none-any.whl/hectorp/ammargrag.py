...
#
#
# This file is part of HectorP 0.0.4.
#
...
class AmmarGrag(metaclass=SingletonMeta):
    ...
    def compute_leastsquares(self,t,H,x,F,samenoise=False):
        ...
        if samenoise==False:
            ...
            self.z  = np.zeros(m)
            Fx      = fft.fft(np.concatenate((xm,self.z)))
            self.y1 = (fft.ifft(self.Fl1 * Fx).real)[0:m]
            self.y2 = (fft.ifft(self.Fl2 * Fx).real)[0:m]
            ...
            if k>0:
                ...
                for i in range(0,k):
                    FF = fft.fft(np.concatenate((F[:,i].T,self.z)))
                    self.G1[i,:] = (fft.ifft(self.Fl1 * FF).real)[0:m]
                    self.G2[i,:] = (fft.ifft(self.Fl2 * FF).real)[0:m]
        ...
        for i in range(0,n):
            FH = fft.fft(np.concatenate((Hm[:,i].T,self.z)))
            A1[i,:] = (fft.ifft(self.Fl1 * FH).real)[0:m]
            A2[i,:] = (fft.ifft(self.Fl2 * FH).real)[0:m]
