...
def get_intdash_data_filter():
    ...
    # 説明変数
    for i in ['L', 'R']:
        for j in ['LFV_INITIALCONTACTACCELERATION', 'LFV_INITIALCONTACTANGULARVELOCITY', 'LFV_TOEOFFACCELERATION', 'LFV_TOEOFFANGULARVELOCITY', 'LFV_SWINGPHASESTRIDEVELOCITY']:
            for m in ['MIN', 'MAX']:
                for p in ['X', 'Y', 'Z']:
                    data_id = i + '_' + j + m + '_' + p
                    for k in [1, 2, 3, 4]:
    ...
    # 3次元パラメータ
    for i in ['L', 'R']:
        for j in ['MRA_ACCELERATION', 'MRA_ANGULARVELOCITY', 'MRA_STRIDEGLOBALACCELERATION', 'MRA_STRIDEVELOCITY', 'MRA_STRIDEDISPLACEMENT', 'MRA_STANCEPHASEANGLE']:
            for p in ['X', 'Y', 'Z']:
                data_id = i + '_' + j + '_' + p
                for k in [1, 2, 3, 4]:
                    filters.append(DataFilter(
                        data_type=DataType.float.value, data_id=data_id, channel=k))
        for j in ['MRA_INITIALCONTACTFLAG', 'MRA_FOOTFLATFLAG', 'MRA_TOEOFFFLAG', 'MRA_VERTICALJUMPFLAG']:
            data_id = i + '_' + j
            for k in [1, 2, 3, 4]:
                filters.append(DataFilter(
                    data_type=DataType.float.value, data_id=data_id, channel=k))
