...
class AccessoryInterface(i2c.I2CBusMaster):
    ...
    def __init__(self, spm: spm.Spm):
        ...
        """
        super().__init__(spm_ctrl=spm, bus_id=i2c.I2CBusID.Undef, 
            instance_id=i2c.I2CBusMaster.I2CInstances.MAIN_APP, 
            master_type=i2c.I2CBusMaster.I2CMasterType.ACCESSORY_MASTER, 
            bus_speed=i2c.I2CBusMaster.I2CBusSpeed.kHz_200)
    ...
    def get_list_of_available_interfaces(self) -> list[str]:
        ...
        count_ai = self._test_obj.GetAccessoryInterfaceInUseCount
        for i in range(count_ai):
            self._list_of_found_ai.append(self._test_obj.GetAccessoryInterfaceInUseSerial(i))

        count_ai = self._test_obj.GetAccessoryInterfaceAvailableCount
        for i in range(count_ai):
            self._list_of_found_ai.append(self._test_obj.GetAccessoryInterfaceAvailableSerial(i))
    ...
    def select_port(self, port_nr: int):
        ...
        ret = self._test_obj.I2CSetupMetaData(0,1,self._bus_id,0x70,0,1) # Setup for switch(multiplexer)
        assert ret == 0, "Accessory Interface: Error: Cannot connect to bus switch"
        ret = self._test_obj.I2CIsConnected
        ...
        ret = self._test_obj.I2CWrite(0,1,[1 << port_nr])
    ...
    def is_slave_device_connected(self) -> bool:
        ...
        assert self._bus_id != i2c.I2CBusID.Unassigned, "Accessory Interface: Error: Slave Access, but connection not assigned"
        ret = self._test_obj.I2CSetupMetaData(0,1,self._bus_id,0x57,2,1) # Setup for eeprom
        assert ret == 0, "Accessory Interface: Error: Cannot setup device metadata"
        id_eeprom_connected = self._test_obj.I2CIsConnected
    ...
    def get_slave_device_id(self) -> AISlaveIDHeader:
        ...
        ret = self._test_obj.I2CSetupMetaData(0,1,self._bus_id,0x57,2,1) # Setup for eeprom
        assert ret == 0, "Accessory Interface: Error: Cannot setup device metadata"
        id_eeprom_data = self._test_obj.I2CReadEx(0, 19)
    ...
    def select_port_with_slave(self, serial_nr: str) -> bool:
        """ Auto select the port where a device with 'serial_nr' is connected
            ...
            returns True if found othervise False
        """
        device_found = False
        try:
    ...
    def get_bus_addr_of_ai_device(self, serial_no: str) -> int:
        ...
        count_ai = self._test_obj.GetAccessoryInterfaceInUseCount
        for i in range(count_ai):
            if serial_no == self._test_obj.GetAccessoryInterfaceInUseSerial(i):
                found_bus_addr = self._test_obj.GetAccessoryInterfaceInUseI2CBus(i)
                break
        ...
        if found_bus_addr == i2c.I2CBusID.Unassigned:
            count_ai = self._test_obj.GetAccessoryInterfaceAvailableCount
            for i in range(count_ai):
                if serial_no == self._test_obj.GetAccessoryInterfaceAvailableSerial(i):
                    found_bus_addr = self._test_obj.ActivateAccessoryInterfaceAvailable(i)
                    break
