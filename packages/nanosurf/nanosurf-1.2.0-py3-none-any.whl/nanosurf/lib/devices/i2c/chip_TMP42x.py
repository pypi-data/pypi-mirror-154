...
class Chip_TMP42X(i2c.I2CChip): 
    ...
    class ConverstionRate(enum.IntEnum):
        Convert_every_16s = 0
        Convert_every_8s = 1
        Convert_every_4s = 2
        Convert_every_2s = 3
    ...
    def __init__(self, bus_addr: int, **kwargs):
        super().__init__(bus_addr, offset_mode=self.I2COffsetMode.NoOffset,**kwargs)

    def start_measuring(self, rate: ConverstionRate = ConverstionRate.Convert_1_per_sec, active_sensors: ConfigReg2 = ConfigReg2.EnableLocal):
        self.ConverstionRate = rate
    ...
    def reg_conversion(self, rate: ConverstionRate):
