...
import nanosurf.lib.datatypes.sci_channel as sc
from PySide2.QtCore import Signal
from app import app_common, background_worker, module_base
from modules.scan_module import settings

class ScanData(background_worker.WorkerData):
    def __init__(self):
        self.stream = ss.SciStream(channels=2)
        self.scan_line_index = -1

class ScanFrameWorker(background_worker.SingleRunWorker):
    ...
    def do_work(self):
        ...
        if self._connect_to_controller():
            ...
            self.tick_time = 0.0
            while (self.next_scan_line < self.par_points_per_line) and not self.stop_request_flag:
                
                # wait for new data 
                self.tick_time = 0.0
                while (self.next_scan_line > (self.spm_scan.Currentline - 1)) and not self.stop_request_flag:
    ...
    def _read_scan_line(self, line: int, channel: int, forward=True, raw: bool = True) -> sc.SciChannel:
        ...
        line_str = line_str.replace(',','.') # needed in case the numbers are formated with a ',' as decimal separator (e.g. standard german windows number format setting)
        line_split_str = line_str.split(";")
        scan_data = [float(p) for p in line_split_str]
        return sc.SciChannel(scan_data, unit=unit_str)
