...
def daily_factor_on300500(fac,hs300=False,zz500=False,zz1000=False,gz2000=False,other=False):
    ...
    if fac.shape[0]/last.shape[0]>2:
        if hs300:
            df=pd.read_feather(homeplace.daily_data_file+'沪深300成分股.feather').set_index('date')
            df=df*fac
            df=df.dropna(how='all')
        elif zz500:
            df=pd.read_feather(homeplace.daily_data_file+'中证500成分股.feather').set_index('date')
            df=df*fac
            df=df.dropna(how='all')
        elif zz1000:
            ...
            tr=np.sign(tr)
            df1=(tr*pd.read_feather(homeplace.daily_data_file+'沪深300成分股.feather').set_index('date')).fillna(0)
            df2=(tr*pd.read_feather(homeplace.daily_data_file+'中证500成分股.feather').set_index('date')).fillna(0)
        ...
        if hs300:
            df=pd.read_feather(homeplace.daily_data_file+'沪深300成分股.feather').set_index('date').resample('M').last()
            df=df*fac
            df=df.dropna(how='all')
        elif zz500:
            df=pd.read_feather(homeplace.daily_data_file+'中证500成分股.feather').set_index('date').resample('M').last()
            df=df*fac
            df=df.dropna(how='all')
        elif zz1000:
            ...
            tr=np.sign(tr)
            df1=(tr*pd.read_feather(homeplace.daily_data_file+'沪深300成分股.feather').set_index('date').resample('M').last()).fillna(0)
            df2=(tr*pd.read_feather(homeplace.daily_data_file+'中证500成分股.feather').set_index('date').resample('M').last()).fillna(0)
...
def boom_four(df,minus=None,backsee=20,daily=False,min_periods=20):
    '''使用20天均值和标准差，生成4个因子'''
    min_periods=min(min_periods,backsee)
    if not daily:
...
def make_relative_comments(ret_fac,hs300=0,zz500=0,zz1000=0):
    if hs300:
        net_index=read_index_three()[0].iloc[:,0]
    elif zz500:
        net_index=read_index_three()[1].iloc[:,0]
    elif zz1000:
        net_index=read_index_three()[2].iloc[:,0]
    else:
