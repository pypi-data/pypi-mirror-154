        ...
        {{  addDTablev2(
            ...
            map(lambda x:
                ...
                [ [{",".join(map(lambda x: f'{{value:{nl}{x["name"]}{nl}}}', data["header"]))}]
                  {",".join(
            map(
                lambda row: f'[{",".join(map(lambda col: f"{{value:{nl}{empty if col is None else col}{nl},state:{ignore if col is None else dontignore}}}", row))}]', data["rowdata"]))}
                ]
                )}}"""
