...
from accnetbio.window.base import Pair as ecabPair
from accnetbio.net.Reader import reader as prrcreader
...
class bipartite(ecabPair.pair):
    ...
    def __init__(
        ...
        if kind == 'patch':
            self.to_dos = self.computlib.patch(length=patch_size)
        if kind == 'cross':
            self.to_dos = [
                [-1, 0], [1, 0], [0, 0], [0, 1], [0, -1],
            ]
    ...
    def pairids(self):
        ...
        -------
            1>. pairids
                ...
                2nd dimension: (2 * window_size + 1) * length of to_dos list
                    ...
                    |--->block 1.1.2:
                        assigning calc results to rest of amino acids.
                            |--->block 1.1.2.1:
                                ...
                                amino acid index minus number in to_dos
        ...
        for i in range(self.num_pairs):
            ...
            for j in range(num_aa_in_window):
                ...
                # #/*** block 1.1.2 ***/
                    ...
                    for k in range(num_pending):
                        # #/*** block 1.1.2.1 ***/
                        left = self.window_m_ids[i][0][j] - self.to_dos[k][0]
                        right = self.window_m_ids[i][1][j] - self.to_dos[k][1]
                        left_inf = left < 1
                        left_sup = left > n
                        right_inf = right < 1
                        right_sup = right > n
