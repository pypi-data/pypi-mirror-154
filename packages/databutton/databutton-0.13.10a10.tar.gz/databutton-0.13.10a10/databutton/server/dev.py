...
import click
import psutil
from anyio import CancelScope, create_task_group, move_on_after, open_process
...
class DatabuttonRunner:
    ...
    async def create_streamlit_watcher(self, task_status):
        ...
        try:
            ...
            with CancelScope(shield=True):
                logger.debug("cleaning up streamlit-watcher")
                await streamlit_watcher.clear()
                logger.debug("successfully cleaned up streamlit-watcher")
    ...
    async def run(self, debug=False):
        ...
        async with create_task_group() as tg:
            # This should be awaited so we know it succeeds before starting the webserver
            click.secho("starting streamlit apps", fg="cyan")
            await tg.start(self.create_streamlit_watcher, name="streamlit")
