...
class Scheduler(schedule.Scheduler):
    ...
    def load_schedules(self, components: "ArtifactDict", loop=None):
        if not loop:
        ...
        return loop.create_task(run_continously())
    ...
    def create(cls):
        ...
        if platform.system() == "Windows":
            loop.set_event_loop_policy(asyncio.WindowsProactorEventLoopPolicy())
        task = sched.load_schedules(components, loop)
        try:
