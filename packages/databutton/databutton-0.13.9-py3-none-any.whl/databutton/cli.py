...
def create(
    ...
    if res.ok:
        ...
        full_path = Path.cwd() / project_directory
        if login_info:
...
def start(open=False):
    ...
    # With the following line, we'll get a CancelledError on termination (and ctrl+c)
    # It's disabled for now, until we need it.
    # When enabled, it will _not_ wait for background tasks to complete.
...
def serve():
    ...
    uvicorn.run(
        "prod:app", app_dir=str(app_dir), reload=False, port=int(port), host="0.0.0.0"
    )
