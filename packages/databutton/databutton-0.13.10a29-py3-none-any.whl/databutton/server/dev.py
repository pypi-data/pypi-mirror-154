...
class StreamlitWatcher:
    ...
    async def start_process(self, uid: str, app: StreamlitApp) -> Process:
        ...
        print("Parent", psutil.Process(process.pid).parent())
        print("ParentPID", psutil.Process(process.pid).parent().pid)
        print("ParentPPID", psutil.Process(process.pid).ppid())
        print("ParentChild", psutil.Process(process.pid).children())
    ...
    async def stop_process(self, process: Process, app_filename: str):
        try:
            ...
            print("Parent", psutil.Process(process.pid).parent().pid)
            process.kill()
            await process.wait()
