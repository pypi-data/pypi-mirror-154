...
class KeyInfo:
    ...
    def __init__(self, pubkey: bytes, enc_symkey: bytes):
        ...
        try:
            m = verify(self.enc_symkey[64:], self.enc_symkey[64:], self.pubkey)
            if m != self.enc_symkey[64:]:
                raise CryptoError(f"Expected message {self.enc_symkey[64:].hex()}, "
                                  f"found {m.hex()}")
        except CryptoError as exc:
            raise CryptoError(
                f"Failed to verify signature {self.enc_symkey[64:].hex()} with "
                f"pk {self.pubkey}"
            ) from exc
