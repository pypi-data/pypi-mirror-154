...
# int cs_sgx__enclave_x25519_pubkey(unsigned char *pk);
crypto_lib.cs_sgx__enclave_x25519_pubkey.argtypes = [POINTER(c_ubyte)]
crypto_lib.cs_sgx__enclave_x25519_pubkey.restype = c_int
...
#                                     unsigned char sealed_symkey[static 144],
#                                     const unsigned char signer_pk);
crypto_lib.cs_sgx__encrypt_with_sealed_key.argtypes = [
    POINTER(c_ubyte), POINTER(c_ubyte), c_ulonglong, POINTER(c_ubyte), POINTER(c_ubyte)
]
...
def enclave_x25519_pubkey() -> bytes:
    """Get enclave's public key for X25519."""
    assert is_running_in_enclave()

    pk = (c_ubyte * 32)()

    if crypto_lib.cs_sgx__enclave_x25519_pubkey(cast(pk, POINTER(c_ubyte))) != 0:
        raise CryptoError("Failed to get enclave's X25519 pubkey!")
...
def enclave_encrypt(data: bytes, sealed_key: bytes, signer_pubkey: bytes) -> bytes:
...
def enclave_decrypt(encrypted_data: bytes,
    ...
    """Decrypt `encrypted_data` using the sealed symmetric key if signature
    is verified with `signer_pubkey`."""
    assert is_running_in_enclave()
