...
def create_title_and_route(method, extra_radii, opt=True, freq=True, ecp=False,
                           chelpg=False, extra_route=[], extra_title=""):
    """
    ...
    if freq:
        title += "FREQ "
        routecard += "freq=HPModes iop(7/33=3) "
    ...
    if chelpg:
        if extra_radii:
            routecard += "pop=(CHELPG, AtomDipole, ReadRadii) "
        else:
...
def subsititute_element(sub_list, labels, labels_nn):
    ...
    for it, val in enumerate(labels):
        if old in val:
            labels_nn[it] = xyzp.remove_numbers([new])[0]
            num = ''
            for c in labels[it]:
                if c.isdigit():
                    num += c
...
def create_com_file(charge, mult, f_name_head, labels, coords,
    ...
    # Unfreeze all atoms if frozen not given
    if not len(frozen):
        frozen = [0]*len(labels)
    ...
    with open(comfile, "w") as f:
        ...
        f.write(
            "%chk={}/{}.chk \n"
            .format(cwd, f_name_head)
        )
        f.write("%mem=MEMORYGB \n")
        f.write(routecard + "\n")
        f.write("\n")
        f.write(title + "\n")
        f.write("\n")
    ...
    if verbose:
        ...
        message = "Submit with \033[0;32m"
        message += "gaussian_suite submit {} NCORES \033[0m".format(comfile)
        print(message)

    return


def gen_input(f_name, labels, coords, charge, mult, method, bs_spec,
              ecp_spec={}, subiso=[], chelpg=False, opt=True,
              freq=True, extra_route=[], extra_title="", verbose=True,
              frozen=[]):
    """
    ...
    -----------
        ...
        subiso : list, optional
            ...
            e.g. Dy1 or Dy
        chelpg : bool, default False
            If True, enables decomposition of electronic potential using CHELPG
            scheme
        opt : bool, default True
    ...
    # Create basis list for this set of coordinates
