...
class Shioaji:
    ...
    def login(
        ...
        if self.simulation:
            self._real_person_id = person_id
            person_id = config.SJ_PAPIUSER_ID
            passwd = config.SJ_PAPIUSER_PASSWORD
            hashed = False
        ...
        if not self.simulation:
            ...
            self._solace.trace_log(
                person_id=self._real_person_id, action="login", accounts=accounts
            )
        return accounts
    ...
    def place_order(
        ...
        if self.simulation:
            self._solace.trace_log(
                person_id=self._real_person_id, action="place_order", trade=trade
            )
    ...
    def update_status(
        ...
        if trade:
            self._solace.update_status(
                trade.order.account, seqno=trade.order.seqno, timeout=timeout, cb=cb
            )
