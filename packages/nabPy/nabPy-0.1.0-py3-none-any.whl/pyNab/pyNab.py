...
try:
	...
	try:
		...
		print('deltaRice / NabCompression libaries unavailable')
...
import temperatureFileClass as tmf
...
class DataRun:
	def __init__(self, directory, runNumber, ignoreEventFile = False):
	...
	def __openWaveformFiles(self):
		self.__coincFiles = wf.waveformFile(self.__coincFilenames, pixelMapping = self.parameterFile().BoardChannelPixelMap['pixel'])
		self.__noiseFiles = wf.waveformFile(self.__noiseFilenames, pixelMapping = self.parameterFile().BoardChannelPixelMap['pixel'])
		self.__pulsrFiles = wf.waveformFile(self.__pulsrFilenames, pixelMapping = self.parameterFile().BoardChannelPixelMap['pixel'])
		self.__singlFiles = wf.waveformFile(self.__singlFilenames, pixelMapping = self.parameterFile().BoardChannelPixelMap['pixel'])
