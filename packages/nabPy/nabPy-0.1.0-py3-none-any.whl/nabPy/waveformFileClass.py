		...
		if method is None or params is None:
			...
			if method=='trap': #handle the trap filter method
				...
				if not (len(params) >=3 and len(params) <= 6):
					...
					eners, times = bf.applyTrapFilter(self.waves(prep=False), *params, pretrigger=pretrig, batchsize=batchsize, useGPU = useGPU, shift = shift, mean = mean, hdf5ChunkSize = self.hdf5ChunkSize, rechunk=rechunk, useFFTW = useFFTW)
					headers = None
