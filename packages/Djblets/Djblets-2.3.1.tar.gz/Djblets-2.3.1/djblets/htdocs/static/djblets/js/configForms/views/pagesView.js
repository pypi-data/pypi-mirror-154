...
Djblets.Config.PagesView = Backbone.View.extend({
   ...
   */
  _onPageChanged: function _onPageChanged(pageID) {
    this._$activeNav.removeClass('-is-active');

    this._$activePage.removeClass('-is-active');

    this._$activePage = $("#page_".concat(pageID));
    ...
    if (this._$activePage.length === 0) {
      ...
      this._$activeNav = this._$pageNavs.filter(":has(a[href=\"#".concat(pageID, "\"])")).addClass('-is-active');
