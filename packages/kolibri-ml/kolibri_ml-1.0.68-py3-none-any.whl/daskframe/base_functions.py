...
class Functions(type):
    ...
    def __getattr__(self, func) -> callable:
        ...
        def _func(df_or_value, *args):
            # Get the internal series in the daskframe dataframe
            if isinstance(df_or_value, BaseDaskFrame):
                _df = df_or_value.data[df_or_value.cols.names(0)[0]]
                # Convert the frame to a dataframe and to a daskframe Dataframe
