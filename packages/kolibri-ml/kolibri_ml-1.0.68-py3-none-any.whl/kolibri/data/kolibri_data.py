...
"""
Functions to find and load Kolibri resource files, such as corpora,
models, and saved processing objects.  Resource files are identified
using URLs, such as ``corpora/abc/rural.txt``
"""
...
from kolibri.data.settings import path
try:
...
def normalize_resource_url(resource_url):
    ...
    try:
        ...
        # the resource url has no protocol, use the kolibri protocol by default
        protocol = "kolibri"
        name = resource_url
    # use file protocol if the path is an absolute path
    if protocol == "kolibri" and os.path.isabs(name):
        protocol = "file://"
...
class PathPointer(metaclass=ABCMeta):
    """
    An abstract base class for 'path pointers,' used by Kolibri's data
...
def find(resource_name, paths=None):
    ...
    the installation instructions for the Kolibri downloader.
    ...
    Zip File Handling:
      ...
      - If any element of ``kolibri.data.path`` has a ``.zip`` extension,
    ...
    msg = str(
        ...
        "Please use the Kolibri Downloader to obtain the resource:\n\n"
        "\33[31m"  # To display red text in terminal.
        ">>> import kolibri\n"
        ">>> kolibri.download('{resource}')\n"
        "\033[0m"
...
def retrieve(resource_url, filename=None, verbose=True):
    ...
    :param resource_url: A URL specifying where the resource should be
        loaded from.  The default protocol is "kolibri:", which searches
        for the file in the the Kolibri data package.
    """
...
#: A dictionary describing the formats that are supported by Kolibri's
...
def load(
    ...
    """
    Load a given resource from the Kolibri data package.  The following
    ...
    if format == "raw":
        ...
        if format == "text":
            ...
            raise AssertionError(
                "Internal Kolibri error: Format %s isn't "
                "handled by kolibri.data.load()" % (format,)
            )
...
def _open(resource_url):
    ...
    given its resource URL.  If the given resource URL uses the "kolibri:"
    protocol, or uses no protocol, then use ``kolibri.data.find`` to find
    ...
    if protocol is None or protocol.lower() == "kolibri":
