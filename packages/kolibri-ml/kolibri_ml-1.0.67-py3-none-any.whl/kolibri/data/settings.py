...
"""
utilities to find and load Kolibri resource files, such as corpora,
grammars, and saved processing objects.
"""
...
_paths_from_env = os.environ.get("KOLIBRI_DATA", "").split(os.pathsep)
path += [d for d in _paths_from_env if d]
if "APPENGINE_RUNTIME" not in os.environ and os.path.expanduser("~/") != "~/":
    path.append(os.path.expanduser("~/.kolibri"))
...
class PathPointer(metaclass=ABCMeta):
    """
    An abstract base class for 'path pointers,' used by Olibri's data
