    ...
    def prepare_for_correlation_stats(self):
        def make_info_collector(layer):
            ...
            if isinstance(layer, nn.Conv2d) or isinstance(layer, nn.Linear):
                ...
                layer.Ey = torch.zeros(layer.weight.size(0)).to(
                    layer.weight.device)  # init as floatTensor sometimes inits it with NaN values. wtf is that about
    ...
    def get_patterns(self):
        ...
        for layer in list(self.net.children()):
            if isinstance(layer, nn.Conv2d) or isinstance(layer, nn.Linear):
                ...
                num = Exy - ExEy
                denom = (layer.weight * Exy).view((num.size(0), -1)).sum(dim=1) - (layer.weight * ExEy).view(
                    (num.size(0), -1)).sum(dim=1)
