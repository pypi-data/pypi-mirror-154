...
#bn=nn.BatchNorm2d
class BatchNorm2d_nobias(nn.Module):
    def __init__(self,out_channels,momentum=0.999):
        super().__init__()
        #self.bn=nn.BatchNorm2d(out_channels,affine=False)
        self.alpha=nn.Parameter(torch.ones(1,out_channels,1,1))
        self.register_buffer('running_mean', torch.zeros(1, out_channels, 1, 1))
        self.register_buffer('running_var', torch.ones(1, out_channels, 1, 1))
    ...
    def forward(self, x):
        if not self.training:
            return x/torch.sqrt(self.running_var)*self.alpha
        self.running_mean = self.running_mean * self.momentum + mean(x).data * (
                    1 - self.momentum)
        self.running_var = self.running_var * self.momentum + mean((x - self.running_mean).data ** 2) * (1 - self.momentum)
        out=x/torch.sqrt(self.running_var)*self.alpha
        return out

def mean(x):
    temp=x.permute(1,0,2,3)
    temp=temp.reshape(x.size(1),-1)
    temp=torch.mean(temp,1)
    return temp.view(1,-1,1,1)
bn=BatchNorm2d_nobias
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, num_block, indim=3,num_classes=10):
        ...
        if indim==1:
            lastdim=8
        self.fc = nn.Linear(temp*4 * block.expansion, num_classes,bias=False)
        self.weights_init()
