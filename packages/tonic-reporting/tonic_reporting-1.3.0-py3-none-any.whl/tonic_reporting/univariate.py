...
def compute_empirical_cdf(samples:np.ndarray, ci_val = .95, sort=False):
...
def compare_stepwise_curves(x_0: np.ndarray, y_0:np.ndarray, x_1:np.ndarray, y_1:np.ndarray, lower_bound = 0):
    ...
    return np.all(np.logical_or( y_0[idx[comparable]] <= lower_bound, y_1[comparable] <= y_0[idx[comparable]] ))
...
def plot_all_marginals( real_data: pd.DataFrame,
    ...
    fig, axarr = plt.subplots(n_rows, 2)
    for col, ax in zip(numeric_columns + categorical_columns, axarr.ravel()):
