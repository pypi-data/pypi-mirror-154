...
class TestTrainValidatePredict(tf.test.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.pred_filepath = os.path.join(
            cls.pred_dir, "pred_model_epoch_1_file_3_on_listfilename_val_file_1.h5"
        )
        cls.file_sizes = [
            500,
        ]
    ...
    def setUp(self):
        ...
        self.orga.cfg.get_list_file = MagicMock(
            return_value="/path/to/a/listfilename.toml"
        )
    ...
    def test_train(self):
        ...
        self.assertSequenceEqual(
            batch_nos, list(range(int(self.file_sizes[0] / self.orga.cfg.batchsize)))
        )
    ...
    def test_predict(self):
        ...
        target_shapes = [(500,), (500,), (500, 1), (500, 1), (500,)]
        target_contents = [
...
def build_dummy_model(input_shapes, output_shapes):
    ...
    for name, shape in output_shapes.items():
        outputs.append(
            layers.Dense(
                shape, name=name, kernel_initializer="Ones", bias_initializer="Zeros"
            )(conc)
        )
