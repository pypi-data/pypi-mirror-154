...
class TestSummaryLogger(TestCase):
    ...
    def test_writing_summary_file_update_epoch_1(self):
        ...
        epoch = (1, 1)
        lr = 0.001
        self.smry.write_line(
            self.orga.io.get_epoch_float(*epoch), lr, history_train=history_train_0
        )
        ...
        lr = 0.002
        self.smry.write_line(
            self.orga.io.get_epoch_float(*epoch), lr, history_train=history_train_1
        )
        self._check_file(target)
        ...
        lr = np.nan
        self.smry.write_line(
            self.orga.io.get_epoch_float(*epoch), lr, history_val=history_val
        )
        self._check_file(target)
...
class TestBatchLogger(TestCase):
    def setUp(self):
        self.temp_dir = os.path.join(os.path.dirname(__file__), ".temp", "batch_logger")
    ...
    def _make_and_get_lines(self, epoch):
        ...
        logfile_path = "{}/log_epoch_{}_file_{}.txt".format(
            os.path.join(self.temp_dir, "train_log"), epoch[0], epoch[1]
        )
...
def build_test_model():
    ...
    x = layers.Dense(
        1, kernel_initializer="Ones", bias_initializer="Zeros", trainable=False
    )(inp)
    outp = layers.Dense(
        1,
        name="out",
