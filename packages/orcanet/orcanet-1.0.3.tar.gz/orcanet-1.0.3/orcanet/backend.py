...
def h5_inference(
    orga, model, files_dict, output_path, samples=None, use_def_label=True
):
    """
    ...
    file_size = h5_get_number_of_rows(
        list(files_dict.values())[0], datasets=[orga.cfg.key_y_values]
    )
    ...
    try:
        ...
        with h5py.File(temp_output_path, "x") as h5_file:
            ...
            for s in range(steps):
                if s % print_every == 0:
                    print(
                        "Predicting in step {}/{} ({:0.2%})".format(s, steps, s / steps)
                    )
                ...
                for dataset_name, data in datasets.items():
                    if s == 0:
                        # create datasets in the first step
                        h5_file.create_dataset(
                            dataset_name,
                            shape=(file_size,) + data.shape[1:],
                            dtype=data.dtype,
                            chunks=True,
                            compression="gzip",
                            compression_opts=1,
                        ...
                        dataset_last_element[dataset_name] = 0
    ...
    orga.io.print_log("Statistics of model prediction:")
    orga.io.print_log(f"\tTotal time:\t{model_time_total / 60:.2f} min")
    orga.io.print_log(f"\tPer batch:\t{1000 * model_time_total / steps:.5} ms")
    generator.print_timestats(print_func=orga.io.print_log)
