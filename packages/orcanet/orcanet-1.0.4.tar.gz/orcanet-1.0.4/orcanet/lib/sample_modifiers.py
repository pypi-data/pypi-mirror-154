...
class GraphEdgeConv:
    ...
    def _cache_column_names(self, x_dataset):
        try:
            self.column_names = [
                x_dataset.attrs[f"hit_info_{i}"] for i in range(x_dataset.shape[-1])
            ]
        except Exception:
    ...
    def __call__(self, info_blob):
        ...
        if np.any(n_items < self.knn + 1):
            x_values, n_items = _pad_disjoint(x_values, n_items, min_items=self.knn + 1)
        ...
        if self.ragged is True:
            ...
            return {
                "nodes": nodes_t.to_tensor(
                    default_value=0.0, shape=sh + [nodes_t.shape[-1]]
                ),
                "is_valid": tf.ones_like(nodes_t[:, :, 0]).to_tensor(
                    default_value=0.0, shape=sh
                ),
                "coords": coords_t.to_tensor(
                    default_value=0.0, shape=sh + [coords_t.shape[-1]]
                ),
            }
