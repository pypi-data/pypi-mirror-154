...
class TestIOHandler(TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.temp_dir = os.path.join(os.path.dirname(__file__), ".temp", "test_in_out")
        cls.pred_dir = os.path.join(
            os.path.dirname(__file__), ".temp", "test_in_out", "predictions"
        )
    ...
    def test_copy_to_ssd(self):
        ...
        try:
            ...
            target_dirs_train = {
                "input_A": (
                    scratch_dir + self.file_names[0],
                    scratch_dir + self.file_names[1],
                ),
                "input_B": (
                    scratch_dir + self.file_names[2],
                    scratch_dir + self.file_names[3],
                ),
            }
            target_dirs_val = {
                "input_A": (
                    scratch_dir + self.file_names[4],
                    scratch_dir + self.file_names[5],
                ),
                "input_B": (
                    scratch_dir + self.file_names[6],
                    scratch_dir + self.file_names[7],
                ),
            }
    ...
    def test_get_pred_path(self):
        ...
        target = (
            self.io.get_subfolder("predictions")
            + "/pred_model_epoch_1_file_2_on_in_out_test_list_val_file_3.h5"
        )
        self.assertEqual(value, target)
...
class TestIOHandlerLR(TestCase):
    ...
    def test_get_learning_rate_read_csv_file(self):
        ...
        for test in tests:
            self.assertEqual(self.get_learning_rate(test["inp"]), test["target"])
...
def save_dummy_h5py(path, shape, value_xs=1.0, value_ys=1.0, size=50):
