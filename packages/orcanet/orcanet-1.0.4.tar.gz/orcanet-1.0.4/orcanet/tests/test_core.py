...
class TestOrganizer(tf.test.TestCase):
    ...
    def setUp(self):
        ...
        self.temp_dir = self.tmpdir.name
    ...
    def test_inference_on_file(self):
        model_path = os.path.join(self.temp_dir, "temp_model.h5")
        input_file = os.path.join(self.temp_dir, "temp_inp_file.h5")
        model = build_test_model(
            compile=True, kernel_initializer=tf.keras.initializers.Constant(2.5)
        )

        model.save(model_path)
        with h5py.File(input_file, "w") as f:
            f.create_dataset(name="x", data=np.ones((40, 1)))
            f.create_dataset(
                name="y", data=np.core.records.fromarrays([np.zeros(40)], names=["out"])
            )

        outfile = self.orga.inference_on_file(input_file, saved_model=model_path)
        with h5py.File(outfile, "r") as f:
            np.testing.assert_array_equal(f["pred_out"][()], np.ones((40, 1)) * 2.5)
...
class TestConfiguration(TestCase):
    ...
    def test_load_list_file_no_train_files(self):
        cfg = Configuration("./", None, None)
        val_files = {"input_A": ("input_A_val_1.h5",), "input_B": ("input_B_val_1.h5",)}
        content = [
            "[input_A]",
            'validation_files = ["input_A_val_1.h5"]',
            "[input_B]",
