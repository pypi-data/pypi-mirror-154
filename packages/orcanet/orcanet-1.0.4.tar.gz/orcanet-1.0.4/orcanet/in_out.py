...
class IOHandler(object):
    ...
    def get_all_epochs(self):
        ...
        if os.path.exists(saved_models_folder):
            ...
            for file in files:
                ...
                f_epoch, file_no = file_base.split("model_epoch_")[-1].split("_file_")
    ...
    def get_previous_epoch(self, epoch):
        ...
        if epoch[1] == 1:
            if epoch[0] == 1:
                raise ValueError(
                    "Can not get previous epoch of epoch {} file {}".format(*epoch)
                )
    ...
    def get_model_path(self, epoch, fileno, local=False):
        ...
        if epoch < 1 or fileno < 1:
            raise ValueError(
                "Invalid epoch/file number {}, {}: Must be "
    ...
    def get_pred_path(self, epoch, fileno, pred_file_no):
        ...
        if list_file is None:
            raise ValueError(
                "No toml list file specified. Can not look up " "saved prediction"
            )
        list_name = os.path.splitext(os.path.basename(list_file))[0]

        pred_filepath = self.get_subfolder(
            "predictions"
    ...
    def get_file_sizes(self, which):
        ...
        for n, file_no_set in enumerate(self.yield_files(which)):
            ...
            file_sizes_full[n] = [
                h5_get_number_of_rows(file, datasets=[self.cfg.key_y_values])
                for file in file_no_set.values()
            ]
            if not file_sizes_full[n].count(file_sizes_full[n][0]) == len(
                file_sizes_full[n]
            ):
        ...
        if len(error_file_sizes) != 0:
            err_msg = (
                "The files you gave for the different inputs of the model "
                "do not all have the same number of samples!\n"
            )
            for n in error_file_sizes:
                err_msg += (
                    "File no {} in {} has the following files sizes "
    ...
    def check_connections(self, model):
        ...
        layer_inp_shapes = {
            key: layer_inputs[key].input_shape[0][1:] for key in layer_inputs
        }
        list_inp_shapes = self.get_n_bins()

        print(
            "The data in the files of the toml list have the following "
            "names and shapes:"
        )
        ...
        if len(err_inp_names) == 0 and len(err_inp_shapes) == 0:
            ...
            if len(err_inp_names) != 0:
                err_msg_inp += (
                    "No matching input name from the input files "
                    "for input layer(s): "
                    + (", ".join(str(e) for e in err_inp_names) + "\n")
                )
            if len(err_inp_shapes) != 0:
                err_msg_inp += (
                    "Shapes of layers and labels do not match for "
        ...
        print(
            "The following {} label names are in the first file of the "
        ...
        if self.cfg.label_modifier is not None:
            ...
            print(
                "The following {} labels get produced from them by your "
            ...
            print(
                "You did not specify a label_modifier. The output layers "
        ...
        if len(err_out_names) == 0:
            ...
            if len(err_out_names) != 0:
                err_msg_out += (
                    "No matching label name from the input files "
                    "for output layer(s): "
    ...
    def get_learning_rate(self, epoch):
        ...
        """
        error_msg = (
            "The learning rate must be either a float, a tuple of "
        ...
        try:
            ...
            if length != 2:
                raise LookupError(
                    "{} (Your tuple has length {})".format(error_msg, len(user_lr))
                )
        ...
        try:
            ...
            if n_params != 2:
                raise TypeError(
                    "A custom learning rate function must have two "
            ...
            raise TypeError(
                "{} (You gave {} of type {}) ".format(error_msg, user_lr, type(user_lr))
            )
...
def h5_get_number_of_rows(h5_filepath, datasets=None):
    ...
    if not number_of_rows.count(number_of_rows[0]) == len(number_of_rows):
        err_msg = (
            "Datasets do not have the same number of samples " "in file " + h5_filepath
        )
...
def use_local_tmpdir(files):
    ...
    for input_key in files:
        ...
        for f_path in old_pathes:
            ...
            f_path_ssd = os.path.join(local_scratch_path, os.path.basename(f_path))
