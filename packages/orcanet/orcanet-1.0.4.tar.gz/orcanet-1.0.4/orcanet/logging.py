...
class TrainfileLogger:
    ...
    def write_line(self, values):
        ...
        if len(values) != len(self.column_names):
            raise ValueError(
                "Can not log: Expected {} values, but got "
...
def gen_line_str(
    data, widths=None, seperator=" | ", float_precision=4, minimum_cell_width=9
):
    """
...
class SummaryLogger:
        ...
        if os.path.isfile(self.logfile_name):
            ...
            if not set(column_names) == set(file_column_names):
                raise NameError(
                    ...
                    "{}, from summary file: {}".format(column_names, file_column_names)
                )
    ...
    def _init_writing(self):
        ...
        if (
            not os.path.isfile(self.logfile_name)
            or os.stat(self.logfile_name).st_size == 0
        ):
...
def merge_arrays(base, supp, exclude=None):
    ...
    for i in range(len(base)):
        ...
        if base[i] == supp[i]:
            ...
            raise ValueError(
                "Cannot merge arrays at index {}: Base {}, supplement {}".format(
                    i, base[i], supp[i]
                )
            )
    return base
...
class BatchLogger(ks.callbacks.Callback):
    ...
    def _write_line(self):
        ...
        batch_frctn = (
            self.epoch_number
            - 1
            + (self.previous_batches + self.seen - self.display / 2.0)
            / self.total_batches
        )
