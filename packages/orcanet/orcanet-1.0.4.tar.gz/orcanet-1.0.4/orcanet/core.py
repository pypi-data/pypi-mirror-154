...
class Organizer:
    ...
    def train(self, model=None):
        ...
        for input_name, input_file in files_dict.items():
            self.io.print_log(
                "   {}: \t{}".format(input_name, os.path.basename(input_file))
            )
    ...
    def validate(self):
        ...
        if self.history.get_state()[-1]["is_validated"] is True:
            raise ValueError(
                "Can not validate in epoch {} file {}: "
    ...
    def predict(self, epoch=None, fileno=None, samples=None):
        ...
        """
        epoch, fileno = self._get_auto_epoch(epoch, fileno)
        if self._check_if_pred_already_done(epoch, fileno):
            ...
            start_time = time.time()
            backend.make_model_prediction(self, model, epoch, fileno, samples=samples)
            elapsed_s = int(time.time() - start_time)
    ...
    def inference(self, epoch=None, fileno=None, as_generator=False):
        ...
        """
        gen = self._inference(epoch=epoch, fileno=fileno)
        if as_generator:
            return gen
        else:
            return [filename for filename in gen]

    def _inference(self, epoch=None, fileno=None):
        """Get generator of doing inference, file by file."""
        epoch, fileno = self._get_auto_epoch(epoch, fileno)
        if self._stored_model is None:
            model = self.load_saved_model(epoch, fileno, logging=False)
        else:
        ...
        for files_dict in self.io.yield_files("inference"):
            ...
            output_path = os.path.join(
                self.io.get_subfolder("inference"), output_filename
            )
            filenames.append(output_path)
            if os.path.exists(output_path):
                print("File {} exists already, skipping...".format(output_filename))
                continue
    ...
    def inference_on_file(
        self, input_file, output_file=None, saved_model=None, epoch=None, fileno=None
    ):
        """
        ...
        if saved_model is None:
            ...
            model = self._load_model(saved_model)
        self._set_up(model)

        if isinstance(input_file, str):
            input_file = {model.input_names[0]: input_file}
        if output_file is None:
            out_path, first_filename = os.path.split(list(input_file.values())[0])
            output_file = os.path.join(out_path, "dl_pred_{}".format(first_filename))
        start_time = time.time()
        backend.h5_inference(
            orga=self,
            model=model,
        ...
        elapsed_s = int(time.time() - start_time)
        print(f"Finished inference in {elapsed_s / 60} min")
        return output_file
    ...
    def _get_auto_epoch(self, epoch, fileno):
        """Automatically retrieve best epoch/fileno if they are none."""
        if fileno is None and epoch is None:
            ...
            raise ValueError("Either both or none of epoch and fileno must be None")
        return epoch, fileno
    ...
    def get_xs_mean(self, logging=False):
        ...
        if self.xs_mean is None:
            if self.cfg.zero_center_folder is None:
                raise ValueError(
                    "Can not calculate zero center: " "No zero center folder given"
                )
    ...
    def _get_model(self, model, logging=False):
        ...
        if latest_epoch is None:
            ...
            if model is None:
                raise ValueError(
                    "You need to provide a compiled keras model "
    ...
    def _set_up(self, model, logging=False):
        ...
        if self.cfg.label_modifier is None:
            self._setup_auto_lmod(model)

        if self.cfg.zero_center_folder is not None:
            self.get_xs_mean(logging)

    def _setup_auto_lmod(self, model):
        """Set up the auto label modifier for the given model."""
    ...
    def val_is_due(self, epoch=None):
        ...
        val_sched = (epoch[1] == n_train_files) or (
            self.cfg.validate_interval is not None
            and epoch[1] % self.cfg.validate_interval == 0
        )
        return val_sched
    ...
    def get_strategy(self):
        ...
        if self._strategy is None:
            if self.cfg.multi_gpu and len(tf.config.list_physical_devices("GPU")) > 1:
...
class Configuration(object):
    ...
    def import_list_file(self, list_file):
        ...
        if self._list_file is not None:
            raise ValueError(
                "Can not load list file: Has already been loaded! "
...
def _extract_filepaths(file_content, which):
    ...
    if n_files and n_files.count(n_files[0]) != len(n_files):
        raise ValueError("Input with different number of {} in toml list".format(which))
