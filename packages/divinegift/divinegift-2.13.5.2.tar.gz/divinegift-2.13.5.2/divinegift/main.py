...
def get_list_files_recursive(path: str, filter=None, filter_not_contain: str = None, add_path: bool = False, path2: str = ''):
    ...
    if filter:
        ...
        for path_, subdirs, files in os.walk(path):
            list_files = [os.path.join(path_, x) for x in files]

    list_files_ = []
    for file in list_files:
        path_ = os.path.split(file)
        if path == '.' and not add_path:
            path = '.' + os.sep
        l = [x.replace(path, path2) for x in path_ if x.replace(path, path2) != '']
        list_files_.append(os.path.join(*l))
