...
TRunMetricLogger = TypeVar('TRunMetricLogger', bound=LightningLoggerBase)
...
class DefaultMachineLearningExperimentationService(Generic[TRunMetricLogger], MachineLearningExperimentationService[TModel]):
    def __init__(self,
    ...
    run_metric_logger_factory: Callable[[str, UUID, UUID, MachineLearningRunSettings, bool], LightningLoggerBase],
    ...
    load_experiment_checkpoint_hook: Optional[Callable[[Logger, str, UUID], Optional[ExperimentCheckpoint]]] = None):
        ...
        if run_metric_logger_factory is None:
            raise ValueError("run_metric_logger_factory")
        ...
        self.__run_metric_logger_factory: Optional[Callable[[str, UUID, UUID, MachineLearningRunSettings, bool], LightningLoggerBase]] = run_metric_logger_factory
    ...
    def execute_run(self, run_settings: MachineLearningRunSettings, experiment_id: Optional[UUID] = None, run_id: Optional[UUID] = None) -> MachineLearningRunResult[TModel]:
        ...
        run_metric_logger: TRunMetricLogger = self.__run_metric_logger_factory(run_settings.experiment_name, experiment_id, run_id, run_settings)
