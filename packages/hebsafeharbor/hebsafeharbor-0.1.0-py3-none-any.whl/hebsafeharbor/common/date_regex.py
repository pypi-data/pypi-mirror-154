...
# Hebrew Dates
heb_months_re = '|'.join(
    ["אלול", "אב", "תמוז", "סיון", "סיוון", "אייר", "ניסן", "אדר", "שבט", "טבת", "כסלו", "כיסלו", "חשוון", "תשרי"])
...
en_full_months_list = ['january', 'february', 'march', 'april', 'may', 'june', 'july', 'august', 'september', 'octber',
                       'november', 'december']
...
EN_DMY_REGEX = rf"\b{preposition_re}?(?:-|:)?(?P<day>[0-2][1-9]|3[0-1]|[1-2]0])(?P<month>1[1-2]|0[1-9])(?P<year>\d\d\d\d)\b"
EN_MDY_REGEX = rf"\b{preposition_re}?(?:-|:)?(?P<month>0[1-9]|1[0-2])(?P<day>0[1-9]|[1-2][0-9]|3[0-1]|[1-2]0)(?P<year>\d\d\d\d)\b"
EN_YMD_REGEX = rf"\b{preposition_re}?(?:-|:)?(?P<year>\d\d\d\d)(?P<month>1[1-2]|0[1-9])(?P<day>0[1-9]|[1-2][0-9]|3[0-1]|[1-2]0)\b"

# Dates with spaces around separator
SPLIT_BY_SPACE_REGEX = rf"(?P<year>\d\d\d\d)(\s*[\.\-\/]\s*)(?P<month>1[0-2]|0[1-9]|[1-9])(\s*[\.\-\/]\s*)(?P<day>[0-2][1-9]|3[0-1]|[1-2]0|[1-9])"
