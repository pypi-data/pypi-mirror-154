...
def extractArea(area_coordinates, raster):
    ...
    area_img, area_trans = rasterio.mask.mask(raster, [area_coordinates], crop=True)

    area_profile = createNewProfile(area_trans, raster.profile, img_height=area_img.shape[1], img_width=area_img.shape[2])

    return createMemRaster(area_profile, area_img)
    pass