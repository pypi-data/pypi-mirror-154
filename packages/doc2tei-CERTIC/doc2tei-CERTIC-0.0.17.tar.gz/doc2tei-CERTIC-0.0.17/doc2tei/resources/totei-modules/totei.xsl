    ...
    <text xml:id="text">
      <front>
          ...
          <xsl:if test="//*:p[starts-with(@text:style-name,'TEI_abstract')]">
              <xsl:for-each-group select="//*:p[starts-with(@text:style-name,'TEI_abstract')]" group-by="@text:style-name">
                  <div type="abstract">
                      <xsl:attribute name="xml:lang" select="substring-after(current-grouping-key(),':')"/>
      ...
      <body>
        <xsl:choose>
            <xsl:when test="//text/text:*[following::*:div[@type='section1'][1]]">
...
<!--        <xsl:comment>*3-start*</xsl:comment>-->
        <xsl:apply-templates select="//*:div[@type='section1'][last()]"/>
<!--        <xsl:comment>*3-end*</xsl:comment>  -->
      </body>
      <xsl:if test="//*:div[@type='bibliography' or @type='appendix']">
        <back>
