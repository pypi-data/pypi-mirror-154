...
class OWMaskDetection(OWTriggerWidget, ewokstaskclass=MaskDetection):
    ...
    def _update_image(self, idx, inputs, outputs):
        ...
        if not utils.is_data(image):
            return
        plot.addImage(utils.get_image(image, gui=True))

        mask = outputs.get("mask")
        if not utils.is_data(mask):
            return
    ...
    def _update_ratio(self, inputs, outputs):
        ...
        if (
            not monitor1
            or not monitor2
            or not utils.is_data(image1)
            or not utils.is_data(image2)
        ):
            return
        image1 = utils.get_image(image1, gui=True)
        image2 = utils.get_image(image2, gui=True)
        monitor1 = utils.get_data(monitor1, gui=True)
        monitor2 = utils.get_data(monitor2, gui=True)
