...
from ewoksxrpd.tasks import utils
...
class OWCalibrateMulti(OWTriggerWidget, ewokstaskclass=CalibrateMulti):
    ...
    def _accept_refined_parameters(self):
        ...
        parameters = self.get_task_output_value.get("parameters")
        position = self.get_task_input_value.get("reference_position")
        if not parametrization or not parameters or not position:
            return
        position = utils.get_data(position, gui=True)
        parametrization = utils.data_from_storage(parametrization)
        parameters = utils.data_from_storage(parameters)
    ...
    def _update_image(self, inputs, plot_index):
        ...
        if not utils.is_data(image):
            return
        image = utils.get_image(image, gui=True)
    ...
    def _update_theoretical_rings(self, values, legend, plot_index) -> List[str]:
        ...
        if (
            ...
            return
        position = utils.get_data(positions[plot_index], gui=True)
        parametrization = utils.data_from_storage(parametrization)
        parameters = utils.data_from_storage(parameters)
        geometry, energy = calculate_geometry(parametrization, parameters, position)
    ...
    def _update_detected_rings(self, values, plot_index):
        ...
        if not rings:
            return
        rings = utils.data_from_storage(rings)
        plot = self._get_plot(plot_index)
    ...
    def _values_from_form(self, values: Mapping, checked: Dict[str, bool]) -> Mapping:
        values = pack_geometry(values, checked)
        values = pack_parametrization(values)
        return values

    def _values_to_form(self, values: Mapping) -> Tuple[Mapping, Dict[str, bool]]:
        values, checked = unpack_geometry(values)
        values = unpack_parametrization(values)
        return values, checked

    def _enabled_to_form(self, enabled: Dict[str, bool]) -> Dict[str, bool]:
    ...
    def _update_tabs(self):
        ...
        if images:
            images = [utils.get_image(image, gui=True) for image in images]
            positions = self.get_task_input_value("positions")
            if positions:
                positions = [
                    utils.get_data(position, gui=True) for position in positions
                ]
            else:
                positions = list()
