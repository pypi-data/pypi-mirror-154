...
def register_temp_view_for_data_source(
    ...
    if not register_stream and raw_data_time_limits:
        validate_data_source_timestamp_present_for_feature_view(data_source.batch_data_source)
    df = get_ds_dataframe(
        spark,
        data_source,
        register_stream,
...
def validate_data_source_timestamp_present_for_feature_view(data_source: BatchDataSource):
    """
    Verifies the data_source has the timestamp_column_properties field.

    :param data_source: batch data source
    """
