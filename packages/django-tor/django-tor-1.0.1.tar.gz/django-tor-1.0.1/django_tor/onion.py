...
class Onion(object):
    ...
    def connect(self, settings=False, tor_status_update_func=None):
        ...
        if self.settings.get('connection_type') == 'bundled':
            if not self.bundle_tor_supported:
                raise BundledTorNotSupported('settings_error_bundled_tor_not_supported')
            ...
            if self.system == 'Windows':
                ...
                torrc_template = open(os.path.join(common.get_path(),'torrc_template-windows')).read()
                ...
                with open(os.path.join(common.get_path(),'torrc_template')) as f:
            ...
            try:
                ...
                raise BundledTorBroken('settings_error_bundled_tor_broken', True).format(e.args[0])
            ...
            while True:
                ...
                print("connecting_to_tor: {}% - {}{}".format(progress, summary, "\033[K"), end="\r")
            ...
            try:
                if self.settings.get('connection_type') == 'control_port':
                    ...
                    raise TorErrorInvalidSetting("settings_error_unknown")

            except:
                if self.settings.get('connection_type') == 'control_port':
                    raise TorErrorSocketPort("settings_error_socket_port").format(self.settings.get('control_port_address'), self.settings.get('control_port_port'))
                else:
                    raise TorErrorSocketFile("settings_error_socket_file").format(self.settings.get('socket_file_path'))
            ...
            try:
                ...
                raise TorErrorAuthError('settings_error_auth').format(self.settings.get('control_port_address'), self.settings.get('control_port_port'))
    ...
    def start_onion_service(self, port):
        ...
        if not self.supports_ephemeral:
            raise TorTooOld('error_ephemeral_not_supported')
        if self.stealth and not self.supports_stealth:
            raise TorTooOld('error_stealth_not_supported')

        # print(f"config_onion_service {int(port)}")
