...
1) check setup.py for environment. 
   Check EPICS base path
   TKLIB/TKINC location and WITH_TK flag setting.
2) run 
...
/* we should never try to decref Py_None*/
/*#define X_XDECREF(obj) if ((obj) != Py_None) {Py_XDECREF(obj);}*/

static inline void X_XDECREF(PyObject *obj){
  // if ((obj) != Py_None) {Py_XDECREF(obj);}
  if ((obj) == Py_None) {/* do nothing */}
  else                  {Py_XDECREF(obj);}
}

/* ca_detach_contex is in library but not in header files. */ /* now in cadef.h. NY 2018.1.20 */
/* ca_detach_contex requires EPICS R3.14.8 or later **/
// epicsShareFunc void epicsShareAPI ca_detach_context (void ) ; // now in cadef.h in R7. 2020.11.18 
#include <cadef.h>
...
static PyObject *Py_sg_stat(PyObject *self, PyObject *args);//epicsShareFunc int epicsShareAPI ca_sg_stat (CA_SYNC_GID gid);
//static PyObject *Py_ca_dump_dbr(PyObject *self, PyObject *args);//epicsShareFunc void epicsShareAPI ca_dump_dbr (chtype type, unsigned count, const void * pbuffer);
...
static PyMethodDef CA_Methods[]={
  ...
  {"sg_stat", Py_sg_stat, METH_VARARGS, "sync. group status (id)"},
  ...
  {"__ca_task_exit", Py_ca_task_exit, METH_VARARGS, "Terminate CA libary, also calls destroy_context"},
  {"__ca_destroy_context", Py_ca_destroy_context, METH_VARARGS, "Terminate CA libary/destroy context"}, 
...
//  PyEval_ThreadsInitialized:Deprecated since version 3.9, will be removed in version 3.11.
...
Py_ca_get(PyObject *self, PyObject *args){
  ...
  if(!PyArg_ParseTuple(args, "kOll", &ch_id, &pcallback, &type, &ecount))
    return NULL;
...
#define MoveValuesToBuffer(DBRBUFTYPE, DBRTYPE, FORMAT) \
	...
	DBRBUFTYPE buf;\
        assert(ptr);\
	for(cur=0; cur<count; cur++){\
          register PyObject *item=PySequence_Fast_GET_ITEM(value, cur); \
...
static PyObject *Py_ca_put(PyObject *self, PyObject *args)
{
  ...
  if (count == 1){
    ...
    default:
      ...
      X_XDECREF(item); // XDECREF is safe for NULL pointer.
      return NULL;
    }
    if (pcallback == Py_None ){
      Py_BEGIN_ALLOW_THREADS{
...
Py_ca_monitor(PyObject *self, PyObject *args){
  ...
  if(!PyArg_ParseTuple(args, "kOl|l", &ch_id, &pcallback, &ecount, &evmask))
    return NULL;
...
Py_ca_pend_io(PyObject *self, PyObject *args){
  ...
  {
    register int rval=CA_EXTRACT_MSG_NO(status);
    return Py_BuildValue("i", rval);
  }
} 
...

  {
	...
	if (PyCallable_Check(func)){
	  ...
	  try{
	    ...
	    PyObject_CallObject(func, uargs);
	  }
	  catch(...){
	    PyErr_WarnEx(NULL, "exception while fd reigster\n", 1);
	  }
	  X_XDECREF(func);X_XDECREF(uargs);X_XDECREF(arg);
	}
      }
    }
    pframe->unlock();
...
static PyObject *
Py_sg_stat(PyObject *self, PyObject *args){
  ...
  Py_BEGIN_ALLOW_THREADS{
    ENTER_CA{
      status=ca_sg_stat(gid);
      SEVCHK(status, "Py_sg_stat")
    }LEAVE_CA;
  }Py_END_ALLOW_THREADS;
...
Py_sg_reset(PyObject *self, PyObject *args){
  ...
  if (status != ECA_NORMAL){
    ...
    Py_BEGIN_ALLOW_THREADS{
      ENTER_CA{
	rval=CA_EXTRACT_MSG_NO(status);
      }LEAVE_CA;
    }Py_END_ALLOW_THREADS;
    robj=Py_BuildValue("l", rval);
    return  robj;
  }
}
...
Py_sg_put(PyObject *self, PyObject *args){
  ...
  if(!PyArg_ParseTuple(args, "IklOl", &gid, &ch_id, &pbuf, &value, &type)){
    return NULL;
  }
  ...
  if(!pbuf){
	...
	size=dbr_size_n(dbf_type_to_DBR_TIME(pyca_field_type), count) ; 
	pbuf=calloc(1, size);
...
Py_sg_get(PyObject *self, PyObject *args){
  ...
  if(!PyArg_ParseTuple(args, "Ikl", &gid, &ch_id, &pbuf)){
    return NULL;
  }
...
Py_ca_convert(PyObject *self, PyObject *args){
  ...
  if(!PyArg_ParseTuple(args, "kl", &ch_id, &pbuf)){
    return NULL;
  }
