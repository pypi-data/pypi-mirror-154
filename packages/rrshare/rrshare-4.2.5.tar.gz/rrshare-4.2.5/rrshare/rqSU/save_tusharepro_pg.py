# coding: utf-8
""" save aily data of stock day and swl_day to pdsql from  tushare pro by trade_date"""
import pandas as pd
import numpy as np
import os
import time
...
from rrshare.rqFetch.fetch_basic_tusharepro import  fetch_stock_list_tusharepro
from rrshare.rqUtil.rqParameter import startDate
from rrshare.rqFetch.fetch_swl_index_daily_and_realtime import Swsindex, get_sw_index_daily_L1_L2
...
def download_day_data_from_tushare(trade_date=startDate):   
...
def fetch_stock_daily_adj_from_tusharepro(trade_date=lastTD):
    trade_date = trade_date.replace('-', '') #兼容设置以防日期格式为2001-10-20格式
    lastEx = None
    retry = 3
    for _ in range(retry):
        try:
            ...
            return None   
    #df=pd.merge(df_daily,df_factor,how='left')
    # adj_factor delist & ipo (not list)
...
def rq_save_stock_day_adj_fillna_pg(start_date=startDate): #20050101
    table_name='stock_day_adj_fillna'    
    t = time.localtime(time.time())
    if int(time.strftime('%H%M%S',t))<170000:   #晚上5点之后在更新当天数据，以免不及时
