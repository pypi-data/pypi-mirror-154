...
def read_stock_day_N_to_pickle(table_name='stock_day_adj_fillna', N=265):
    ...
    # TODO if file exists ,read and get lasy max date compare with last tradedate
    # if == pass; else continue
...
def stock_RT_HH_MA_pre(table_name='stock_day_adj_fillna', N=255): # ok
    """caculate i-1 day rt_ma
       prepare to next day  day rs_OH_ma
       迭代， 只算一次， 速度快。
       if stock_RT_HH_MA_pre tabble exist, compare max tradedate and lastTD
       is_new , pass, else pass
    """
    trade_date = rq_util_get_last_tradedate()
    #print(trade_date)
    start_date = rq_util_get_pre_trade_date(trade_date,N)
    ...
    df = read_sql_from_pg(start_date=start_date, data=\
            "trade_date,open,high,low,close,pre_close,vol,amount,\
            adj_factor,code,pct_chg,avg",table_name=table_name, client=client_rrshare)
    #df.to_pickle("~/.rrsdk/data/stock_day_250.pkl")
    '''
    file_path_name = f"{file_path}/{table_name}_{N}.pkl"
    ...
    vol_mean =  (df_un_fill['vol']*df_un_fill['adj_factor']).rolling(49).mean().unstack()
    vol_chg = 100*((df_un_fill['vol']*df_un_fill['adj_factor']) / \
            (df_un_fill['vol']*df_un_fill['adj_factor']).rolling(49).mean() - 1).unstack()
...
def stock_RS_OH_MA_last(table_name='stock_day_adj_fillna', N=255): # ok
    ...
    df = read_sql_from_pg(start_date=start_date, data=\
            "trade_date,open,high,low,close,pre_close,vol,amount,\
            adj_factor,code,pct_chg,avg",table_name=table_name, client=client_rrshare)
    cols = ['open', 'high', 'low', 'close','pre_close','pct_chg', 'vol', 'amount', 'adj_factor','avg']
    for i in cols:
    ...
    L = (df_un_fill['low']*df_un_fill['adj_factor']).expanding().min()
    LL = (L/df_un_fill['adj_factor']).unstack()
    OH = 100*(df_un_fill['close']*df_un_fill['adj_factor']/H ).unstack()
    OL = 100*(df_un_fill['close']*df_un_fill['adj_factor']/L -1).unstack()
    ...
    'rs5':rt[5],'rs10':rs[10],'rs20':rs[20],'rs60':rs[60],'rs120':rs[120], 'rs250': rt[250],
    'H':HH,'L':LL, 'OH':OH, 'OL':OL,
    })
    ...
    save_PRS_to_pg(table_name="stock_RS_OH_MA_last",data=data,client=client_rrfactor)
    return data 


def stock_RS_OH_MA():
    df = read_data_from_pg(table_name='stock_RS_OH_MA_last',client=client_rrfactor)
...
def stock_RS_OH_MA_new():
    ...
    if (trade_date > df_last_date): 
        ...
        return read_data_from_pg(table_name='stock_RS_OH_MA',client=client_rrfactor)
...
def update_stock_PRS_day(N=255):
    stock_RT_HH_MA_pre(N=N)
    stock_RS_OH_MA_last(N=N)
    stock_RS_OH_MA()
