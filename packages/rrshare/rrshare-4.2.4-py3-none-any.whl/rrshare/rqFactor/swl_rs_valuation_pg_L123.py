...
class SwlPRS(object): #TODO
    
    def __init__(self, level):
        self.level = level

    def swl_PRS_level(self, period):
        #cols = "trade_date, index_symbol, name_level, level, change_pct" #, pe, pb"
        start_date = rq_util_get_pre_trade_date(rq_util_get_last_tradedate(), period)
        sql = f"SELECT  * FROM swl_day WHERE trade_date >= '{start_date}' ;"
        df = pd.read_sql_query(sql=sql, con=client_rrshare)
        print(df)
        
       
        df = df[df['level']==self.level]
        df = df[['index_symbol','name_level', 'change_pct']]
                
        rt = df.groupby(by='index_symbol').sum()
        ...
        RS = RS[['index_symbol','name_level', f'rt_{str(period)}',f'rs_{str(period)}']]
        print(RS)
        return RS
...
def update_swl_rs_valuation(L=L[:2]):
    for l in L:
        prs = SwlPRS(l)
