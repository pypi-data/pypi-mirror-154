...
sw_index_day, list from rrshare , swl_rs_valuation save to rrfactor
"""
import time
import pandas as pd
...
class SwlPRS(object):
    ...
    def swl_PRS_level(self, period):
        cols = "trade_date, index, name_level, level, change_pct"#, pe, pb"
        start_date = rq_util_get_pre_trade_date(rq_util_get_last_tradedate(), period)
        df = read_sql_from_pg(start_date=start_date, data=cols, table_name='sw_index_day',client=client_rrshare)
        df = df[df['level']==self.level]
        df = df[['index','name_level', 'change_pct']]
                
        rt = df.groupby(by='index').sum()
        rt['rank'] = rt['change_pct'].rank(ascending=False)
        rt['rs'] = round(100*(1 - rt['rank']/len(rt)), 2)
        rt = rt.rename(columns={'change_pct': f'rt_{str(period)}'})
    ...
    def swl_Prs_all_periods(self):
        ...
        logging.info(f'鍐欏叆鏁版嵁搴撶殑琛╯wl_rs_{self.level}, ok')


    def swl_rt_all_periods(self, level):
        RT = pd.DataFrame()
        for period in periods:
            p = period -1
            cols = "trade_date, index, name_level, level, change_pct" #, pe, pb"
            ...
            df = df[['index','name_level', 'change_pct']]
            rt = df.groupby(by='index').sum()
            rt = rt.rename(columns={'change_pct': f'rt_{str(p)}'})
            rt =rt.round(2)
            #print(rt)
            RT = pd.concat([RT, rt], axis=1, sort=True)
        
        df_name = swl_index_to_name(swl_level=level).set_index('index', drop=False)  # keep index in columns
        #df_name['name'] = df_name['name_level']
        print(df_name)
        RT = pd.concat([df_name, RT], axis=1, sort=True)
        logging.info(f'\n {RT}')
        #print(RT)
        save_data_to_postgresql(RT,f'swl_rt_{level}',client_rrfactor)
        logging.info(f'save_swl_rt_{level}, ok')
    ...
    def save_swl_valuation_pg(self):
        ...
        logging.info(f'鍐欏叆鏁版嵁搴撶殑琛╯wl_valuation_{self.level}, ok')
    ...
    def save_swl_rs_valuation_pg(self):
        ...
        logging.info(f'鍐欏叆鏁版嵁搴撶殑琛╯wl_rs_valuation_{self.level}, ok')


def swl_index_realtime_prs(level="L1"):
    rt = read_data_from_pg(f'swl_rt_{level}', client_rrfactor)
    #print(rt)
    if level == "L1":
        new_rt  = Swsindex().get_swsindex_L1_realtime()[['index','pct_change','trade_date']] 
        #print(new_rt)
    if level == "L2":
        new_rt  = Swsindex().get_swsindex_L2_realtime()[['index','pct_change','trade_date']]
    else:
        logging.info("No this level !")
        new_rt = None
    logging.info(new_rt)
    ...
    if not new_rt:
        ...
        df.rename(columns={'pct_change':'chang_pct'}, inplace=True)
        df.sort_values(by='change_pct', ascending=False, inplace=True)
        save_data_to_postgresql(df, f'swl_rs_{level1}_realtime', client_rrfactor)
        logging.info(f"\n {df}")
        return df
...
if __name__ == '__main__':
    ...
    #swl_index_realtime_prs(level="L1")
