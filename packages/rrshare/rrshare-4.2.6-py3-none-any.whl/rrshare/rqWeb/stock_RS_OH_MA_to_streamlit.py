...
def write_swl_RS_OH_MA_new():
    table_name_new = 'swl_L2_RS_OH_MA_new' 
    #print(table_name)
    ...
    try:
        st.text('申万行业L2相对强度')
        df = pd.read_sql_table(table_name_new,conn)
        df = df[cols]
        #df.rename(columns={'name':'cn_name'},inplace=True)
        #df.drop_duplicates(subset=None,keep='first',inplace=True)
        
        #data_by_pctchg = df.sort_values(by='chang_pct', ascending=False)
        #st.write("更新时间: ",data[1],data[2])
        #st.write(data[0])
        #df2 = df.dropna(axis=0,how='any')
        df2 = df.sort_values(by='rs_5', ascending=False)
        st.write(df2)
        
        st.text("申万行业L2估值和股息率")
        df_v = pd.read_sql(f"SELECT DISTINCT * FROM swl_day_valuation WHERE trade_date = '{lastTD}' and level='L2'", 
                           con=client_pgsql('rrshare'))
        st.write(set(df_v.trade_date.values))
        
        df_v.drop(columns=['trade_date', 'level','index_code'], inplace=True)
        st.write(df_v)
