...
import utils.files.file as cfu
...
def insert_sql(data,table_name,schema_name=None):
    '''
        ...
        sql = csu.gen.sql.insert_sql(data,'kitties','titties')
...
def parse_list(value):
    ...
    vals = value.split(",")
    vals = [mod.strip_any(x,[" "]) for x in vals]
    return ','.join(vals)
...
def parse_table_columns(sql):
    ...
    cols = {}
    parsed = sqlparse.parse(file_data['sql'])
    for stmt in parsed:
...
def parse_col_comment(col_data):
    ...
    for s in col_data['shlexed']:
        ...
        if s.lower().startswith('comment'):
            ...
            col_data['comment'] = mod.strip_any(cmt,[" "])
            break
...
def parse_primary_keys(sql):
    ...
    if match is not None:
        if len(match) > 0:
            ...
            keys = keys.split(",")
            keys = [mod.strip_any(x,[" "]) for x in keys]
            primary_keys = keys
        # print(match)
    return primary_keys
...
#             if cfu.exists(self.data['file_path']):
#                 self.data['raw_sql'] = cfu
...
# cfu.write.to_json("colums.delete.json",cols)
