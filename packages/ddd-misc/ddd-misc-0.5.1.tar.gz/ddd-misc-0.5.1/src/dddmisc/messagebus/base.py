...
from dddmisc import (DDDEvent, DDDCommand)
from dddmisc.messagebus.abstract import EventHandlerType, CommandHandlerType
...
class EventConfig:
    ...
    @property
    def domain(self) -> str:
        return self.event_type.__domain__
...
class CommandConfig:
    ...
    @property
    def domain(self) -> str:
        return self.command_type.__domain__
...
class BaseExternalMessageBus:
    ...
    def register_event(self, event: t.Type[DDDEvent], *handlers: EventHandlerType):
        ...
        if self.domain != event.__domain__ and not handlers:
            raise RuntimeError('Required set handler for consume to other domain events')
