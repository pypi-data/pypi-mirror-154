...
class PrintStep:
    def __init__(self, text, additional_space=None, catch_exc=False):
        self.text = text
...
def convert_mps_apps_list_to_pretty_table(apps):
    ...
    for app in apps:
        table.add_row(
            [
                app["id"],
                app["ead"]["content"]["name_short"],
                app["ead"]["content"]["namespace"],
                app["aim"]["namespace"],
                datetime.strptime(app["aim"]["uploaded_at"], "%Y-%m-%d %H:%M:%S.%f").strftime("%Y-%m-%d %H:%M:%S"),
            ]
        )
    return table
...
def convert_slides_list_to_pretty_table(slides):
    table = PrettyTable()
    table.field_names = ["SLIDE ID", "PATH", "TISSUE", "STAIN", "BLOCK", "CASE ID", "CREATED", "UPDATED"]
...
def convert_jobs_list_to_pretty_table(jobs):
    ...
    table = PrettyTable()
    table.field_names = ["JOB ID", "STATUS", "APP ID", "CREATED", "STARTED", "ENDED"]
