...
class Tunnel(object):
    ...
    def install(cls):
        ...
        if not pathlib.Path(zip_dest).exists():
            ...
            cmd = f"curl -s -o {zip_dest} {src_url}"
            os.system(cmd)

        # unzip ngrok.zip
        if linux(platform):
            cmd = f"unzip -d {cache_dir} {zip_dest}"
        elif platform == "windows":
            cmd = f"tar --force-local -xf {zip_dest} -C {cache_dir}"
        else:
            cmd = f"tar -xf {zip_dest} -C {cache_dir}"
        os.system(cmd)

        # rm zip
        pathlib.Path(zip_dest).unlink(missing_ok=True)
