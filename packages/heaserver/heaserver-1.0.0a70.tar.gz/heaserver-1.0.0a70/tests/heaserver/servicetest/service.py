...
@action('component-get-open-choices', rel='hea-opener-choices', path='/components/{id}/opener',
        include_root=True)  # only add if the object is openable
@action('component-duplicate', rel='hea-duplicator', path='/components/{id}/duplicator', include_root=True)
...
async def get_component_opener(request: web.Request) -> web.Response:
    """
    Gets choices for opening the component's content. We wouldn't ordinarily open a component's content because
    components do not have any content.
...
async def post_component(request: web.Request) -> web.Response:
    """
    Creates a new component, provided in the body.

    :param request: the HTTP request.
    :return: a Response object with a status of Created and the object's URI in the
    """
    with StringIO('The quick brown fox jumps over the lazy dog') as f:
        return await mongoservicelib.post(request, MONGODB_COMPONENT_COLLECTION, Component, default_content=f)
...
async def delete_component(request: web.Request) -> web.Response:
    """
    Deletes the component with the specified id and its associated content.
...
@action('heaserver-organizations-organization-open-aws-accounts', rel='hea-default hea-opener text/plain',
        path='/organizations/{id}/content', include_root=True)
