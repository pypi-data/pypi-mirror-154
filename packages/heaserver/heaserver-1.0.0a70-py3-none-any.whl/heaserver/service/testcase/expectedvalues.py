...
def expected_one(fixtures: Dict[str, List[DesktopObjectDict]], coll: str, wstl_builder: wstl.RuntimeWeSTLDocumentBuilder,
                 include_root=False,
    ...
    def item_links() -> List[Dict[str, Any]]:
        ...
        for action, action_name, rel, url in ((wstl_builder.find_action(a.name), a.name, a.rel or [], a.url) for a in
            ...
            if url is not None:
                match = extra_tvars(route=url, path=URL(href).path)
            else:
                match = {}
        ...
        for action, action_name, rel, url in ((wstl_builder.find_action(a.name), a.name, a.rel or [], a.url) for a in
            ...
            if 'item' in targets and 'href' in targets and 'cj' in targets:
                return {
                    'prompt': action['prompt'],
                    'href': uritemplate.expand(url, {k: v for k, v in (match | obj).items() if
                                                     isinstance(v, (int, float, str))}) if url else (
    ...
    def top_level_links() -> List[Dict[str, Any]]:
        ...
        for action, action_name, rel, url in ((wstl_builder.find_action(a.name), a.name, a.rel or [], a.url) for a in
            ...
            if url is not None:
                match: Mapping = extra_tvars(route=url, path=URL(href).path)
            else:
                match = {}
            targets = action.get('target', '').split()
            if action['type'] == 'safe' and 'app' in targets and 'cj' in targets and (
                ...
                links.append({
                    'prompt': action['prompt'],
                    'href': uritemplate.expand(url, match | {'id': id_}) if url else (
                        str(href) if include_root else str(href.path)),
                    'rel': ' '.join(rel)
                })
        return links
...
def expected_all(fixtures: Dict[str, List[DesktopObjectDict]], coll: str, wstl_builder: wstl.RuntimeWeSTLDocumentBuilder,
                 include_root=False, get_all_actions: Optional[List[Action]] = None) -> List[Dict[str, Any]]:
    """
    ...
    def item_links(obj: DesktopObjectDict):
        ...
        for action, name, rel, url in ((wstl_builder.find_action(a.name), a.name, a.rel or [], a.url) for a in
            ...
            if url is not None:
                match = extra_tvars(route=url, path=URL(href_).path)
            else:
                match = {}
            targets = action.get('target', '').split()
            if 'item' in targets and 'read' in targets and 'cj' in targets:
                links.append({
                    'prompt': action['prompt'],
                    'href': uritemplate.expand(url, match | obj) if url else (
                        str(URL(href_) / obj['id']) if include_root else str((URL(href_) / obj['id']).path)),
                    'rel': ' '.join(rel)
                })
        return links
