...
class Mongo(Database):
    ...
    async def put_content(self, request: web.Request, collection: str, sub: Optional[str] = None) -> bool:
        ...
        try:
            ...
            failed = True
            try:
                await fs.upload_from_stream_with_id(obj['id'], str(obj), fileobj)
                fileobj.close()
                failed = False
...
MONGO_RELEVANT_COLLECTIONS = ['components', 'folders', 'folders_items', 'organizations', 'volumes', 'filesystems', 'credentials', 'people', 'dataadapters']
...
class MongoManager(DatabaseManager):
    ...
    @classmethod
    def get_relevant_collections(cls) -> Optional[list[str]]:
        super_colls = super().get_relevant_collections()
        return super_colls + MONGO_RELEVANT_COLLECTIONS if super_colls is not None else MONGO_RELEVANT_COLLECTIONS
...
"""
    ...
    def insert_desktop_objects(self, desktop_objects: Optional[Dict[str, List[root.DesktopObjectDict]]]):
        ...
        for k, v in desktop_objects.items() if desktop_objects is not None else {}:
            rel_coll = self.get_relevant_collections()
            if v and k in (rel_coll if rel_coll is not None else [k]):
                db_[k].insert_many(replace_id_with_object_id(f) for f in v)
    ...
    def insert_content(self, content: Optional[Dict[str, Dict[str, bytes]]]):
        ...
        for k in content_:
            rel_coll = self.get_relevant_collections()
            if k in (rel_coll if rel_coll is not None else [k]):
                fs = gridfs.GridFSBucket(db_, bucket_name=k)
