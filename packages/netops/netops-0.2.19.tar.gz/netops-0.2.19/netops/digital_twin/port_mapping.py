...
def get_junos_lo_configs(nornir=None, dev_name=None, dev=None, con_ints=None):
    """
    ...
    Input:
            ...
            con_ints: (connected interfaces) list of interface names of connected interfaces and its link id
                        {id: <int>, iface: <str>}
    Output:
    ...
    # Get PO interface names
    po_int = junos_pyez_get_interfaces(device_configs)
    # Get LO available interface names
    ...
    last_i = 0
    if con_ints:
        for i in range(len(con_ints)):
            data.append( (i, con_ints[i]['iface'], get_interface(avail_lo_int)) )
            last_i = i
    for i in range(last_i+1, len(po_int)):
