...
class Bdd:
    ...
    def addFullTextMetadatas(self,r : Resource,folder : str) -> None:
        ...
        try:
            ...
            self.connect.commit()
            #self.cursor.execute(sql, ([m.sha1, m.fileName.value, m.extension.value, m.pathName.value, m.creationDate, m.modificationDate, m.sizeCompressed, m.sizeUncompressed, str(r.type.value), folder, m.createdAt, content]))
            #self.connect.commit()
        except sqlite3.Error as e:
            print(e)
        ...
        for option in options:
            ...
            match option:
                ...
                case "createdAtStart":
                    ...
                    params.append(Metadata.convertDateInTimeStamp(options[option]))
                    if "createdAtEnd" in options:
                        params.append(Metadata.convertDateInTimeStamp(options["createdAtEnd"]))
                    else:
                        params.append(Metadata.convertDateInTimeStamp(options["createdAtEnd"]))
                case "type":
