...
def patch_flask():
    ...
    if patch_werkzeug:
        ...
        app.logger.info("AML_FLASK_ONE_COMPATIBILITY is set. Patched Flask to ensure compatibility with Flask 1.")
    else:
        app.logger.info("AML_FLASK_ONE_COMPATIBILITY is set, but patching is not necessary.")


if os.environ.get("AML_FLASK_ONE_COMPATIBILITY", "true").lower() == "true":
    try:
        patch_flask()
    except Exception:
        app.logger.warning(
...
def create():
    # Import routes to bind the routes to app. We will need to move away from the create_app design pattern as it is for
    # blueprints and not suitable if we're going to use app directly.
    import routes
