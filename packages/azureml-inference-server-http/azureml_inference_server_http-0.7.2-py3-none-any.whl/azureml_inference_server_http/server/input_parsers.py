...
class ObjectInput(InputParserBase):
    ...
    run() function. An error is raised if ``parameter_name`` is not found in the body JSON. This is used when the user's
    run function is decorated with @input_schema.
    """
    ...
    def _extract_parameters(self, body: Dict) -> Dict[str, Any]:
        ...
        for parameter_name in self.parameter_names:
            input_value = body.get(parameter_name, _MISSING)
            if input_value is _MISSING:
                raise BadInput(f"A value is not provided for the {parameter_name!r} parameter.")

            parameters[parameter_name] = input_value
