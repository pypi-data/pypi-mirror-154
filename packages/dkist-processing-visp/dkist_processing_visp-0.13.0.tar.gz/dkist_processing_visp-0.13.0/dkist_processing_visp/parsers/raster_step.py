...
class TotalRasterStepsBud(Stem):
    ...
    def setter(self, fits_obj: VispL0FitsAccess) -> Union[Type[SpilledDirt], Tuple[int, int]]:
        """
        Setter for the bud.
    ...
    def getter(self, key) -> Union[str, float, int]:
        ...
        num_steps = num_steps_set.pop()

        # Now check that all the steps we expect are present
        all_steps = sorted(list(set([v[1] for v in values])))
        if all_steps != list(range(0, num_steps)):
            raise ValueError(
                f"Did not find the correct set of raster step values. Found: {all_steps}"
            )
