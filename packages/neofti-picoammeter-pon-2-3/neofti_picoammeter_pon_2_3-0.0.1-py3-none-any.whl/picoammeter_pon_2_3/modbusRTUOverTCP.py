...
class Handler():
    def __init__(self,host:str,port:int,id:int):
        self.__client = ModbusTcpClient(host, port=port, framer=ModbusRtuFramer,timeout=3,skip_encode = True)
        self.__id = id
        self.status = False
    ...
    def readRegisters(self, startAdrress:int, count:int=1):
        log.debug("Read modbus register")
    ...
    def __checkArrayValueByNegative(self,array):
        ...
        for i in range(np.size(array)):
            resultArray[i] = array[i] if array[i] < 2**8 else array[i] -2**16
        return resultArray