...
#            print("1 {} {} {} {} {}".format(self.id, self.start, self.end, start, end))
            return (self.end-start) / (end-start)
        elif start <= self.start <= end <= self.end:
#            print("2 {} {} {} {} {}".format(self.id, self.start, self.end, start, end))
            return (end-self.start) / (end-start)
        elif start <= self.start and self.end <= end:
#            print("3 {} {} {} {} {}".format(self.id, self.start, self.end, start, end))
            return (self.end-self.start) / (end-start)
        elif self.start <= start and end <= self.end:
#            print("4 {} {} {} {} {}".format(self.id, self.start, self.end, start, end))
            return 1.0
        else:
            return 0.0


    def compute_ratio(self, no_product, tr_ratio, no_evidence_adjust=False):

        if tr_ratio > 1.0:
            raise Exception("Ratio: {}, higher than 1.0".format(tr_ratio))
        else:
            if no_evidence_adjust:
                if self.best_tr_evidence[1] == 1.0 and self.best_bx_evidence[1] < 1.0:
                    self.evidence_ratio = self.best_bx_evidence[1]
                elif self.best_tr_evidence[1] < 1.0 and self.best_bx_evidence[1] == 1.0:
                    self.evidence_ratio = self.best_tr_evidence[1]
                else:
                    if no_product:
                        self.evidence_ratio = self.best_tr_evidence[1] * tr_ratio + self.best_bx_evidence[1] * (1 - tr_ratio)
                    else:
                        self.evidence_ratio = self.best_tr_evidence[1] * self.best_bx_evidence[1]
            else:
                if no_product:
                    self.evidence_ratio = self.best_tr_evidence[1] * tr_ratio + self.best_bx_evidence[1] * (1 - tr_ratio)
                else:
    ...
    def infer_cds_parts_from_start_end(self, start, end):
        ...
        for e in self.lExons:
            ...
            self.add_cds(CDS("cds:{}".format(self.id),self.seqid,cds_start,cds_end,self.strand,None,self.source,self.id))
...
#    def infer_utrs(self):
#        """infer utrs from exon and cds"""

#        print("UTR infer")
#        s = self.get_min_cds_start() 
#        e = self.get_max_cds_end()
#        print("UTR infer")
    ...
    def get_nb_shared_bases_with_another_transcript(self, other, self_no_utr=False, other_no_utr=False):
        ...
        if self.seqid != other.seqid:
            return 0
        #min_pos, max_pos = self.__get_boundaries_for_comparison(other, no_utr)
        min_pos, max_pos = self.__get_boundaries_for_comparison(other,self_no_utr, other_no_utr)
        ...
        return bin(int(self_bits,2) & int(other_bits,2)).count("1")
    ...
    def is_cds_included_in_other_cds(self, tr):
        ...
        for cds in self.lCDS:
            if self.get_bit_repr_for_comparison_spe(cds.start, cds.end, True) != tr.get_bit_repr_for_comparison_spe(cds.start, cds.end, True):
                return False
        return True
    ...
    def get_bit_repr_for_comparison(self,min_pos, max_pos, no_utr, code=1):
        ...
        return "".join([str(x) for x in bitarray])
        ...
        for feat in lFeatures:
            for pos in range(feat.start,feat.end+1):
                if (pos >= min_pos) & (pos <= max_pos):
                    #print(len(bitarray), pos)
                    bitarray[pos-min_pos] = 1
    ...
    def has_same_exons(self, other):
        ...
        return sorted([(ex.start,ex.end) for ex in self.lExons])== sorted([(ex.start, ex.end) for ex in other.lExons])
    ...
    def is_cds_partial(self, genome):
        ...
        if len(cds_seq) % 3 != 0:
            return True
        if Utils.translate(cds_seq[0:3].upper()) != 'M':
            return True
        if Utils.translate(cds_seq[-3:].upper()) != '*':
            return True
...
#                    logging.info("Warning change frame {} to {}".format(cds.frame, next_frame))
    ...
    def to_gff3(self,atts=None, source=None):
        ...
        if atts:
            ...
            #return "{}\t{}\tmRNA\t{}\t{}\t.\t{}\t.\t{}\n".format(self.seqid,'ingenannot',self.start,self.end,strand,str_atts)
            return "{}\t{}\tmRNA\t{}\t{}\t.\t{}\t.\t{}\n".format(self.seqid,src,self.start,self.end,strand,str_atts)
        else:
            return "{}\t{}\tmRNA\t{}\t{}\t.\t{}\t.\tID={};Parent={}\n".format(self.seqid,src,self.start,self.end,strand,self.id,self.gene_id)
