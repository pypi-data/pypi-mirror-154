...
class Instance():
  ...
  def __run_command(self, comm: str, port: int) -> str:
    ...
    try:
      ...
      result = run(command, stdout=sys.stdout, stderr=sys.stderr, universal_newlines=True)
  ...
  def ls(self, path: str, *, port: int = 6174):
    ...
    comm = "ssh"
    if U.ENVVARS.NBOX_SSH_NO_HOST_CHECKING(False): comm += " -o StrictHostKeychecking=no"
  ...
  def mv(self, src: str, dst: str, force: bool = False, *, port: int = 6174):
    ...
    comm = "scp"
    if U.ENVVARS.NBOX_SSH_NO_HOST_CHECKING(False): comm += " -o StrictHostKeychecking=no"
    comm += f" {src_folder} -P {port} {src} localhost:{dst}"
    result = self.__run_command(comm, port)
    return result
