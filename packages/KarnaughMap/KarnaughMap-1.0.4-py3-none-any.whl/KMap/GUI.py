...
class _ToggleButton(Canvas):
    ...
    Class variables:
        ...
        Public:
            self.master    - total number of variables in the expression (used for table dimensions)
            self.state = 0 - unsimplified expression
        Private:
            self._day      - list of valid symbols that can be used
            self._night    - debug mode (enables some prints)
            self._button   - the table (can also get access through __repr__)
    """
...
class _CreditsApp:
    ...
    Functions:
        ...
        (None see tkinter.Canvas for inherited functions)

    Class variables:

        (None see tkinter.Canvas for inherited variables)
    """
