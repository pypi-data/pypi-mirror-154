...
class App(object):
    ...
    def __init__(self, **kwargs):
        ...
        # 增加配置项，方便热更新
        self.config = {}
        self.add_route_plus(self.app_config, methods=["GET", "POST"])  # 配置
        self.add_route_plus(self.proxy_app, methods=["GET", "POST"])  # 代理服务，自己调自己有问题
    ...
    def _handler_plus(self, func, **kwargs):
        ...
        async def handler(request: Request):
            ...
            if body.startswith(b'{'):  # 主要分支
                ...
                input.update({'__body__': body})  # 非 json 请求体

            # input4str 方便 cache
            if 'str' in str(func.__annotations__):
                input = str(input)
