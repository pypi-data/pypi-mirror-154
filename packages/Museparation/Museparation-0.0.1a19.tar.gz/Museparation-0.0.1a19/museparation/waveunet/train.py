    ...
    while state["worse_epochs"] < args.patience:
        ...
        with tqdm(total=len(train_data) // args.batch_size) as pbar:
            ...
            for example_num, (x, targets) in enumerate(dataloader):
                ...
                optimizer.zero_grad()
                outputs, avg_loss = model_utils.compute_loss(
                    model, x, targets, criterion, compute_grad=True
                )
                ...
                t = time.time() - t
                avg_time += (1.0 / float(example_num + 1)) * (t - avg_time)
                ...
                if example_num % args.example_freq == 0:
                    input_centre = torch.mean(
                        x[
                            0,
                            :,
                            model.shapes["output_start_frame"] : model.shapes[
                                "output_end_frame"
                            ],
                        ],
                        0,
                    ...
                    writer.add_audio(
                        "input", input_centre, state["step"], sample_rate=args.sr
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--hdf_dir", type=str, default="hdf", help="Dataset path")
    parser.add_argument(
        "--checkpoint_dir",
        type=str,
    ...
    parser.add_argument(
        "--cycles", type=int, default=2, help="Number of LR cycles per epoch"
    )
