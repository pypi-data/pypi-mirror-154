...
def predict(audio, model):
    ...
    with torch.no_grad():
        for target_start_pos in range(0, target_outputs, model.shapes["output_frames"]):
            ...
            curr_input = audio[
                :, target_start_pos : target_start_pos + model.shapes["input_frames"]
            ...
            for key, curr_targets in compute_model_output(model, curr_input).items():
                outputs[key][
                    :,
                    target_start_pos : target_start_pos + model.shapes["output_frames"],
                ] = (
...
def predict_song(args, audio_path, model):
    ...
    sources = {
        key: data_utils.resample(sources[key], args.sr, mix_sr)
    ...
    with torch.no_grad():
        for example in dataset:
            ...
            target_sources = np.stack(
                [
                    data_utils.load(example[instrument], sr=None, mono=False)[0].T
...
def validate(args, model, criterion, test_data):
    ...
    with tqdm(total=len(test_data) // args.batch_size) as pbar, torch.no_grad():
        for example_num, (x, targets) in enumerate(dataloader):
            ...
            total_loss += (1.0 / float(example_num + 1)) * (avg_loss - total_loss)
