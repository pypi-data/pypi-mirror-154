...
@LastEditTime: 2022-06-07 09:49:52
...
class UserBaseModel(FrameBaseModel):
    ...
    def save_user_by_openid(self, app_id, act_id, open_id, user_nick, avatar, union_id=""):
        ...
        try:
            user_account = user_account_model.get_cache_entity("open_id=%s", params=[open_id], dependency_key=f"user_account:openid_{open_id}")
            if not user_account:
                ...
                if config.get_value("is_cache_empty",False) == True:
                    user_info_model.delete_dependency_key(f"user_account:openid_{open_id}")
            else:
                is_change = False
                if user_nick and user_account.user_nick != user_nick:
                    ...
                    is_change = True
                if avatar and user_account.avatar != avatar:
                    user_account.avatar = avatar
                    is_change = True
                if union_id and user_account.union_id != union_id:
                    ...
                    is_change = True
                if is_change == True:
                    user_account_model.update_entity(user_account, "user_nick,user_nick_encrypt,avatar,union_id")
                    user_account_model.delete_dependency_key(f"user_account:openid_{open_id}")

            user_info = user_info_model.get_cache_entity("act_id=%s and open_id=%s", params=[act_id, open_id], dependency_key=self.get_user_info_dependency_key(act_id,"",open_id))
            if not user_info:
                user_info = UserInfo()
                ...
                if config.get_value("is_cache_empty",False) == True:
                    user_info_model.delete_dependency_key([self.get_user_info_dependency_key(act_id, user_info.id_md5), self.get_user_info_dependency_key(act_id, "", user_info.open_id)])
            else:
                ...
                user_info.modify_date = now_datetime
                user_info.login_token = SevenHelper.get_random(16)
                result = user_info_model.update_entity(user_info, "modify_date,login_token,is_new,user_nick,user_nick_encrypt,avatar,is_auth,union_id")
                if result > 0:
                    user_info_model.set_dependency_cache(dependency_key=self.get_user_info_dependency_key(act_id, user_info.id_md5),cache_key=user_info_model.get_cache_key(prefix_key="dict",field="*",where="id_md5=%s",group_by="",order_by="",limit="1",params=[user_info.id_md5]),value=user_info,cache_expire=60)
                    user_info_model.set_dependency_cache(dependency_key=self.get_user_info_dependency_key(act_id,"", user_info.open_id),cache_key=user_info_model.get_cache_key(prefix_key="entity",field="*",where="act_id=%s and open_id=%s",group_by="",order_by="",limit="1",params=[act_id, open_id]),value=user_info,cache_expire=60)
        ...
        try:
            ...
            if not user_info_dict:
                ...
                if update_sql:
                    ...
                    user_account_model.delete_dependency_key(f"user_account:openid_" + user_info_dict["open_id"])
