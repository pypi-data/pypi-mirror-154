...
@LastEditTime: 2022-06-07 09:10:22
    ...
    def save_user_by_openid(self, app_id, act_id, open_id, user_nick, avatar, union_id=""):
        ...
        try:
            ...
            if not user_info:
                ...
                if config.get_value("is_cache_empty",False) == True:
                    user_info_model.delete_dependency_key([f"user_account:openid_{open_id}",self.get_user_info_dependency_key(act_id, user_info.id_md5), self.get_user_info_dependency_key(act_id, "", user_info.open_id)])
            else:
                ...
                user_info.modify_date = now_datetime
                user_info.login_token = SevenHelper.get_random(16)
                result = user_info_model.update_entity(user_info, "modify_date,login_token,is_new,user_nick,user_nick_encrypt,avatar,is_auth")
                if result > 0:
