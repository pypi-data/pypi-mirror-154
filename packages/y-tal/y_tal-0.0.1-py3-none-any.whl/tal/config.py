...
def _default_value(param_name: Config):
    if param_name == Config.MITSUBA2_FOLDER:
        return get_home_path()


def ask_for_config(param_name: Config):
    """Most useful: ask for a specific key in the config,
    and if it does not exist ask the user for a value"""
    config_dict = read_config()
    if 'param_name' not in config_dict or len(config_dict[param_name].strip()) == 0:
