...
def get_entry(times,values,value_index):
    VALUE_INDEX = 0x0
    HEADER_BYTE2 = 0x0
    START_POS2 = 0x1
    LENGTH2 = 0x0
    ...
    DT=0 if VALUE_INDEX == 0 else HEADER_BYTE-0x20
    if LENGTH < 3:
        LENGTH2 = 3 - LENGTH
...
org_values =np.genfromtxt('out2.csv', delimiter=' ', dtype=np.int8)
org_times = np.genfromtxt('out3.csv', delimiter=' ')
time_idxs = np.where(org_values[..., 0] == 0x10)[0]
value_len=len(org_times)
cumulated_values=[]
for i in range(100):
    state,times,values,dt=get_entry(org_times,org_values,1)
    if not state:
        break
    idx2 = np.where(np.isin(values, WHITE_LIST))[0]
    times=np.delete(times,idx2)
    values = np.delete(values, idx2)
    if len(times)==0:
        continue

    full_values = np.zeros(value_len, dtype=np.uint32)

    idxs = np.intersect1d(org_times, times, return_indices=True)[1]
    idxs=idxs-dt
    full_values[idxs]=values

    np.hstack([cumulated_values,full_values])
times=org_times[time_idxs]

print(times,cumulated_values)
#return (times, values)
