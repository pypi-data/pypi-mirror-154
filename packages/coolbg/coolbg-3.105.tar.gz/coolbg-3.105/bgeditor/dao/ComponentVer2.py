...
from bgeditor.common.utils import cache_file, download_file, upload_file, upload_static_file
...
def create_lyric_outro(deezer_artist_id, lyric_outro_repeat,  arr_comp, outro_claim_topic = None, time_o=5):
    ...
    try:
        ...
        tmp_clip.write_videofile(tmp_path_composite_outro, bitrate='500k', fps=24, codec='libx264', audio=False)
        tmp_clip.close()
...
def create_video(list_comp_data, path_video, job_id, mf_server):
    ...
    for comp in arr_comps:
        if indexVidBG < 5 and  (comp.type == "image" or comp.type == "video"):
            ...
            indexVidBG+=1
        if is_lyric_ver2 and comp.duration>intro_time:
    ...
    for comp in arr_comps:
        ...
        if is_lyric_ver2 and (comp.type == "lyric" or comp.type == "mix_lyric"):
            audio_compilation_path = comp.get_audio()
            lyric_comp = comp
            if deezer_artist_id and f"{deezer_artist_id}".isnumeric() and int(deezer_artist_id) < 907996688:
        ...
        if comp.type == "video":
            ...
            if comp.is_intro:
                ...
                rs.write_videofile(introPath, bitrate='500k', fps=24, codec='libx264', audio=False)
                is_cont = True
                rs.close()
            if comp.is_outro:
                ...
                if not rs.audio:
                    ...
                    vidPathTmp = FFmpeg.add_null_sound(comp.video_path)
                    rs = VideoFileClip(vidPathTmp)
                rs.write_videofile(outroPath, bitrate='500k', fps=24, codec='libx264', audio=False)
                is_cont = True
                rs.close()
            if is_cont:
                continue
    ...
    if not is_music:
        ...
        final_clip.write_videofile(path_video, bitrate='500k', fps=24, codec='libx264', logger=logger)
        final_clip.close()
    else:
        # make Compilation
        ...
        if intro_time_real > 0:
            ...
            tmp_clip.write_videofile(tmp_path_composite_intro, bitrate='500k', fps=24, codec='libx264', audio=False, logger=logger)
            # tmp_clip.close()
        ...
        final_clip.write_videofile(tmp_path, bitrate='500k', fps=24, codec='libx264', audio=False, logger=logger)
        #crop big video
        ...
        # final_clip.write_videofile(tmp_path, bitrate='500k', fps=24, codec='libx264', audio=False)
        final_clip.close()
        if cnt_effect_vid > 1:
            ...
            clip.write_videofile(tmp_path_maked_loop, bitrate='500k', fps=24, codec='libx264', audio=False, logger=logger)
            clip.close()
            final_clip.close()
