...
def load_files(filelist: list[str]) -> tuple[np.ndarray, np.ndarray, np.ndarray, list[dict]]:
    ...
    y, wl, ord, h = _load_file(filelist[0])
    Y = np.full((len(y), len(filelist)), np.nan)
    Y[:,0] = y
    head = [h]
    if len(filelist) > 1:
        for i,file in enumerate(filelist[1:], start=1):
            y,x,o,h = _load_file(file)
            assert np.all(x == wl), "Can only merge spectra with identical wavelength axis"
            assert np.all(o == ord), "Can only merge spectra with identical orders"
            Y[:,i] = y
            head.append(h)
    return Spectra(Y, wl, ord, head)


spec_extensions = ['.ary', '.aryx']


def find_spectra_in_folder(folder: str, extensions=spec_extensions) -> list[str]:
    """
    ...
    """
    return [os.path.join(folder, file) for file in os.listdir(folder) if any(ext == os.path.splitext(file)[1] for ext in extensions)]


def load_folder(folder, extensions=spec_extensions) -> Spectra:
    """
