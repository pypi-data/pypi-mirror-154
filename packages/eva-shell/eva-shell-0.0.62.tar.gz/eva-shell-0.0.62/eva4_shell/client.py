...
def call_rpc(method, params=None, target='eva.core'):
    common.elbus_conn_no += 1
    name = f'{common.elbus_name}.{common.elbus_conn_no}'
    if current_command.debug:
        debug(f'calling ELBUS {common.elbus_path} as {name}')
    if common.bus is None or not common.bus.is_connected():
        common.bus = elbus.client.Client(common.elbus_path, name)
        common.bus.connect()
