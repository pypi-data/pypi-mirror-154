...
def HLLC_flux(qL, qR, n):
    ...
    fL = ti.Vector([
        rL * vnL, rL * vnL * uL + pL * nx, rL * vnL * vL + pL * ny,
        rL * vnL * HL
    ])
    fR = ti.Vector([
        rR * vnR, rR * vnR * uR + pR * nx, rR * vnR * vR + pR * ny,
        rR * vnR * HR
    ])
    ...
    sL = min(vnL - aL, vn - a)
    sR = max(vnR + aR, vn + a)
    sM = (pL - pR + rR * vnR * (sR - vnR) - rL * vnL *
          (sL - vnL)) / (rR * (sR - vnR) - rL * (sL - vnL))
        ...
        if is_interior_x_face(i, j):
            ...
            for f in ti.static(range(4)):
                ...
                wR[f] = W[i, j][f] - 0.5 * mc_lim(ratio_r) * (W[i + 1, j][f] -
                                                              W[i, j][f])
            F_x[i, j] = HLLC_flux(w_to_q(wL), w_to_q(wR), ti.Vector([1.0,
                                                                     0.0]))
        ...
        if is_interior_y_face(i, j):
            ...
            for f in ti.static(range(4)):
                ...
                wR[f] = W[i, j][f] - 0.5 * mc_lim(ratio_r) * (W[i, j + 1][f] -
                                                              W[i, j][f])
            F_y[i, j] = HLLC_flux(w_to_q(wL), w_to_q(wR), ti.Vector([0.0,
                                                                     1.0]))
...
def compute_F_thinc():
    ...
    for i, j in Q:
        if is_interior_cell(i, j):
            for f in ti.static(range(4)):
                ...
                w0, w1 = thinc(W[i - 1, j][f], W[i, j][f], W[i + 1, j][f],
                               beta_smooth)
                W_xr[i, j, 0][f] = w0
                W_xl[i + 1, j, 0][f] = w1
                ...
                w0, w1 = thinc(W[i, j - 1][f], W[i, j][f], W[i, j + 1][f],
                               beta_smooth)
                W_yr[i, j, 0][f] = w0
                W_yl[i, j + 1, 0][f] = w1
    ...
    for i, j in Q:
        ...
        if is_interior_x_face(i, j):
            # muscl reconstrucion of left and right states with HLLC flux
            F_x[i, j] = HLLC_flux(w_to_q(W_xl[i, j, 2]), w_to_q(W_xr[i, j, 2]),
                                  ti.Vector([1.0, 0.0]))
        elif is_boundary_x_face(i, j):
            F_x[i, j] = HLLC_flux(Q[i - 1, j], Q[i, j], ti.Vector([1.0, 0.0]))

        if is_interior_y_face(i, j):
            F_y[i, j] = HLLC_flux(w_to_q(W_yl[i, j, 2]), w_to_q(W_yr[i, j, 2]),
                                  ti.Vector([0.0, 1.0]))
        elif is_boundary_y_face(i, j):
            F_y[i, j] = HLLC_flux(Q[i, j - 1], Q[i, j], ti.Vector([0.0, 1.0]))
