...
def tableprint(datain,header=None,fields=None,fieldcolors=None,short=False,enum=False,keyname='key',title=None,emptyremove=False):
	...
	for d in data:
		for var in fields:
			if d.get(var) and isinstance(d[var], bool):
				if d[var]:
					d[var]="Yes"
				else:
					d[var]="No"
	...
	for d in data:
		...
		for var in fields:
			...
			if fieldcolors.get(var):
				print(c.get(fieldcolors[var]), end="")
			else:
				print(c.get("none"), end="")
			print(strf,end="")
			if sizes is None:
				print(c.get("none")+" ",end="")
			else:
...
def replacevars(data, varlist, flags=[], __debug=False, __debugiter=0):
	#__debug=False
	#__debug=True
...
def replacevars_str(data, varlist, flags=[], __debug=False, __debugiter=0):
	#__debug=False
	#__debug=True
	if __debug:
