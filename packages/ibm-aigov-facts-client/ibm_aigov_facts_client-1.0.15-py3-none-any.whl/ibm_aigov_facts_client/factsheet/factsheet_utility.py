...
class FactSheetElements:
    ...
    def register_model_entry(self,model_entry_props:'ModelEntryProps'):
        ...
        :param ModelEntryProps mdoel_entry_props: Properties about model asset and model entry catalog.
        ...
        _logger.debug("props are {}".format(meta_props))
        ...
        _logger.debug("final payload {}".format(payload))
        ...
        _logger.debug("final url: {} params {}".format (url,params))
        if model_entry_id:
            _logger.info("Initiate linking model to existing model entry {}......".format(model_entry_id))
        else:
            _logger.info("Initiate linking model to new model entry......")
        ...
        if response.status_code == 200:
            ...
            _logger.info("Successfully finished linking Model {} to Model Entry".format(model_asset_id))
    ...
    def unregister_model_entry(self, asset_id, catalog_id=None ,project_id=None, space_id=None):
        ...
        if response.status_code == 204:
            _logger.info("Successfully finished unregistering WKC Model {} from Model Entry.".format(asset_id))
        else:
