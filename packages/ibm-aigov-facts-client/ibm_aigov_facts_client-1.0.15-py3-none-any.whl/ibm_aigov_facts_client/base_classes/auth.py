...
class FactsheetServiceClientAutolog:
    ...
    def add_payload(self, container_type: str, container_id: str, run_id: str, token: str, payload: Dict[str, Any], external:bool,is_cp4d:bool, **kwargs) -> DetailedResponse:
        ...
        _logger.debug("params are {}".format(params))
        ...
        _logger.debug("url using {} with headers {}".format(url,headers))
       ...
       # try:
        ...
        _logger.debug("final response..{}".format(response.json()))

        if not response:
            if self.run_id:
                set_custom_tag_autolog(run_id)
        ...
        if response.status_code == 403:
            ...
            if self.run_id and asset_id:
                set_custom_tag_autolog(run_id, "True")
            ...
            if not external and params_key is not None:
                _logger.info("Successfully logged results to Factsheet service for run_id {} under asset_id: {} and {} : {}".format(
                    self.run_id, asset_id, params_key, self.container_id))
            else:
                _logger.info("Successfully logged results to Factsheet service for run_id {} under asset_id: {} ".format(
                    self.run_id, asset_id))

        else:
            _logger.error(
                "Something went wrong when posting to factsheet,{}".format(response.json()['message']))
...
class FactsheetServiceClientManual:
    ...
    def add_payload(self, run_id: str, token: str, payload: Dict[str, Any],external:bool,is_cp4d:bool, **kwargs) -> DetailedResponse:
        headers = {}
        params={}
        self.run_id = run_id
        ...
        if self.is_cp4d:
            ...
            if get_env() == 'dev':
                ...
                url = prod_config["DEFAULT_SERVICE_URL"] + \
                    '/v1/aigov/factsheet/notebook_experiments'
        _logger.debug("url using {}".format(url))

       # try:
        _logger.debug(
            "Sending payload to factsheet......... {} url {} params {}".format(data,url,params))
        response = requests.post(url=url,
                                 headers=headers,
                                 params=params,
                                 data=data)
