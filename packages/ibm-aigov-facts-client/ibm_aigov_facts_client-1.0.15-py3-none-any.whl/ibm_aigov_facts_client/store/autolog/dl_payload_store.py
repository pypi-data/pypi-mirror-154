...
class GetFinalPayloadDl(AbstractPayloadStore):
    ...
    def get_final_payload_and_publish(self, parent_path, metric_files):
        self.parent_path = parent_path
        self.metric_files = metric_files

        #_logger.debug("metric files......{}".format(metric_files))

        if DL_EPOCHS_TAG in self.current_data.params:
            tag_name = DL_EPOCHS_TAG
        else:
            tag_name = TF_EPOCHS_TAG

        if(self.current_data.params.get(tag_name) != None):
            ...
            num_to_match = get_epochs_num_to_match(
                self.current_data.params, tag_name)

            _logger.debug("number to match....{}".format(num_to_match))
            ...
            if (max(max_step_count) == num_to_match):
                ...
                _logger.debug("auth status {}".format(check_auth))
