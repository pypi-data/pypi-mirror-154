...
class GetFinalPayloadDl(AbstractPayloadStore):
    ...
    def get_metrics_files(self, exp_id, run_id, root_dir):
        ...
        _logger.debug("run dir...{}".format(run_dir))
        _logger.debug("subfolder...{}".format(self.subfolder))
        source_dirs = find(run_dir, self.subfolder, full_path=True)
        _logger.debug("source dir...{}".format(source_dirs))
    ...
    def gen_payload(self, run_data, root_dir=None):
        ...
        if (fact_trace.FactsClientAdapter._autolog):
            _logger.debug("cur exp names are {} {}".format(
                cur_run_exp_name, autolog.AutoLog._cur_exp.name))
            ...
            _logger.debug("cur exp names are {} {}".format(
                cur_run_exp_name, manual_log.ManualLog._cur_exp.name))
        ...
        runs["created_date"] = datetime.today().strftime('%Y%m%d')
    ...
    def get_payload_and_publish(self, root_dir, exp_id, run_id):
        ...
        _logger.debug("files.....{}".format(metric_files))
        ...
        _logger.debug("final metrics.............{}".format(allmetrics))
        ...
        # _logger.debug("metric files......{}".format(metric_files))
        ...
        _logger.debug("tag_name {} and num_max_step: {}".format(
            tag_name, num_max_step))

        if(run_data.params.get(tag_name) != None):

            num_to_match = get_epochs_num_to_match(
                run_data.params, tag_name)
            _logger.debug("num to match {}".format(num_to_match))

            if (max(max_step_count) == num_to_match):
                allmetrics = format_metrics(allmetrics)
                run_data.metrics.clear()
                run_data.metrics['data'] = allmetrics
                _logger.debug("run data...{}".format(run_data))
                self.final_payload = self.gen_payload(run_data)
