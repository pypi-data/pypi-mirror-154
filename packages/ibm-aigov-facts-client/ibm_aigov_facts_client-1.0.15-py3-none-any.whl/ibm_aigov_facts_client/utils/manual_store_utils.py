...
def get_metric_from_file_custom(parent_path, metric_name):
    ...
    _logger.debug("metrcis obj...{}".format(metric_objs))
    _logger.debug("max step..{}".format(max_step))
...
def check_framework_support_manual(tags, values=[], key=None):
    if key is None:
...
def check_framework_support_manual_custom(tags, values=[], key=None):
...
def _has_experiment(experiment_id, root_dir):
    return _get_experiment_path(experiment_id, root_dir) is not None


def get_run_dir(experiment_id, run_uuid, root_dir):
    validate_run_id(run_uuid)
    if not _has_experiment(experiment_id, root_dir):
        return None
    return os.path.join(_get_experiment_path(experiment_id, root_dir, assert_exists=True), run_uuid)


def _get_experiment_path(experiment_id, root_dir, view_type=ViewType.ALL, assert_exists=False):
    _logger.debug("run dir......{}".format(root_dir))
    parents = []
    if view_type == ViewType.ACTIVE_ONLY or view_type == ViewType.ALL:
        parents.append(root_dir)
    if view_type == ViewType.DELETED_ONLY or view_type == ViewType.ALL:
        parents.append(TRASH_FOLDER)
...
def make_containing_dirs(path):
    """
    Create the base directory for a given file path if it does not exist and creates parent
    directories.
    """
