...
class GenerateExpId:
    ...
    def generate_guid(self, cur_exp):
        _logger.debug("generating guid for experiment {}".format(cur_exp.name))
        if not exists(self.root_directory):
            new_guid = self.get_new_guid()
        else:
            chk_exp_exists = mlflow.get_experiment_by_name(cur_exp.name)
            _logger.debug(
                "check if exp exists flag..{}".format(chk_exp_exists))
            if chk_exp_exists is not None:
                _logger.debug("current experiment exists")
                check_runs = mlflow.search_runs([cur_exp.experiment_id])
                _logger.debug(
                    "current available run_ids: {}".format(check_runs.run_id))
                if not check_runs.empty and "tags.GUID" in check_runs:
                    _logger.debug("current runs found..getting guid")
                    new_guid = self.get_cur_exp_guid(check_runs)
                else:
                    _logger.debug("no current runs found")
                    new_guid = self.get_new_guid()
            else:
                new_guid = self.get_new_guid()
    ...
    def get_exp_guid(self, experiment_name):
        _logger.debug("getting guid for exp {}".format(experiment_name))
        check_if_exp_exists = mlflow.get_experiment_by_name(experiment_name)
        _logger.debug(
            "check if exp exists flag..{}".format(check_if_exp_exists))
        if check_if_exp_exists is not None:
            ...
            if not check_runs.empty and "tags.GUID" in check_runs:
                ...
                _logger.debug("current exp guid...{}".format(cur_guid))
            else:
    ...
    def get_cur_exp_guid(self, cur_runs):
        _logger.debug("Getting GUID from existing runs {}".format(cur_runs))
        _logger.debug("existing GUIDs, {} and shape {}".format(
            cur_runs["tags.GUID"].unique(), cur_runs.shape[0]))
        cur_id = cur_runs[(~cur_runs["tags.GUID"].isnull()) & (
            cur_runs["tags.GUID"] != "")]["tags.GUID"].unique()[0]
        _logger.debug("Found existing guids {} and selected guid {}".format(
            pd.Series(cur_runs["tags.GUID"].values), cur_id))
        return cur_id

    def get_new_guid(self):
        _logger.debug("Creating new GUID")
    ...
    def gen_new_tag(self):
        ...
        _logger.debug("exp {}".format(self.experiment))
        self.EXP_GUID = self.generate_guid(self.experiment)
        _logger.debug("guid {}".format(self.EXP_GUID))
