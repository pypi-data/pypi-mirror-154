...
class Manager:
  """The ``laboro.module.manager.Manager`` object manages all aspect of module handling such as loading and, registration,
  """
  modules = dict()
  ...
  def install_package(self, package):
    ...
    try:
      ...
      if re.match(reg, package) is not None:
        ...
        logging.critical(f"Package is not a valid Laboro package: {package}")
        raise LaboroError(f"InvalidPackageNameError: Package is not a valid Laboro package: {package}")
    except subprocess.CalledProcessError as err:
      logging.critical(f"Unable to install Laboro package: {package}")
  ...
  def register_class_from_module(self, cls, module):
    ...
    Registering a module is used to import external Laboro modules (search the documentation for 'external Laboro modules').
    ...
    In the following example illustrate how to import the ``PgClient`` class from the ``laboro_database`` package.
    ...
    ..  code-block:: python
      ...
      module_mgr = Manager()
      module_mgr.register(cls="PgClient", module="laboro_database")

    """
    if cls not in self.modules:
      logging.info(f"  [+] Registering class: {module}.{cls}")
      try:
        ...
        logging.critical(f"No such module: {module}")
  ...
  def get_class_from_module(self, cls, module):
    ...
    Raises:
      ``laboro.error.LaboroError``: When the specified module or class is unknown.
    """
    try:
      return self.modules[module][cls]
    except KeyError as err:
      logging.critical(f"No such class: {module}.{cls}")
