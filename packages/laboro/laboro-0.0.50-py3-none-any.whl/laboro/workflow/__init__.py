...
class Workflow:
  ...
  def __exit__(self, kind, value, traceback):
    self.context.exit(self.name, self.session, kind, value)

  def _instantiate(self, module, cls, args):
    logging.info(f"[+] Object instantiation: {module}.{cls}")
    mod = self.context.modulemgr.get_class_from_module(cls=cls, module=module)
    return mod(context=self.context, args=args)
