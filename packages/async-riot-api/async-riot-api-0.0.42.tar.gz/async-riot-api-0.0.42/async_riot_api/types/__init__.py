...
class RiotApiResponse:
    ...
    def to_string(
            ...
            quote_attrs: bool = False,
            skip_none: bool = False,
            lower_bools: bool = False
    ) -> str:
        """
        Returns a prettified string representation of the object.

        :param level: starting level of indentation. Default: 0
        :param indent_char: character sequence for indentation. Default: 4 spaces
        :param nl: new line sequence. Default '\\n'
        :param o_par: parenthesis to use when opening the object. Default: '('
        :param c_par: parenthesis to use when closing the object. Default: ')'
        :param show_class_name: whether to show the class name at the beginning of each complex object. Default: True
        :param sep: separator to use when printing the name value of an object. Default: '='
        :param quote_attrs: whether to quote the attribute name. Default: False
        :type level: int
        :type indent_char: str
        :type nl: str
        :type o_par: str
        :type c_par: str
        :type show_class_name: bool
        :type sep: str
        :type quote_attrs: bool
        """

        def recursion(obj, level: int = level, indent_char = indent_char):
            if isinstance(obj, RiotApiResponse):
                return obj.to_string(
                    ...
                    skip_none = skip_none,
                    lower_bools = lower_bools
                )
            if isinstance(obj, list):
                return f'[{nl}{indent_char * (level + 2)}' + (
                    f',{nl}{indent_char * (level + 2)}'.join(
                        map(lambda x: recursion(x, level + 1, indent_char), obj)
                    )) + f'\n{indent_char * (level + 1)}]'
            if isinstance(obj, str):
                return f'"{obj}"'
            if isinstance(obj, bool):
                return str(obj).lower() if lower_bools else str(obj)
            return str(obj)

        to_skip = ['success', 'error', '_RiotApiResponse__success']
        return '{}{}{}{}{}{}{}{}'.format(
            type(self).__name__ if show_class_name else '',
            o_par,
            nl,
            indent_char * (level + 1),
            f',{nl}{indent_char * (level + 1)}'.join(
                '{}{}{} {} {}'.format('"' if quote_attrs else '', item[0], '"' if quote_attrs else '', sep, item[1])
                for item in map(
                    lambda x: (x[0], recursion(x[1])),
                    filter(lambda x: x[0] not in to_skip and (not skip_none or x[1] is not None), vars(self).items())
                )
            ),
            nl,
            indent_char * level,
...
class MTLFrameDto(RiotApiResponse):
    def __init__(self, events: List[dict], participantFrames: dict, timestamp: int, **kwargs):
        ...
        self.participantFrames: List[MTLParticipantFrameDto] = list(
            map(
                lambda x: MTLParticipantFrameDto(**x),
                map(lambda x: x[1], sorted(participantFrames.items(), key = lambda x: int(x[0])))
            )
        )
