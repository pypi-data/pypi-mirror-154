...
@throw_with_message
def get_cluster_aws_account_id(cluster_id: str) -> str:
    c = GridRestClient(create_swagger_client())
    resp = c.cluster_service_get_cluster(cluster_id)

    role_arn = resp.spec.driver.kubernetes.aws.role_arn
    if not isinstance(role_arn, str) or role_arn == "" or role_arn.count(":") != 4:
        raise ValueError("invalid cluster role returned from server")
