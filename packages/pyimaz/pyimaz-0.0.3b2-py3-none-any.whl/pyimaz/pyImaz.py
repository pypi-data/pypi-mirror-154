...
import numpy as np
import pyimaz._pyImaz as _pyImaz


def imaz(lat,lon,year,day,hour,rio_abs, magAp,f10_7,alt):
    # REAL iriinput(9), imazoutput(4,60) FORTRAN-КОД
    iriinput = np.arange(0,10)
    iriinput[0] = lat
    iriinput[1] = lon
    iriinput[2]= year
    iriinput[3] = day
    ...
    iriinput[7] = f10_7
    iriinput[8] = alt
    inf = np.arange(0.0, 8.0)  # +1 по сравнению с фортран_кодом
    of = np.arange(0.0, 3.0)  # +1 по сравнению с фортран_кодом
    pressure = _pyImaz._get_imaz_pressure()  # местоположение!!
    ninf = np.arange(0.0, 7.0)  # +1 по сравнению с фортран_кодом
    ...
    if (iriinput[5] == -1.0):
        ...
        nolv = 0
        lra = calcresabs(iriinput[3], chp, iriinput[7])  # call
        Lv = iriinput[5]
        Li = Lv + lra
...
def nolvcase(nolinps, nolout=None):
...
def imaz_network(in_netw):
    ...
    #print('weight', act.shape, weights.shape)
    #count1=0
    #count2=0
# hidden layer
...
def imaz_errnetwork(in_err):
    ...
    out = np.arange(0, OUTNODES)
    weights = np.arange(0, N)
    biases = np.arange(0, M)
    act = np.arange(0, M)

    data_weights = np.array([-0.105310, -1.127650, 0.392770, -0.713520, 0.925380, -1.606360,
...
def imaz_nolv_network(IMAZ_IN):
    ...
    for member in range(0, HIDNODES1):
        ...
        if (IMAZ_sum + data_biases[unit]) < 10000.0:
            act[unit] = 1 / (1 + np. np.exp(-sum - data_biases[unit]))
        else:
            act[unit] = 0.0
...
def csza(ld, t, flat, flon):
    ...
    # real*8 tf, eqt, et, fa, phi, a, b, np.cosx
    # real*8 declin, p(0:6)
...
def calcchap(z):
    ...
    #print(values.shape)
    # unp.sing chi calculate chapman function индексы пофикшены
...
d = imaz(70,60,2005,190,10,0,1,50,80)
print (d)