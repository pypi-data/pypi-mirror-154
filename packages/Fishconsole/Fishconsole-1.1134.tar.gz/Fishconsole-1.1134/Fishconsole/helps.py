    ...
    if True:
        try:
            res = window.列表选择对话框(
                ...
                ["Matplotlib中文辅助模块", "easygui中文辅助模块", "logs控制台输出辅助模块", "Fishsys模块", "fcv_debug控制台","files模块"])
        except:
                    ...
                    for a in res:
                        ...
                        if a == "缓存模块":
                            window.弹窗(
                                ...
                                "\n-----------------------------\n\n"
                                "语法就是\n\n"
                                "files.缓存(模式, '字典数据','文件名','变量名'\n（请注意，字符串用单引号括起来，千万不要用双引号，否则将过不了加密',缓存文件的名字)\n\n"
                                "模式有三种\n\n1:写\n覆写模式就是创建缓存文件，你需要两个模式和缓存一起使用\n\n2：读\n你只需要两个参数（模式，文件名），他会返回给你一个列表\n但如果你在之前没有创建文件，那么我们将会返回一个布尔值（False）\n\n3：插入模式，你只需要指定一个dict，dict中填写你想改变或插入的dict键值对，我们就会直接在原来的缓存文件中将他们插入进去"
                                "但是每次都在后面指定文件的名字的话就有点废手了，为了避免这种情况，我们可以在开头就指定好文件的名字，这样就可以了\n\n"
                                "示例语法\n"
                                "Filename='1.txt'\n"
                                "a = 缓存(1, 数据='{'a':123,'b':456}',文件名=Filename) #覆写模式\n"
                                "a = 缓存(2,变量名='a',文件名=Filename) #读模式\n"
                                "a = 缓存(3,文件名=Filename,数据='{'a':123,'b':456}') #插入/修改模式\n"
                                , "缓存模块", "ok")
