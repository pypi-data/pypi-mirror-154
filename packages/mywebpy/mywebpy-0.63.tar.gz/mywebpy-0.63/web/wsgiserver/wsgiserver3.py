...
class HTTPConnection(object):
    ...
    def communicateWithConn(self,req,sendMessage,timeout=0):
        """Read each request and respond appropriately."""
        request_seen = False
        try:
            req.simple_response("200 OK", sendMessage)
            while True:
                ...
                request_seen = True
                #req.respond()
                cl = int(req.inheaders.get(b"Content-Length", 0))

                return req.rfile.read(cl),req
                if req.close_connection:
                    return
        except socket.error:
            e = sys.exc_info()[1]
...
class WSGIGateway_10(WSGIGateway):
    ...
    def get_environ(self):
        ...
        env = {
            ...
            'SERVER_REQ':req,
	        'SERVER_CONN':req.conn,
            'SERVER_SOCKET':req.conn.socket,
            'SERVER_SERVER':req.conn.server,
