...
def get_current_pad():
    ...
    for ao_i in range(18, 20):
        print("need AO pad for {ao}".format(ao=ao_i))
        pad_pattern = arc_url + "/aux/org/AO{ao}/pad_{ao}_%04d.fits".format(ao=ao_i)
        pad_list = []
        ...
        for pad in pad_list[::-1]:
            try:
                ...
                print("WARNING: unable to open pad {}, choosing previous one".format(ao), e)

    return np.concatenate(ao_list)


def get_pointings(revid, kind="historic", getdata=True):
    if getdata:
    ...
    if kind == 'historic':
        pt = pvphase_url + "/aux/adp/%s.000/pointing_definition_%s.fits" % (revid, kind)
    elif kind == 'predicted':
