...
class RNNBase_cdopt(Module):
    ...
    def A_test(self, _flat_wights):
        return [self.manifold.A(weight) if self.cdopt_weight_hh_bool[i] else weight  for i, weight in enumerate(_flat_wights) ]


    def A(self, _flat_wights):
        return self.flatten_parameters_cdopt([self.manifold.A(weight) if self.cdopt_weight_hh_bool[i] else weight  for i, weight in enumerate(_flat_wights) ])

    def C(self, _flat_wights):
        return [ self.manifold.C(_flat_wights[i]) for i in  self.cdopt_weight_hh_index]
...
class RNNCellBase_cdopt(Module):
    ...
    def __init__(self, input_size: int, hidden_size: int, bias: bool, num_chunks: int,
        ...
        self.manifold = manifold_class((num_chunks * hidden_size, hidden_size), device= device, dtype= dtype, **kwargs)
        self.A = self.manifold.A
        self.C = self.manifold.C
