...
class DingTalkNotifyPlugin(CorePluginMixin, notify.NotificationPlugin):
    ...
    def _post(self, group, project):
        ...
        # payload = f"{payload} > status: {group.status} \n\n"
        payload = f"{payload} > 出现次数: {group.times_seen} \n\n"
        payload = f"{payload} > 查看详情: <font color=blue>[{group.title if group.title else 'unknown'}]({issue_link})</font> \n\n"
        ...
        # 获取预设的阀值
        counts = self.get_option("warning_counts", project)
        if int(group.status) == 0:
            data_1 = {
                "msgtype": "markdown",
                "markdown": {
                    "title": group.title,
            ...
            for webhook_url in self.get_webhook_urls(group.project):
                requests.post(webhook_url, data=json.dumps(data_1), headers=headers)

        # if int(group.times_seen) > int(counts):
        #     data_2 = {
        #         "msgtype": "markdown",
        #         "markdown": {
        #             "title": group.title,
        ...
        #
        #     for webhook_url in self.get_webhook_urls(group.project):
        #         requests.post(webhook_url, data=json.dumps(data_2), headers=headers)
