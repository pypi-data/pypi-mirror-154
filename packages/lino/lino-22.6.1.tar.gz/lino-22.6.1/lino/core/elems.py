...
class DisplayElement(FieldElement):
    ...
    def __init__(self, *args, **kw):
        ...
        if hasattr(self.field, 'max_length') and self.field.max_length is not None:  # might be explicit 0
...
class LightWeightContainer(HtmlBoxElement):
    ...
    Note that this creates an automatic VirtualField which is a bit special because it
    is created during :func:`create_layout_element` after the startup
    analysis.
...
class SlaveSummaryPanel(LightWeightContainer):
    """
    The panel used to display a slave table with `display_mode_main` 'summary'.
    """
...
class StoryElement(LightWeightContainer):
    ...
    tables with `display_mode_main` 'story'.
...
def create_layout_element(lh, name, **kw):
    ...
    if isinstance(de, type) and issubclass(de, tables.AbstractTable):
        ...
        if isinstance(lh.layout, FormLayout):
            ...
            # `display_mode_main` of the table.
            ...
            if de.display_mode_slave in [
            ...
            elif de.display_mode_slave == constants.DISPLAY_MODE_HTML:
                # if de.editable:
                #     a = de.insert_action
                #     if a is not None:
            ...
            elif de.display_mode_slave == constants.DISPLAY_MODE_STORY:
                e = StoryElement(lh, de, name, **kw)
                lh.add_data_elem(e.field)
                return e

            elif de.display_mode_slave == constants.DISPLAY_MODE_SUMMARY:
            ...
            elif de.display_mode_slave == 'reactive':  # TODO: is this still being used?
                ...
                raise Exception(
                    "Invalid display_mode %r" % de.display_mode_slave)
