...
from typing import Callable, Dict, Any, cast, Iterator, NamedTuple, Awaitable, List, Optional
...
from .ipython_pb2 import JupyterMessage, CodeRequest, ShutdownRequest, EmptyMessage, NotebookRequest
...
class AsyncGrpcClient:
  channel = None
  def __init__(self, runtimeToken: Optional[str]):
  ...
  async def authenticatedClient(self, kernelInfo: KernelInfo, syncClient:bool=False) -> KernelWrapperStub:
    ...
    authCreds = grpc.metadata_call_credentials(GrpcAuth(kernelInfo.sharedSecret, self.runtimeToken))  # type: ignore
...
class AsyncJupyterClient(AsyncGrpcClient):
  ...
  def __init__(self, apiClient: ModelbitApiClient, processMessage: Callable[[JupyterMessage], Awaitable[Optional[JupyterMessage]]]):
...
class CloudKernelClient:
  ...
  def __init__(self, sendResponse:
               ...
               sendHtml: Callable[[str, Optional[str]], None]):
    logging.basicConfig(level=logging.DEBUG, datefmt='%X.%f')
    self.sendResponse = sendResponse
    self.sendHtml = sendHtml
    self._apiClient = ModelbitApiClient(self._setState, self.sendStatusHtml)
  ...
  def clearMessage(self, dispId) -> None:
    self.sendResponse(
      "update_display_data",
      {
        "data": {},
  ...
  async def _checkpoint(self) -> Dict[str, Any]:
    if self.stateDispId is not None:
      self.stateDispId = self.clearMessage(self.stateDispId)
...
class ModelbitApiClient:
  ...
  def runtimeToken(self) -> Optional[str]:
    return self._mbCore._state.signedToken
  ...
  def _callWeb(self, path: str, data: Dict[str, Any]) -> NotebookResponse:
    if not self._mbCore.isAuthenticated(True):
      ...
      self.sendStatusHtml(self._mbCore.getAuthMessage())
      self._waitForAuth()
      if not self._mbCore.isAuthenticated(True):
          raise NotConnectedError()
