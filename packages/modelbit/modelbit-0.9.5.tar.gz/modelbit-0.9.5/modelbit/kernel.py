...
class ModelbitKernel(Kernel):
  ...
  async def _processLocalMagic(self, code):
    if code.startswith("%%mbinfo"):
      self.sendText(f"kernelId={self._client._apiClient.kernelId} remoteAddress={self._client._apiClient.remoteAddress}")
