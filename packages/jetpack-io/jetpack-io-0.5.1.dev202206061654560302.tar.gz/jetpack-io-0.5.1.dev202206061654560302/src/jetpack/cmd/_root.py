...
def exec_task(
    ...
    _exec_task(entrypoint, exec_id, qualified_symbol, encoded_args)
...
@click.command(help="De-registers jetpack functions with runtime")
def deregister() -> None:
...
def jetworker_server(port: str) -> None:
    ...
    jetworker_pb2_grpc.add_JetworkerServicer_to_server(
        jetworker_pb2_grpc.JetworkerServicer(), server
    )
...
# todo: move this class to a proper file/folder
class JetworkerServicer(jetworker_pb2_grpc.JetworkerServicer):
    # TODO savil. Add param types
    def StartJetroutine(self, request, context):
        # TODO handle the ENTRYPOINT_PARAMS["envvar"]
        entrypoint = ENTRYPOINT_PARAMS["default"]
        _exec_task(
            str(entrypoint),
            request.GetExecId(),
            request.GetQualifiedSymbol(),
            request.GetEncodedArgs(),
        )
...
def _exec_task(
    ...
    print(f"started _exec_task {qualified_symbol}")
    _util.load_user_entrypoint(entrypoint)
    ...
    print(f"finished _exec_task {qualified_symbol}")
