...
"repo_id",list_of_datasets)
    ...
    def add_to_cohort(
        self, repo_key=None, entity_id=None, dataset_id=None, sample_id=None
    ) -> None:
        """
...
instantiated.
        ...
        if not (
            (repo_key and entity_id)
            or (repo_key and dataset_id and sample_id)
            or (repo_key and dataset_id)
        ):
            raise InvalidParameterException("repo_key entity_id dataset_id sample_id")
...
instantiated.
        ...
        if entity_type == "dataset":
            self.core_obj.remove_single_from_cohort(self._cohort_path, entity_id)
        else:
            self.core_obj.remove_multiple_from_cohort(self._cohort_path, entity_id)
