...
converter functions.
    ...
    def _get_columns_with_array_dtype(self, local_file_path: str) -> list:
        ...
        with open(local_file_path) as f:
            reader = csv.reader(f)
            columns_in_csv = next(reader)
            dict_reader = csv.DictReader(f, delimiter=",", fieldnames=columns_in_csv)
            conv_columns = []
            for row in dict_reader:
                for index in range(0, len(columns_in_csv)):
                    key = columns_in_csv[index]
                    val = row[key]
                    if re.search(r"\[(.*?)\]", val):
                        conv_columns.append(key)
                    elif val == "[]":
                        conv_columns.append(key)
                break
