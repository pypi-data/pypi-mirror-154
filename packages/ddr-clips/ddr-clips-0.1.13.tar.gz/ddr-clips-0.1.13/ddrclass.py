...
#
                        if trigger[1] != []:
                            try:
                                ...
                                self.print_log("\n%%%% DDR Error: Asserting syslog static fact: " + str(trigger[1]) + "\n" + str(e))
                            break
                    except:
 ...
 <config xmlns="urn:ietf:params:xml:ns:netconf:base:1.0">
  <ddr-control xmlns="http://cisco.com/ns/yang/cisco-ios-xe-ddr-control">
   ...
   #
    ...
    #
                if self.control["debug-fact"] == 1:
                    self.print_log("**** DDR Debug: get_template_multifacts_index: " + str(fact1))
                   
                try:
                    template.assert_fact(**fact1)
                except Exception as e:
                    if self.control["debug-fact"] == 1:
                        self.print_log("%%%% DDR Exception: get_template_multifacts_index: " + str(e))
                    pass
...
#
    ...
    def run_assert_sim_fact(self, fact_index):
        ...
        try:
            ...
            self.print_log('\n%%%% DDR Error: run_assert_sim_fact fact assert error: ' + str(self.sim_data[int_index]))
    ...
    def run_cli_command(self, cli_index, filename, parameter1, parameter2, addtime):
        ...
        #######################################################################
                try:
                    ...
                    pass
#PVH redirect to file timeout                    self.print_log("\n%%%% DDR ERROR: run_cli_cmd Error: device cli command timeout:\n" + str(cmdline) )
            except Exception as e:
    ...
    def run_delete_file(self, filepath):
        ...
        if self.control["actions"] == 1:
            ...
            try:
                ...
                self.print_log("\n%%%% DDR ERROR: run_delete_file Error: Exception deleting file: " + str(filepath) + " " + str(e))
...
#
    ...
    #
                if mode == 'ssh':
                    ...
                    try:
                        ...
                        child.close()
                        self.print_log("\n%%%% DDR ERROR: ssh run_ping_action SSH or timeout Error: " + str(ssh_cmd) + "\n")
    ...
    #
                    ...
                    try:
                        ...
                        self.print_log("\n%%%% DDR ERROR: cli run_ping_action ssh command Error: " + str(e) + "\n")
                        message = '(' + str(fact_result) + ' (neighbor ' + str(neighbor) + ') (percent 0) (result FALSE))'
                        if self.control["debug-fact"] == 1:
                            self.print_log("**** DDR Debug: cli Error Assert run_ping_action SSH FACT: " + str(message))
                        try:
   ...
   #
                    ...
                    self.print_log("\n%%%% DDR ERROR: invalid run_ping_action ssh command Error: " + str(e) + "\n")
                    message = '(' + str(fact_result) + ' (neighbor ' + str(neighbor) + ') (percent 0) (result FALSE))'
                    if self.control["debug-fact"] == 1:
                ...
                self.print_log("\n%%%% DDR ERROR: global run_ping_action ssh command Error: " + str(e) + "\n")
                message = '(' + str(fact_result) + ' (neighbor ' + str(neighbor) + ') (percent 0) (result FALSE))'
                if self.control["debug-fact"] == 1:
    ...
    #
        ...
        try:
            ...
            if self.control["debug-fact"] == 1:
                self.print_log("\n%%%% DDR ERROR: run_trace fact assert error: " + str(message) + " " + str(e))
            return
