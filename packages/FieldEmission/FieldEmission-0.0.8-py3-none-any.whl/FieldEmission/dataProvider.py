...
class DataProvider:
    def __init__(self, ColumnHeaderKeysOrDict, NumpyArray: np.ndarray):
        if type(ColumnHeaderKeysOrDict) == dict:
            self.ColumnHeader = ColumnHeaderKeysOrDict
        else:
    ...
    def ClipData(self, ColumnKeyOrIndex, ValueExcluded, ClipFlag: ClipFlags):
    ...
    def RemoveColumn(self, ColumnKeyOrIndex):
        colIndex = self.__GetColumnIndexFromKey__(ColumnKeyOrIndex)

        self.Data = np.delete(self.Data, colIndex, axis=1)  # Remove data-column
        self.ColumnHeader.pop(self.__GetKeyFromColumnIndex__(colIndex))  # Remove corresponding HeaderColumn-Entry
        self.__DetermineColumns__()  # Update self.Columns

        # Update the dictionary to match the new data-column-order
        for newColIndex in range(colIndex + 1, self.Columns + 1):
            colKey = self.__GetKeyFromColumnIndex__(newColIndex)
            self.ColumnHeader[colKey] = newColIndex - 1
