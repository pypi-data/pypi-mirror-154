...
class BaseEntities:
    ...
    def players(team_id: int):
        return "select rp.wyscout_id, rp.rankset_name from rankset.players as rp, rankset.teams as rt, wyscout.teams as wt " \
...
class Players:
    ...
    def players_filter(object_id: int, data_type: int, argument: list = None):
        ...
        match data_type:
            case 1:
                return BaseEntities.players(team_id=object_id)
            case 2:
                return f"select rp.wyscout_id, rp.rankset_name from rankset.players as rp, rankset.teams as rt, " \
            ...
            case 3 | 4:
                return "select distinct rp.wyscout_id, rp.rankset_name from rankset.players as rp " \
            ...
            case 5 | 6:
                val = argument[0]
                position = [argument[1]]
                return "select distinct rp.wyscout_id, rp.rankset_name " \
    ...
    def stats(player_id: int, required_stats: list):
        ...
        return "select distinct stat_name, " \
               "cast(sum(stat_value) / (select count(distinct event_id) from wyscout.player_stats " \
               f"where object_id = {player_id}) as decimal(10,2)) as stat_value " \
               f"from wyscout.player_stats where object_id = {player_id} " \
