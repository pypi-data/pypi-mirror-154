...
def get_dir_content(dir_path, recursive=False, collect_dirs=True, collect_files=True, ignored_files_arr=['.DS_Store'], sort_by_name=True):
    ...
    for path, dirs, files in os.walk(dir_path):
        ...
        if collect_files:
            ...
            path_files = append_path_to_list(path, files)
            legal_files = list(filter(lambda x: (not is_dir_exists(x)), path_files))
            f_list += legal_files
...
# will create a file. If you want to create a text file, call os-file-stream-handler
def create_file(path, content: list = None):
    parent_dir = get_parent_path(path)
    if not is_dir_exists(parent_dir):
...
def copy_file(src, dst, create_path_if_needed=True, overwrite_if_needed=True):
    """
    Will copy a file to a dest.

    Args:
        param create_path_if_needed: set to true if the parent directory doesn't exists and you want to create it
        param overwrite_if_needed: toggle to true to overwrite if file exists
    """
    if is_file_exists(dst) and not overwrite_if_needed:
        return
...
# will search for a file in a path by prefix, suffix, full name with/without an extension. The extension can be string or a list '.java'/['.java','.kt']
def search_file(path_to_search, full_name=None, prefix=None, suffix=None, by_extension=None, recursive=True, filter_files_list=['.DS_Store']):
    from pathlib import Path
    files = []
    search_queue = ''
    if recursive:
    ...
    # fail safe against "fetch all files", recursive or not
    ...
    if recursive and not search_queue.endswith('*'):
        search_queue += '*'

    for filename in Path(path_to_search).glob(search_queue):
        if is_dir_exists(filename) and by_extension is not None:  # filter directories, if required
            continue
        if by_extension is not None:
            file_ext = get_extension_from_file(filename)
            if isinstance(by_extension, list):
                end = next((x for x in by_extension if x.endswith(file_ext)), None)
                if end is None:
                    continue
            if isinstance(by_extension, str) and not file_ext == by_extension:
                continue
        if filter_files_list is not None and get_file_name_from_path(filename) in filter_files_list:
            continue
...
# call this function if you had a permission issue with the normal copy. Specifically permission denied
def copy_dir_with_permission_denied(src, dst, ignored_files=['.DS_Store'], overwrite_dir_if_exists=False):
    if overwrite_dir_if_exists:
        remove_dir(dst)
    if is_dir_exists(dst):
        return
    create_dir(dst)
    files = get_dir_content(src, recursive=True, collect_dirs=False, collect_files=True)
    for file in files:
        filename = get_file_name_from_path(file, with_extension=True)
        if filename in ignored_files:
            continue
        rel_dest = str(file).replace(f'{src}/', '')
        new_dest = os.path.join(dst, rel_dest)
        copy_file(file, new_dest)
...
def dict_to_json_file(dictt, output_json):
    import json
    if not is_dir_exists(get_parent_path(output_json)):
        create_dir(get_parent_path(output_json))
...
# Will return the latest added file from a given directory
def get_latest_added_file(dir_path):
    import glob
    import os
