...
from cubicweb.predicates import authenticated_user, anonymous_user, match_form_params
...
class ViewController(Controller):
    ...
    def publish(self, rset=None):
        ...
        if hasattr(self._cw, "path"):
            logger.info(
                f"on {self._cw.path} controller {self} selected view {view} for rset"
                f" {rset}"
            )
        ...
        return self._cw.vreg["views"].main_template(
            self._cw, template, rset=rset, view=view
        )
    ...
    def _select_view_and_rset(self, rset):
            ...
            for mimetype in rdf.RDF_MIMETYPE_TO_FORMAT:
                try:
                    req.headers_out.addRawHeader(
                        ...
                        % (f"{self._cw.base_url()}{rset.one().rest_path()}", mimetype),
                    )
                except NotAnEntity:
        ...
        try:
            ...
            if rset:
                req.set_message(
                    req._("The view %s can not be applied to this query") % vid
                )
            else:
                req.set_message(
                    req._(
                        "You have no access to this view or it can not "
                        "be used to display the current data."
                    )
                )
            vid = req.form.get("fallbackvid") or vid_from_rset(
                req, rset, req.vreg.schema
            )
