...
def _select_main_var(relations):
    ...
    if principal is None:
        if others:
            return others[0]
        raise BadRQLQuery(
            "unable to find principal in %s"
            % ", ".join(r.as_string() for r in relations)
        )
    return principal
...
class RQLAnnotator:
    ...
    def __init__(self, schema):
        self.schema = schema
        self.nfdomain = frozenset(
            eschema.type for eschema in schema.entities() if not eschema.final
        )
        ...
        for var in rqlst.defined_vars.values():
            ...
            if stinfo["typerel"] is None:
                ...
                if any(
                    ...
                    r
                    for r in var.stinfo["relations"] - var.stinfo["rhsrelations"]
                    if r.r_type != "eid"
                    ...
                    stinfo["principal"] = None
                    continue
            if stinfo["selected"] and var.valuable_references() == 1 + bool(
                stinfo["constnode"]
            ):
            ...
            for ref in var.references():
                ...
                if rel.optional:
                    ...
                    # reference for the outer join
                        ...
                        if not (
                            ostinfo.get("optcomparisons")
                            or any(
                                orel
                                for orel in ostinfo["relations"]
            ...
            if invariant and joins:
                ...
                # (eg exists)
                try:
                    stinfo["principal"] = principal = _select_principal(
                        var.scope, joins
                    )
                    if getrschema(principal.r_type).inlined:
                        ...
                        sstinfo["scope"] = common_parent(
                            sstinfo["scope"], stinfo["scope"]
                        ).scope
...
class IsAmbData:
    ...
    def restrict(self, var, restricted_domain):
        ...
        if (
            var in self.ambiguousvars
            and self.varsols[var] == var.stinfo["possibletypes"]
        ):
    ...
    def _debug_print(self):
        print(
            "varsols",
            {x: sorted(str(v) for v in values) for x, values in self.varsols.items()},
        )
