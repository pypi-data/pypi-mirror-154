...
class Affaire(EntityType):
    ...
    ref = String(fulltextindexed=True, indexed=True, constraints=[SizeConstraint(16)])
...
class Note(Para):
    ...
    whatever = Int(
        default=0
    )  # keep it before `date` for unittest_migraction.test_add_attribute_int
    ...
    shortpara = String(
        maxsize=11, default="hop", vocabulary=["hop", "hop hop", "hop hop hop"]
    )
    ecrit_par = SubjectRelation(
        "Personne", constraints=[RQLConstraint("S concerne A, O concerne A")]
    )
...
class Folder(EntityType):
    ...
    name = String(required=True, indexed=True, internationalizable=True, maxsize=64)
