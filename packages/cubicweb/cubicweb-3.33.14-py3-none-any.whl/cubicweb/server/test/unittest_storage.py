...
class StorageTC(CubicWebTC):
    ...
    def setup_database(self):
        ...
        storages.set_attribute_storage(
            self.repo, "BFSSTestable", "opt_attr", bfs_storage
        )
    ...
    def create_file(self, cnx, content=b"the-data"):
        return cnx.create_entity(
            "File", data=Binary(content), data_format="text/plain", data_name="foo.pdf"
        )

    def fspath(self, cnx, entity):
        fspath = cnx.execute(
            "Any fspath(D) WHERE F eid %(f)s, F data D", {"f": entity.eid}
        )[0][0].getvalue()
    ...
    def test_bfss_sqlite_fspath(self):
        with self.admin_access.repo_cnx() as cnx:
            f1 = self.create_file(cnx)
            expected_filepath = osp.join(self.tempdir, f"{f1.eid}_data_{f1.data_name}")
    ...
    def test_bfss_fs_importing_doesnt_touch_path(self):
        with self.admin_access.repo_cnx() as cnx:
            ...
            f1 = cnx.create_entity(
                "File",
                data=Binary(filepath.encode(sys.getfilesystemencoding())),
    ...
    def test_source_mapped_attribute_error_cases(self):
        with self.admin_access.repo_cnx() as cnx:
            ...
            self.assertEqual(
                str(cm.exception),
                'can\'t use File.data (X data ILIKE "hop") in restriction',
            )
            with self.assertRaises(QueryError) as cm:
                cnx.execute(
            ...
            self.assertEqual(
                str(cm.exception), "query fetch some source mapped attribute, some not"
            )
            with self.assertRaises(QueryError) as cm:
                cnx.execute(
                    "(Any D WHERE X data D, X is File)"
            ...
            try:
                ...
                self.assertEqual(
                    str(cm.exception),
                    "query fetch some source mapped attribute, some not",
                )
            finally:
    ...
    def test_source_mapped_attribute_advanced(self):
        with self.admin_access.repo_cnx() as cnx:
            ...
            rset = cnx.execute(
                "Any X,LENGTH(D) WHERE X eid %(x)s, X data D", {"x": f1.eid}
            )
            self.assertEqual(len(rset), 1)
            self.assertEqual(rset[0][0], f1.eid)
            self.assertEqual(rset[0][1], len("the-data"))
            rset = cnx.execute(
                "Any X,LENGTH(D) WITH D,X BEING ("
                " (Any D, X WHERE X eid %(x)s, X data D)"
                "  UNION "
                " (Any D, X WHERE X eid %(x)s, X data D)"
                ")",
            ...
            with self.assertRaises(QueryError) as cm:
                cnx.execute("Any X,UPPER(D) WHERE X eid %(x)s, X data D", {"x": f1.eid})
            self.assertEqual(
                str(cm.exception), "UPPER can not be called on mapped attribute"
            )
    ...
    def test_bfss_update_with_existing_data(self):
        with self.admin_access.repo_cnx() as cnx:
            ...
            cnx.commit()
            f2 = cnx.execute(
                "Any F WHERE F eid %(f)s, F is File", {"f": f1.eid}
            ).get_entity(0, 0)
    ...
    def test_bfss_update_with_different_extension_commited(self):
        with self.admin_access.repo_cnx() as cnx:
            ...
            cnx.execute(
                "SET F data %(d)s, F data_name %(dn)s, "
    ...
    def test_bfss_update_with_fs_importing(self):
        with self.admin_access.repo_cnx() as cnx:
            ...
            cnx.execute(
                "SET F data %(d)s WHERE F eid %(f)s",
                {
                    "d": Binary(new_fspath.encode(sys.getfilesystemencoding())),
                    "f": f1.eid,
                },
            )
            cnx.commit()
