...
class BaseSecurityTC(CubicWebTC):
    def setup_database(self):
        ...
        with self.admin_access.client_cnx() as cnx:
            ...
            self.create_user(cnx, "oldpassword", password=Binary(hash.encode("ascii")))


class LowLevelSecurityFunctionTC(BaseSecurityTC):
    def test_check_relation_read_access(self):
        rql = 'Personne U WHERE U nom "managers"'
        ...
        with self.temporary_permissions((nom, {"read": ("users", "managers")})):
            ...
            with self.new_access("anon").repo_cnx() as cnx:
                self.assertRaises(
                    Unauthorized, check_relations_read_access, cnx, rqlst, {}
                )
    ...
    def test_get_local_checks(self):
        ...
        with self.temporary_permissions(Personne={"read": ("users", "managers")}):
            ...
            with self.new_access("anon").repo_cnx() as cnx:
                self.assertRaises(Unauthorized, get_local_checks, cnx, rqlst, solution)
                self.assertRaises(Unauthorized, cnx.execute, rql)

    def test_upassword_not_selectable(self):
        with self.admin_access.repo_cnx() as cnx:
            self.assertRaises(
                Unauthorized, cnx.execute, "Any X,P WHERE X is CWUser, X upassword P"
            )
...
class SecurityTC(BaseSecurityTC):
    ...
    def test_update_security_1(self):
        ...
        with self.admin_access.repo_cnx() as cnx:
            self.assertEqual(
                cnx.execute('Personne X WHERE X nom "bidulechouette"').rowcount, 0
            )
    ...
    def test_insert_immutable_attribute_update(self):
        with self.admin_access.repo_cnx() as cnx:
            cnx.create_entity("Old", name="Babar")
            cnx.commit()
            # this should be equivalent
            o = cnx.create_entity("Old")
            o.cw_set(name="Celeste")
            cnx.commit()
    ...
    def test_delete_security(self):
        ...
        with self.new_access("iaminusersgrouponly").repo_cnx() as cnx:
            self.assertRaises(
                Unauthorized, cnx.execute, "DELETE CWGroup Y WHERE Y name 'staff'"
            )
    ...
    def test_insert_relation_rql_permission(self):
        with self.new_access("iaminusersgrouponly").repo_cnx() as cnx:
            ...
            self.assertFalse(
                cnx.execute("Any P,S WHERE P travaille S,P is Personne, S is Societe")
            )
            self.assertRaises(Unauthorized, ent.cw_check_perm, "update")
            self.assertRaises(
                Unauthorized,
                cnx.execute,
                "SET P travaille S WHERE P is Personne, S is Societe",
            )
            self.assertRaises(QueryError, cnx.commit)  # can't commit anymore
            cnx.rollback()
            # test nothing has actually been inserted:
    ...
    def test_delete_relation_rql_permission(self):
        ...
        with self.admin_access.repo_cnx() as cnx:
            ...
            cnx.execute(
                'SET X owned_by U WHERE X eid %(x)s, U login "iaminusersgrouponly"',
                {"x": eid},
            )
    ...
    def test_user_can_change_its_upassword(self):
        ...
        with self.new_access("user").repo_cnx() as cnx:
            cnx.execute(
                "SET X upassword %(passwd)s WHERE X eid %(x)s",
    ...
    def test_read_base(self):
        with self.temporary_permissions(Personne={"read": ("users", "managers")}):
            with self.new_access("anon").repo_cnx() as cnx:
                self.assertRaises(
                    Unauthorized, cnx.execute, 'Personne U where U nom "managers"'
                )
    ...
    def test_read_erqlexpr_base(self):
        ...
        with self.new_access("iaminusersgrouponly").repo_cnx() as cnx:
            ...
            self.assertRaises(
                Unauthorized, cnx.execute, "Any X WHERE X eid %(x)s", {"x": eid}
            )
            # cache test
            self.assertRaises(
                Unauthorized, cnx.execute, "Any X WHERE X eid %(x)s", {"x": eid}
            ...
            self.assertRaises(
                Unauthorized,
                cnx.execute,
                'SET X sujet "hacked" WHERE X eid %(x)s',
                {"x": eid},
            )
    ...
    def test_read_erqlexpr_has_text1(self):
        ...
        with self.new_access("iaminusersgrouponly").repo_cnx() as cnx:
            ...
            soc1 = cnx.execute("INSERT Societe X: X nom 'chouette'")[0][0]
            cnx.execute(
                "SET A concerne S WHERE A eid %(a)s, S eid %(s)s",
                {"a": aff2, "s": soc1},
            )
            cnx.commit()
            self.assertRaises(
                Unauthorized, cnx.execute, "Any X WHERE X eid %(x)s", {"x": aff1}
            )
            self.assertTrue(cnx.execute("Any X WHERE X eid %(x)s", {"x": aff2}))
            self.assertTrue(cnx.execute("Any X WHERE X eid %(x)s", {"x": card1}))
            rset = cnx.execute("Any X WHERE X has_text 'cool'")
            self.assertEqual(sorted(eid for eid, in rset.rows), [card1, aff2])
    ...
    def test_read_erqlexpr_has_text2(self):
        ...
        with self.temporary_permissions(Personne={"read": ("managers",)}):
            with self.new_access("iaminusersgrouponly").repo_cnx() as cnx:
                ...
                rset = cnx.execute(
                    'Any N WITH N BEING (Any N WHERE N has_text "bidule")'
                )
                self.assertEqual(len(rset.rows), 1, rset.rows)
    ...
    def test_attribute_security(self):
        ...
        with self.new_access("iaminusersgrouponly").repo_cnx() as cnx:
            ...
            self.assertRaises(Unauthorized, cnx.commit)
            eid = cnx.execute(
                "INSERT Personne X: X nom 'bidule', " "X web 'http://www.debian.org'"
            )[0][0]
            cnx.commit()
            cnx.execute("SET X test FALSE WHERE X eid %(x)s", {"x": eid})
            self.assertRaises(Unauthorized, cnx.commit)
            ...
            self.assertRaises(Unauthorized, cnx.commit)
            cnx.execute(
                'SET X web "http://www.logilab.org" WHERE X eid %(x)s', {"x": eid}
            )
            cnx.commit()
        with self.new_access("iaminusersgrouponly").repo_cnx() as cnx:
    ...
    def test_attribute_security_rqlexpr(self):
        ...
        with self.new_access("iaminusersgrouponly").repo_cnx() as cnx:
            ...
            self.assertEqual(
                len(
                    cnx.execute(
                        'Any X WHERE X in_state S, S name "todo", X eid %(x)s',
                        {"x": note2.eid},
                    )
                ),
                0,
            ...
            cnx.commit()
            note.cw_set(something="B")
            cnx.commit()
            note.cw_set(something=None, para="zogzog")
            cnx.commit()
    ...
    def test_attribute_read_security(self):
        ...
        with self.temporary_permissions(
            (login_rdef, {"read": ("users", "managers")}),
    ...
    def test_yams_inheritance_and_security_bug(self):
        with self.temporary_permissions(
            Division={"read": ("managers", ERQLExpression("X owned_by U"))}
        ):
            with self.new_access("iaminusersgrouponly").repo_cnx() as cnx:
                rqlst = self.repo.vreg.rqlhelper.parse(
...
class BaseSchemaSecurityTC(BaseSecurityTC):
    ...
    def test_user_can_delete_object_he_created(self):
        ...
        with self.new_access("iaminusersgrouponly").repo_cnx() as cnx:
            ...
            cnx.execute(
                'INSERT Affaire X: X ref "ARCT01", X concerne S WHERE S nom "ARCTIA"'
            )
            cnx.commit()
        with self.admin_access.repo_cnx() as cnx:
            ...
            cnx.commit()
            self.assertEqual(
                len(cnx.execute('TrInfo X WHERE X wf_info_for A, A ref "ARCT01"')), 1
            )
            self.assertEqual(
                len(
                    cnx.execute(
                        'TrInfo X WHERE X wf_info_for A, A ref "ARCT01",'
                        'X owned_by U, U login "admin"'
                    )
                ),
                1,
            )  # TrInfo at the above state change
        with self.new_access("iaminusersgrouponly").repo_cnx() as cnx:
    ...
    def test_users_and_groups_non_readable_by_guests(self):
        ...
        with self.new_access("anon").repo_cnx() as cnx:
            ...
            self.assertRaises(
                Unauthorized, cnx.execute, "CWUser X WHERE X eid %(x)s", {"x": admineid}
            )
    ...
    def test_in_group_relation(self):
        with self.new_access("iaminusersgrouponly").repo_cnx() as cnx:
            rql = "DELETE U in_group G WHERE U login 'admin'"
            self.assertRaises(Unauthorized, cnx.execute, rql)
            rql = "SET U in_group G WHERE U login 'admin', G name 'users'"
            self.assertRaises(Unauthorized, cnx.execute, rql)
    ...
    def test_owned_by(self):
        with self.admin_access.repo_cnx() as cnx:
            ...
            cnx.commit()
        with self.new_access("iaminusersgrouponly").repo_cnx() as cnx:
            rql = "SET X owned_by U WHERE U login 'iaminusersgrouponly', X is Personne"
            self.assertRaises(Unauthorized, cnx.execute, rql)

    def test_bookmarked_by_guests_security(self):
        with self.admin_access.repo_cnx() as cnx:
            beid1 = cnx.execute(
                'INSERT Bookmark B: B path "?vid=manage", B title "manage"'
            )[0][0]
            beid2 = cnx.execute(
                'INSERT Bookmark B: B path "?vid=index", B title "index", '
                'B bookmarked_by U WHERE U login "anon"'
            )[0][0]
            cnx.commit()
        with self.new_access("anon").repo_cnx() as cnx:
            ...
            self.assertEqual(
                cnx.execute(
                    "Any T,P ORDERBY lower(T) WHERE B is Bookmark,B title T,B path P,"
                    "B bookmarked_by U, U eid %s" % anoneid
                ).rows,
                [["index", "?vid=index"]],
            )
            self.assertEqual(
            ...
            self.assertEqual(
                cnx.execute("Any B where B is Bookmark, NOT B bookmarked_by U").rows,
                [[beid1]],
            )
            ...
            self.assertRaises(
                Unauthorized,
                cnx.execute,
                "SET B bookmarked_by U WHERE U eid %(x)s, B eid %(b)s",
                {"x": anoneid, "b": beid1},
            )
    ...
    def test_in_state_without_update_perm(self):
        ...
        with self.new_access("iaminusersgrouponly").repo_cnx() as cnx:
            ...
            affschema = self.schema["Affaire"]
            with self.temporary_permissions(
                Affaire={"update": affschema.get_groups("update"), "read": ("users",)}
            ):
                self.assertRaises(
                    Unauthorized, affschema.check_perm, cnx, "update", eid=eid
                )
                ...
                self.assertRaises(
                    ValidationError,
                    user.cw_adapt_to("IWorkflowable").fire_transition,
                    "deactivate",
                )
    ...
    def test_trinfo_security(self):
        with self.admin_access.repo_cnx() as cnx:
            ...
            cnx.execute(
                'SET TI comment %(c)s WHERE TI wf_info_for X, X ref "ARCT01"',
                {"c": "bouh!"},
            )
            cnx.commit()
    ...
    def test_emailaddress_security(self):
        ...
        with self.admin_access.repo_cnx() as cnx:
            ...
            cnx.execute(
                'INSERT EmailAddress X: X address "anon", '
                'U use_email X WHERE U login "anon"'
            ).get_entity(0, 0)
            cnx.commit()
