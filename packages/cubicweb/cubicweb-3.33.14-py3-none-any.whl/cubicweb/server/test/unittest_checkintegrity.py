...
from cubicweb.server.checkintegrity import (
    check,
    check_indexes,
    reindex_entities,
)  # noqa: E402
...
class CheckIntegrityTC(unittest.TestCase):
    def setUp(self):
        handler = devtools.get_test_db_handler(
            devtools.TestServerConfiguration("data", __file__)
        )
    ...
    def test_checks(self):
        with self.repo.internal_cnx() as cnx:
            check(
                self.repo,
                cnx,
                ("entities", "relations", "text_index", "metadata"),
                reindex=False,
                fix=True,
                withpb=False,
            )
    ...
    def test_reindex_etype(self):
        with self.repo.internal_cnx() as cnx:
            ...
            cnx.commit()
            reindex_entities(self.repo.schema, cnx, withpb=False, etypes=("Personne",))
            self.assertTrue(cnx.execute('Any X WHERE X has_text "tutu"'))
