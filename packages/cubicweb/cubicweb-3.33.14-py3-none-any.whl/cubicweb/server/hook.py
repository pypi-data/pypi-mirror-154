...
.. _`emacs`: https://www.gnu.org/software/emacs/manual/html_node/emacs/Hooks.html
...
from logilab.common.registry import NotPredicate, OrPredicate, objectify_predicate
...
class HooksRegistry(CWRegistry):
    ...
    def call_hooks(self, event, cnx=None, **kwargs):
        ...
        if cnx is None:  # True for events such as server_start
            for hook in sorted(
                self.possible_objects(cnx, **kwargs), key=lambda x: x.order
            ):
                hook()
        else:
            if "entities" in kwargs:
                assert (
                    "entity" not in kwargs
                ), 'can\'t pass "entities" and "entity" arguments simultaneously'
                assert (
                    "eids_from_to" not in kwargs
                ...
                entities = []
                eids_from_to = []
            pruned = self.get_pruned_hooks(cnx, event, entities, eids_from_to, kwargs)

            # by default, hooks are executed with security turned off
            with cnx.security_enabled(read=False):
                for _kwargs in _iter_kwargs(entities, eids_from_to, kwargs):
                    hooks = sorted(
                        self.filtered_possible_objects(pruned, cnx, **_kwargs),
                ...
                for hook in hooks:
                    ...
                    if main_filter is not None:
                        if isinstance(main_filter, match_rtype) and (
                            main_filter.frometypes is not None
                            or main_filter.toetypes is not None
                        ):
                            continue
                        first_kwargs = next(
                            _iter_kwargs(entities, eids_from_to, kwargs)
                        )
    ...
    def filtered_possible_objects(self, pruned, *args, **kwargs):
        for appobjects in self.values():
            ...
            obj = self._select_best(filtered_objects, *args, **kwargs)
            if obj is None:
                continue
            yield obj
...
class match_rtype(ExpectedValuePredicate):
    ...
    def __call__(self, cls, req, *args, **kwargs):
        if kwargs.get("rtype") not in self.expected:
            return 0
        if (
            self.frometypes is not None
            and req.entity_type(kwargs["eidfrom"]) not in self.frometypes
        ):
            return 0
        if (
            self.toetypes is not None
            and req.entity_type(kwargs["eidto"]) not in self.toetypes
        ):
            return 0
        return 1
        ...
        try:
            for event in cls.events:
                if event not in ALL_HOOKS:
                    raise Exception(
                        "bad event %s on %s.%s" % (event, cls.__module__, cls.__name__)
                    )
        except AttributeError:
            raise
        except TypeError:
            raise Exception(
                "bad .events attribute %s on %s.%s"
...
class CleanupDeletedEidsCacheOp(DataOperationMixIn, SingleLastOperation):
    ...
    def postcommit_event(self):
        ...
        try:
            ...
            self.cnx.repo.app_instances_bus.publish(
                ["delete"] + list(str(eid) for eid in eids)
            )
        except KeyError:
            pass
