...
You can set multiple groups by separating them by a comma.",
    ...
    def init(self, source_entity):
        ...
        self.base_filters = [
            "(objectclass=%s)" % replace_filter(o) for o in typedconfig["user-classes"]
        ]
        ...
        self.group_base_filters = [
            "(objectClass=%s)" % replace_filter(o) for o in typedconfig["group-classes"]
        ]
    ...
    def authenticate(self, cnx, login, password=None, **kwargs):
        ...
        searchfilter = [
            "(%s=%s)" % (replace_filter(self.user_login_attr), replace_filter(login))
        ]
