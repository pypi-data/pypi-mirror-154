...
class BytesFileSystemStorage(Storage):
    ...
    def new_fs_path(self, entity, attr):
        ...
        fd, fspath = uniquify_path(self.default_directory, "_".join(basename))
        if fspath is None:
    ...
    def current_fs_path(self, entity, attr):
        ...
        if rawvalue is None:  # no previous value
            return None
        fspath = sysource._process_value(rawvalue, cu.description[0], binarywrap=bytes)
    ...
    def rollback_event(self):
        for filepath in self.get_data():
            ...
            try:
                ...
                self.error(f"can't remove {filepath}: {ex}")
