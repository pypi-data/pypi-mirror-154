...
from cubicweb.server.ssplanner import (
    READ_ONLY_RTYPES,
        ...
        for rel in select.where.iget_nodes(Relation):
            for solution in select.solutions:
                ...
                if rschema.final:
                    ...
                    rdef = rschema.rdef(
                        term_etype(cnx, rel.children[0], solution, args),
                        term_etype(cnx, rel.children[1].children[0], solution, args),
                    )
                if not user.matching_groups(rdef.get_groups("read")):
                    if DBG:
                        print(
                            "check_read_access: %s %s does not match %s"
                ...
                if DBG:
                    print(
                        "check_read_access: %s %s matches %s"
                        % (rdef, user.groups, rdef.get_groups("read"))
                    )
        ...
        if not user.matching_groups(eschema.get_groups("read")):
            ...
            if varinfo["selected"] or (
                len(
                    [
                        r
                        for r in varinfo["relations"]
                        if (
                            not schema.rschema(r.r_type).final
                            and (
                                (
                                    isinstance(r.parent, Exists)
                                    and r.parent.neged(strict=True)
                                )
...
class InsertPlan(ExecutionPlan):
    ...
    def substitute_entity_def(self, edef, edefs):
        ...
        for i, row in enumerate(self.e_defs[:]):
            ...
            for edef_ in edefs[1:]:
                row = [ed.clone() for i, ed in enumerate(samplerow) if i != colidx]
    ...
    def insert_entity_defs(self):
        ...
        for row in self.e_defs:
            results.append([repo.glob_add_entity(cnx, edef) for edef in row])
        return results
        ...
        if eids is None:
            ...
            for eid, etype in zip(eids, etypes):
                ...
                if etype is not None:
                    cache.pop(("{} X WHERE X eid {}".format(etype, eid),), None)
        ...
        * `build_descr` is a boolean flag indicating if the description should
        ...
        if server.DEBUG & (server.DBG_RQL | server.DBG_SQL):
            if server.DEBUG & (server.DBG_MORE | server.DBG_SQL):
                print("*" * 80)
            print(
                "querier input", highlight_terminal(repr(rql)[1:-1], "RQL"), repr(args)
            )
        try:
        ...
        if rqlst.TYPE != "select":
            ...
            if args:
                ...
                cachekey += tuple(sorted([k for k, v in args.items() if v is None]))
        ...
        if build_descr:
            if rqlst.TYPE == "select":
                ...
                if len(rqlst.children) == 1 and len(rqlst.children[0].solutions) == 1:
                    ...
                    variables = [
                        self._get_projected_name(projected, rqlst.children[0].stinfo)
...
def _rql_cache_key(cnx, rql, args, eidkeys):
    ...
    for key in sorted(eidkeys):
        try:
            ...
            raise QueryError("bad cache key {} (value: {!r})".format(key, args[key]))
        ...
        for index, isfinal in todetermine:
            ...
            if isfinal:
                ...
                try:
                    ...
                    cnx.error(
                        "wrong eid %s in repository, you should "
