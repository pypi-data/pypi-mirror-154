...
def sqlexec(
    sqlstmts, cursor_or_execute, withpb=True, pbtitle="", delimiter=";", cnx=None
):
...
def sqlgrants(
    ...
    stmts += grant_schema(
        schema, user, set_owner, skip_entities=skip_entities, prefix=SQL_PREFIX
    )
    return stmts
...
def sqlschema(
    ...
    if dbhelper.users_support and user:
        stmts += sqlgrants(
            schema, driver, user, text_index, set_owner, skip_relations, skip_entities
        )
    return stmts
...
def sql_drop_all_user_tables(driver_or_helper, sqlcursor):
    ...
    if hasattr(dbhelper, "list_views"):
        stmts += [
            "DROP VIEW %s;" % name
            for name in filter(
                _SQL_DROP_ALL_USER_TABLES_FILTER_FUNCTION,
    ...
    stmts += [
        "DROP TABLE %s;" % name
        for name in filter(
            _SQL_DROP_ALL_USER_TABLES_FILTER_FUNCTION, dbhelper.list_tables(sqlcursor)
        )
    ]
    return stmts
...
class SQLAdapterMixIn(object):
    ...
    def __init__(self, source_config, repairing=False):
        ...
        self.dbhelper.record_connection_info(
            dbname,
            dbhost,
            dbport,
            dbuser,
        ...
        if not repairing:
            db_statement_timeout = source_config.get("db-statement-timeout", 0)
            # If we have db-statement-timeout= in sources, then the key exists
            # in source_config, but its value is None, so we need to check it.
            statement_timeout = int(db_statement_timeout if db_statement_timeout else 0)

            if statement_timeout > 0:

                def set_postgres_timeout(cnx):
                    cnx.cursor().execute(
                        "SET statement_timeout to %d" % statement_timeout
                    )
                    cnx.commit()
    ...
    def _backup_restore_env(self):
        if (
            self.config["db-driver"] == "postgres"
    ...
    def backup_to_file(self, backupfile, confirm):
        ...
        for cmd in self.dbhelper.backup_commands(backupfile, keepownership=False):
    ...
    def restore_from_file(self, backupfile, confirm, drop=True):
        extra_env = self._backup_restore_env()
        for cmd in self.dbhelper.restore_commands(
            backupfile, keepownership=False, drop=drop
        ):
            if _run_command(cmd, extra_env=extra_env):
                if not confirm("   [Failed] Continue anyway?", default="n"):
                    raise Exception("Failed command: %s" % cmd)
    ...
    def merge_args(self, args, query_args):
        if args is not None:
            ...
            # should not collide
            assert not (
                frozenset(newargs) & frozenset(query_args)
    ...
    def iter_process_result(self, cursor, cnx, column_callbacks=None):
        ...
        if not column_callbacks:
            return self.dbhelper.dbapi_module.process_cursor(
                cursor, self._dbencoding, Binary
            )
        assert cnx
        ...
        for attr, value in entity.cw_edited.items():
            if value is not None and eschema.subjrels[attr].final:
                ...
                if atype in converters:
                    ...
                    atype == "Password"
                ):  # XXX could be done using a TYPE_CONVERTERS callback
            ...
            if rset.description:
                ...
                for row, rowdesc in zip(rset, rset.description):
                    for cellindex, (value, vtype) in enumerate(zip(row, rowdesc)):
                        if vtype in ("TZDatetime", "Date", "Datetime") and isinstance(
                            value, str
                        ):
                            found_date = True
                            value = value.rsplit(".", 1)[0]
                            try:
                                row[cellindex] = datetime.strptime(
                                    value, "%Y-%m-%d %H:%M:%S"
                                )
                            except Exception:
