...
def has_eid(cnx, sqlcursor, eid, eids):
    ...
    if etype not in cnx.vreg.schema:
        ...
        return False
    sqlcursor.execute(
        "SELECT * FROM {}{} WHERE {}eid={}".format(SQL_PREFIX, etype, SQL_PREFIX, eid)
    )
    ...
    if len(result) == 0:
        ...
        msg = (
            "  More than one entity with eid %s exists in source!\n"
    ...
    if containers:
        for rschema, target in containers:
            ...
            for targeteschema in targets:
                ...
                yield from etype_fti_containers(targeteschema, _done)
    else:
        yield eschema
...
def reindex_entities(schema, cnx, withpb=True, etypes=None):
    ...
    if etypes is None:
        ...
        print(
            "Reindexing entities of type %s" % ", ".join(sorted(str(e) for e in etypes))
        )
        # clear fti table first. Use subquery for sql compatibility
        cnx.system_sql(
            "DELETE FROM %s WHERE EXISTS(SELECT 1 FROM ENTITIES "
...
def check_schema(schema, cnx, eids, fix=1):
    ...
    rql = (
        "Any COUNT(X),RN,SN,ON,CTN GROUPBY RN,SN,ON,CTN ORDERBY 1 "
    ...
    for count, rn, sn, on, cstrname in cnx.execute(rql):
        ...
        if cstrname in UNIQUE_CONSTRAINTS:
            print(
                "ERROR: got %s %r constraints on relation %s.%s.%s"
...
def check_text_index(schema, cnx, eids, fix=1):
    ...
    msg = (
        "  Entity with eid %s exists in the text index but not in any "
...
def check_entities(schema, cnx, eids, fix=1):
    ...
    msg = (
        '  Entity %s with eid %s exists in "entities" table but not in any '
    ...
    cursor = cnx.system_sql(
        "SELECT e.eid FROM entities as e, cw_CWSource as s "
    ...
    if fix:
        cnx.system_sql(
            "INSERT INTO cw_source_relation (eid_from, eid_to) "
    ...
    cursor = cnx.system_sql(
        "SELECT e.type, e.eid FROM entities as e, cw_CWEType as s "
    ...
    if fix:
        cnx.system_sql(
            "INSERT INTO is_relation (eid_from, eid_to) "
    ...
    if fix:
        cnx.system_sql(
            "INSERT INTO is_instance_of_relation (eid_from, eid_to) "
            "SELECT e.eid, s.cw_eid FROM entities as e, cw_CWEType as s "
            "WHERE s.cw_name=e.type AND NOT EXISTS(SELECT 1 FROM is_instance_of_relation as cs "
            "  WHERE cs.eid_from=e.eid AND cs.eid_to=s.cw_eid)"
        )
    ...
    msg = (
        '  Entity with eid %s exists in the %s table but not in "entities" '
    ...
    for eschema in schema.entities():
        ...
        cursor = cnx.system_sql("SELECT {} FROM {};".format(column, table))
        for row in cursor.fetchall():
            eid = row[0]
            ...
            if not eid in eids or not eids[eid]:
                ...
                if fix:
                    cnx.system_sql(
                        "DELETE FROM {} WHERE {}={};".format(table, column, eid)
                    )
                notify_fixed(fix)
...
def bad_related_msg(rtype, target, eid, fix):
    msg = (
        "  A relation %(rtype)s with %(target)s eid %(eid)d exists but "
...
def bad_inlined_msg(rtype, parent_eid, eid, fix):
    msg = (
        "  An inlined relation %s from %s to %s exists but the latter "
    ...
    for rschema in schema.relations():
        ...
        if rschema.inlined:
            for subjtype in rschema.subjects():
                ...
                sql = "SELECT cw_eid,{} FROM {} WHERE {} IS NOT NULL;".format(
                    column,
                    table,
                    column,
                )
                ...
                for row in cursor.fetchall():
                    ...
                    if not has_eid(cnx, cursor, eid, eids):
                        ...
                        if fix:
                            sql = "UPDATE {} SET {}=NULL WHERE {}={};".format(
                                table,
                                column,
                                column,
                                eid,
        ...
        for row in cursor.fetchall():
            ...
            if not has_eid(cnx, cursor, eid, eids):
                ...
                if fix:
                    sql = "DELETE FROM {}_relation WHERE eid_from={};".format(
                        rschema, eid
                    )
                    cnx.system_sql(sql)
        ...
        for row in cursor.fetchall():
            ...
            if not has_eid(cnx, cursor, eid, eids):
                ...
                if fix:
                    sql = "DELETE FROM {}_relation WHERE eid_to={};".format(
                        rschema, eid
                    )
                    cnx.system_sql(sql)
...
def check_mandatory_relations(schema, cnx, eids, fix=1):
    ...
    for rschema in schema.relations():
        if (
            rschema.final
            or rschema in PURE_VIRTUAL_RTYPES
            or rschema in ("is", "is_instance_of")
        ):
            continue
        smandatory = set()
        omandatory = set()
        ...
        for role, etypes in (("subject", smandatory), ("object", omandatory)):
            for etype in etypes:
                if role == "subject":
                    rql = "Any X WHERE NOT X {} Y, X is {}".format(rschema, etype)
                else:
                    rql = "Any X WHERE NOT Y {} X, X is {}".format(rschema, etype)
...
def check_metadata(schema, cnx, eids, fix=1):
    ...
    for (etype,) in cursor.fetchall():
        if etype not in cnx.vreg.schema:
            ...
            if fix:
                cnx.system_sql(
                    "DELETE FROM entities WHERE type = %(type)s", {"type": etype}
                )
            continue
        ...
        for rel, default in (
            ...
            cursor = cnx.system_sql(
                "SELECT {} FROM {} WHERE {} is NULL".format(eidcolumn, table, column)
            )
            for (eid,) in cursor.fetchall():
                sys.stderr.write(msg % (etype, eid, rel))
                if fix:
                    cnx.system_sql(
                        "UPDATE %s SET %s=%%(v)s WHERE %s=%s ;"
...
def expected_indexes(cnx):
    ...
    if source.dbdriver == "postgres":
        ...
        schema_indexes.update(
            {
                "appears_uid": ("appears", "uid"),
    ...
    for rschema in schema.relations():
        ...
        if rschema.final or rschema.inlined:
            for rdef in rschema.rdefs.values():
                table = "cw_{}".format(rdef.subject)
                column = "cw_{}".format(rdef.rtype)
            ...
            table = "{}_relation".format(rschema)
            if source.dbdriver == "postgres":
                # index built after the primary key constraint
                schema_indexes[
                    build_index_name(table, ["eid_from", "eid_to"], "key_")
...
def database_indexes(cnx):
    ...
    return {idx for idx in dbh.list_indices(cnx.cnxset.cu) if index_filter(idx)}
...
def check_indexes(cnx):
    ...
    if missing_indexes:
        ...
        missing = [
            "{0} ON {1[0]} {1[1]}".format(idx, schema_indexes[idx])
