...
class StatsService(Service):
    ...
    def call(self):
        ...
        for size, maxsize, hits, misses, title in (
            (
                len(querier.rql_cache),
                repo.config["rql-cache-size"],
                querier.rql_cache.cache_hit,
                querier.rql_cache.cache_miss,
                "rqlt_st",
            ),
            (
                len(source._cache),
...
class GcStatsService(Service):
    ...
    def call(self, nmax=20):
        ...
        results = {}
        counters, ocounters, garbage = gc_info(lookupclasses, viewreferrersclasses=())
...
class RegisterUserService(Service):
    ...
    def call(self, login, password, email=None, groups=None, **cwuserkwargs):
        ...
        cnx.execute(
            "SET X in_group G WHERE X eid %%(x)s, G name IN (%s)" % group_names,
            {"x": user.eid},
        )
