...
class SupervisingTC(CubicWebTC):
    def setup_database(self):
        with self.admin_access.client_cnx() as cnx:
            cnx.create_entity("Card", title="une news !", content="cubicweb c'est beau")
            card = cnx.create_entity(
                "Card", title="une autre news !", content="cubicweb c'est beau"
            )
            cnx.create_entity("Bookmark", title="un signet !", path="view?vid=index")
            cnx.create_entity("Comment", content="Yo !", comments=card)
            cnx.commit()
    ...
    def test_supervision(self):
        ...
        with self.admin_access.repo_cnx() as cnx:
            ...
            cnx.execute(
                "SET X bookmarked_by U WHERE X is Bookmark, U eid %(x)s",
                {"x": user.eid},
            )
            cnx.execute('SET X content "duh?" WHERE X is Comment')
            cnx.execute(
                'DELETE Comment C WHERE C comments Y, Y is Card, Y title "une autre news !"'
            )
            # check only one supervision email operation
            sentops = [
                op for op in cnx.pending_operations if isinstance(op, SupervisionMailOp)
            ]
            ...
            data = re.sub(r"#\d+", "#EID", data)
            data = re.sub(r"/\d+", "/EID", data)
            self.assertMultiLineEqual(
                """user admin has made the following change(s):
  ...
  http://testing.fr/cubicweb/cwuser/toto""",
    ...
    def test_nonregr1(self):
        with self.admin_access.repo_cnx() as cnx:
            ...
            cnx.execute(
                "SET X last_login_time NOW WHERE X eid %(x)s", {"x": cnx.user.eid}
            )
            cnx.commit()  # no crash
