    ...
    for change in changes[:]:
        ...
        if event == "add_entity":
            ...
            if entity.e_schema == "TrInfo":
                ...
                change = (
                    event,
                    (entity.wf_info_for[0], entity.from_state[0], entity.to_state[0]),
                )
...
class SupervisionEmailView(Component):
    ...
    def subject(self):
        return (
            self._cw._("[%s supervision] changes summary") % self._cw.vreg.config.appid
        )
    ...
    def add_entity(self, changedescr):
        ...
        self.w("  %s", changedescr.entity.absolute_url())
    ...
    def change_state(self, args):
        ...
        self.w(
            "  from state %(fromstate)s to state %(tostate)s\n",
            {"fromstate": _(fromstate.name), "tostate": _(tostate.name)},
        )
        self.w("  %s", entity.absolute_url())
