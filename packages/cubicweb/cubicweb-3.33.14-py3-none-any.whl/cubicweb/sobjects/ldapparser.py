...
class UserMetaGenerator(stores.MetadataGenerator):
    ...
    def base_etype_rels(self, etype):
        rels = super().base_etype_rels(etype)
        if etype == "CWUser":
            wf_state = self._cnx.execute(
                "Any S WHERE ET default_workflow WF, ET name %(etype)s, "
                "WF initial_state S",
                {"etype": etype},
            ).one()
...
class DataFeedLDAPAdapter(datafeed.DataFeedParser):
    ...
    non_attribute_keys = {"email", "eid", "member", "modification_date"}
    ...
    def user_source_entities_by_extid(self):
        ...
        if source.user_base_dn.strip():
            ...
            return {
                userdict["dn"]: userdict
                for userdict in source._search(
                    self._cw,
                    source.user_base_dn,
    ...
    def group_source_entities_by_extid(self):
        ...
        if source.group_base_dn.strip():
            attrs = list(
                map(str, ["modifyTimestamp"] + list(source.group_attrs.keys()))
            )
            return {
                groupdict["dn"]: groupdict
                for groupdict in source._search(
                    self._cw,
    ...
    def process_urls(self, *args, **kwargs):
        ...
        self._source_uris = {}
        self._group_members = {}
        error = super().process_urls(*args, **kwargs)
        if not error:
    ...
    def process(self, url, raise_on_error=False):
        ...
        self.debug(
            "processing ldapfeed source %s %s", self.source, self.searchfilterstr
        )
        ...
        for group, members in self._group_members.items():
            ...
            if members:
                ...
                rql = (
                    "SET U in_group G WHERE G name %%(g)s, U login IN (%s)"
                    % ",".join(members)
                )
        ...
        # deactivated
        self._cw.execute(
            "SET X primary_email E WHERE NOT X primary_email E, X use_email E, "
    ...
    def build_importer(self, raise_on_error):
        ...
        for etype in etypes:
            rset = self._cw.execute(
                "Any XURI, X WHERE X cwuri XURI, X is {},"
        ...
        for rtype in ("in_group", "use_email", "owned_by"):
            ...
            existing_relations[rtype] = {tuple(x) for x in rset}
    ...
    def extentities_generator(self):
        self.debug(
            "processing ldapfeed source %s %s", self.source, self.searchgroupfilterstr
        )
        ...
        for userdict in self.user_source_entities_by_extid.values():
            ...
            extuser.values["owned_by"] = {extuser.extid}
            yield from self._process_email(extuser, userdict)
            groups = list(
                filter(
                    None,
                    [self._get_group(name) for name in self.source.user_default_groups],
                )
            )
            if groups:
    ...
    def _process_email(self, extuser, userdict):
        ...
        for emailaddr in emailaddrs:
            ...
            rset = self._cw.execute(
                "EmailAddress X WHERE X address %(addr)s", {"addr": emailaddr}
            )
            ...
            if not rset:
                ...
                yield importer.ExtEntity(
                    "EmailAddress", emailextid, dict(address=[emailaddr])
                )
    ...
    def _get_group(self, name):
        try:
            return self._cw.execute(
                "Any X WHERE X is CWGroup, X name %(name)s", {"name": name}
            )[0][0]
        except IndexError:
            self.error(
                "group %r referenced by source configuration %r does not exist",
                name,
                self.source.uri,
            )
            return None
