...
def mangle_email(address):
    try:
        ...
        return address
    return "{} at {}".format(name, host.replace(".", " dot "))
...
class EmailAddress(AnyEntity):
    ...
    def dc_title(self):
        if self.alias:
            return f"{self.alias} <{self.display_address()}>"
    ...
    def printable_value(self, attr, value=_marker, attrtype=None, format="text/html"):
        ...
        if attr == "address":
            ...
            return address
        return super().printable_value(attr, value, attrtype, format)
