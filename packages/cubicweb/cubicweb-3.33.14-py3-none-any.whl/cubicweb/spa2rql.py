...
def order_limit_offset(sparqlst):
    ...
    if sparqlst.orderby:
        sortterms = ", ".join(
            f"{var.name.upper()} {ascdesc.upper()}" for var, ascdesc in sparqlst.orderby
        )
    ...
    def infer_types(self):
        ...
        while modified:
            ...
            for yams_predicates, subjvar, obj in self.infer_types_info:
                ...
                # current predicate
                svptypes = {s for s, r, o in yams_predicates}
                if not "*" in svptypes:
                ...
                if isinstance(obj, ast.SparqlVar):
                    ...
                    ovptypes = {o for s, r, o in yams_predicates}
                    if not "*" in ovptypes:
    ...
    def finalize(self):
        ...
        for releq, subjvar, obj in self.union_params:
            ...
            for st, rt, ot in releq:
                ...
                if st != "*":
                    thisunions[-1].append(f"{subjvar} is {st}")
                if isinstance(obj, ast.SparqlVar) and ot != "*":
                    objvar = obj.name.upper()
                    thisunions[-1].append(f"{objvar} is {objvar}")
            if not unions:
                unions = thisunions
            else:
        ...
        if unions:
            baserql = "{} WHERE {}".format(selection, ", ".join(self.restrictions))
            rqls = [
                "({}, {})".format(baserql, ", ".join(unionrestrs))
                for unionrestrs in unions
            ]
            rql = " UNION ".join(rqls)
            if sparqlst.orderby or sparqlst.limit or sparqlst.offset:
                rql = "{}{} WITH {} BEING ({})".format(
                    selection,
                    order_limit_offset(sparqlst),
                    ", ".join(self.selection),
                    rql,
                )
        else:
    ...
    def translate(self, sparql):
        ...
        for subj, predicate, obj in sparqlst.where:
            ...
            if predicate in [
                ...
                qi.set_possible_types(subjvar, xy.yeq(":".join(obj), isentity=True))
            else:
