...
def compare_steps(self, step, expected):
    try:
        self.assertEqual(
            step[0],
            expected[0],
            "expected step type %s, got %s" % (expected[0], step[0]),
        )
        if (
            len(step) > 2
            ...
            for i, (rql, sol) in enumerate(queries):
                ...
                self.assertEqual(
                    sorted(sorted(x.items()) for x in sol),
                    sorted(sorted(x.items()) for x in equeries[i][1]),
                )
            idx = 2
        else:
            idx = 1
...
class BaseQuerierTC(BaseTestCase):
    ...
    def _prepare(self, cnx, rql, kwargs=None):
        ...
        rqlst.solutions = remove_unused_solutions(
            rqlst, rqlst.solutions, self.repo.schema
        )[0]
        return rqlst
    ...
    def user_groups_session(self, *groups):
        ...
        with self.admin_access.cnx() as cnx:
            ...
            cnx.user._cw.transaction_data[
                user_session_cache_key(user_eid, "groups")
            ] = set(groups)
            yield cnx
...
def _build_variantes(self, newsolutions):
    ...
    for variante in variantes:
        ...
        variante = DumbOrderedDict(
            sorted(variante.items(), key=lambda a: (a[0][1], a[0][2], a[1]))
        )
...
def _check_permissions(*args, **kwargs):
    ...
    res = DumbOrderedDict(
        sorted(res.items(), key=lambda x: [list(y.items()) for y in x[1]])
    )
