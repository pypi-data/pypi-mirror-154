...
class ResultSet:
    ...
    def __str__(self):
        ...
        return "<resultset {} ({} rows)>".format(self.rql, len(self.rows))
    ...
    def __repr__(self):
        ...
        return pattern % (
            self.rql,
            len(self.rows),
            "\n".join(f"{r} ({d})" for r, d in zip(rows, self.description)),
        )
    ...
    def possible_actions(self, **kwargs):
        ...
        if not kwargs:
            raise ValueError(
                "ResultSet.possible_actions is expecting to receive "
        ...
        if self._actions_cache is None:
            ...
            if key != self._actions_cache[0]:
                raise ValueError(
                    "ResultSet.possible_actions expects to always "
    ...
    def sorted_rset(self, keyfunc, reverse=False, col=0):
        ...
        if col >= 0:
            entities = sorted(
                enumerate(self.entities(col)),
                key=lambda t: keyfunc(t[1]),
                reverse=reverse,
            )
        else:
            entities = sorted(
                enumerate(self), key=lambda t: keyfunc(t[1]), reverse=reverse
            )
    ...
    def limited_rql(self):
        ...
        #     cubicweb)
        nav = self.req.vreg["components"].select_or_none(
            "navigation", self.req, rset=self
        )
        if nav:
    ...
    def _make_entity(self, row, col):
        ...
        etype = self.description[row][col]
        entity = self.req.vreg["etypes"].etype_class(etype)(
            self.req, rset=self, row=row, col=col
        )
