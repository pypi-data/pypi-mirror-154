...
class HTMLTranslator(CSS1HTMLTranslator):
    ...
    def visit_title(self, node):
        ...
        if isinstance(node.parent, nodes.topic):
            ...
            if node.parent.hasattr("id"):
                self.body.append(self.starttag({}, "a", "", name=node.parent["id"]))
            ...
            self.body.append(
                self.starttag(node, "h%d" % default_level, "", CLASS="title")
            )
            self.context.append("</h%d>\n" % default_level)
        else:
            self.body.append(
                self.starttag(
                    node, "h%s" % (default_level + self.section_level - 1), ""
                )
            )
            atts = {}
            if node.hasattr("refid"):
                atts["class"] = "toc-backref"
                atts["href"] = "{}#{}".format(self.base_url, node["refid"])
    ...
    def visit_reference(self, node):
        ...
        if "refuri" in node:
            ...
            href = "{}#{}".format(self.base_url, node["refid"])
    ...
    def visit_system_message(self, node):
        ...
        self.body.append(
            '<div class="system-message"><b>ReST / HTML errors:</b>%s</div>'
            % xml_escape(error)
        )
