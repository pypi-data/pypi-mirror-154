...
def _generate_link_to_source(file_path, start=None, end=None, tag_body="[source]"):
    if start:
        ...
        if end:
            return (
                '<a href="../%s?file=%s&line=%s&end=%s#line-%s" target="_blank">%s</a>'
                % (
            ...
            return (
                '<a href="../{}?file={}&line={}#line-{}" target="_blank">{}</a>'.format(
                    DEBUG_DISPLAY_SOURCE_CODE_PATH,
                    file_path,
                    start,
                    line_anchor,
    ...
    return '<a href="../{}?file={}" target="_blank">{}</a>'.format(
        DEBUG_DISPLAY_SOURCE_CODE_PATH,
        file_path,
        tag_body,
    )
...
def source_code_url_in_stack(stack, highlighted=False):
    ...
    if highlighted:
        for i in stack.split(" File "):
            ...
            line_number = int(
                "".join(dropwhile(lambda x: not x.isdigit(), rest)).split("<")[0]
            )
        ...
        for i in stack.split("\n"):
            ...
            new_stack.append(
                ...
                % (
                    beginning,
                    _generate_link_to_source(
                        file_path, start=line_number, tag_body='"%s"' % file_path
                    ),
                    rest,
                )
            )
...
def debug_display_source_code(request):
    ...
    if "file" not in request.params:
        return Response(
            "Error: you should have end up on this page following a link in the "
    ...
    if not os.path.exists(source_code_file):
        return Response(
            "Error: file '%s' doesn't exist on the filesystem." % source_code_file
        )
    ...
    try:
        content = open(source_code_file).read()
    except Exception as e:
        return Response(
            f"Error: while opening file '{source_code_file}' got the error: {e}"
        )
    ...
    template_filename = os.path.join(
        this_file_directory, "debug_toolbar_templates/debug_source_code.mako"
    )
