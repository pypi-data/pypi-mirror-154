        ...
        for rtype, targeteids in rels.items():
            ...
            try:
                for targeteid in targeteids:
                    self._add_relation(
                        cnx, entity.eid, rtype, targeteid, inlined, **kwargs
                    )
            except TypeError:
                self._add_relation(
                    cnx, entity.eid, rtype, targeteids, inlined, **kwargs
                )
...
class MetadataGenerator(object):
    ...
    * `source`: optional source to be used as `cw_source` for imported entities
    ...
    META_RELATIONS = frozenset(
        META_RTYPES
        - VIRTUAL_RTYPES
        - set(("eid", "cwuri", "is", "is_instance_of", "cw_source"))
    )
...
class MetaGenerator(object, metaclass=class_deprecated):
    ...
    * `source`: optional source to be used as `cw_source` for imported entities
    ...
    __deprecation_warning__ = (
        "[3.23] this class is deprecated, use MetadataGenerator instead"
    )
    ...
    def gen_cwuri(self, entity):
        ...
        return "%s%s" % (self.baseurl, entity.eid)
