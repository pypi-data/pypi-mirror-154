    ...
    def __call__(
        ...
        **kwargs,
        ...
        if select is not None and filtered_variable is not None:
            ...
            if not accept_none and any(row[col] is None for row in rset):
                return 0
        ...
        if isinstance(values, dict):
            if isinstance(self.expected, dict):
                ...
                for key, expected_value in self.expected.items():
                    if key in values:
                        if (
                            isinstance(expected_value, (list, tuple, frozenset, set))
    ...
    * `regids`, adapter identifiers (e.g. interface names) to which the context
    ...
    def __call__(self, cls, req, **kwargs):
        ...
        if (
            score == 0
            and kwargs.get("rset")
            and len(kwargs["rset"]) > 1
            and not "row" in kwargs
        ):
...
def sorted_rset(cls, req, rset=None, **kwargs):
    ...
    if (
        len(selects) > 1
        or not selects[0].orderby
        or (
            isinstance(selects[0].orderby[0].term, Function)
            and selects[0].orderby[0].term.name == "FTIRANK"
        )
    ):
        return 0
    return 2
    ...
    * `strict`, boolean (default to False) telling what to do when the user has
    ...
    def score_entity(self, entity):
        ...
        if self.action:
            if self.target_etype is not None:
                try:
                    rschema = rschema.role_rdef(
                        entity.e_schema, self.target_etype, self.role
                    )
                except KeyError:
                    return 0
...
class partial_relation_possible(PartialPredicateMixIn, relation_possible):
    ...
    def __init__(self, action="read", **kwargs):
        super(partial_relation_possible, self).__init__(
            None, None, None, action, **kwargs
        )
...
class partial_has_related_entities(PartialPredicateMixIn, has_related_entities):
    ...
    def __init__(self, **kwargs):
        super(partial_has_related_entities, self).__init__(None, None, None, **kwargs)
...
class has_add_permission(EClassPredicate):
    ...
    def score_class(self, eclass, req):
        ...
        if (
            eschema.final
            or eschema.is_subobject(strict=True)
...
class is_in_state(score_entity):
    ...
    * you must use the latest tr info thru the workflow adapter for repository
    ...
    def _validate(self, adapted):
        ...
        if unknown:
            raise ValueError("%s: unknown state(s): %s" % (wf.name, ",".join(unknown)))
...
class match_transition(ExpectedValuePredicate):
    ...
    def __call__(self, cls, req, transition=None, **kwargs):
        ...
        if (
            ...
            and getattr(transition, "name", None) in self.expected
        ):
            return 1
        return 0
...
def match_context_prop(cls, req, context=None, **kwargs):
    ...
    * the appobject's context property value is None
    ...
    if context is None:
        return 1
    propval = req.property_value("%s.%s.context" % (cls.__registry__, cls.__regid__))
...
class attribute_edited(EntityPredicate):
    ...
    def score_entity(self, entity):
        return (
            eid_param(role_name(self._attribute, "subject"), entity.eid)
            in entity._cw.form
        )
