...
def guess_rrqlexpr_mainvars(expression):
    ...
    if not mainvars:
        raise BadSchemaDefinition(
            "unable to guess selection variables in %r" % expression
        )
    return mainvars
...
def normalize_expression(rqlstring):
    ...
    """
    union = parse("Any 1 WHERE %s" % rqlstring).as_string()
...
def _check_valid_formula(rdef, formula_rqlst):
    ...
    if len(formula_rqlst.children) != 1:
        raise BadSchemaDefinition(
            ...
            % {"attr": rdef.rtype, "etype": rdef.subject.type, "form": rdef.formula}
        )
...
class RQLExpression(object):
    ...
    def __init__(self, expression, mainvars, eid):
        ...
        for mainvar in mainvars:
            if len(self.snippet_rqlst.defined_vars[mainvar].references()) < 2:
                _LOGGER.warning(
                    "You did not use the %s variable in your RQL " "expression %s",
                    mainvar,
                    self,
                )
    ...
    def rqlst(self):
        ...
        for varname in self.predefined_variables:
            if varname in defined:
                select.add_eid_restriction(
                    select.get_variable(varname), varname.lower(), "Substitute"
                )
        return select
    ...
    def minimal_rql(self):
        return "Any %s WHERE %s" % (",".join(sorted(self.mainvars)), self.expression)
...
ybo.DEFAULT_ATTRPERMS["update"] = (
    "managers",
    ERQLExpression("U has_update_permission X"),
)
...
def order_eschemas(eschemas):
    ...
    if cycles:
        ...
        raise Exception("cycles in entity schema specialization: %s" % cycles)
    eschemas = []
    while graph:
...
def may_have_permission(self, action, req):
    if action != "read" and not (
        self.has_local_role("read") or self.has_perm(req, "read")
    ):
        return False
...
def check_perm(self, _cw, action, **kwargs):
    ...
    if "owners" in groups and (
        kwargs.get("creating") or ("eid" in kwargs and _cw.user.owns(kwargs["eid"]))
    ):
        if DBG:
            print(
                "check_perm: %r %r: user is owner or creation time"
                % (action, _self_str)
            )
        return
    ...
    if DBG:
        print(
            ...
            % (
                ...
                _self_str,
                [
                    (rqlexpr, kwargs, rqlexpr.check(_cw, **kwargs))
                    for rqlexpr in self.get_rqlexprs(action)
                ],
            )
        )
    if any(rqlexpr.check(_cw, **kwargs) for rqlexpr in self.get_rqlexprs(action)):
        return
...
class CubicWebEntitySchema(EntitySchema):
    ...
    def is_subobject(self, strict=False, skiprels=None):
        if skiprels is None:
            ...
            skiprels += SKIP_COMPOSITE_RELS
        return super(CubicWebEntitySchema, self).is_subobject(strict, skiprels=skiprels)
    ...
    def main_attribute(self):
        ...
        for rschema, __ in self.attribute_definitions():
            if not (rschema in META_RTYPES or self.is_metadata(rschema)):
                return rschema
    ...
    def _update_has_text(self, deletion=False):
        ...
        if need_has_text and not has_has_text and not deletion:
            rdef = ybo.RelationDefinition(
                self.type, "has_text", "String", __permissions__=RO_ATTR_PERMS
            )
...
class CubicWebSchema(Schema):
    ...
    def add_entity_type(self, edef):
        ...
        if not eschema.final:
            ...
            rdef = ybo.RelationDefinition(
                eschema.type, "identity", eschema.type, __permissions__=RO_REL_PERMS
            )
            self.add_relation_def(rdef)
...
class BaseRQLConstraint(RRQLExpression, BaseConstraint):
    ...
    def serialize(self):
        return cstr_json_dumps(
            {"mainvars": sorted(self.mainvars), "expression": self.expression}
        )
...
class workflowable_definition(ybo.metadefinition):
    ...
    def __new__(mcs, name, bases, classdict):
        ...
        cls = super(workflowable_definition, mcs).__new__(mcs, name, bases, classdict)
        if not abstract:
...
def make_workflowable(cls, in_state_descr=None):
    ...
    if "in_state" not in existing_rels:
        rdef = ybo.RelationDefinition(
            etype, "in_state", "State", description=in_state_descr
        )
        yams_add_relation(cls.__relations__, rdef)
...
class BootstrapSchemaLoader(SchemaLoader):
    ...
    def load(
        self, config, modnames=(["cubicweb", "cubicweb.schemas.bootstrap"],), **kwargs
    ):
