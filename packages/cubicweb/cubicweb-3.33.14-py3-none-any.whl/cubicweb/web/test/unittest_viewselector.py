...
USERACTIONS = [
    actions.UserPreferencesAction,
    actions.UserInfoAction,
    actions.LogoutAction,
]
SITEACTIONS = [actions.ManageAction]
FOOTERACTIONS = [wdoc.HelpAction, wdoc.AboutAction, actions.PoweredByAction]
MANAGEACTIONS = [
    actions.SiteConfigurationAction,
...
class VRegistryTC(ViewSelectorTC):
    ...
    def test_propertiesform_admin(self):
        ...
        with self.admin_access.web_request() as req:
            ...
            self.assertTrue(
                self.vreg["views"].select("propertiesform", req, rset=rset1)
            )
            rset2 = req.execute('CWUser X WHERE X login "anon"')
            self.assertTrue(
                self.vreg["views"].select("propertiesform", req, rset=rset2)
            )
    ...
    def test_possible_actions_none_rset(self):
        with self.admin_access.web_request() as req:
            self.assertDictEqual(
                self.pactionsdict(req, None, skipcategories=()),
                {
    ...
    def test_possible_actions_no_entity(self):
        with self.admin_access.web_request() as req:
            rset = req.execute("Any X WHERE X eid 999999")
            self.assertDictEqual(
                self.pactionsdict(req, rset, skipcategories=()),
                {
                    "useractions": USERACTIONS,
                    "siteactions": SITEACTIONS,
                    "manage": MANAGEACTIONS,
        ...
        with self.admin_access.web_request() as req:
            ...
            self.assertDictEqual(
                self.pactionsdict(req, rset, skipcategories=()),
                {
                    ...
                    "moreactions": [actions.DeleteAction, actions.AddNewAction],
                },
            )
    ...
    def test_select_view(self):
        ...
        with self.admin_access.web_request() as req:
            self.assertIsInstance(
                self.vreg["views"].select("index", req, rset=rset), startup.IndexView
            )
            self.assertRaises(
                NoSelectableObject, self.vreg["views"].select, "primary", req, rset=rset
            )
            self.assertRaises(
                NoSelectableObject, self.vreg["views"].select, "table", req, rset=rset
            )

            # no entity
            rset = req.execute("Any X WHERE X eid 999999")
            self.assertRaises(
                NoSelectableObject, self.vreg["views"].select, "index", req, rset=rset
            )
            self.assertRaises(
                NoSelectableObject,
                self.vreg["views"].select,
            ...
            self.assertIsInstance(
                self.vreg["views"].select("list", req, rset=rset), baseviews.ListView
            )
            self.assertIsInstance(
            ...
            self.assertRaises(
                NoSelectableObject, self.vreg["views"].select, "list", req, rset=rset
            )
            self.assertRaises(
                NoSelectableObject, self.vreg["views"].select, "edition", req, rset=rset
            )
            # mixed query
    ...
    def test_interface_selector(self):
        with self.admin_access.web_request() as req:
            req.create_entity("File", data_name="bim.png", data=Binary(b"bim"))
    ...
    def test_score_entity_selector(self):
        with self.admin_access.web_request() as req:
            ...
            rset = req.execute('File X WHERE X data_name "bim.png"')
            self.assertIsInstance(
                self.vreg["views"].select("image", req, rset=rset),
                idownloadable.ImageView,
            )
            self.assertRaises(
                NoSelectableObject, self.vreg["views"].select, "ehtml", req, rset=rset
            )

            fileobj = req.create_entity(
                "File", data_name="bim.html", data=Binary(b"<html>bam</html")
            )
            # image/ehtml primary view priority
            rset = req.execute('File X WHERE X data_name "bim.html"')
            self.assertIsInstance(
                self.vreg["views"].select("ehtml", req, rset=rset),
                idownloadable.EHTMLView,
            )
            self.assertRaises(
                NoSelectableObject, self.vreg["views"].select, "image", req, rset=rset
            )

            fileobj = req.create_entity(
                "File", data_name="bim.txt", data=Binary(b"boum")
            )
            # image/ehtml primary view priority
    ...
    def test_properties(self):
        self.assertEqual(
            sorted(
                k
                for k in self.vreg["propertydefs"]
        ...
        self.assertEqual(
            [
                k
                for k in self.vreg["propertyvalues"]
                if not k.startswith("system.version")
            ],
            [],
        )
        self.assertEqual(
            self.vreg.property_value("ctxcomponents.edit_box.visible"), True
        )
        self.assertEqual(self.vreg.property_value("ctxcomponents.edit_box.order"), 2)
        self.assertEqual(
            self.vreg.property_value("ctxcomponents.possible_views_box.visible"), False
        )
        self.assertEqual(
            self.vreg.property_value("ctxcomponents.possible_views_box.order"), 10
        )
        self.assertRaises(
            UnknownProperty, self.vreg.property_value, "ctxcomponents.actions_box"
        )
