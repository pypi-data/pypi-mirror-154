...
class TimeTableView(EntityView):
    ...
    def call(self, title=None):
        ...
        for row in range(self.cw_rset.rowcount):
            ...
            if (
                len(self.cw_rset[row]) > 1
                and self.cw_rset.description[row][1] == "CWUser"
            ):
            ...
            if icalendarable.start and icalendarable.stop:
                if icalendarable.start.toordinal() == icalendarable.stop.toordinal():
                    ...
                    the_dates += date_range(
                        icalendarable.start, icalendarable.stop + ONEDAY
                    )
        ...
        if title:
            self.w("<h4>%s</h4>\n", title)
        self.w('<table class="listing timetable">')
    ...
    def render_col_headers(self, users, widths):
        ...
        self.w('<th class="ttdate">&#160;</th>\n')
        columns = []
        for user, width in zip(users, widths):
            self.w('<th colspan="%s">', max(MIN_COLS, width))
    ...
    def render_rows(self, rows):
        ...
        for row in rows:
            ...
            if date.weekday() in (5, 6) and not empty_line:
                klass = "odd"
            self.w('<tr class="%s">', klass)
            odd = not odd

            if not empty_line:
                self.w('<th class="ttdate">%s</th>', self._cw.format_date(date))
            else:
                self.w("<th>...</th>")
            ...
            for uid, group in enumerate(row[1:]):
                for i, value in enumerate(group):
                    ...
                    if value:
                        ...
                        if first_row:
                            url = task_descr.task.absolute_url(vid="edition")
                            self.w(
                                '<td rowspan="%d" class="%s %s" onclick="document.location=\'%s\'">&#160;<div>',
                                task_descr.lines,
                                task_descr.color,
                                filled_klasses[kj],
                                url,
                        ...
                        if empty_line:
                            self.w('<td class="ttempty">&#160;</td>')
                        else:
                            self.w('<td class="%s">&#160;</td>', empty_klasses[kj])
            self.w("</tr>\n")
