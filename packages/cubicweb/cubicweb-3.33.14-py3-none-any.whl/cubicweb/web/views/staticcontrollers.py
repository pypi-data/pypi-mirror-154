...
class StaticFileController(Controller):
    ...
    def static_file(self, path):
        ...
        if not debugmode:
            ...
            self._cw.set_header(
                "Expires", generateDateTime(mktime(expires.timetuple()))
            )
...
class ConcatFilesHandler:
    ...
    def build_filepath(self, paths):
        ...
        fname = (
            "cache_concat_"
            + hashlib.md5((";".join(paths)).encode("ascii")).hexdigest()
            + ext
        )
                ...
                try:
                    ...
                    for path in paths:
                        ...
                        if rid is None:
                            ...
                            self.logger.error(
                                "concatenated data url error: %r file "
                                "does not exist",
                                path,
                            )
                            if self.config.debugmode:
...
STATIC_CONTROLLERS = [DataController, FCKEditorController, StaticDirectoryController]
...
class StaticControlerRewriter(URLRewriter):
    ...
    def rewrite(self, req, uri):
        for ctrl in STATIC_CONTROLLERS:
            ...
            raise KeyError(uri)
        self._cw.form["static_relative_path"] = self._cw.relative_path(
            includeparams=True
        )
