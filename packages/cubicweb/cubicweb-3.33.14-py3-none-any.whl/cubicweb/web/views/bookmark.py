...
class BookmarkPrimaryView(primary.PrimaryView):
    ...
    def cell_call(self, row, col):
        ...
        self.w("&#160;")
        self.w("<span class='title'><b>")
        self.w("%s : %s", self._cw._("Bookmark"), entity.title)
        self.w("</b></span>")
        self.w('<br/><br/><div class="content"><a href="%s">', entity.actual_url())
        self.w("</a>")
        self.w(
            "<p>%s%s</p>",
            self._cw._("Used by:"),
            ", ".join(u.name() for u in entity.bookmarked_by),
        )
        self.w("</div>")
...
class BookmarksBox(component.CtxComponent):
    ...
    def init_rendering(self):
        ...
        self.can_edit = eschema.has_perm(self._cw, "add") and rschema.has_perm(
            self._cw, "add", toeid=ueid
        )
    ...
    def render_body(self, w):
        ...
        for bookmark in self.bookmarks_rset.entities():
            ...
            if self.can_delete:
                dlink = (
                    '[<a class="action" href="javascript:removeBookmark(%s)" title="%s">-</a>]'
                    % (bookmark.eid, req._("delete this bookmark"))
                )
                label = f"<div>{dlink} {label}</div>"
            self.append(label)
        if self.can_edit:
            ...
            url = (
                req.vreg["etypes"]
                .etype_class("Bookmark")
                .cw_create_url(req, __linkto=linkto, path=path)
            )
            ...
            if self.bookmarks_rset:
                if req.user.is_in_group("managers"):
                    ...
                    bookmarksrql = (
                        "Bookmark B WHERE B bookmarked_by U, B owned_by U, U eid %(x)s"
                    )
                    erset = req.execute(bookmarksrql, {"x": ueid}, build_descr=False)
                    bookmarksrql %= {"x": ueid}
                if erset:
                    url = req.build_url(vid="muledit", rql=bookmarksrql)
                    menu.append(self.link(req._("edit bookmarks"), url))
            url = req.user.absolute_url(
                vid="xaddrelation", rtype="bookmarked_by", target="subject"
            )
