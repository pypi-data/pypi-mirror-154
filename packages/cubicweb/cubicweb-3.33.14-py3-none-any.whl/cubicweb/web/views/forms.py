...
class FieldsForm(form.Form):
    ...
    **Form posting methods**
    ...
    def build_context(self, formvalues=None):
        ...
        if fields:
            self.add_hidden("_cw_fields", ",".join(fields))
        if eidfields:
            self.add_hidden("_cw_entity_fields", ",".join(eidfields), eidparam=True)
    ...
    def process_posted(self):
        ...
        with tempattr(self, "formvalues", {}):  # init fields value cache
            ...
            if errors:
                errors = {f.role_name(): str(ex) for f, ex in errors}
...
class EntityFieldsForm(FieldsForm):
    ...
    def field_by_name(cls_or_self, name, role=None, eschema=None):
        ...
        try:
            ...
            if isinstance(cls_or_self, type):
                ...
                req = cls_or_self._cw
            field = guess_field(
                eschema, rschema, role, req=req, eidparam=True, **kwargs
            )
            if field is None:
                raise
            return field
    ...
    def __init__(self, _cw, rset=None, row=None, col=None, **kwargs):
        ...
        super().__init__(_cw, rset, row, col, **kwargs)
        ...
        if kwargs.get("mainform", True) or kwargs.get("mainentity", False):
            self.add_hidden("__maineid", self.edited_entity.eid)
            # If we need to directly attach the new object to another one
            if "__linkto" in self._cw.form:
                if msg:
                    msg = "{} {}".format(msg, self._cw._("and linked"))
                else:
    ...
    def add_generation_time(self):
        ...
        self.add_hidden("__form_generation_time", "%.6f" % time.time(), eidparam=True)
    ...
    def add_linkto_hidden(self):
        ...
        for (rtype, role), eids in self.linked_to.items():
            ...
            try:
                ...
                for eid in eids:
                    self.add_hidden("__linkto", f"{rtype}:{eid}:{role}")
...
class CompositeFormMixIn:
    ...
    def build_context(self, formvalues=None):
        super().build_context(formvalues)
