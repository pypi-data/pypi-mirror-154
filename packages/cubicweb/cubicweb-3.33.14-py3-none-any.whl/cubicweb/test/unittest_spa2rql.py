...
class XYTC(unittest.TestCase):
    ...
    LIMIT 20""",
            "Any NAME LIMIT 20 WHERE PROJECT name NAME, PROJECT is Project",
        )
        self._test(
            """
    PREFIX doap: <http://usefulinc.com/ns/doap#>
    ...
    LIMIT 20 OFFSET 10""",
            "Any NAME LIMIT 20 OFFSET 10 WHERE PROJECT name NAME, PROJECT is Project",
        )
    ...
    ORDER BY ?name DESC(?created)""",
            "Any NAME ORDERBY NAME ASC, CREATED DESC WHERE PROJECT name NAME, PROJECT creation_date CREATED, PROJECT is Project",
        )
    ...
    LIMIT 20 OFFSET 10""",
                "Any X, CD LIMIT 20 OFFSET 10 WITH X, CD BEING ((Any X, CD WHERE , X creation_date CD) UNION (Any X, CD WHERE , X publication_date CD, X is Version))",
            )
        finally:
            xy.remove_equivalence("Version publication_date", "doap:Version dc:date")
    ...
    ORDER BY DESC(?cd)""",
                "Any X, CD ORDERBY CD DESC WITH X, CD BEING ((Any X, CD WHERE , X creation_date CD) UNION (Any X, CD WHERE , X publication_date CD, X is Version))",
            )
        finally:
            xy.remove_equivalence("Version publication_date", "doap:Version dc:date")
    ...
    WHERE  {
      ?project a doap:Project;
            ...
            "Any PROJECT WHERE PROJECT name %(a)s, PROJECT is Project",
            {"a": "cubicweb"},
        )
