...
class SchemaModificationHooksTC(CubicWebTC):
    ...
    def index_exists(self, cnx, etype, attr, unique=False):
        ...
        return dbhelper.index_exists(
            sqlcursor, SQL_PREFIX + etype, SQL_PREFIX + attr, unique=unique
        )

    def _set_perms(self, cnx, eid):
        cnx.execute(
            "SET X read_permission G WHERE X eid %(x)s, G is CWGroup", {"x": eid}
        )
        cnx.execute(
            "SET X add_permission G WHERE X eid %(x)s, G is CWGroup, "
            'G name "managers"',
            {"x": eid},
        )
        cnx.execute(
            "SET X delete_permission G WHERE X eid %(x)s, G is CWGroup, "
            'G name "owners"',
            {"x": eid},
        )
    ...
    def _set_attr_perms(self, cnx, eid):
        ...
        cnx.execute(
            'SET X update_permission G WHERE X eid %(x)s, G is CWGroup, G name "managers"',
            {"x": eid},
        )
    ...
    def test_base(self):
        with self.admin_access.repo_cnx() as cnx:
            ...
            eeid = cnx.execute(
                'INSERT CWEType X: X name "Societe2", '
                'X description "", X final FALSE'
            )[0][0]
            ...
            concerne2_rdef_eid = cnx.execute(
                ...
                'WHERE RT name "concerne2", E name "Societe2"'
            )[0][0]
            ...
            cnx.commit()
            cnx.execute(
                "DELETE CWRelation X WHERE X eid %(x)s", {"x": concerne2_rdef_eid}
            )
            cnx.commit()
    ...
    def test_metartype_with_nordefs(self):
        with self.admin_access.repo_cnx() as cnx:
            ...
            cnx.execute(
                'INSERT CWRType X: X name "custom_meta", X description "", '
                "X final FALSE, X symmetric FALSE"
            )
            cnx.commit()
            eeid = cnx.execute(
                'INSERT CWEType X: X name "NEWEtype", '
                'X description "", X final FALSE'
            )[0][0]
            self._set_perms(cnx, eeid)
            cnx.commit()
    ...
    def test_metartype_with_somerdefs(self):
        with self.admin_access.repo_cnx() as cnx:
            ...
            rdefeid = cnx.execute(
                'INSERT CWRelation X: X cardinality "**", X relation_type RT, '
                "   X from_entity E, X to_entity E "
                'WHERE RT name "custom_meta", E name "CWUser"'
            )[0][0]
            self._set_perms(cnx, rdefeid)
            cnx.commit()
            eeid = cnx.execute(
    ...
    def test_is_instance_of_insertions(self):
        with self.admin_access.repo_cnx() as cnx:
            ...
            is_etypes = [
                etype
                for etype, in cnx.execute(
                    "Any ETN WHERE X eid %s, " "X is ET, ET name ETN" % seid
                )
            ]
            ...
            instanceof_etypes = [
                etype
                for etype, in cnx.execute(
                    "Any ETN WHERE X eid %s, " "X is_instance_of ET, ET name ETN" % seid
                )
            ]
            self.assertEqual(
                sorted(instanceof_etypes), ["BaseTransition", "Transition"]
            )
            snames = [
                name
                for name, in cnx.execute("Any N WHERE S is BaseTransition, S name N")
            ]
            self.assertNotIn("subdiv", snames)
            snames = [
                name
                for name, in cnx.execute(
                    "Any N WHERE S is_instance_of BaseTransition, " "S name N"
                )
            ]
    ...
    def test_perms_synchronization_1(self):
        with self.admin_access.repo_cnx() as cnx:
            schema = self.repo.schema
            self.assertEqual(schema["CWUser"].get_groups("read"), {"managers", "users"})
            self.assertTrue(
                cnx.execute(
                    'Any X, Y WHERE X is CWEType, X name "CWUser", '
                    'Y is CWGroup, Y name "users"'
                )[0]
            )
            cnx.execute(
                "DELETE X read_permission Y "
                'WHERE X is CWEType, X name "CWUser", Y name "users"'
            )
            self.assertEqual(
                schema["CWUser"].get_groups("read"),
                {
                    "managers",
                    "users",
                },
            ...
            cnx.commit()
            self.assertEqual(schema["CWUser"].get_groups("read"), {"managers"})
            cnx.execute(
                "SET X read_permission Y WHERE X is CWEType, "
    ...
    def test_perms_synchronization_2(self):
        with self.admin_access.repo_cnx() as cnx:
            ...
            self.assertEqual(schema.get_groups("read"), {"managers", "users", "guests"})
            cnx.execute(
            ...
            self.assertEqual(schema.get_groups("read"), {"managers", "users", "guests"})
            cnx.commit()
            self.assertEqual(schema.get_groups("read"), {"managers", "users"})
            cnx.execute(
                "SET X read_permission Y WHERE X relation_type RT, "
                'RT name "in_group", Y name "guests"'
            )
            self.assertEqual(schema.get_groups("read"), {"managers", "users"})
            cnx.commit()
    ...
    def test_nonregr_user_edit_itself(self):
        with self.admin_access.repo_cnx() as cnx:
            ...
            cnx.execute(
                "SET X read_permission Y WHERE X eid %s, Y eid in (%s, %s)"
    ...
    def test_indexed_change(self):
        with self.admin_access.repo_cnx() as cnx:
            try:
                cnx.execute(
                    'SET X indexed FALSE WHERE X relation_type R, R name "name"'
                )
    ...
    def test_unique_change(self):
        with self.admin_access.repo_cnx() as cnx:
            try:
                eid = cnx.execute(
                    'INSERT CWConstraint X: X cstrtype CT, X value "{}", '
                ...
                self.assertFalse(
                    self.index_exists(cnx, "Workflow", "name", unique=True)
                )
                cnx.commit()
    ...
    def test_change_fulltextindexed(self):
        with self.admin_access.repo_cnx() as cnx:
            target = cnx.create_entity(
                "Email", messageid="1234", subject="rick.roll@dance.com"
            )
            cnx.commit()
            rset = cnx.execute('Any X WHERE X has_text "rick.roll"')
            self.assertIn(target.eid, [item[0] for item in rset])
            assert cnx.execute(
                "SET A fulltextindexed FALSE "
    ...
    def test_change_fulltext_container(self):
        with self.admin_access.repo_cnx() as cnx:
            target = cnx.create_entity("EmailAddress", address="rick.roll@dance.com")
            target.cw_set(reverse_use_email=cnx.user)
            cnx.commit()
            rset = cnx.execute('Any X WHERE X has_text "rick.roll"')
            self.assertIn(cnx.user.eid, [item[0] for item in rset])
            assert cnx.execute(
                "SET R fulltext_container NULL " 'WHERE R name "use_email"'
            )
            cnx.commit()
            rset = cnx.execute('Any X WHERE X has_text "rick.roll"')
            self.assertIn(target.eid, [item[0] for item in rset])
            assert cnx.execute(
                'SET R fulltext_container "subject" ' 'WHERE R name "use_email"'
            )
            cnx.commit()
            rset = cnx.execute('Any X WHERE X has_text "rick.roll"')
            self.assertIn(cnx.user.eid, [item[0] for item in rset])
    ...
    def test_update_constraint(self):
        with self.admin_access.repo_cnx() as cnx:
            ...
            cnx.execute(
                "SET X value %(v)s WHERE X eid %(x)s",
                {"x": cstr.eid, "v": "u'normal', u'auto', u'new'"},
            )
            cnx.execute(
                "INSERT CWConstraint X: X value %(value)s, X cstrtype CT, "
                "EDEF constrained_by X WHERE CT name %(ct)s, EDEF eid %(x)s",
                {"ct": "SizeConstraint", "value": "max=10", "x": rdef.eid},
            )
            cnx.commit()
            cstr = rdef.constraint_by_type("StaticVocabularyConstraint")
            self.assertEqual(cstr.values, ("normal", "auto", "new"))
    ...
    def test_add_constraint(self):
        with self.admin_access.repo_cnx() as cnx:
            ...
            cnx.execute(
                "INSERT CWConstraint X: X value %(v)s, X cstrtype CT, "
                "EDEF constrained_by X WHERE CT name %(ct)s, EDEF eid %(x)s",
            ...
            self.assertEqual(set(rdef.constraints), {cstr, cstr3})
    ...
    def test_eschema_composite_properties(self):
        with self.admin_access.repo_cnx() as cnx:
            ...
            self.assertEqual(
                email_eschema.composite_rdef_roles, [(parts_rdef, "subject")]
            )
