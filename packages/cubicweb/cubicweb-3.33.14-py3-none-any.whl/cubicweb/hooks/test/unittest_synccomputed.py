...
class FormulaDependenciesMatrixTC(TestCase):
    ...
    def test_computed_attribute_by_etype_attrs(self):
        ...
        values = {(rdef.formula, tuple(v)) for rdef, v in values.items()}
        self.assertEqual(
            values,
            {
                ("Any 2014 - D WHERE X birth_year D", tuple(("birth_year",))),
                (
...
class ComputedAttributeTC(CubicWebTC):
    ...
    def setup_entities(self, req):
        self.societe = req.create_entity("Societe", nom="Foo")
        req.create_entity(
            "Person", name="Titi", salaire=1000, travaille=self.societe, birth_year=2001
        )
        self.tata = req.create_entity(
            "Person", name="Tata", salaire=2000, travaille=self.societe, birth_year=1990
        )
    ...
    def test_update_on_add_remove_relation(self):
        ...
        with self.admin_access.web_request() as req:
            ...
            # Add relation.
            toto = req.create_entity(
                "Person",
                name="Toto",
                salaire=1500,
                travaille=self.societe,
                birth_year=1988,
            )
            req.cnx.commit()
            rset = req.execute('Any S WHERE X salaire_total S, X nom "Foo"')
    ...
    def test_init_on_entity_creation(self):
        ...
        with self.admin_access.web_request() as req:
            p = req.create_entity("Person", name="Tata", salaire=2000, birth_year=1990)
            req.cnx.commit()
    ...
    def test_recompute_on_ambiguous_relation(self):
        ...
        with self.admin_access.client_cnx() as cnx:
            societe = cnx.create_entity("Societe", nom="Foo")
            cnx.create_entity("MirrorEntity", mirror_of=societe, extid="1")
            cnx.commit()
