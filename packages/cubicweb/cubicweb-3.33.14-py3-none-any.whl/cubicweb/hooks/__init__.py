...
class DataImportsCleanupStartupHook(hook.Hook):
    ...
    def __call__(self):
        ...
        def expire_dataimports(repo=self.repo):
            for uri, source in repo.sources_by_uri.items():
                if uri == "system" or not repo.config.source_enabled(source):
                    continue
                with repo.internal_cnx() as cnx:
                    mindate = datetime.utcnow() - timedelta(
                        seconds=source.config["logs-lifetime"]
                    )
                    cnx.execute(
                        "DELETE CWDataImport X WHERE X start_timestamp < %(time)s",
                        {"time": mindate},
                    )
                    cnx.commit()
