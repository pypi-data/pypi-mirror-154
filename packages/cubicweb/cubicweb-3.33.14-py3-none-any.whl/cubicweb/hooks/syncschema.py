...
CORE_TYPES = (
    ...
    | SCHEMA_TYPES
    | META_RTYPES
    | set(("CWUser", "CWGroup", "login", "upassword", "name", "in_group"))
)
...
def add_inline_relation_column(cnx, etype, rtype):
    ...
    try:
        cnx.system_sql(
            str(
                "ALTER TABLE %s ADD %s integer REFERENCES entities (eid)"
                % (table, column)
            ),
        ...
        cnx.exception("error while adding column %s to table %s", table, column)
...
def insert_rdef_on_subclasses(cnx, eschema, rschema, rdefdef, props):
    ...
    for specialization in eschema.specialized_by(False):
        if (specialization, rdefdef.object) in rschema.rdefs:
            continue
        sperdef = RelationDefinitionSchema(
            specialization, rschema, object, None, values=props
        )
        ss.execschemarql(
            cnx.execute, sperdef, ss.rdef2rql(sperdef, cstrtypemap, groupmap)
        )
...
class DropRelationTable(DropTable):
    def __init__(self, cnx, rtype):
        super(DropRelationTable, self).__init__(cnx, table="%s_relation" % rtype)
    ...
    def precommit_event(self):
        ...
        for etype, attr in self.get_data():
            ...
            if source.dbhelper.alter_column_support:
                ...
                self.error(
                    "dropping column not supported by the backend, handle "
                    "it yourself (%s.%s)",
                    table,
                    column,
                )
...
class CWETypeAddOp(MemSchemaOperation):
    ...
    * add <meta rtype> relation by creating the necessary CWRelation entity
    ...
    def precommit_event(self):
        ...
        etype = ybo.EntityType(
            eid=entity.eid, name=entity.name, description=entity.description
        )
        eschema = schema.add_entity_type(etype)
        # create the necessary table
        for sql in y2sql.eschema2sql(
            cnx.repo.system_source.dbhelper, eschema, prefix=SQL_PREFIX
        ):
            cnx.system_sql(sql)
        # add meta relations
        ...
        for rtype in META_RTYPES - VIRTUAL_RTYPES:
            try:
                ...
                self.critical(
                    "rtype %s was not handled at cwetype creation time", rtype
                )
                continue
...
class CWETypeRenameOp(MemSchemaOperation):
    ...
    def rename(self, oldname, newname):
        ...
        for rschema in eschema.subject_relations():
            if rschema.inlined or (
                rschema.final and eschema.rdef(rschema.type).indexed
            ):
                source.drop_index(cnx, old_table, SQL_PREFIX + rschema.type)
            if rschema.final and any(
                isinstance(cstr, UniqueConstraint)
                for cstr in eschema.rdef(rschema.type).constraints
            ):
                source.drop_index(
                    cnx, old_table, SQL_PREFIX + rschema.type, unique=True
                )
        sql = dbhelper.sql_rename_table(old_table, new_table)
        sqlexec(sql)
        self.info("renamed table %s to %s", oldname, newname)
        sqlexec(
            "UPDATE entities SET type=%(newname)s WHERE type=%(oldname)s",
        ...
        for rschema in eschema.subject_relations():
            ...
            if rschema.final and any(
                ...
                source.create_index(
                    cnx, new_table, SQL_PREFIX + rschema.type, unique=True
                )
    ...
    def precommit_event(self):
        ...
        if not inlined:
            ...
            for etype in rschema.subjects():
                ...
                sqlexec(
                    "INSERT INTO %s_relation SELECT %s, %s FROM %s WHERE NOT %s IS NULL"
            ...
            for etype in rschema.subjects():
                ...
                table = SQL_PREFIX + str(etype)
                cursor = sqlexec(
                    "SELECT eid_from, eid_to FROM %(table)s, "
                ...
                if args:
                    column = SQL_PREFIX + rtype
                    cursor.executemany(
                        "UPDATE %s SET %s=%%(val)s WHERE %s=%%(x)s"
                        % (table, column, eidcolumn),
                        args,
                    )
                # drop existant table
    ...
    * register an operation to add the relation definition to the
    ...
    def init_rdef(self, **kwargs):
        ...
        try:
            ...
            # probably buggy)
            rdef = self.cnx.vreg.schema.rschema(rdefdef.name).rdefs[
                rdefdef.subject, rdefdef.object
            ]
            assert rdef.infered
        else:
            rdef = self.cnx.vreg.schema.rschema(rdefdef.name).rdefs[
    ...
    def precommit_event(self):
        ...
        try:
            cnx.system_sql(
                str("ALTER TABLE %s ADD %s %s" % (table, column, attrtype)),
                rollback_on_failure=False,
            )
        ...
        if entity.indexed:
            try:
                ...
                self.error(
                    "error while creating index for %s.%s: %s", table, column, ex
                )
        ...
        if default is not None:
            ...
            cnx.system_sql(
                "UPDATE %s SET %s=%%(default)s" % (table, column), {"default": default}
            )
    ...
    * if this is an inlined relation, add the necessary column else if it's the
    ...
    def precommit_event(self):
        ...
        if rschema.inlined:
            ...
            insert_rdef_on_subclasses(
                cnx, eschema, rschema, rdefdef, {"composite": entity.composite}
            )
        else:
            ...
            # transaction
            if not (
                relation_already_defined
                or rtype in cnx.transaction_data.get("createdtables", ())
            ):
                rschema = schema.rschema(rtype)
                # create the necessary table
                for sql in y2sql.rschema2sql(rschema):
                    cnx.system_sql(sql)
                cnx.transaction_data.setdefault("createdtables", []).append(rtype)
...
class RDefDelOp(MemSchemaOperation):
    ...
    def precommit_event(self):
        ...
        execute = cnx.execute
        rset = execute(
            "Any COUNT(X) WHERE X is %s, X relation_type R," "R eid %%(x)s" % rdeftype,
            {"x": rschema.eid},
        )
        lastrel = rset[0][0] == 0
        ...
        if rschema.final or rschema.inlined:
            if not cnx.deleted_in_transaction(rdef.subject.eid):
                rset = execute(
                    "Any COUNT(X) WHERE X is %s, X relation_type R, "
                    "R eid %%(r)s, X from_entity E, E eid %%(e)s" % rdeftype,
                    {"r": rschema.eid, "e": rdef.subject.eid},
                )
                if rset[0][0] == 0:
                    ptypes = cnx.transaction_data.setdefault("pendingrtypes", set())
                    ptypes.add(rschema.type)
                    DropColumn.get_instance(cnx).add_data(
                        (str(rdef.subject), str(rschema))
                    )
                elif rschema.inlined:
                    cnx.system_sql(
                        "UPDATE %s%s SET %s%s=NULL WHERE "
...
class RDefUpdateOp(MemSchemaOperation):
    ...
    def precommit_event(self):
        ...
        if (
            "cardinality" in self.values
            and rdef.rtype.final
...
class CWConstraintDelOp(MemSchemaOperation):
    ...
    def precommit_event(self):
        ...
        if self.oldcstr in rdef.constraints:
            ...
            self.critical(
                "constraint %s for rdef %s was missing or already removed",
                self.oldcstr,
                rdef,
            )
...
class CWConstraintAddOp(hook.LateOperation, CWConstraintDelOp):
    ...
    def precommit_event(self):
        ...
        # constraint changes
        syssource = cnx.repo.system_source
        if cstrtype == "SizeConstraint" and (
        ...
        if cstrtype in (
            ...
            cstrname, check = y2sql.check_constraint(
                rdef, newcstr, syssource.dbhelper, prefix=SQL_PREFIX
            )
...
class CWUniqueTogetherConstraintDelOp(MemSchemaOperation):
    ...
    def postcommit_event(self):
        ...
        cols = set(self.cols)
        unique_together = [ut for ut in eschema._unique_together if set(ut) != cols]
...
class MemSchemaPermissionAdd(MemSchemaOperation):
    ...
    def precommit_event(self):
        ...
        try:
            perms.index(perm)
            self.warning(
                "%s already in permissions for %s on %s", perm, self.action, erschema
            )
        except ValueError:
            perms.append(perm)
...
class MemSchemaPermissionDel(MemSchemaPermissionAdd):
    ...
    def precommit_event(self):
        ...
        try:
            ...
            self.error(
                "can't remove permission %s for %s on %s", perm, self.action, erschema
            )
...
class DelCWRTypeHook(SyncSchemaHook):
    ...
    * instantiate an operation to delete the relation type on commit
    ...
    def __call__(self):
        ...
        self._cw.execute(
            "DELETE CWAttribute X WHERE X relation_type Y, Y eid %(x)s",
            {"x": self.entity.eid},
        )
        self._cw.execute(
            "DELETE CWRelation X WHERE X relation_type Y, Y eid %(x)s",
            {"x": self.entity.eid},
        )
...
class AfterAddCWComputedRTypeHook(AfterAddCWRTypeHook):
    ...
    def rtype_def(self):
        entity = self.entity
        return ybo.ComputedRelation(name=entity.name, eid=entity.eid, rule=entity.rule)
...
class BeforeUpdateCWRTypeHook(SyncSchemaHook):
    ...
    def __call__(self):
        ...
        if newvalues:
            ...
            CWRTypeUpdateOp(self._cw, rschema=rschema, entity=entity, values=newvalues)
...
class BeforeUpdateCWComputedRTypeHook(SyncSchemaHook):
    ...
    def __call__(self):
        ...
        if "rule" in entity.cw_edited:
            ...
            if old != new:
                rschema = self._cw.vreg.schema.rschema(entity.name)
                CWComputedRTypeUpdateOp(
                    self._cw, rschema=rschema, entity=entity, rule=new
                )
...
class AfterUpdateCWRDefHook(SyncSchemaHook):
    __regid__ = "syncaddcwattribute"
    __select__ = SyncSchemaHook.__select__ & is_instance("CWAttribute", "CWRelation")
    events = ("before_update_entity",)
    ...
    def __call__(self):
        ...
        if newvalues:
            RDefUpdateOp(
                self._cw, rschema=rschema, rdefkey=(subjtype, objtype), values=newvalues
            )
...
class BeforeDeleteConstraintOfHook(SyncSchemaHook):
    ...
    def __call__(self):
        ...
        CWUniqueTogetherConstraintDelOp(
            self._cw, entity=entity, cstrname=cstr.name, cols=cols
        )
...
class AfterAddPermissionHook(SyncSchemaHook):
    ...
    __select__ = SyncSchemaHook.__select__ & hook.match_rtype(
        "read_permission", "add_permission", "delete_permission", "update_permission"
    )
    events = ("after_add_relation",)
    ...
    def __call__(self):
        ...
        if self._cw.entity_type(self.eidto) == "CWGroup":
            MemSchemaPermissionAdd(
                self._cw, action=action, eid=self.eidfrom, group_eid=self.eidto
            )
        else:  # RQLExpression
            expr = self._cw.entity_from_eid(self.eidto).expression
            MemSchemaPermissionAdd(self._cw, action=action, eid=self.eidfrom, expr=expr)
...
class BeforeDelPermissionHook(AfterAddPermissionHook):
    ...
    def __call__(self):
        ...
        if self._cw.entity_type(self.eidto) == "CWGroup":
            ...
            expr = self._cw.entity_from_eid(self.eidto).expression
            MemSchemaPermissionDel(self._cw, action=action, eid=self.eidfrom, expr=expr)
...
class UpdateFTIndexOp(hook.DataOperationMixIn, hook.SingleLastOperation):
    ...
    def postcommit_event(self):
        ...
        for etype in to_reindex:
            ...
            self.info(
                "Reindexing full text index for %i entity of type %s", len(rset), etype
            )
...
class AfterAddSpecializesHook(SyncSchemaHook):
    ...
    def __call__(self):
        MemSchemaSpecializesAdd(
            self._cw, etypeeid=self.eidfrom, parentetypeeid=self.eidto
        )
