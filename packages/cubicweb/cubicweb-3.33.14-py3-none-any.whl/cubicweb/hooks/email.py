...
class SetUseEmailRelationOp(hook.Operation):
    ...
    def condition(self):
        ...
        return not any(e for e in self.entity.use_email if self.email.eid == e.eid)
...
class SetPrimaryEmailHook(hook.Hook):
    ...
    def __call__(self):
        ...
        if "primary_email" in entity.e_schema.subject_relations():
            SetPrimaryEmailRelationOp(
                self._cw, entity=entity, email=self._cw.entity_from_eid(self.eidto)
            )
