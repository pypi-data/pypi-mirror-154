...
class _CheckRequiredRelationOperation(hook.DataOperationMixIn, hook.LateOperation):
    ...
    def precommit_event(self):
        ...
        for eid, rtype in self.get_data():
            ...
            if not cnx.execute(self.base_rql % rtype, {"x": eid}):
                ...
                msg = _(
                    "at least one relation %(rtype)s is required on "
                    "%(etype)s (%(eid)s)"
                )
...
class _EnsureSymmetricRelationsAdd(hook.Hook):
    ...
    def __call__(self):
        self._cw.repo.system_source.add_relation(
            self._cw, self.eidto, self.rtype, self.eidfrom
        )
...
class _EnsureSymmetricRelationsDelete(hook.Hook):
    ...
    def __call__(self):
        self._cw.repo.system_source.delete_relation(
            self._cw, self.eidto, self.rtype, self.eidfrom
        )
...
class CheckCardinalityHookBeforeDeleteRelation(IntegrityHook):
    ...
    def __call__(self):
        ...
        if (rdef.subject, rtype, rdef.object) in cnx.transaction_data.get(
            "pendingrdefs", ()
        ):
            return
        ...
        for values in self.get_data():
            ...
            for constraint in constraints:
                ...
                try:
                    ...
                    self.critical(
                        "can't check constraint %s, not supported", constraint
                    )
    ...
    def __call__(self):
        ...
        for attr in self.entity.cw_edited:
            if eschema.subjrels[attr].final:
                constraints = [
                    c
                    for c in eschema.rdef(attr).constraints
...
class DontRemoveOwnersGroupHook(IntegrityHook):
    ...
    def __call__(self):
        ...
        elif self.event == "before_update_entity" and "name" in entity.cw_edited:
...
class DeleteCompositeOrphanHook(hook.Hook):
    ...
    def __call__(self):
        ...
        for rdef, role in self.entity.e_schema.composite_rdef_roles:
            ...
            self._cw.execute(f"DELETE {target} X WHERE C eid %(c)s, {expr}", {"c": eid})


def registration_callback(vreg):
    vreg.register_all(globals().values(), __name__)
    symmetric_rtypes = [
        rschema.type for rschema in vreg.schema.relations() if rschema.symmetric
    ]

    class EnsureSymmetricRelationsAdd(_EnsureSymmetricRelationsAdd):
        __select__ = _EnsureSymmetricRelationsAdd.__select__ & hook.match_rtype(
            *symmetric_rtypes
        )
    ...
    class EnsureSymmetricRelationsDelete(_EnsureSymmetricRelationsDelete):
        __select__ = _EnsureSymmetricRelationsDelete.__select__ & hook.match_rtype(
            *symmetric_rtypes
        )
