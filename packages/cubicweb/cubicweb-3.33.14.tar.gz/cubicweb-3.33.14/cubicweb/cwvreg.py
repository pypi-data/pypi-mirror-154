...
class CWRegistry(Registry):
    ...
    def poss_visible_objects(self, *args, **kwargs):
        ...
        return sorted(
            [
                x
                for x in self.possible_objects(*args, **kwargs)
...
class ETypeRegistry(CWRegistry):
    ...
    def initialization_completed(self):
        ...
        super().initialization_completed()
    ...
    def register(self, obj, **kwargs):
        ...
        if oid != "Any" and oid not in self.schema:
            self.error(
                "don't register %s, %s type not defined in the " "schema", obj, oid
            )
            return
        kwargs["clear"] = True
    ...
    def parent_classes(self, etype):
        ...
        parents = tuple(
            self.etype_class(e.type) for e in self.schema.eschema(etype).ancestors()
        )
...
class ViewsRegistry(CWRegistry):
    ...
    def possible_views(self, req, rset=None, **kwargs):
        ...
        for vid, views in self.items():
            if vid[0] == "_":
                continue
            views = [view for view in views if not isinstance(view, class_deprecated)]
            try:
                view = self._select_best(views, req, rset=rset, **kwargs)
                if view is not None and view.linkable():
                    yield view
            except Exception:
                self.exception("error while trying to select %s view for %s", vid, rset)
...
class CtxComponentsRegistry(CWRegistry):
    def poss_visible_objects(self, *args, **kwargs):
        ...
        try:
            ...
            ctxcomps = super().poss_visible_objects(*args, **kwargs)
            if cache is None:
                components = []
    ...
    * 'controllers', primary objects to handle request publishing, directly
    ...
    def __init__(self, config, initlog=True):
        if initlog:
            ...
            config.init_log()
        super().__init__(config.debugmode)
        self.config = config
        ...
        for webdir in (
            join(dirname(realpath(__file__)), "web"),
    ...
    def items(self):
        return [
            item
            for item in super(CWRegistryStore, self).items()
    ...
    def register_all(self, objects, modname, butclasses=()):
        butclasses = {related_appobject(obj) for obj in butclasses}
        objects = [related_appobject(obj) for obj in objects]
        super().register_all(objects, modname, butclasses)

    def register_and_replace(self, obj, replaced):
        obj = related_appobject(obj)
        replaced = related_appobject(replaced)
        super().register_and_replace(obj, replaced)
    ...
    def load_file(self, filepath, modname):
        ...
        return super().load_file(filepath, modname)
        ...
        if self.config.cleanup_unused_appobjects:
            ...
            for obj, (regname, regids) in self._needs_appobject.items():
                ...
                for regid in regids:
                    ...
                    self.debug(
                        "unregister %s (no %s object in registry %s)",
                        registry.objid(obj),
                        " or ".join(regids),
                        regname,
                    )
    ...
    def rqlhelper(self):
        return RQLHelper(
            self.schema, special_relations={"eid": "uid", "has_text": "fti"}
        )
    ...
    def user_property_keys(self, withsitewide=False):
        if withsitewide:
            return sorted(
                k for k in self["propertydefs"] if not k.startswith("sources.")
            )
        return sorted(
            k
            for k, kd in self["propertydefs"].items()
            if not kd["sitewide"] and not k.startswith("sources.")
        )

    def register_property(
        self, key, type, help, default=None, vocabulary=None, sitewide=False
    ):
