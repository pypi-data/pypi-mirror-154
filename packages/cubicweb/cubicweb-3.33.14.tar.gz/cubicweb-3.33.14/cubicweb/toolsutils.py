...
def iter_dir(directory, condition_file=None, ignore=()):
    ...
    for sub in listdir(directory):
        if sub in ("CVS", ".svn", ".hg"):
            continue
        if condition_file is not None and not exists(
            join(directory, sub, condition_file)
        ):
            continue
        if sub in ignore:
            continue
...
def copy_skeleton(skeldir, targetdir, context, exclude=SKEL_EXCLUDE, askconfirm=False):
    import shutil
    ...
    for dirpath, dirnames, filenames in walk(skeldir):
        ...
        if "cubicweb_CUBENAME" in tdirpath:
            tdirpath = tdirpath.replace(
                "cubicweb_CUBENAME", "cubicweb_" + context["cubename"]
            )
        create_dir(tdirpath)
        for fname in filenames:
            ...
            if fname.endswith(".tmpl"):
                ...
                if (
                    not askconfirm
                    or not exists(tfpath)
                    or ASK.confirm("%s exists, overwrite?" % tfpath)
                ):
    ...
    try:
        ...
        if raise_if_unreadable:
            raise ExecutionError(
                "%s. Are you logged with the correct user " "to use this instance?" % ex
            )
        else:
            warning(
                "missing or non readable configuration file %s (%s)", config_file, ex
            )
    return config
