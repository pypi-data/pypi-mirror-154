...
""",
    ...
    def test_rql_role_with_vid(self):
        with self.admin_access.web_request() as req:
            context = self.context(req)
            out = rest_publish(
                context, ":rql:`Any X ORDERBY XL WHERE X is CWUser, X login XL:table`"
            )
            self.assertTrue(
                out.endswith(
    ...
    def test_rql_role_with_vid_empty_rset(self):
        with self.admin_access.web_request() as req:
            context = self.context(req)
            out = rest_publish(
                context, ':rql:`Any X WHERE X is CWUser, X login "nono":table`'
            )
            self.assertTrue(
                out.endswith(
    ...
    def test_rql_role_with_unknown_vid(self):
        with self.admin_access.web_request() as req:
            ...
            self.assertTrue(
                out.startswith(
                    "<p>an error occurred while interpreting this "
    ...
    def test_rql_role_without_vid(self):
        with self.admin_access.web_request() as req:
            context = self.context(req)
            out = rest_publish(
                context, ":rql:`Any X,XL ORDERBY XL WHERE X is CWUser, X login XL`"
            )
            self.assertEqual(
                out,
                '<p><h1>CWUser_plural</h1><div class="section">'
...
.. rql-table::
   ...
   %(rql)s
                ...
                % {
                    "rql": rql,
                    "colvids": ", ".join(
                        ["%d=%s" % (k, v) for k, v in colvids.items()]
                    ),
                },
            )
            view = self.vreg["views"].select("table", req, rset=req.execute(rql))
