...
if applcubicwebversion < (3, 18, 0) and cubicwebversion >= (3, 18, 0):
    ...
    if not driver == "postgres":
        ...
        print(
            "This migration is not supported for backends other than postgres (yet).",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    rql(
        'SET ATTR defaultval NULL WHERE ATTR from_entity E, E name "CWSource", ATTR relation_type T, T name "in_synchronization"'
    )
    ...
    rql(
        'DELETE CWConstraint C WHERE C cstrtype T, T name "SizeConstraint", R constrained_by C, R eid %(eid)s',
        {"eid": rdefeid},
    )
    ...
    for cwattr in rql("CWAttribute X").entities():
        ...
        if olddefault is not None:
            ...
            args = {
                "val": dbh.binary_value(
                    convert_defaultval(cwattr, olddefault).getvalue()
                ),
                "eid": cwattr.eid,
            }
    ...
    rql(
        'SET X to_entity B WHERE X is CWAttribute, X from_entity Y, Y name "CWAttribute", '
    ...
    for rschema in schema.relations():
        if rschema.symmetric:
            ...
            martians = set(
                str(eid)
                for eid, in sql(
                    "SELECT eid_to FROM %s_relation, entities WHERE eid_to = eid AND type NOT IN (%s)"
            ...
            martians |= set(
                str(eid)
                for eid, in sql(
                    "SELECT eid_from FROM %s_relation, entities WHERE eid_from = eid AND type NOT IN (%s)"
                    % (rschema.type, ",".join(subjects))
                )
            )
            if martians:
                ...
                print(
                    "deleting broken relations %s for eids %s"
                    % (rschema.type, martians)
                )
                sql(
                    "DELETE FROM %s_relation WHERE eid_from IN (%s) OR eid_to IN (%s)"
                    % (rschema.type, martians, martians)
                )
            with session.deny_all_hooks_but():
                rql(
                    "SET X %(r)s Y WHERE Y %(r)s X, NOT X %(r)s Y" % {"r": rschema.type}
                )
            commit()
        ...
        if rschema.final:
            if rschema.type in fsschema:
                ...
                sync_schema_props_perms(
                    rschema.type, syncprops=False, ask_confirm=False, commit=False
                )
            else:
...
if applcubicwebversion < (3, 17, 0) and cubicwebversion >= (3, 17, 0):
    try:
        ...
        if not confirm(
            "In cubicweb 3.17 sioc views have been moved to the sioc "
    ...
    try:
        ...
        if not confirm(
            "In cubicweb 3.17 embedding views have been moved to the embed "
            "cube, which is not installed.  Continue anyway?"
        ):
            raise
    try:
...
if applcubicwebversion == (3, 6, 0) and cubicwebversion >= (3, 6, 0):
    CSTRMAP = dict(
        rql("Any T, X WHERE X is CWConstraintType, X name T", ask_confirm=False)
    )
    _add_relation_definition_no_perms("CWAttribute", "update_permission", "CWGroup")
    _add_relation_definition_no_perms(
        "CWAttribute", "update_permission", "RQLExpression"
    )
    ...
    with session.allow_all_hooks_but("integrity"):
        for rschema in repo.schema.relations():
            ...
            for rdef in rschema.rdefs.values():
                for action in rdef.ACTIONS:
                    actperms = []
                    for something in rpermsdict.get(
                        action == "update" and "add" or action, ()
                    ):
        ...
        for action in ("read", "add", "delete"):
            _add_relation_definition_no_perms(
                "CWRelation", "%s_permission" % action, "CWGroup"
            )
            _add_relation_definition_no_perms(
                "CWRelation", "%s_permission" % action, "RQLExpression"
            )
        for action in ("read", "update"):
            _add_relation_definition_no_perms(
                "CWAttribute", "%s_permission" % action, "CWGroup"
            )
            _add_relation_definition_no_perms(
                "CWAttribute", "%s_permission" % action, "RQLExpression"
            )
        for action in ("read", "add", "delete"):
            rql(
                "SET X %s_permission Y WHERE X is CWRelation, "
                "RT %s_permission Y, X relation_type RT, Y is CWGroup"
                % (action, action)
            )
            rql(
                "INSERT RQLExpression Y: Y exprtype YET, Y mainvars YMV, Y expression YEX, "
        ...
        rql(
            "SET X read_permission Y WHERE X is CWAttribute, "
        ...
        rql(
            "SET X update_permission Y WHERE X is CWAttribute, "
        ...
        for action in ("read", "add", "delete"):
            drop_relation_definition(
                "CWRType", "%s_permission" % action, "CWGroup", commit=False
            )
            drop_relation_definition(
                "CWRType", "%s_permission" % action, "RQLExpression"
            )
    sync_schema_props_perms(
        "read_permission", syncperms=False
    )  # fix read_permission cardinality

if (
    applcubicwebversion < (3, 9, 6)
    and cubicwebversion >= (3, 9, 6)
    and not "CWUniqueTogetherConstraint" in schema
):
...
if applcubicwebversion < (3, 5, 0) and cubicwebversion >= (3, 5, 0):
    ...
    try:
        ...
        repo.hm.unregister_hook(
            after_del_relation_type, "after_delete_relation", "relation_type"
        )
        try:
            drop_relation_definition("State", "allowed_transition", "Transition")
        finally:
            repo.hm.register_hook(
                after_del_relation_type, "after_delete_relation", "relation_type"
            )
    schema.rebuild_infered_relations()  # need to be explicitly called once everything is in place

    for et in rql(
        "DISTINCT Any ET,ETN WHERE S state_of ET, ET name ETN", ask_confirm=False
    ).entities():
        wf = add_workflow("default %s workflow" % et.name, et.name, ask_confirm=False)
        rql(
            "SET S state_of WF WHERE S state_of ET, ET eid %(et)s, WF eid %(wf)s",
            {"et": et.eid, "wf": wf.eid},
            "et",
            ask_confirm=False,
        )
        rql(
            "SET T transition_of WF WHERE T transition_of ET, ET eid %(et)s, WF eid %(wf)s",
            {"et": et.eid, "wf": wf.eid},
            "et",
            ask_confirm=False,
        )
        rql(
            "SET WF initial_state S WHERE ET initial_state S, ET eid %(et)s, WF eid %(wf)s",
            {"et": et.eid, "wf": wf.eid},
            "et",
            ask_confirm=False,
        )
    ...
    rql(
        "SET TI by_transition T WHERE TI from_state FS, TI to_state TS, "
...
def sync_constraint_types():
    ...
    repo_constraints = set(
        row[0] for row in rql("Any N WHERE X is CWConstraintType, X name N")
    )
