...
from cubicweb.schemas import PUB_SYSTEM_ENTITY_PERMS, PUB_SYSTEM_REL_PERMS, RO_REL_PERMS
...
class Workflow(EntityType):
    ...
    initial_state = SubjectRelation(
        ...
        constraints=[
            RQLConstraint(
                "O state_of S", msg=_("state doesn't belong to this workflow")
            )
        ],
...
class State(EntityType):
    ...
    name = String(required=True, indexed=True, internationalizable=True, maxsize=256)
    description = RichString(
        default_format="text/rest", description=_("semantic description of this state")
    )
...
class WorkflowTransition(BaseTransition):
    ...
    subworkflow_exit = SubjectRelation(
        "SubWorkflowExitPoint", cardinality="*1", composite="subject"
    )
...
class in_state(RelationType):
    ...
    constraints = [
        RQLConstraint(
            "S is ET, O state_of WF, WF workflow_of ET",
