...
class CWEType(EntityType):
    ...
    name = String(
        required=True, indexed=True, internationalizable=True, unique=True, maxsize=64
    )
...
class CWRType(EntityType):
    ...
    symmetric = Boolean(
        description=_("is this relation equivalent in both direction ?")
    )
    inlined = Boolean(
        description=_(
            "is this relation physically inlined? you should know what you're doing if you are changing this!"
        )
    )
    fulltext_container = String(
        description=_(
            "if full text content of subject/object entity "
...
class CWAttribute(EntityType):
    ...
    __permissions__ = PUB_SYSTEM_ENTITY_PERMS
    relation_type = SubjectRelation(
        "CWRType",
        cardinality="1*",
        constraints=[RQLConstraint("O final TRUE")],
        composite="object",
    )
    from_entity = SubjectRelation(
        "CWEType",
        cardinality="1*",
        constraints=[RQLConstraint("O final FALSE")],
        composite="object",
    )
    to_entity = SubjectRelation(
        "CWEType",
        cardinality="1*",
        constraints=[RQLConstraint("O final TRUE")],
        composite="object",
    )
    constrained_by = SubjectRelation(
        "CWConstraint", cardinality="*1", composite="subject"
    )
    ...
    indexed = Boolean(
        description=_("create an index for quick search on this attribute")
    )
    fulltextindexed = Boolean(
        description=_("index this attribute's value in the plain text index")
    )
    internationalizable = Boolean(
        description=_("is this attribute's value translatable")
    )
...
class CWRelation(EntityType):
    ...
    ordernum = Int(description=_("control subject entity's relations order"), default=0)
    composite = String(
        description=_(
            "is the subject/object entity of the relation "
    ...
    description = RichString(
        internationalizable=True, description=_("semantic description of this relation")
    )
...
class RQLExpression(EntityType):
    ...
    mainvars = String(
        maxsize=8,
        description=_(
            "name of the main variables which should be "
    ...
    expression = String(
        required=True,
        description=_(
            "restriction part of a rql query. "
...
class CWUniqueTogetherConstraint(EntityType):
    ...
    constraint_of = SubjectRelation(
        "CWEType", cardinality="1*", composite="object", inlined=True
    )
...
class CWProperty(EntityType):
    ...
    pkey = String(
        ...
        maxsize=256,
        description=_(
            "defines what's the property is applied for. "
