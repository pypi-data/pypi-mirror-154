...
class RQLRewriter:
    ...
    def insert_local_checks(self, select, kwargs, localchecks, restricted, noinvariant):
        """
        ...
        if nbtrees > 1 and (
            ...
            aliases = [
                n.VariableRef(newselect.get_variable(vref.name, i))
                for i, vref in enumerate(select.selection)
            ]
            selected = {vref.name for vref in aliases}
    ...
    def rewrite(self, select, snippets, kwargs, existingvars=None):
        ...
        newsolutions = remove_solutions(
            solutions, select.solutions, select.defined_vars
        )
    ...
    def insert_varmap_snippets(self, varmap, rqlexprs, varexistsmap):
        try:
            ...
            RQLRewriter(self.session).rewrite(
                subselect, [(varmap, rqlexprs)], self.kwargs
            )
            return
    ...
    def _insert_snippet(self, varmap, previous, new):
        ...
        if new is not None:
            ...
            if self._insert_scope is None and any(
                vi.get("stinfo", {}).get("optrelations") for vi in self.varinfos
            ):
    ...
    def snippet_subquery(self, varmap, transformedsnippet):
        ...
        if self.u_varname:
            ...
            while argname in self.kwargs:
                argname = subselect.allocate_varname()
            subselect.add_constant_restriction(
                subselect.get_variable(self.u_varname), "eid", argname, "Substitute"
            )
            self.kwargs[argname] = self.session.user.eid
        add_types_restriction(
            self.schema, subselect, subselect, solutions=self.solutions
        )
        myunion = stmts.Union()
        myunion.append(subselect)
        aliases = [
            n.VariableRef(self.select.get_variable(name, i))
    ...
    def remove_ambiguities(self, snippets, newsolutions):
        ...
        self.compute_solutions()
        # clean solutions according to initial solutions
        return remove_solutions(
            self.solutions, self.select.solutions, self.select.defined_vars
        )
    ...
    def build_variantes(self, newsolutions):
        ...
        for sol in newsolutions:
            ...
            for key, var_name in self.rewritten.items():
                ...
                if len(var.stinfo["relations"]) == 1 and isinstance(
                    var.scope.parent, n.Not
                ):
                    continue
    ...
    def _may_be_shared_with(self, sniprel, target):
        ...
        for vi in self.varinfos:
            ...
            if _has_multiple_cardinality(
                vi["stinfo"]["possibletypes"], rdef, ttypes_func, cardindex
            ):
                continue
        ...
        if vname == "U":
            ...
            if self.u_varname is None:
                ...
                stmt.add_constant_restriction(
                    stmt.get_variable(self.u_varname), "eid", argname, "Substitute"
                )
                self.kwargs[argname] = self.session.user.eid
