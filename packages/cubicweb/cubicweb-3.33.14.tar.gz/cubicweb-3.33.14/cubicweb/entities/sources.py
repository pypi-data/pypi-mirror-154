    ...
    def host_config(self):
        ...
        for hostcfg in self.host_configs:
            if hostcfg.match(host):
                self.info(
                    "matching host config %s for source %s",
                    hostcfg.match_host,
                    self.name,
                )
...
class CWDataImport(AnyEntity):
    ...
    def dc_title(self):
        return "{} [{}]".format(
            self.printable_value("start_timestamp"),
    ...
    def write_log(self, session, **kwargs):
        if "status" not in kwargs:
            kwargs["status"] = getattr(self, "_status", "success")
        self.cw_set(log="<br/>".join(self._logs), **kwargs)
        self._logs = []
