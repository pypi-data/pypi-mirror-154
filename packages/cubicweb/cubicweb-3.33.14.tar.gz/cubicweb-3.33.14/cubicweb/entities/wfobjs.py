    ...
    def add_state(self, name, initial=False, **kwargs):
        ...
        if initial:
            assert not self.initial, (
                "Initial state already defined as %s" % self.initial
            )
            self._cw.execute(
    ...
    def may_be_fired(self, eid):
        ...
        if groups:
            ...
            if matches:
                if DBG:
                    print(
                        "may_be_fired: %r may fire: user matches %s"
                        % (self.name, groups)
                    )
                return matches
...
class IWorkflowableAdapter(EntityAdapter):
    ...
    def _get_transition(self, tr):
        ...
        if isinstance(tr, str):
            ...
            assert _tr is not None, "not a %s transition: %s" % (self.__regid__, tr)
            tr = _tr
        return tr
        ...
        if hasattr(statename, "eid"):
            ...
            if state is None:
                raise WorkflowException(
                    "not a %s state: %s" % (self.__regid__, statename)
                )
            stateeid = state.eid
