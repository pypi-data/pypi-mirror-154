...
class WorkflowTC(CubicWebTC):
    ...
    def _test_stduser_deactivate(self):
        ...
        with self.new_access("tutu").web_request() as req:
            iworkflowable = req.entity_from_eid(self.member_eid).cw_adapt_to(
                "IWorkflowable"
            )
            with self.assertRaises(ValidationError) as cm:
                iworkflowable.fire_transition("deactivate")
    ...
    def test_swf_base(self):
        ...
        with self.admin_access.shell() as shell:
            ...
            tr1 = swf.add_transition("tr1", (swfstate1,), swfstate2)
            tr2 = swf.add_transition("tr2", (swfstate1,), swfstate3)
            # main workflow
        ...
        with self.admin_access.web_request() as req:
            group = req.create_entity("CWGroup", name="grp1")
            req.cnx.commit()
    ...
    def test_swf_fire_in_a_row(self):
        with self.admin_access.shell() as shell:
            ...
            xcomplete = subwf.add_transition(
                "xcomplete", (xsigning,), xsigned, type="auto"
            )
            # main workflow
...
class AutoTransitionTC(CubicWebTC):
    ...
    def test_auto_transition_custom_initial_state_fired(self):
        wf = self.setup_custom_wf()
        with self.admin_access.web_request() as req:
            user = self.create_user(req, "member", surname="toto")
            req.execute(
                "SET X custom_workflow WF WHERE X eid %(x)s, WF eid %(wf)s",
                {"wf": wf.eid, "x": user.eid},
            )
