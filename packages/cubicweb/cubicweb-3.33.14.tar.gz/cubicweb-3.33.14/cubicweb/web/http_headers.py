...
def casemappingify(d):
    ...
    newd = {key.lower(): key for key in d}
...
class HeaderHandler:
    ...
    def parse(self, name, header):
        ...
        if parser is None:
            raise ValueError(
                "No header parser for header '%s', either add one or use getHeaderRaw."
                % (name,)
            )
    ...
    def generate(self, name, header):
        ...
        if generator is None:
            ...
            raise ValueError(
                "No header generator for header '%s', either add one or use setHeaderRaw."
                % (name,)
            )
...
def checkSingleToken(tokens):
    if len(tokens) != 1:
        raise ValueError(f"Expected single token, not {tokens}.")
    return tokens[0]
...
def parseKeyValue(val):
    if len(val) == 1:
        ...
        return val[0], val[2]
    raise ValueError(f"Expected key or key=value, but got {val}.")
...
def parseAllowOrigin(origin):
    ...
    if p.scheme not in ("http", "https"):
        raise ValueError(
            "Unsupported Accept-Control-Allow-Origin URL scheme %s" % origin
        )
    if not p.netloc:
        raise ValueError(
            "Accept-Control-Allow-Origin: host name cannot be unset  (%s)" % origin
        )
    return origin
...
def parseAllowCreds(cred):
    ...
    if cred and cred != "true":
        raise ValueError(
            'Accept-Control-Allow-Credentials can only be "true" (%s)' % cred
        )
    return cred
...
def generateTrueFalse(value):
    ...
    *   'false' values are `False`, `0`, `"false"`
    ...
    """
    if value in (True, 1) or isinstance(value, str) and value.lower() == "true":
        return "true"
    if value in (False, 0) or isinstance(value, str) and value.lower() == "false":
        return "false"
...
class MimeType:
    ...
    def __repr__(self):
        return f"MimeType({self.mediaType!r}, {self.mediaSubtype!r}, {self.params!r})"

    def __hash__(self):
        return (
            hash(self.mediaType)
            ^ hash(self.mediaSubtype)
            ^ hash(tuple(self.params.items()))
        )
...
def parseRange(range):
    ...
    if len(range) < 3 or range[1] != Token("="):
        raise ValueError(f"Invalid range header format: {range}")

    type = range[0]
    if type != "bytes":
        raise ValueError(f"Unknown range unit: {type}.")
    ...
    for byterangespec in rangeset:
        ...
        if start and end and start > end:
            raise ValueError(f"Invalid range header, start > end: {range}")
...
def generateAccept(accept):
    ...
    out = f"{mimeType.mediaType}/{mimeType.mediaSubtype}"
    if mimeType.params:
        out += ";" + generateKeyValues(mimeType.params.items())

    if q != 1.0:
        out += (f";q={q:.3f}").rstrip("0").rstrip(".")
...
def removeDefaultEncoding(seq):
    for item in seq:
        if item[0] != "identity" or item[1] != 0.0001:
            yield item
    ...
    if v is None:
        ...
        if k == "no-cache" or k == "private":
            ...
            v = quoteString(
                generateList(
                    [
                        header_case_mapping.get(name) or dashCapitalize(name)
                        for name in v
                    ]
                )
            )
...
def generateRange(range):
    ...
    return (
        ...
        + ",".join(
            [
                "{}-{}".format(noneOr(startend[0]), noneOr(startend[1]))
    ...
    for seq in headers:
        ...
        try:
            ...
            for k, v in dict(challenge).items():
                l.append("{}={}".format(k, quoteString(v)))

            _generated.append("{} {}".format(scheme, ", ".join(l)))
        except ValueError:
            _generated.append(f"{scheme} {challenge}")
...
class ETag:
    ...
    def __repr__(self):
        return f"Etag({self.tag!r}, weak={self.weak!r})"
    ...
    def parse(tokens):
        ...
        if len(tokens) == 3 and tokens[0] == "w" and tokens[1] == Token("/"):
...
class Cookie:
    ...
    def __repr__(self):
        s = f"Cookie({self.name!r}={self.value!r}"
        if self.path is not None:
            s += f", path={self.path!r}"
        if self.domain is not None:
            s += f", domain={self.domain!r}"
        if self.ports is not None:
            s += f", ports={self.ports!r}"
        if self.expires is not None:
            s += f", expires={self.expires!r}"
        if self.secure:
            s += ", secure"
        if self.httponly:
            s += ", HttpOnly"
        if self.comment is not None:
            s += f", comment={self.comment!r}"
        if self.commenturl is not None:
            s += f", commenturl={self.commenturl!r}"
        if self.version != 0:
            s += f", version={self.version!r}"
        s += ")"
        return s
    ...
    def __eq__(self, other):
        return (
            ...
            and other.ports == self.ports
            and other.expires == self.expires
            and other.secure == self.secure
            and other.comment == self.comment
            and other.commenturl == self.commenturl
                    ...
                    if last_cookie is not None:
                        if name == "$path":
                            ...
                            if value is None:
                                ...
                                last_cookie.ports = tuple(
                                    [int(s) for s in value.split(",")]
                                )
                else:
    ...
    if max_version == 0:
        ...
        for cookie in cookies:
            if cookie.version == 0:
                ...
                str_cookies.append(f"{cookie.name}={value}")
            else:
                # V1 cookie, nice and easy
                str_cookies.append(
                    "{}={}".format(cookie.name, quoteString(cookie.value))
                )
            ...
            if cookie.ports is not None:
                if len(cookie.ports) == 0:
                    ...
                    str_cookies.append(
                        "$Port=%s"
                        % quoteString(",".join([str(x) for x in cookie.ports]))
                    )
...
def parseSetCookie2(toks):
    outCookies = []
    for cookie in [
        [parseKeyValue(x) for x in split(y, Token(";"))]
...
def generateSetCookie(cookies):
    setCookies = []
    for cookie in cookies:
        out = [f"{cookie.name}={cookie.value}"]
        if cookie.expires:
...
def generateSetCookie2(cookies):
    setCookies = []
    for cookie in cookies:
        out = ["{}={}".format(cookie.name, quoteString(cookie.value))]
        if cookie.comment:
        ...
        if cookie.ports is not None:
            if len(cookie.ports) == 0:
                ...
                out.append(
                    "Port=%s" % quoteString(",".join([str(x) for x in cookie.ports]))
                )
        if cookie.secure:
            out.append("Secure")
        if cookie.httponly:
...
def parseDepth(depth):
    if depth not in ("0", "1", "infinity"):
        raise ValueError(f"Invalid depth header value: {depth}")
    return depth
...
def parseOverWrite(overwrite):
    if overwrite == "F":
        ...
        return True
    raise ValueError(f"Invalid overwrite header value: {overwrite}")
...
class Headers:
    ...
    def __repr__(self):
        return f"<Headers: Raw: {self._raw_headers} Parsed: {self._headers}>"
    ...
    def _mutateRaise(self, *args):
        raise AttributeError(
            "This header object is immutable as the headers have already been sent."
        )
...
generator_request_headers = {
    ...
    "Accept-Encoding": (
        ...
        listGenerator(generateAcceptQvalue),
        singleHeader,
    ),
...
parser_response_headers = {
    ...
    "WWW-Authenticate": (
        lambda h: tokenize(h, foldCase=False),
