...
from cubicweb.rtags import (
    RelationTags,
    RelationTagsBool,
    RelationTagsSet,
    RelationTagsDict,
    NoTargetRelationTagsDict,
    rtags_chain,
    _ensure_str_key,
)
...
class PrimaryViewSectionRelationTags(RelationTags):
    ...
    _allowed_values = frozenset(("attributes", "relations", "sideboxes", "hidden"))
...
class DisplayCtrlRelationTags(NoTargetRelationTagsDict):
    ...
    def _init(self, sschema, rschema, oschema, role):
        if role == "subject":
            ...
            sschema = "*"
        self.counter += 1
        self.setdefault((sschema, rschema, oschema, role), "order", self.counter)
...
class AutoformSectionRelationTags(RelationTagsSet):
    ...
    _allowed_values = {
        "main": ("attributes", "inlined", "relations", "metadata", "hidden"),
        "inlined": ("attributes", "inlined", "hidden"),
        "muledit": ("attributes", "hidden"),
    }

    def init(self, schema, check=True):
        super().init(schema, check)
        if self._parent is None:
            ...
            for key in list(self._tagdefs):
                ...
                for sschema, oschema in concrete_rdefs:
                    ...
                    parent_formsections = self._parent.get(
                        sschema, rschema, oschema, role
                    )
                    ...
                    for formtype, section in parent_sectdict.items():
                        if formtype not in sectdict:
                            ...
                            formsections.add(f"{formtype}_{section}")
    ...
    def tag_relation(self, key, formtype, section):
        if isinstance(formtype, tuple):
            ...
            return
        assert (
            formtype in self._allowed_form_types
        ), "formtype should be in ({}), not {}".format(
            ",".join(self._allowed_form_types),
            formtype,
        )
        assert (
            section in self._allowed_values[formtype]
        ), "section for {} should be in ({}), not {}".format(
            formtype,
            ",".join(self._allowed_values[formtype]),
            section,
        )
        rtags = self._tagdefs.setdefault(_ensure_str_key(key), self.tag_container_cls())
        # remove previous section for this form type if any
        if rtags:
            for tag in rtags.copy():
                if tag.startswith(formtype):
                    rtags.remove(tag)
        rtags.add(f"{formtype}_{section}")
        return rtags
    ...
    def init_get(self, stype, rtype, otype, tagged):
        ...
        rtags = cls("_".join([section, value]) for section, value in rtags.items())
        return rtags
    ...
    def relations_by_section(self, entity, formtype, section, permission, strict=False):
        ...
        """
        tag = f"{formtype}_{section}"
        eschema = entity.e_schema
        cw = entity._cw
        permsoverrides = cw.vreg["uicfg"].select(
            "autoform_permissions_overrides", cw, entity=entity
        )
        if entity.has_eid():
            eid = entity.eid
        else:
        ...
        for rschema, targetschemas, role in eschema.relation_definitions(True):
            ...
            for tschema in targetschemas:
                ...
                if rschema.final:
                    if not rdef.has_perm(cw, permission, eid=eid, creating=eid is None):
                        continue
            ...
            if not rschema.final and role == "subject":
                ...
                if (
                    ...
                    and eid
                    and entity.related(rschema.type, role)
                    and not rdef.has_perm(
                        cw,
                        "delete",
                        fromeid=eid,
    ...
    def hide_field(self, etype, attr, desttype="*", formtype="main"):
        ...
        """
        self._tag_etype_attr(etype, attr, desttype, formtype=formtype, section="hidden")
    ...
    def edit_inline(self, etype, attr, desttype="*", formtype=("main", "inlined")):
        ...
        """
        self._tag_etype_attr(
            etype, attr, desttype, formtype=formtype, section="inlined"
        )
    ...
    def edit_as_attr(self, etype, attr, desttype="*", formtype=("main", "muledit")):
        ...
        """
        self._tag_etype_attr(
            etype, attr, desttype, formtype=formtype, section="attributes"
        )
...
class ReleditTags(NoTargetRelationTagsDict):
    ...
    * `edit_target`, may be either 'rtype' (to edit the relation) or 'related'
    ...
    __regid__ = "reledit"
    _keys = frozenset(
        "novalue_label novalue_include_rtype reload rvid edit_target".split()
    )

    def tag_relation(self, key, tag):
        for tagkey in tag:
            assert tagkey in self._keys, "tag {!r} not in accepted tags: {!r}".format(
                tag,
                self._keys,
            )
        return super().tag_relation(key, tag)
    ...
    def _init(self, sschema, rschema, oschema, role):
        ...
        if not rschema.final:
            ...
            if edittarget not in (None, "rtype", "related"):
                self.warning(
                    "reledit: wrong value for edit_target on relation %s: %s",
                    rschema,
                    edittarget,
                )
                edittarget = None
            if not edittarget:
                ...
                self.tag_relation(
                    (sschema, rschema, oschema, role), {"edit_target": edittarget}
                )
