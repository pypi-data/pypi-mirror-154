...
class SimpleReqRewriter(URLRewriter):
    ...
    rules = [
        ...
        (
            rgx("/cwuser", re.I),
            dict(
                vid="cw.users-and-groups-management", tab=domid("cw.users-management")
            ),
        ),
        (
            rgx("/cwgroup", re.I),
            dict(
                vid="cw.users-and-groups-management", tab=domid("cw.groups-management")
            ),
        ),
...
def build_rset(
    rql, rgxgroups=None, setuser=False, vid=None, vtitle=None, form={}, **kwargs
):
...
class SchemaBasedRewriter(URLRewriter):
    ...
    rules = [
        ...
        (
            rgx("/search/(.+)"),
            build_rset(
                rql=r"Any X ORDERBY FTIRANK(X) DESC WHERE X has_text %(text)s",
                rgxgroups=[("text", 1)],
            ),
        ),
    ]
