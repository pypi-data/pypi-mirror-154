...
class has_editable_relation(EntityPredicate):
    ...
    def score_entity(self, entity):
        ...
        form = entity._cw.vreg["forms"].select(
            "edition", entity._cw, entity=entity, mainform=False
        )
...
def addable_etype_empty_rset(cls, req, rset=None, **kwargs):
    if rset is not None and not rset.rowcount:
        ...
        if len(select.defined_vars) == 1 and len(select.solutions) == 1:
            ...
            if not (
                eschema.final or eschema.is_subobject(strict=True)
...
class SelectAction(action.Action):
    ...
    __regid__ = "select"
    __select__ = (
        match_search_state("linksearch") & nonempty_rset() & match_searched_etype()
    )
...
class ModifyAction(action.Action):
    __regid__ = "edit"
    __select__ = action.Action.__select__ & one_line_rset() & has_editable_relation()
...
class MultipleEditAction(action.Action):
    ...
    def url(self):
        return self._cw.build_url(
            "view", rql=self.cw_rset.printable_rql(), vid="muledit"
        )
...
class ManagePermissionsAction(action.Action):
    ...
    def url(self):
        return self.cw_rset.get_entity(self.cw_row or 0, self.cw_col or 0).absolute_url(
            vid="security"
        )
...
class CopyAction(action.Action):
    __regid__ = "copy"
    __select__ = action.Action.__select__ & one_line_rset() & has_permission("add")
...
class AddNewAction(MultipleEditAction):
    ...
    def url(self):
        return (
            self._cw.vreg["etypes"].etype_class(self.rsettype).cw_create_url(self._cw)
        )
...
class AddRelatedActions(action.Action):
    ...
    def actual_actions(self):
        ...
        for rschema, teschema, role in self.add_related_schemas(entity):
            if (
                rschema.role_rdef(eschema, teschema, role).role_cardinality(role)
                in "1?"
            ):
                if entity.related(rschema, role):
                    continue
            if role == "subject":
                label = f"add {eschema} {rschema} {teschema} {role}"
                url = self.linkto_url(entity, rschema, teschema, "object", **params)
            else:
                label = f"add {teschema} {rschema} {eschema} {role}"
    ...
    def add_related_schemas(self, entity):
        ...
        req = self._cw
        eschema = entity.e_schema
        for role, rschemas in (
            ("subject", eschema.subject_relations()),
            ...
            for rschema in rschemas:
                ...
                for teschema in rschema.targets(eschema, role):
                    if not appearsin_addmenu.etype_get(
                        eschema, rschema, role, teschema
                    ):
                        continue
    ...
    def linkto_url(self, entity, rtype, etype, target, **kwargs):
        return (
            self._cw.vreg["etypes"]
            .etype_class(etype)
            .cw_create_url(
                self._cw, __linkto=f"{rtype}:{entity.eid}:{target}", **kwargs
            )
        )
...
class ViewSameCWEType(action.Action):
    ...
    __select__ = (
        one_line_rset() & is_instance("CWEType") & score_entity(lambda x: not x.final)
    )
    category = "mainactions"
    order = 40
    ...
    def title(self):
        return (
            self._cw.__("view all %s")
            % display_name(self._cw, self.etype, "plural").lower()
        )
