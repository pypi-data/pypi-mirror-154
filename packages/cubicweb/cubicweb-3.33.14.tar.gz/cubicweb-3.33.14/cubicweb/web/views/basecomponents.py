...
class RQLInputForm(component.Component):
    ...
    def call(self, view=None):
        ...
        self.w(
            """<div id="rqlinput" class="%s"><form action="%s"><fieldset>
...
""",
        ...
        self.w("</fieldset></form></div>")


class HeaderComponent(
    component.CtxComponent
):  # XXX rename properly along with related context
...
class ApplicationName(HeaderComponent):
    ...
    def render(self, w, **kwargs):
        ...
        if title:
            w(
                '<span id="appliName"><a href="%s">%s</a></span>',
                self._cw.base_url(),
                title,
            )
...
class CookieLoginComponent(HeaderComponent):
    ...
    __select__ = (
        HeaderComponent.__select__
        & anonymous_user()
        & configuration_values("auth-mode", "cookie")
    )
    context = "header-right"
    loginboxid = "popupLoginBox"
    _html = """<a class="logout icon-login" title="%s" href="javascript:
cw.htmlhelpers.popupLoginBox('%s', '__login');">%s</a>"""
    ...
    def call(self):
        ...
        self._cw.view(
            ...
            rset=self.cw_rset,
            id=self.loginboxid,
            klass="%s hidden" % self.loginboxid,
            title=False,
            showmessage=False,
            w=self.w,
        )
...
class HTTPLoginComponent(CookieLoginComponent):
    ...
    def render(self, w):
        ...
        req = self._cw
        w(
            '[<a class="logout" title="%s" href="%s">%s</a>]'
...
class SectionLayout(component.Layout):
    __select__ = match_context(
        "navtop", "navbottom", "navcontenttop", "navcontentbottom"
    )
    cssclass = "section"
