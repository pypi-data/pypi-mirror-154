...
if rdflib is not None:
    ...
    SKIP_RTYPES = VIRTUAL_RTYPES | {"cwuri", "is", "is_instance_of"}
    ...
    class RDFView(EntityView):
        ...
        def call(self):
            ...
            self.w(graph.serialize(format=self.format, encoding="utf-8"))
        ...
        def entity2graph(self, graph, entity):
            ...
            try:
                ...
                pass
            for rschema, eschemas, role in entity.e_schema.relation_definitions(
                "relation"
            ):
                rtype = rschema.type
                ...
                for eschema in eschemas:
                    if eschema.final:
                        ...
                        if value is not None:
                            ...
                            try:
                                for item in xy.xeq(f"{entity.e_schema.type} {rtype}"):
                        ...
                        for related in entity.related(
                            ...
                            if role == "subject":
                                ...
                                try:
                                    for item in xy.xeq(
                                        f"{entity.e_schema.type} {rtype}"
                                    ):
                                        add(
                                            (
                                                cwuri,
                                                urijoin(item[1]),
                                                URIRef(related.cwuri),
                                            )
                                        )
                                except xy.UnsupportedVocabulary:
                                    pass
