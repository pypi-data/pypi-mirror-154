...
def optional_kwargs(extraargs):
    ...
    return {str(key): value for key, value in extraargs.items()}
    ...
    def publish(self, rset=None):
        ...
        try:
            ...
            raise RemoteCallFailed(
                "no method specified", status=http_client.BAD_REQUEST
            )
        try:
            func = self._cw.vreg["ajax-func"].select(fname, self._cw)
        except ObjectNotFound:
            raise RemoteCallFailed(
                "no %s method" % xml_escape(fname), status=http_client.BAD_REQUEST
            )
        ...
        try:
            ...
            if debug_mode:
                self.exception(
                    "error while decoding json arguments for " "js_%s: %s (err: %s)",
                    fname,
                    args,
                    exc,
                )
            raise RemoteCallFailed(
                exc_message(exc, self._cw.encoding), status=http_client.BAD_REQUEST
            )
        try:
            result = func(*args)
...
class AjaxFunction(AppObject):
    ...
    def _rebuild_posted_form(names, values, action=None):
        ...
        if action:
            form["__action_%s" % action] = "whatever"
        return form
    ...
    def _call_view(self, view, paginate=False, **kwargs):
        ...
        if divid == "contentmain":
            ...
            paginate = True
        if divid == "contentmain":
            stream.write('<div id="contentmain">')
        nav_html = UStringIO()
        ...
        if extresources:
            stream.write('<div class="ajaxHtmlHead">\n')  # XXX use a widget?
...
def _ajaxfunc_factory(
    implementation, selector=yes(), _output_type=None, _check_pageid=False, regid=None
):
    ...
    class AnAjaxFunc(AjaxFunction):
        ...
        def serialize(self, content):
            if self.output_type is None:
                ...
                return "".join(("<div>", content.strip(), "</div>"))
...
def component(self, compid, rql, registry="components", extraargs=None):
    ...
    # try:
    comp = self._cw.vreg[registry].select(
        compid, self._cw, rset=rset, **optional_kwargs(extraargs)
    )
    # except NoSelectableObject:
...
def render(self, registry, oid, eid=None, selectargs=None, renderargs=None):
    if eid is not None:
        ...
        rset = None
    viewobj = self._cw.vreg[registry].select(
        oid, self._cw, rset=rset, **optional_kwargs(selectargs)
    )
