...
class EditBox(component.CtxComponent):
    ...
    def render_title(self, w):
        title = xml_escape(self._cw._(self.title))
        if self.cw_rset:
            ...
            if len(etypes) == 1:
                ...
                title = "{} - {}".format(title, xml_escape(etypelabel.lower()))
        w(title, escape=False)
        ...
        if len(submenu.items) == 1 and not appendanyway:
            ...
            if submenu.label_prefix:
                ...
                if hasattr(boxlink, "label"):
                    boxlink.label = f"{submenu.label_prefix} {boxlink.label}"
                else:
                    boxlink = f"{submenu.label_prefix} {boxlink}"
            box.append(boxlink)
        elif submenu.items:
            box.append(submenu)
        elif appendanyway:
...
<input type="submit" class="rqlsubmit" value="" />
    ...
    def render_title(self, w):
        w(
            """<span onclick="javascript:toggleVisibility('rqlinput')">%s</span>""",
...
class PossibleViewsBox(component.CtxComponent):
    ...
    def init_rendering(self):
        self.views = [
            v
            for v in self._cw.vreg["views"].possible_views(self._cw, rset=self.cw_rset)
...
class StartupViewsBox(PossibleViewsBox):
    ...
    def init_rendering(self):
        self.views = [
            v
            for v in self._cw.vreg["views"].possible_views(self._cw)
            if v.category == "startupview"
        ]
        if not self.views:
            raise component.EmptyComponent()
...
class RsetBox(component.CtxComponent):
    ...
    @property
    def domid(self):
        return super().domid + str(abs(id(self))) + str(abs(id(self.cw_rset)))
...
class ContextualBoxLayout(component.Layout):
    ...
    def render(self, w):
        if self.init_rendering():
            ...
            w('<div class="%s %s" id="%s">', self.cssclass, view.cssclass, view.domid)
            with wrap_on_write(
                w, '<div class="boxTitle"><span>', "</span></div>"
            ) as wow:
                view.render_title(wow)
            w('<div class="boxBody">')
            view.render_body(w)
            # boxFooter div is a CSS place holder (for shadow for example)
            w('</div><div class="boxFooter"></div></div>\n')
