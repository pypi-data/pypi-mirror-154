...
class hCalView(EntityView):
    ...
    def call(self):
        self.w('<div class="hcalendar">')
        for i in range(len(self.cw_rset.rows)):
            task = self.cw_rset.complete_entity(i, 0)
            self.w('<div class="vevent">')
            self.w('<h3 class="summary">%s</h3>', task.dc_title())
            self.w(
            ...
            if icalendarable.stop:
                self.w(
                    '<abbr class="dtstop" title="%s">%s</abbr>',
    ...
    def cell_call(self, row, col, dates=False):
        ...
        if dates:
            ...
            if icalendarable.start and icalendarable.stop:
                self.w(
                    "<br/> %s",
                    self._cw._("from %(date)s")
                    % {"date": self._cw.format_date(icalendarable.start)},
                )
                self.w(
                    "<br/> %s",
                    self._cw._("to %(date)s")
                    % {"date": self._cw.format_date(icalendarable.stop)},
                )
            else:
                self.w(
                    "<br/>%s",
                    self._cw.format_date(icalendarable.start or icalendarable.stop),
                )
...
class _TaskEntry:
    ...
    def is_one_day_task(self):
        return (
            self.start
            and self.stop
            and self.start.isocalendar() == self.stop.isocalendar()
        )
...
class CalendarView(EntityView):
    ...
    def call(self, cssclass=""):
        ...
        if cssclass:
            self.w('<div class="%s" id="%s"></div>', cssclass, self.calendar_id)
        else:
            self.w('<div id="%s"></div>', self.calendar_id)
    ...
    def add_onload(self):
        ...
        """  # "
        self._cw.add_onload(
            js % (self._cw.lang, json_dumps(fullcalendar_options), self.calendar_id)
        )
