...
class BaseFacetTC(CubicWebTC):
    def prepare_rqlst(
        ...
        filtered_variable, baserql = facet.init_facets(
            rset, rqlst.children[0], mainvar=mainvar
        )
    ...
    def _in_group_facet(self, req, cls=facet.RelationFacet, no_relation=False):
        ...
        guests, managers = [
            eid
            for eid, in req.cnx.execute(
                "CWGroup G ORDERBY GN "
    ...
    def test_relation_simple(self):
        with self.admin_access.web_request() as req:
            ...
            self.assertEqual(
                f.vocabulary(), [("guests", guests), ("managers", managers)]
            )
            # ensure rqlst is left unmodified
            self.assertEqual(f.select.as_string(), "DISTINCT Any  WHERE X is CWUser")
            # rqlst = rset.syntax_tree()
            self.assertEqual(f.possible_values(), [str(guests), str(managers)])
            # ensure rqlst is left unmodified
            self.assertEqual(f.select.as_string(), "DISTINCT Any  WHERE X is CWUser")
    ...
    def test_relation_multiple_and(self):
        with self.admin_access.web_request() as req:
            ...
            self.assertEqual(
                f.select.as_string(),
                "DISTINCT Any  WHERE X is CWUser, X in_group A, B eid %s, X in_group B, A eid %s"
                % (guests, managers),
            )
    ...
    def test_relation_multiple_or(self):
        with self.admin_access.web_request() as req:
            ...
            self.assertEqual(
                f.select.as_string(),
                "DISTINCT Any  WHERE X is CWUser, X in_group A, A eid IN(%s, %s)"
                % (guests, managers),
            )

    def test_relation_optional_rel(self):
        with self.admin_access.web_request() as req:
            rset = req.cnx.execute(
                "Any X,GROUP_CONCAT(GN) GROUPBY X "
            ...
            f = facet.RelationFacet(
                req, rset=rset, select=select, filtered_variable=filtered_variable
            )
            f.rtype = "in_group"
            f.role = "subject"
            f.target_attr = "name"
            ...
            self.assertEqual(
                rqlst.as_string(),
                'DISTINCT Any  WHERE X in_group G?, G name GN, NOT G name "users"',
            )
            # rqlst = rset.syntax_tree()
            self.assertEqual(sorted(f.possible_values()), [str(guests), str(managers)])
            # ensure rqlst is left unmodified
            self.assertEqual(
                rqlst.as_string(),
                'DISTINCT Any  WHERE X in_group G?, G name GN, NOT G name "users"',
            ...
            self.assertEqual(
                f.select.as_string(),
                'DISTINCT Any  WHERE X in_group G?, G name GN, NOT G name "users", X in_group D, D eid %s'
                % guests,
            )
    ...
    def test_relation_no_relation_1(self):
        with self.admin_access.web_request() as req:
            ...
            self.assertEqual(f.possible_values(), [str(guests), str(managers)])
            f._cw.create_entity("CWUser", login="hop", upassword="toto")
            self.assertEqual(
                f.vocabulary(),
                [("<no relation>", ""), ("guests", guests), ("managers", managers)],
            )
            self.assertEqual(f.possible_values(), [str(guests), str(managers), ""])
    ...
    def test_relation_no_relation_2(self):
        with self.admin_access.web_request() as req:
            ...
            self.assertEqual(
                f.select.as_string(),
                "DISTINCT Any  WHERE X is CWUser, (NOT X in_group B) OR (X in_group A, A eid %s)"
                % guests,
            )
            f.select.recover()
            self.assertEqual(f.select.as_string(), "DISTINCT Any  WHERE X is CWUser")
    ...
    def test_relationattribute(self):
        with self.admin_access.web_request() as req:
            f, (guests, managers) = self._in_group_facet(
                req, cls=facet.RelationAttributeFacet
            )
            self.assertEqual(
                f.vocabulary(), [("guests", "guests"), ("managers", "managers")]
            )
            ...
            self.assertEqual(f.select.as_string(), "DISTINCT Any  WHERE X is CWUser")
            # rqlst = rset.syntax_tree()
            self.assertEqual(f.possible_values(), ["guests", "managers"])
            # ensure rqlst is left unmodified
            self.assertEqual(f.select.as_string(), "DISTINCT Any  WHERE X is CWUser")
            f._cw.form[f.__regid__] = "guests"
            f.add_rql_restrictions()
    ...
    def test_daterange(self):
        with self.admin_access.web_request() as req:
            ...
            self.assertEqual(f.vocabulary(), [(str(mind), mind), (str(maxd), maxd)])
            # ensure rqlst is left unmodified
    ...
    def test_attribute(self):
        with self.admin_access.web_request() as req:
            ...
            f.rtype = "login"
            self.assertEqual(f.vocabulary(), [("admin", "admin"), ("anon", "anon")])
            # ensure rqlst is left unmodified
            self.assertEqual(rqlst.as_string(), "DISTINCT Any  WHERE X is CWUser")
            # rqlst = rset.syntax_tree()
            self.assertEqual(f.possible_values(), ["admin", "anon"])
            # ensure rqlst is left unmodified
            self.assertEqual(rqlst.as_string(), "DISTINCT Any  WHERE X is CWUser")
            ...
            # is not in real life)
            self.assertEqual(
                f.select.as_string(), 'DISTINCT Any  WHERE X is CWUser, X login "admin"'
            )

    def test_bitfield(self):
        with self.admin_access.web_request() as req:
            rset, rqlst, filtered_variable = self.prepare_rqlst(
                req,
            ...
            f.rtype = "ordernum"
            self.assertEqual(f.vocabulary(), [("deux", 2), ("un", 1)])
            # ensure rqlst is left unmodified
            self.assertEqual(
                rqlst.as_string(), "DISTINCT Any  WHERE X ordernum XO, X is CWAttribute"
            )
            # rqlst = rset.syntax_tree()
            self.assertEqual(f.possible_values(), ["2", "1"])
            # ensure rqlst is left unmodified
            self.assertEqual(
                rqlst.as_string(), "DISTINCT Any  WHERE X ordernum XO, X is CWAttribute"
            )
    ...
    def test_bitfield_0_value(self):
        with self.admin_access.web_request() as req:
            ...
            f.rtype = "ordernum"
            self.assertEqual(f.vocabulary(), [("deux", 2), ("un", 1), ("zero", 0)])
            self.assertEqual(f.possible_values(), ["2", "1", "0"])
    ...
    def prepareg_aggregat_rqlst(self, req):
        return self.prepare_rqlst(
            ...
            "HAVING DAY(XD)>=DAY(YD) AND DAY(XM)<=DAY(YD)",
            "X",
    ...
    def test_aggregat_query_rql_path(self):
        with self.admin_access.web_request() as req:
            ...
            self.assertEqual(f.vocabulary(), [("admin", "admin")])
