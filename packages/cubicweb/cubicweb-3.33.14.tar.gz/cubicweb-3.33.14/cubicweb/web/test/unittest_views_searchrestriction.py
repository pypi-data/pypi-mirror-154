...
class InsertAttrRelationTC(CubicWebTC):
    ...
    def _generate(self, rqlst, rel, role, attr):
        ...
        facet.insert_attr_select_relation(select, filtered_variable, rel, role, attr)
        return rqlst.as_string()

    @property
    def select(self):
        return self.parse(
            "Any B,(NOW - CD),S,V,U,GROUP_CONCAT(TN),VN,P,CD,BMD "
    ...
    def test_1(self):
        self.assertEqual(
            self._generate(self.select, "in_state", "subject", "name"),
    ...
    def test_2(self):
        self.assertEqual(
            self._generate(self.select, "tags", "object", "name"),
            'DISTINCT Any A,C ORDERBY C WHERE B in_group P, P name "managers", '
    ...
    def test_3(self):
        self.assertEqual(
            self._generate(self.select, "created_by", "subject", "login"),
            'DISTINCT Any A,C ORDERBY C WHERE B in_group P, P name "managers", '
    ...
    def test_4(self):
        self.assertEqual(
            self._generate(
                self.parse("Any X WHERE X is CWUser"), "created_by", "subject", "login"
            ),
    ...
    def test_5(self):
        self.assertEqual(
            self._generate(
                self.parse("Any X,L WHERE X is CWUser, X login L"),
                "created_by",
                "subject",
                "login",
            ),
    ...
    def test_nonregr1(self):
        select = self.parse(
            "Any T,V WHERE T bookmarked_by V?, "
            'V in_state VS, VS name "published", T created_by U'
        )
        self.assertEqual(
            self._generate(select, "created_by", "subject", "login"),
    ...
    def test_nonregr2(self):
        ...
        select = self.parse(
            "DISTINCT Any V,TN,L ORDERBY TN,L WHERE T nom TN, V connait T, T is Personne, V is CWUser,"
        ...
        try:
            self.assertEqual(
                self._generate(select, "in_state", "subject", "name"),
    ...
    def test_nonregr3(self):
        #'DISTINCT Any X,TMP,N WHERE P name TMP, X version_of P, P is Project, X is Version, not X in_state S,S name "published", X num N ORDERBY TMP,N'
        select = self.parse(
            "DISTINCT Any X, MAX(Y) GROUPBY X WHERE X is CWUser, Y is Bookmark, X in_group A"
        )
        self.assertEqual(
            self._generate(select, "in_group", "subject", "name"),
