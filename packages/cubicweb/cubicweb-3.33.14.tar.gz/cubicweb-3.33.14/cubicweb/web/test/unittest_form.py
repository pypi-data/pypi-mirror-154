...
class FieldsFormTC(CubicWebTC):
    ...
    def test_process_posted(self):
        ...
        with self.admin_access.web_request(
            anint="1", astring="2", _cw_fields="anint,astring"
        ) as req:
            form = AForm(req)
            self.assertEqual(form.process_posted(), {"anint": 1, "astring": "2"})
        with self.admin_access.web_request(
            anint="1a", astring="2b", _cw_fields="anint,astring"
        ) as req:
            form = AForm(req)
...
class EntityFieldsFormTC(CubicWebTC):
    def test_form_field_choices(self):
        with self.admin_access.web_request() as req:
            ...
            choices = [
                reid
                for rview, reid in form1.field_by_name(
                    "tags", "subject", t.e_schema
                ).choices(form1)
            ]
            ...
            choices = [
                reid
                for rview, reid in form2.field_by_name(
                    "tags", "object", t.e_schema
                ).choices(form2)
            ]
    ...
    def test_form_field_choices_new_entity(self):
        with self.admin_access.web_request() as req:
            ...
            unrelated = [
                rview
                for rview, reid in form.field_by_name("in_group", "subject").choices(
                    form
                )
            ]
            # should be default groups but owners, i.e. managers, users, guests
            self.assertEqual(unrelated, ["guests", "managers", "users"])

    def test_consider_req_form_params(self):
        with self.admin_access.web_request() as req:
            e = self.vreg["etypes"].etype_class("CWUser")(req)
            e.eid = "A"
            with self.admin_access.web_request(login="toto") as toto_req:
                form = EntityFieldsForm(toto_req, None, entity=e)
                field = StringField(name="login", role="subject", eidparam=True)
                form.append_field(field)
                form.build_context({})
                self.assertEqual(field.widget.values(form, field), ("toto",))
    ...
    def test_linkto_field_duplication_inout(self):
        with self.admin_access.web_request() as req:
            ...
            pageinfo = self._check_html("\n".join(data), form, template=None)
            inputs = pageinfo.find_tag("select", False)
            ok = False
            for selectnode in pageinfo.matching_nodes(
                "select", name="from_in_group-subject:A"
            ):
    ...
    def test_reledit_composite_field(self):
        with self.admin_access.web_request() as req:
            rset = req.execute(
                'INSERT BlogEntry X: X title "cubicweb.org", X content "hop"'
            )
            form = self.vreg["views"].select(
                "reledit", req, rset=rset, row=0, rtype="content"
            )
            data = form.render(
                row=0, rtype="content", formid="base", action="edit_rtype"
            )
    ...
    def test_form_inheritance(self):
        with self.admin_access.web_request() as req:
            ...
            form = CustomChangeStateForm(
                req, redirect_path="perdu.com", entity=req.user
            )
            data = []
            form.render(
                w=data.append,
                formvalues=dict(state=123, trcomment="", trcomment_format="text/plain"),
            )

    def test_change_state_form(self):
        with self.admin_access.web_request() as req:
            form = ChangeStateForm(req, redirect_path="perdu.com", entity=req.user)
            data = []
            form.render(
                w=data.append,
                formvalues=dict(state=123, trcomment="", trcomment_format="text/plain"),
    ...
    def _test_richtextfield(self, req, expected):
        ...
        form.field_by_name(
            "description", "subject"
        ).format = lambda form, field=None: "text/html"
        self.assertMultiLineEqual(
            self._render_entity_field(req, "description", form),
    ...
    def test_richtextfield_1(self):
        with self.admin_access.web_request() as req:
            ...
            self._test_richtextfield(
                req,
                """<select id="description_format-subject:%(eid)s" name="description_format-subject:%(eid)s" size="1" style="display: block">
"""
                + (
...
<option value="text/rest">text/rest</option>
    ...
    def test_richtextfield_2(self):
        with self.admin_access.web_request() as req:
            ...
            self._test_richtextfield(
                req,
                '<input name="description_format-subject:%(eid)s" type="hidden" value="text/html" /><textarea cols="80" cubicweb:type="wysiwyg" id="description-subject:%(eid)s" name="description-subject:%(eid)s" onkeyup="autogrow(this)" rows="2"></textarea>',
            )

    def test_filefield(self):
        class FFForm(EntityFieldsForm):
            data = FileField(
                format_field=StringField(
                    name="data_format", max_length=50, eidparam=True, role="subject"
                ),
                encoding_field=StringField(
                    name="data_encoding", max_length=20, eidparam=True, role="subject"
                ),
                eidparam=True,
                role="subject",
            )
        ...
        with self.admin_access.web_request() as req:
            ...
            self.assertMultiLineEqual(
                self._render_entity_field(req, "data", form),
    ...
    def test_editablefilefield(self):
        class EFFForm(EntityFieldsForm):
            data = EditableFileField(
                format_field=StringField(
                    "data_format", max_length=50, eidparam=True, role="subject"
                ),
                encoding_field=StringField(
                    "data_encoding", max_length=20, eidparam=True, role="subject"
                ),
                eidparam=True,
                role="subject",
            )
...
<textarea cols="80" name="data-subject:%(eid)s" onkeyup="autogrow(this)" rows="3">new widgets system</textarea>"""
                % {"eid": file.eid},
            )
    ...
    def _modified_tzdatenaiss(self, eid, date_and_time_str=None):
        ...
        with self.admin_access.web_request(**ctx) as req:
            ...
            field = TZDatetimeField(name="tzdatenaiss", eidparam=True, role="subject")
            form.append_field(field)
            form.build_context({})
    ...
    def test_tzdatetimefield(self):
        ...
        with self.admin_access.web_request() as req:
            eid = req.create_entity("Personne", nom="Flo", tzdatenaiss=tzd).eid
            req.cnx.commit()
    ...
    def test_passwordfield(self):
        ...
        with self.admin_access.web_request() as req:
            ...
            self.assertMultiLineEqual(
                self._render_entity_field(req, "upassword", form),
