...
class AutomaticWebTest(AutomaticWebTest):
    ...
    def to_test_etypes(self):
        ...
        etypes = super().to_test_etypes()
        etypes -= {"Blog", "BlogEntry", "CWSession"}
        return etypes
...
class ManualCubicWebTCs(AutoPopulateTest):
    ...
    def test_sortable_js_added(self):
        with self.admin_access.web_request() as req:
            ...
            self.assertNotIn(
                b"jquery.tablesorter.js", self.view("oneline", rset, req=req).source
            )

        with self.admin_access.web_request() as req:
            # but should be included by the tableview
            rset = req.execute(
                "Any P,F,S LIMIT 1 WHERE P is CWUser, P firstname F, P surname S"
            )
            self.assertIn(
                b"jquery.tablesorter.js", self.view("table", rset, req=req).source
            )
    ...
    def test_unrelateddivs(self):
        with self.admin_access.client_cnx() as cnx:
            group = cnx.create_entity("CWGroup", name="R&D")
            cnx.commit()
