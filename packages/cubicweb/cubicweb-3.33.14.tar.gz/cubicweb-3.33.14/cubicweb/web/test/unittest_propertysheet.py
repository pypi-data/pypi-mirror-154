...
class PropertySheetTC(TestCase):
    ...
    def test(self):
        ...
        self.assertEqual(
            ps["stylesheets"],
            ["http://cwtest.com/cubicweb.css", "http://cwtest.com/mycube.css"],
        )
        ...
        # test compilation
        self.assertEqual(
            ps.compile("a {bgcolor: %(bgcolor)s; size: 1%;}"),
            "a {bgcolor: #FFFFFF; size: 1%;}",
        )
        self.assertEqual(ps.process_resource(DATADIR, "pouet.css"), self.cachedir)
