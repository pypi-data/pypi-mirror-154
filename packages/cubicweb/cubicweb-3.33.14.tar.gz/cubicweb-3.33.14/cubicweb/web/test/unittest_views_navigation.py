...
from cubicweb.web.views.navigation import (
    PageNavigation,
    SortedNavigation,
...
class NavigationTC(CubicWebTC):
    ...
    def test_navigation_selection_ordered(self):
        with self.admin_access.web_request() as req:
            ...
            navcomp = self.vreg["components"].select(
                "navigation", req, rset=rset, page_size=20
            )
    ...
    def test_navigation_selection_not_enough_1(self):
        with self.admin_access.web_request() as req:
            ...
            navcomp = self.vreg["components"].select_or_none(
                "navigation", req, rset=rset
            )
    ...
    def test_navigation_selection_not_enough_2(self):
        with self.admin_access.web_request() as req:
            rset = req.execute(
                "Any N, COUNT(RDEF) GROUPBY N ORDERBY N "
    ...
    def test_sorted_navigation_4(self):
        with self.admin_access.web_request() as req:
            rset = req.execute(
                "Any RDEF ORDERBY N " "WHERE RDEF relation_type RT, RT name N"
            )
            navcomp = self.vreg["components"].select("navigation", req, rset=rset)
            html = navcomp.render()
