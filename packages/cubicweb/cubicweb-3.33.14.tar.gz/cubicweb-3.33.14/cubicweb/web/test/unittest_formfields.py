...
class GuessFieldTC(CubicWebTC):
    def test_state_fields(self):
        ...
        with self.admin_access.web_request() as req:
            description_field = guess_field(
                schema["State"], schema["description"], req=req
            )
        ...
        with self.admin_access.web_request() as req:
            description_format_field = guess_field(
                schema["State"], schema["description_format"], req=req
            )
    ...
    def test_cwuser_fields(self):
        with self.admin_access.web_request() as req:
            upassword_field = guess_field(
                schema["CWUser"], schema["upassword"], req=req
            )
        ...
        with self.admin_access.web_request() as req:
            last_login_time_field = guess_field(
                schema["CWUser"], schema["last_login_time"], req=req
            )
        ...
        with self.admin_access.web_request() as req:
            ...
            self.assertEqual(
                in_group_field.help, "groups grant permissions to the user"
            )

        with self.admin_access.web_request() as req:
            owned_by_field = guess_field(
                schema["CWUser"], schema["owned_by"], "object", req=req
            )
    ...
    def test_constraints_priority(self):
        with self.admin_access.web_request() as req:
            salesterm_field = guess_field(
                schema["Salesterm"], schema["reason"], req=req
            )
            constraints = schema["reason"].rdef("Salesterm", "String").constraints
            self.assertEqual(
                [c.__class__ for c in constraints],
    ...
    def test_bool_field_base(self):
        with self.admin_access.web_request() as req:
            ...
            self.assertEqual(
                field.vocabulary(mock(_cw=mock(_=cubicweb._))),
                [("yes", "1"), ("no", "")],
            )
