...
class IDownloadableTC(CubicWebTC):
    ...
    def test_header_simple_case(self):
        with self.admin_access.web_request() as req:
            ...
            self.assertEqual(
                ['attachment;filename="admin.txt"'], get("content-disposition")
            )
            self.assertEqual(["text/plain;charset=ascii"], get("content-type"))
            last_mod = http_headers.parseDateTime(get("last-modified")[0])
            self.assertEqual(
                datetime.fromtimestamp(last_mod, tz=utc),
    ...
    def test_header_with_space(self):
        with self.admin_access.web_request() as req:
            self.create_user(req, login="c c l a", password="babar")
            req.cnx.commit()
        with self.new_access("c c l a").web_request() as req:
            req.form["vid"] = "download"
            req.form["eid"] = str(req.user.eid)
            data = self.ctrl_publish(req, "view")
            get = req.headers_out.getRawHeaders
            self.assertEqual(
                ['attachment;filename="c c l a.txt"'], get("content-disposition")
            )
            self.assertEqual(["text/plain;charset=ascii"], get("content-type"))
            self.assertEqual(b"Babar is not dead!", data)
    ...
    def test_header_with_space_and_comma(self):
        with self.admin_access.web_request() as req:
            self.create_user(req, login='c " l\\ a', password="babar")
            req.cnx.commit()
        with self.new_access('c " l\\ a').web_request() as req:
            req.form["vid"] = "download"
            req.form["eid"] = str(req.user.eid)
            data = self.ctrl_publish(req, "view")
            get = req.headers_out.getRawHeaders
            self.assertEqual(
                [r'attachment;filename="c \" l\\ a.txt"'], get("content-disposition")
            )
            self.assertEqual(["text/plain;charset=ascii"], get("content-type"))
            self.assertEqual(b"Babar is not dead!", data)
    ...
    def test_header_unicode_filename(self):
        with self.admin_access.web_request() as req:
            self.create_user(req, login="cécilia", password="babar")
            req.cnx.commit()
        with self.new_access("cécilia").web_request() as req:
            req.form["vid"] = "download"
            req.form["eid"] = str(req.user.eid)
            ...
            self.assertEqual(
                [
                    """attachment;filename="ccilia.txt";filename*=utf-8''c%C3%A9cilia.txt"""
                ],
                get("content-disposition"),
            )

    def test_header_unicode_long_filename(self):
        name = "Bèrte_hô_grand_nôm_ça_va_totallement_déborder_de_la_limite_là"
        with self.admin_access.web_request() as req:
        ...
        with self.new_access(name).web_request() as req:
            ...
            self.assertEqual(
                [
                    """attachment;filename="Brte_h_grand_nm_a_va_totallement_dborder_de_la_limite_l.txt";filename*=utf-8''B%C3%A8rte_h%C3%B4_grand_n%C3%B4m_%C3%A7a_va_totallement_d%C3%A9border_de_la_limite_l%C3%A0.txt"""
                ],
                get("content-disposition"),
            )
    ...
    def test_download_data_error(self):
        ...
        with self.admin_access.web_request() as req:
            req.form["vid"] = "download"
            req.form["eid"] = str(
                req.execute('CWGroup X WHERE X name "managers"')[0][0]
            )
            with real_error_handling(self.app):
                data = self.app_handle_request(req)
            get = req.headers_out.getRawHeaders
            self.assertEqual(["text/html;charset=UTF-8"], get("content-type"))
            self.assertEqual(None, get("content-disposition"))
