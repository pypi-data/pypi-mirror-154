...
class UrlRewriteTC(CubicWebTC):
        ...
        self.assertListEqual(
            ...
            [
                ...
                (
                    "/cwuser$",
                    {
                        "vid": "cw.users-and-groups-management",
                ...
                (
                    "/schema/([^/]+?)/?$",
                    {"rql": r'Any X WHERE X is CWEType, X name "\1"', "vid": "primary"},
                ),
    ...
    def test_inheritance(self):
        ...
        with self.admin_access.web_request() as req:
            x = req.create_entity("WorkflowTransition", name="test")
            ctrlid, rset = self.app.url_resolver.process(
                req, "basetransition/%s" % x.eid
            )
...
class RgxActionRewriteTC(CubicWebTC):
    ...
    def test_rgx_action_with_transforms(self):
        class TestSchemaBasedRewriter(SchemaBasedRewriter):
            rules = [
                (
                    rgx(r"/(?P<sn>\w+)/(?P<fn>\w+)"),
                    rgx_action(
                        r"Any X WHERE X surname %(sn)s, " "X firstname %(fn)s",
                        argsgroups=("sn", "fn"),
                        transforms={
                            "sn": str.capitalize,
                            "fn": str.lower,
        ...
        with self.admin_access.web_request() as req:
            ...
            _pmid, rset = rewriter.rewrite(req, "/DaLToN/JoE")
