...
from cubicweb.web.views.magicsearch import (
    translate_rql_tree,
    QSPreProcessor,
    QueryTranslator,
)
...
class RQLSuggestionsBuilderTC(CubicWebTC):
    ...
    def test_is_rql(self):
        ...
        self.assertListEqual(
            ["Any X WHERE X is Personne", "Any X WHERE X is Project"],
    ...
    def test_relations_rql(self):
        self.assertListEqual(
            [
                "Any X WHERE X is Personne, X ass A",
    ...
    def test_attribute_value_rql(self):
        ...
        with self.admin_access.web_request() as req:
            for i in range(15):
                req.create_entity("Personne", nom="n%s" % i, prenom="p%s" % i)
            req.cnx.commit()
        self.assertListEqual(
            [
