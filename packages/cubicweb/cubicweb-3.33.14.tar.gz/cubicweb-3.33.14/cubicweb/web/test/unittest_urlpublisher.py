...
class URLPublisherTC(CubicWebTC):
    ...
    def setup_database(self):
        with self.admin_access.repo_cnx() as cnx:
            ...
            c = cnx.create_entity("Tag", name="yo")
            cnx.execute(
                "SET C tags B WHERE C eid %(c)s, B eid %(b)s", {"c": c.eid, "b": b.eid}
            )
            cnx.commit()
    ...
    def test_rest_path_etype(self):
        ...
        with self.admin_access.web_request() as req:
            ...
            self.assertEqual(
                "Any X,AA,AB ORDERBY AB WHERE X is_instance_of CWEType, "
    ...
    def test_rest_path_by_attr(self):
        with self.admin_access.web_request() as req:
            ...
            self.assertEqual(
                "Any X,AA,AB,AC,AD WHERE X is_instance_of CWUser, "
    ...
    def test_rest_path_non_ascii_paths(self):
        with self.admin_access.web_request() as req:
            ...
            self.assertEqual(
                "Any X,AA,AB,AC,AD WHERE X is_instance_of CWUser, "
                "X firstname AA, X login AB, X modification_date AC, "
                'X surname AD, X login "\xffsa\xffe"',
                rset.printable_rql(),
            )
    ...
    def test_rest_path_quoted_paths(self):
        with self.admin_access.web_request() as req:
            ...
            self.assertEqual(
                "Any X,AA,AB,AC WHERE X is_instance_of BlogEntry, "
    ...
    def test_rest_path_errors(self):
        with self.admin_access.web_request() as req:
            ...
            self.assertRaises(
                NotFound, self.process, req, "CWUser/inexistant_attribute/joe"
            )
    ...
    def test_action_path(self):
        ...
        with self.admin_access.web_request() as req:
            ...
            self.assertRaises(
                NotFound, self.process, req, "CWUser/login/admin/non_action"
            )
    ...
    def test_nonascii_path(self):
        ...
        SimpleReqRewriter.rules = [(re.compile(r"/\w+", re.U), dict(vid="foo"))]
        with self.admin_access.web_request() as req:
            try:
                path = str(FakeRequest().url_quote("été"))
