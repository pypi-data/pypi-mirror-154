...
class PropertySheet(dict):
        ...
        for name, type in TYPE_CHECKS:
            if name in self:
                if not isinstance(self[name], type):
                    msg = "Configuration error: {}.{} should be a {}".format(
                        fpath,
                        name,
                        type,
                    )
    ...
    def process_resource(self, rdirectory, rid):
        ...
        self.debug("processing %s/%s into %s", rdirectory, rid, cachefile)
        ...
        try:
            ...
            tmpfd, tmpfile = tempfile.mkstemp(
                dir=rcachedir, prefix=osp.basename(cachefile)
            )
