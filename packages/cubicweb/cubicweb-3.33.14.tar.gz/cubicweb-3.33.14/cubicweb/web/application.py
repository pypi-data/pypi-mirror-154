...
class CookieSessionHandler:
    ...
    def __init__(self, appli):
        ...
        self.session_manager = self.vreg["sessions"].select(
            "sessionmanager", repo=self.repo
        )
...
class CubicWebPublisher:
    ...
    def set_urlresolver(self):
        self.url_resolver = self.vreg["components"].select(
            "urlpublisher", vreg=self.vreg
        )
        ...
        def wrap_set_cnx(func):
            def wrap_execute(cnx):
                ...
                def execute(rql, kwargs=None, build_descr=True):
                    ...
                    cnx.executed_queries.append(
                        (rql, kwargs, time() - tstart, process_time() - cstart)
                    )
                    return rset
    ...
    def add_undo_link_to_msg(self, req):
        ...
        if txuuid is not None:
            msg = '<span class="undo">[<a href="{}">{}</a>]</span>'.format(
                req.build_url("undo", txuuid=txuuid),
                req._("undo"),
            )
