...
def addrheader(addr, uname=None):
    ...
    if uname:
        val = "{} <{}>".format(header(uname).encode(), addr)
    else:
        val = addr
...
def construct_message_id(appid, eid, withtimestamp=True):
    if withtimestamp:
        addrpart = f"eid={eid}&timestamp={time():.10f}"
    else:
        addrpart = "eid=%s" % eid
    # we don't want any equal sign nor trailing newlines
    leftpart = (
        b64encode(addrpart.encode("ascii"), b".-").decode("ascii").rstrip().rstrip("=")
    )
    return f"<{leftpart}@{appid}.{gethostname()}>"
...
def format_mail(
    ...
    if config and config["sender-addr"] and config["sender-addr"] != email:
        ...
        msg["Reply-to"] = "{}, {}".format(msg["From"], appaddr)
    elif email:
        msg["Reply-to"] = msg["From"]
    if config is not None:
        msg["X-CW"] = config.appid
    unique_addrs = lambda addrs: sorted({addr for addr in addrs if addr is not None})
