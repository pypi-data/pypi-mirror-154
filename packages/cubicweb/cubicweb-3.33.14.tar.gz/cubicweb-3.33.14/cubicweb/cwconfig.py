...
from cubicweb import CW_SOFTWARE_ROOT, CW_MIGRATION_MAP, ConfigurationError, Binary, _
...
def configuration_cls(name):
    ...
    try:
        ...
        raise ConfigurationError(
            'no such config %r (check it exists with "cubicweb-ctl list")' % name
        )
...
def possible_configurations(directory):
    ...
    return [
        name
        for name in ("repository", "all-in-one", "pyramid")
...
def guess_configuration(directory):
    ...
    if len(modes) != 1:
        raise ConfigurationError(
            "unable to guess configuration from %r %s" % (directory, modes)
        )
    return modes[0]
...
this option is set to yes",
    ...
    def cubicweb_version():
        ...
        str_base_version = pkg_resources.get_distribution(
            "cubicweb"
    ...
    def cube_dir(cls, cube):
        ...
        raise ConfigurationError(msg % {"cube": cube, "pkg": _cube_pkgname(cube)})
    ...
    def load_available_configs(cls):
        for confmod in ("web.webconfig", "server.serverconfig", "pyramid.config"):
            try:
    ...
    def init_log(self, logthreshold=None, logfile=None, syslog=False):
        ...
        if sys.platform == "win32":
            ...
            init_log(
                not self.log_to_file, syslog, logthreshold, logfile, self.log_format
            )
    ...
    def schema_modnames(self):
        ...
        for cube in reversed(self.cubes()):
            for modname, filepath in _expand_modname(
                "{}.schema".format(_cube_pkgname(cube)), recursive=False
            ):
...
class CubicWebConfiguration(CubicWebNoAppConfiguration):
    ...
    def instance_home(cls, appid):
        ...
        if not exists(home):
            raise ConfigurationError(
                "no such instance %s (check it exists with"
    ...
    def default_log_file(self):
        ...
        if self.mode == "user":
            ...
            basepath = join(
                tempfile.gettempdir(), "%s-%s" % (basename(self.appid), self.name)
            )
            path = basepath + ".log"
            i = 1
    ...
    def __init__(self, appid, debugmode=False, creating=False, log_to_file=False):
        ...
        super(CubicWebConfiguration, self).__init__(debugmode, log_to_file=log_to_file)
...
set_log_methods(CubicWebNoAppConfiguration, logging.getLogger("cubicweb.configuration"))
