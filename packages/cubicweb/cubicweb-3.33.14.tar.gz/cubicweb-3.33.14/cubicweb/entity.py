...
class Entity(AppObject):
    ...
    @classmethod
    def fetch_rql(
        cls, user, fetchattrs=None, mainvar="X", settype=True, ordermethod="fetch_order"
    ):
        st = cls.fetch_rqlst(
            user,
            mainvar=mainvar,
    ...
    def fetch_rqlst(
        ...
        select._varmaker = rqlvar_maker(
            defined=select.defined_vars, aliases=select.aliases, index=26
        )
        if settype:
    ...
    def _fetch_restrictions(
        cls, mainvar, select, fetchattrs, user, ordermethod="fetch_order", visited=None
    ):
        ...
        for attr in sorted(fetchattrs):
            ...
            if rschema.final or rdef.cardinality[0] in "?1":
                ...
                cls.warning(
                    "bad relation %s specified in fetch attrs for %s", attr, cls
                )
                continue
            if not rschema.final:
                ...
                if len(targettypes) > 1:
                    ...
                    cls._fetch_ambiguous_rtypes(
                        select, var, fetchattrs, targettypes, vreg.schema
                    )
                else:
                    fetchattrs = etypecls.fetch_attrs
                etypecls._fetch_restrictions(
                    var, select, fetchattrs, user, None, visited=visited
                )
    ...
    def _cw_build_entity_query(cls, kwargs):
        ...
        for attr, value in kwargs.items():
            if attr.startswith("reverse_"):
                ...
                role = "subject"
            assert eschema.has_relation(attr, role), "%s %s not found on %s" % (
                attr,
                role,
                eschema,
            )
            rschema = (
                eschema.subjrels[attr] if role == "subject" else eschema.objrels[attr]
            )
    ...
    def _cw_handle_pending_relations(cls, eid, pendingrels, execute):
        for attr, role, values in pendingrels:
            ...
            if values is None:
                ...
                continue
            execute(
                "SET %s WHERE X eid %%(x)s, Y eid IN (%s)"
    ...
    def cw_instantiate(cls, execute, **kwargs):
        ...
        try:
            ...
            raise Exception(
                "could not create a %r with %r (%r)" % (cls.__regid__, rql, qargs)
            )
    ...
    def _cw_update_attr_cache(self, attrcache):
        ...
        uncached_attrs = trdata.get(
            "%s.storage-special-process-attrs" % self.eid, set()
        )
    ...
    def _cw_dont_cache_attribute(self, attr, repo_side=False):
        ...
        if repo_side:
            trdata.setdefault("%s.storage-special-process-attrs" % self.eid, set()).add(
                attr
            )
    ...
    def view(
        self, __vid, __registry="views", w=None, initargs=None, **kwargs
    ):  # XXX cw_view
        ...
        view = self._cw.vreg[__registry].select(
            __vid,
            self._cw,
            rset=self.cw_rset,
            row=self.cw_row,
        ...
        if mainattr != "eid":
            ...
            if not can_use_rest_path(value):
                ...
                try:
                    ...
                    nbresults = self.__unique = self._cw.execute(rql, {"value": value})[
                        0
                    ][0]
    ...
    def printable_value(
        ...
        if attrtype == "String":
            ...
            if attrformat:
                return self._cw_mtc_transform(
                    value, attrformat, format, self._cw.encoding
                )
        elif attrtype == "Bytes":
            attrformat = self.cw_attr_metadata(attr, "format")
            if attrformat:
                ...
                return self._cw_mtc_transform(
                    value.getvalue(), attrformat, format, encoding
                )
            return ""
        value = self._cw.printable_value(
            attrtype, value, props, displaytime=displaytime
        )
        if format == "text/html":
    ...
    def _cw_to_complete_relations(self):
        ...
        for rschema in self.e_schema.subject_relations():
            ...
            if rschema.inlined:
                ...
                if all(
                    matching_groups(e.get_groups("read"))
                    and rschema.rdef(self.e_schema, e).get_groups("read")
                    for e in targets
                ):
    ...
    def _cw_to_complete_attributes(self, skip_bytes=True, skip_pwd=True):
        for rschema, attrschema in self.e_schema.attribute_definitions():
            ...
            if not self._cw.user.matching_groups(rdef.get_groups("read")) or (
                attrschema.type == "Password" and skip_pwd
            ):
    ...
    def complete(
        self, attributes=None, skip_bytes=True, skip_pwd=True
    ):  # XXX cw_complete
        ...
        if selected:
            ...
            rql = "Any %s,%s %s" % (
                V,
                ",".join(var for attr, var in selected),
                ",".join(rql),
            )
            try:
                ...
                raise Exception(
                    "unable to fetch attributes for entity with eid %s" % self.eid
                )
            # handle attributes
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    self.critical(
                        "can't get value for attribute %s of entity with eid %s",
                        name,
                        self.eid,
                    )
    ...
    def cw_related_rql(self, rtype, role="subject", targettypes=None, limit=None):
        return self.cw_related_rqlst(
            rtype, role=role, targettypes=targettypes, limit=limit
        ).as_string()

    def cw_related_rqlst(
        self, rtype, role="subject", targettypes=None, limit=None, sort_terms=None
    ):
        ...
        if role == "subject":
            ...
            if targettypes is None:
                ...
                select.add_constant_restriction(mainvar, "is", targettypes, "etype")
        ...
        if len(targettypes) > 1:
            fetchattrs = vreg["etypes"].fetch_attrs(targettypes)
            self._fetch_ambiguous_rtypes(
                select, mainvar, fetchattrs, targettypes, vreg.schema
            )
        else:
            fetchattrs = etypecls.fetch_attrs
        etypecls.fetch_rqlst(self._cw.user, select, mainvar, fetchattrs, settype=False)
    ...
    def cw_linkable_rql(
        ...
        are a list of eids.
        """
        return self._cw_compute_linkable_rql(
            rtype,
            targettype,
            role,
            ordermethod=None,
    ...
    def _cw_compute_linkable_rql(
        ...
        etypecls.fetch_rqlst(
            self._cw.user, select, searchedvar, ordermethod=ordermethod
        )
        ...
        for cstr in rdef.constraints:
            ...
            if isinstance(cstr, cstrcls) and searchedvar.name in cstr.mainvars:
                ...
                varmap = dict(
                    (v, v)
                    for v in (searchedvar.name, evar.name)
        ...
        if not self.has_eid():
            rqlexprs = [
                rqlexpr for rqlexpr in rqlexprs if searchedvar.name in rqlexpr.mainvars
            ]
    ...
    def unrelated(
        ...
        """
        try:
            rql, args = self.cw_unrelated_rql(
                rtype,
                targettype,
                role,
                limit=limit,
    ...
    def cw_set(self, **kwargs):
        ...
        """
        assert kwargs
        assert self.cw_is_saved(), (
            "should not call set_attributes while entity " "hasn't been saved yet"
        )
    ...
    def cw_delete(self, **kwargs):
        ...
        self._cw.execute(
            "DELETE %s X WHERE X eid %%(x)s" % self.e_schema, {"x": self.eid}, **kwargs
        )
...
class Relation(object):
    ...
    def __get__(self, eobj, eclass):
        if eobj is None:
            raise AttributeError("%s can only be accessed from instances" % self._rtype)
...
class EntityAdapter(Adapter):
    ...
    def __init__(self, _cw, **kwargs):
        try:
            ...
            self.entity = kwargs["rset"].get_entity(
                kwargs.get("row") or 0, kwargs.get("col") or 0
            )
