...
def _subst_rql(view, obj):
    ...
    return '<a href="{}">{}</a>'.format(view._cw.build_url(rql=rql), descr)
...
class _JSCallArgs(_JSId):
    ...
    def __str__(self):
        ...
        if self.parent:
            return "{}({})".format(self.parent, ",".join(args))
        return ",".join(args)
...
def sgml_attributes(attrs):
    return " ".join(
        '{}="{}"'.format(attr, xml_escape(str(value)))
...
def toggle_action(nodeid):
    ...
    return "javascript: toggleVisibility('%s')" % nodeid


def toggle_link(nodeid, label):
    """builds a HTML link that uses the js toggleVisibility function"""
    return '<a href="{}">{}</a>'.format(toggle_action(nodeid), label)
...
def html_traceback(info, exception, title="", encoding="ISO-8859-1", body=""):
    ...
    if body:
        ...
        strings.append("</div>")
    if title:
        strings.append('<h1 class="error">%s</h1>' % xml_escape(title))
    try:
        strings.append(
            '<p class="error">%s</p>'
    ...
    for index, stackentry in enumerate(stacktb):
        ...
        try:
            ...
            strings.append("[%s]" % toggle_link(boxid, "+"))
            strings.append(
