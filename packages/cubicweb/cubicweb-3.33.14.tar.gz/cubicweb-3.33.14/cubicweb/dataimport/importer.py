    ...
    def use_extid_as_cwuri_filter(extentities):
        for extentity in extentities:
            if extentity.extid not in extid2eid:
                ...
                extentity.values.setdefault("cwuri", {cwuri})
            yield extentity
...
def drop_extra_values(extentities, schema, import_log):
    ...
    for extentity in extentities:
        ...
        for key, rtype, role in extentity.iter_rdefs():
            ...
            if (rschema.final or (rschema.inlined and role == "subject")) and len(
                entity_dict[key]
            ) > 1:
                ...
                import_log.record_warning(
                    ...
                    % (rtype, values),
                    path=extentity.extid,
                )
                entity_dict[key] = {entity_dict[key].pop()}
        yield extentity
...
class RelationMapping:
    ...
    def __getitem__(self, rtype):
        ...
        return {tuple(x) for x in self.cnx.execute(rql, self._kwargs)}
...
class ExtEntity:
    ...
    * ``values``, a dictionary whose keys are attribute or relation names from the schema (eg.
    ...
    def __repr__(self):
        return f"<{self.etype} {self.extid} {self.values}>"
        ...
        * remove non inlined relations and return them as a [(e1key, relation, e2key)] list
        ...
        for key, rtype, role in self.iter_rdefs():
            ...
            if rschema.final or (rschema.inlined and role == "subject"):
                assert (
                    len(entity_dict[key]) <= 1
                ), "more than one value for {}: {} ({})".format(
                    rtype,
                    entity_dict[key],
                    self.extid,
                )
                if entity_dict[key]:
                    ...
                    if (
                        ...
                        entity_dict[rtype + "_format"] = "text/plain"
                    if (
                        rschema.final
    ...
    def why_not_ready(self, extid2eid):
        ...
        for rtype in entity_dict:
            ...
            if not rschema.final:
                if entity_dict[rtype] not in extid2eid:
                    return "inlined relation {} is not present ({})".format(
                        rtype,
                        entity_dict[rtype],
                    )
...
class ExtEntitiesImporter:
    ...
    def __init__(
        ...
        self.existing_relations = (
            existing_relations if existing_relations is not None else defaultdict(set)
        )
    ...
    def prepare_insert_deferred_relations(self, deferred):
        ...
        for rtype, relations in deferred.items():
            self.import_log.record_debug(
                "importing {} {} relations".format(len(relations), rtype)
            )
    ...
    def _warn_about_missing_work(self, queue, missing_relations):
        ...
        if queue:
            msgs = [
                "can't create some entities, is there some cycle or " "missing data?"
            ]
            for ext_entities in queue.values():
                for ext_entity in ext_entities:
                    msg = "{}: {}".format(
                        ext_entity, ext_entity.why_not_ready(self.extid2eid)
                    )
                    msgs.append(msg)
            map(error, msgs)
        ...
        if missing_relations:
            ...
            for subject_uri, rtype, object_uri, exc in missing_relations:
                msgs.append(
                    "Could not find %s when trying to insert (%s, %s, %s)"
...
class SimpleImportLog:
    ...
    def _log(self, severity, msg, path, line):
        encodedmsg = "{}\t{}\t{}\t{}".format(severity, self.filename, line or "", msg)
...
class HTMLImportLog(SimpleImportLog):
    ...
    def __init__(self, filename):
        super().__init__(xml_escape(filename))

    def _log(self, severity, msg, path, line):
        encodedmsg = "{}\t{}\t{}\t{}<br/>".format(
            severity,
            self.filename,
            line or "",
            xml_escape(msg),
