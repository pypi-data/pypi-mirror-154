...
def all_indexes(cnx):
    ...
    return {r[0] for r in crs.fetchall()}
...
class MassImportSimpleTC(testlib.CubicWebTC):
    ...
    def push_geonames_data(self, dumpname, store):
        # Push timezones
        cnx = store._cnx
        for code, gmt, dst, raw_offset in ucsvreader(
            open(self.datapath("timeZones.txt"), "rb"), delimiter="\t"
        ):
            cnx.create_entity(
                "TimeZone",
                code=code,
        ...
        # Push data
        for ind, infos in enumerate(
            ucsvreader(open(dumpname, "rb"), delimiter="\t", ignore_errors=True)
        ):
            ...
            entity = {
                ...
                "elevation": elevation,
                "gtopo30": gtopo,
                "timezone": timezone_code.get(infos[17]),
                "cwuri": "http://sws.geonames.org/%s/" % int(infos[0]),
    ...
    def test_autoflush_metadata(self):
        with self.admin_access.repo_cnx() as cnx:
            crs = cnx.system_sql(
                "SELECT * FROM entities WHERE type=%(t)s", {"t": "Location"}
            )
            self.assertEqual(len(crs.fetchall()), 0)
            store = MassiveObjectStore(cnx)
            store.prepare_insert_entity("Location", name="toto")
            store.flush()
            store.commit()
            store.finish()
            cnx.commit()
    ...
    def test_massimport_etype_metadata(self):
        with self.admin_access.repo_cnx() as cnx:
            store = MassiveObjectStore(cnx)
            timezone_eid = store.prepare_insert_entity("TimeZone", code="12")
            store.prepare_insert_entity("Location", timezone=timezone_eid)
            store.flush()
            store.commit()
            store.finish()
            eid, etname = cnx.execute(
                "Any X, TN WHERE X timezone TZ, X is T, " "T name TN"
            )[0]
    ...
    def test_index_not_dropped_by_init(self):
        with self.admin_access.repo_cnx() as cnx:
            ...
            self.assertIn(
                build_index_name("owned_by_relation", ["eid_from", "eid_to"], "key_"),
                indexes,
            )
            self.assertIn(
                build_index_name("owned_by_relation", ["eid_from"], "idx_"), indexes
            )
    ...
    def test_eid_entity(self):
        with self.admin_access.repo_cnx() as cnx:
            store = MassiveObjectStore(cnx, eids_seq_range=1000)
            store.restart_eid_sequence(50000)
            eid = store.prepare_insert_entity("Location", name="toto")
            store.flush()
    ...
    def test_eid_entity_2(self):
        with self.admin_access.repo_cnx() as cnx:
            store = MassiveObjectStore(cnx)
            store.restart_eid_sequence(50000)
            eid = store.prepare_insert_entity("Location", name="toto", eid=10000)
            store.flush()
