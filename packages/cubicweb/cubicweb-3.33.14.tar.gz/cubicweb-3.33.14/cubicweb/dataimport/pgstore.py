...
def _execmany_thread_not_copy_from(
    cu, statement, data, table=None, columns=None, encoding="utf-8"
):
    """Execute thread without copy from"""
    cu.executemany(statement, data)


def _execmany_thread_copy_from(cu, statement, data, table, columns, encoding="utf-8"):
        ...
        for statement, data in statements:
            ...
            try:
                ...
                if dump_output_dir is not None:
                    ...
                    try:
                        with open(
                            osp.join(dump_output_dir, "%s.pickle" % filename), "wb"
                        ) as fobj:
...
def _copyfrom_buffer_convert_datetime(value, **opts):
    ...
    # (http://bugs.python.org/issue1777412)
    return "{} {}".format(
        _copyfrom_buffer_convert_date(value, **opts),
    ...
    for row in data:
        ...
        for col in columns:
            try:
                ...
                warnings.warn(
                    f"Column {col} is not accessible in row {row}", RuntimeWarning
                )
...
class SQLGenSourceWrapper:
    def __init__(self, system_source, schema, dump_output_dir=None):
    ...
    def flush(self):
        ...
        try:
            ...
            _execmany_thread(
                self.system_source.get_connection,
    ...
    def add_relation(self, cnx, subject, rtype, object, inlined=False, **kwargs):
        if inlined:
            ...
            if subjtype is None:
                # Try to infer it
                targets = [t.type for t in self.schema.rschema(rtype).subjects()]
                if len(targets) == 1:
                    subjtype = targets[0]
                else:
                    raise ValueError(
                        "You should give the subject etype for "
                        "inlined relation %s"
                        ", as it cannot be inferred: "
                        "this type is given as keyword argument "
                        "``subjtype``" % rtype
                    )
            statement = self.sqlgen.update(SQL_PREFIX + subjtype, data, ["cw_eid"])
        else:
