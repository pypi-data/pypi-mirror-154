...
class CustomMD5Crypt(uh.HasSalt, uh.GenericHandler):
    ...
    def from_string(cls, hash):
        salt, chk = uh.parse_mc2(hash, "")
        if chk is None:
            raise ValueError("missing checksum")
        return cls(salt=salt, checksum=chk)

    def to_string(self):
        return to_hash_str("{}${}".format(self.salt, self.checksum or ""))

    def _calc_checksum(self, secret):
        return md5crypt(secret, self.salt.encode("ascii")).decode("utf-8")


_CRYPTO_CTX = CryptContext(
    ["sha512_crypt", CustomMD5Crypt, "des_crypt", "ldap_salted_sha1"],
...
def manager_userpasswd(user=None, msg=DEFAULT_MSG, confirm=False, passwdmsg="password"):
    if not user:
        if msg:
            print(msg)
        while not user:
...
def schedule_periodic_task(scheduler, interval, func, *args):
    ...
    def task(*args):
        ...
        try:
            ...
            logger.exception('Unhandled exception in periodic task "%s"', func.__name__)
        except BaseException as exc:
            logger = logging.getLogger("cubicweb.scheduler")
            logger.error(
                'periodic task "%s" not re-scheduled due to %r', func.__name__, exc
            )
            restart = False
        finally:
            if restart:
...
class RepoThread(Thread):
    ...
    def getName(self):
        return "{}({})".format(self._name, Thread.getName(self))
