...
READ_ONLY_RTYPES = {"eid", "has_text", "is", "is_instance_of", "identity"}
            ...
            if isinstance(rhs, Constant):
                ...
                if checkread and eid not in neweids:
                    with cnx.security_enabled(read=False):
                        eschema(cnx.entity_type(eid)).check_perm(cnx, "read", eid=eid)
    ...
    def build_set_plan(self, plan, rqlst):
        ...
        for i, relation in enumerate(rqlst.main_relations):
            if relation.r_type in VIRTUAL_RTYPES:
                raise QueryError("can not assign to %r relation" % relation.r_type)
            lhs, rhs = relation.get_variable_parts()
...
class Step:
    ...
    def test_repr(self):
        """return a representation of this step suitable for test"""
        return self.mytest_repr() + ([step.test_repr() for step in self.children],)
...
class OneFetchStep(Step):
    ...
    def mytest_repr(self):
        ...
        return (
            ...
            sorted(
                (r.as_string(kwargs=self.plan.args), r.solutions)
    ...
    def execute(self):
        ...
        for i, row in enumerate(result):
            ...
            for (lhsinfo, rhsinfo, rschema) in self.updatedefs:
                if rschema.rule:
                    raise QueryError("'%s' is a computed relation" % rschema.type)
