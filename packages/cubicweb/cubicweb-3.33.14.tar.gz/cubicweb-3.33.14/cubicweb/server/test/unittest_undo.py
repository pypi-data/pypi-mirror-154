...
class UndoableTransactionTC(CubicWebTC):
    ...
    def test_undo_creation_1(self):
        ...
        with self.admin_access.repo_cnx() as cnx:
            for eid in (p.eid, c.eid):
                self.assertFalse(
                    cnx.system_sql(
                        "SELECT * FROM entities WHERE eid=%s" % eid
                    ).fetchall()
                )
                self.assertFalse(
                    cnx.system_sql(
                        "SELECT 1 FROM owned_by_relation WHERE eid_from=%s" % eid
                    ).fetchall()
                )
                # added by sql in hooks (except when using dataimport)
                self.assertFalse(
                    cnx.system_sql(
                        "SELECT 1 FROM is_relation WHERE eid_from=%s" % eid
                    ).fetchall()
                )
                self.assertFalse(
                    cnx.system_sql(
                        "SELECT 1 FROM is_instance_of_relation WHERE eid_from=%s" % eid
                    ).fetchall()
                )
            self.check_transaction_deleted(cnx, txuuid)
    ...
    def test_undo_inline_rel_remove_ko(self):
        ...
        with self.admin_access.repo_cnx() as cnx:
            self.assertIsNone(
                cnx.system_sql(
                    "SELECT cw_fiche FROM cw_Personne WHERE cw_eid=%s" % p.eid
                ).fetchall()[0][0]
            )
