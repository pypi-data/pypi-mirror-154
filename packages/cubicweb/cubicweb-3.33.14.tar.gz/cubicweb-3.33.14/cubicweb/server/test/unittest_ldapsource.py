...
def create_slapd_configuration(cls):
    ...
    if cls.slapd_process.poll() is None:
        ...
        raise EnvironmentError(
            'Cannot start slapd with cmdline="%s" (from directory "%s")'
...
def terminate_slapd(cls):
    ...
    if cls.slapd_process and cls.slapd_process.returncode is None:
        ...
        if cls.slapd_process.returncode:
            print(
                "slapd returned with status: %s" % cls.slapd_process.returncode,
                file=sys.stderr,
            )
            sys.stdout.write(stdout)
            sys.stderr.write(stderr)
...
class LDAPFeedTestBase(CubicWebTC):
    ...
    def init_slapd(cls):
        for path in ("/usr/sbin/slapd", "/usr/sbin/slapadd"):
            if not os.path.exists(path):
                raise unittest.SkipTest(
                    f"{path} not found, please install the slapd and "
        ...
        if not os.path.exists(path):
            raise unittest.SkipTest(
                f"{path} not found, please install the ldap-utils debian " f"pacakage"
            )
    ...
    def setup_database(self):
        with self.admin_access.repo_cnx() as cnx:
            ...
            cnx.execute(
                "SET S config %(conf)s, S url %(url)s "
...
class LDAPFeedUserTC(LDAPFeedTestBase):
    ...
    def test_authenticate(self):
        ...
        with self.admin_access.repo_cnx() as cnx:
            ...
            self.assertRaises(
                AuthenticationError, source.authenticate, cnx, "toto", "toto"
            )
            self.assertRaises(
                AuthenticationError, source.authenticate, cnx, "syt", "toto"
            )
    ...
    def test_ldapfeed_insert_collision(self):
        ...
        with self.admin_access.cnx() as cnx:
            ...
            with cnx.security_enabled(write=False):
                user.cw_set(cwuri="http://testing.fr/cubicweb/{}".format(user.eid))
    ...
    def test_base(self):
        with self.admin_access.repo_cnx() as cnx:
            ...
            addresses.sort()
            self.assertEqual(
                ["sylvain.thenault@logilab.fr", "syt@logilab.fr"], addresses
            )
            self.assertIn(
                e.primary_email[0].address,
    ...
    def test_copy_to_system_source(self):
        ...
        with self.admin_access.repo_cnx() as cnx:
            source = self.repo.source_by_uri("ldap")
            eid = cnx.execute("CWUser X WHERE X login %(login)s", {"login": "syt"})[0][
                0
            ]
            cnx.execute(
                'SET X cw_source S WHERE X eid %(x)s, S name "system"', {"x": eid}
            )
            cnx.commit()
            rset = cnx.execute("CWUser X WHERE X login %(login)s", {"login": "syt"})
            ...
            self.assertEqual(len(rset), 1)
            self.assertTrue(
                self.repo.system_source.authenticate(cnx, "syt", password="syt")
            )
            ...
            cu = cnx.system_sql(
                "SELECT cw_upassword FROM cw_cwuser WHERE cw_login='syt';"
            )
            pwd = cu.fetchall()[0][0]
    ...
    def test_bad_config(self):
        with self.admin_access.cnx() as cnx:
            ...
            self.assertIn("badly formatted url", str(cm.exception))
            cnx.rollback()
            ...
            self.assertIn("can only have one url", str(cm.exception))
            cnx.rollback()
...
class LDAPFeedUserDeletionTC(LDAPFeedTestBase):
    ...
    def test_a_filter_inactivate(self):
        ...
        with self.repo.internal_cnx() as cnx:
            ...
            self.assertRaises(
                AuthenticationError, repo_source.authenticate, cnx, "syt", "syt"
            )
        with self.admin_access.repo_cnx() as cnx:
            self.assertEqual(
                cnx.execute(
                    'Any N WHERE U login "syt", ' "U in_state S, S name N"
                ).rows[0][0],
                "deactivated",
            )
            self.assertEqual(
                cnx.execute(
                    'Any N WHERE U login "adim", ' "U in_state S, S name N"
                ).rows[0][0],
                "activated",
            )
    ...
    def test_delete(self):
        ...
        with self.repo.internal_cnx() as cnx:
            ...
            source = self.repo.source_by_uri("ldap")
            self.assertRaises(
                AuthenticationError, source.authenticate, cnx, "syt", "syt"
            )
        with self.admin_access.repo_cnx() as cnx:
            self.assertEqual(
                cnx.execute(
        ...
        # reinsert syt
        self.add_ldap_entry(
            "uid=syt,ou=People,dc=cubicweb,dc=test",
            {
                "objectClass": [
                    "OpenLDAPperson",
                    "posixAccount",
    ...
    def test_reactivate_deleted(self):
        ...
        with self.admin_access.repo_cnx() as cnx:
            ...
            cnx.execute(
                'SET X cw_source S WHERE X eid %(x)s, S name "system"', {"x": user.eid}
            )
            cnx.commit()
            # and that we can now authenticate again
            self.assertRaises(
                AuthenticationError, repo_source.authenticate, cnx, "syt", "toto"
            )
...
class LDAPFeedGroupTC(LDAPFeedTestBase):
    ...
    def test_in_group(self):
        with self.admin_access.repo_cnx() as cnx:
            ...
            self.assertEqual(
                set(["adim"]), set([u.login for u in logilabgroup.reverse_in_group])
            )
    ...
    def test_group_member_added(self):
        ...
        with self.admin_access.repo_cnx() as cnx:
            rset = cnx.execute(
                "Any L WHERE U in_group G, G name %(name)s, U login L",
                {"name": "logilab"},
            )
            self.assertEqual(len(rset), 1)
        ...
        try:
            self.update_ldap_entry(
                "cn=logilab,ou=Group,dc=cubicweb,dc=test",
                {("add", "memberUid"): ["syt"]},
            )
            with self.repo.internal_cnx() as cnx:
                self.pull(cnx)
