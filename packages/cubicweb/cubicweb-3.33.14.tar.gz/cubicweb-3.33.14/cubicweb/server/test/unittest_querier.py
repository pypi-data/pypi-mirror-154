...
class MakeDescriptionTC(unittest.TestCase):
    def test_known_values(self):
        ...
        self.assertEqual(
            _make_description((Function("max", "A"), Variable("B")), {}, solution),
            ["Int", "CWUser"],
        )
...
class UtilsTC(BaseQuerierTC):
    ...
    def test_preprocess_1(self):
        with self.admin_access.cnx() as cnx:
            ...
            rqlst = self._prepare(
                cnx,
                "Any COUNT(RDEF) WHERE RDEF relation_type X, X eid %(x)s",
                {"x": reid},
            )
            self.assertEqual(
                [{"RDEF": "CWAttribute"}, {"RDEF": "CWRelation"}], rqlst.solutions
            )
    ...
    def assertRQLEqual(self, expected, got):
        ...
        self.assertMultiLineEqual(str(parse(expected)), str(parse(got)))

    def test_preprocess_security(self):
        with self.user_groups_session("users") as cnx:
            plan = self._prepare_plan(
                cnx, "Any ETN,COUNT(X) GROUPBY ETN " "WHERE X is ET, ET name ETN"
            )
            union = plan.rqlst
            ...
            self.assertEqual(
                [t.as_string() for t in union.children[0].selection],
                ["ETN", "COUNT(X)"],
            )
            self.assertEqual(
                [t.as_string() for t in union.children[0].groupby], ["ETN"]
            )
            partrqls = sorted(
                (rqlst.as_string(), rqlst.solutions) for rqlst in subq.children
            )
            ...
            self.assertRQLEqual(
                rql,
                "Any ETN,X WHERE X is ET, ET name ETN, ET is CWEType, "
            ...
            self.assertEqual(
                solutions, [{"X": "EmailAddress", "ET": "CWEType", "ETN": "String"}]
            )
            ...
            self.assertEqual(
                solutions, [{"X": "Basket", "ET": "CWEType", "ETN": "String"}]
            )
    ...
    def test_preprocess_security_aggregat(self):
        with self.user_groups_session("users") as cnx:
            ...
            self.assertEqual(len(subq.children), 4)
            self.assertEqual(
                [t.as_string() for t in union.children[0].selection], ["MAX(X)"]
            )

    def test_preprocess_nonregr(self):
        with self.admin_access.cnx() as cnx:
            rqlst = self._prepare(
                cnx, "Any S ORDERBY SI WHERE NOT S ecrit_par O, S para SI"
            )
    ...
    def test_build_description(self):
        ...
        rset = self.qexecute(
            "Any X WHERE X eid %(x)s", {"x": self.admin_access._user.eid}
        )
    ...
    def test_build_descr2(self):
        rset = self.qexecute(
            "Any X,Y WITH X,Y BEING ((Any G,NULL WHERE G is CWGroup) UNION "
    ...
    def test_build_descr3(self):
        rset = self.qexecute(
            "(Any G,NULL WHERE G is CWGroup) UNION " "(Any U,G WHERE U in_group G)"
        )
        for x, y in rset.description:
            if y is not None:
                self.assertEqual(y, "CWGroup")
...
class QuerierTC(BaseQuerierTC):
    ...
    def test_bytes_storage(self):
        feid = self.qexecute(
            'INSERT File X: X data_name "foo.pdf", '
    ...
    def test_select_1(self):
        ...
        self.assertEqual(
            descr, [("CWGroup",), ("CWGroup",), ("CWGroup",), ("CWGroup",)]
        )
    ...
    def test_select_2(self):
        ...
        self.assertEqual(
            rset.description, [("CWGroup",), ("CWGroup",), ("CWGroup",), ("CWGroup",)]
        )
    ...
    def test_select_3(self):
        ...
        result.sort()
        self.assertEqual(
            tuplify(result), [("guests",), ("managers",), ("owners",), ("users",)]
        )
    ...
    def test_select_groupby_colnumber(self):
        rset = self.qexecute(
            "Any YEAR(CD), COUNT(X) GROUPBY 1 " "WHERE X is CWUser, X creation_date CD"
        )
        self.assertListEqual(rset.rows, [[date.today().year, 2]])
    ...
    def test_select_6(self):
        ...
        rset = self.qexecute(
            'DISTINCT Any Y where X name TMP, Y nom in (TMP, "bidule")'
        )
        self.assertIn(("Personne",), rset.description)
    ...
    def test_select_is_in(self):
        ...
        self.assertEqual(
            len(self.qexecute("Any X WHERE X is IN (Personne, Societe)")), 2
        )
    ...
    def test_select_nonregr_inlined(self):
        ...
        rset = self.qexecute(
            "Any U,T ORDERBY T DESC WHERE U is CWUser, " "N ecrit_par U, N type T"
        )  # , {'x': self.ueid})
    ...
    def test_select_nonregr_edition_not(self):
        ...
        groupreadperms = {
            r[0]
            for r in self.qexecute(
                'Any Y WHERE X name "CWGroup", '
                "Y eid IN(2, 3, 4), X read_permission Y"
            )
        }
        rset = self.qexecute(
            'DISTINCT Any Y WHERE X is CWEType, X name "CWGroup", '
            "Y eid IN(2, 3, 4), NOT X read_permission Y"
        )
        self.assertEqual(
            sorted(r[0] for r in rset.rows), sorted(groupeids - groupreadperms)
        )
        rset = self.qexecute(
            'DISTINCT Any Y WHERE X name "CWGroup", '
            "Y eid IN(2, 3, 4), NOT X read_permission Y"
        )
        self.assertEqual(
            sorted(r[0] for r in rset.rows), sorted(groupeids - groupreadperms)
    ...
    def test_select_outer_join_optimized(self):
        ...
        rset = self.qexecute(
            "Any X WHERE X eid %(x)s, X require_permission P?", {"x": peid1}
        )
        self.assertEqual(rset.rows, [[peid1]])
    ...
    def test_select_left_outer_join(self):
        ...
        rset = self.qexecute(
            "DISTINCT Any G WHERE U? in_group G, U eid %(x)s",
    ...
    def test_select_ambigous_outer_join(self):
        ...
        self.qexecute(
            "SET X tags Y WHERE X eid %(t)s, Y eid %(g)s", {"g": geid, "t": teid}
        )
        rset = self.qexecute(
    ...
    def test_select_date_extraction(self):
        self.qexecute(
            "INSERT Personne X: X nom 'foo', X datenaiss %(d)s",
        ...
        for funcname, result in test_data:
            rset = self.qexecute(
                "Any %s(D) WHERE X is Personne, X datenaiss D" % funcname
            )
    ...
    def test_select_custom_aggregat_concat_string(self):
        ...
        self.assertEqual(
            sorted(rset[0][0].split(", ")), ["guests", "managers", "owners", "users"]
        )

    def test_select_custom_regproc_limit_size(self):
        rset = self.qexecute(
            'Any TEXT_LIMIT_SIZE(N, 3) WHERE X is CWGroup, X name N, X name "managers"'
        )
        self.assertTrue(rset)
        self.assertEqual(rset[0][0], "man...")
        self.qexecute(
            "INSERT Basket X: X name 'bidule', X description '<b>hop hop</b>', X description_format 'text/html'"
        )
        rset = self.qexecute(
            "Any LIMIT_SIZE(D, DF, 3) WHERE X is Basket, X description D, X description_format DF"
        )
        self.assertTrue(rset)
    ...
    def test_select_regproc_orderby(self):
        rset = self.qexecute(
            'DISTINCT Any X,N ORDERBY GROUP_SORT_VALUE(N) WHERE X is CWGroup, X name N, X name "managers"'
        )
        self.assertEqual(len(rset), 1)
        self.assertEqual(rset[0][1], "managers")
        rset = self.qexecute(
            'Any X,N ORDERBY GROUP_SORT_VALUE(N) WHERE X is CWGroup, X name N, NOT U in_group X, U login "admin"'
        )
    ...
    def test_select_aggregat_having(self):
        rset = self.qexecute(
            "Any N,COUNT(RDEF) GROUPBY N ORDERBY 2,N "
    ...
    def test_select_having_non_aggregat_1(self):
        rset = self.qexecute(
            "Any L WHERE X login L, X creation_date CD "
            "HAVING YEAR(CD) = %s" % date.today().year
        )
        self.assertListEqual(rset.rows, [["admin"], ["anon"]])

    def test_select_having_non_aggregat_2(self):
        rset = self.qexecute(
            "Any L GROUPBY L WHERE X login L, X in_group G, "
    ...
    def test_select_searchable_text_1(self):
        rset = self.qexecute("INSERT Personne X: X nom 'bidüle'")
        rset = self.qexecute("INSERT Societe X: X nom 'bidüle'")
        rset = self.qexecute("INSERT Societe X: X nom 'chouette'")
        rset = self.qexecute("Any X where X has_text %(text)s", {"text": "bidüle"})
        self.assertEqual(len(rset.rows), 2, rset.rows)
        rset = self.qexecute('Any N where N has_text "bidüle"')
        self.assertEqual(len(rset.rows), 2, rset.rows)
        biduleeids = [r[0] for r in rset.rows]
        rset = self.qexecute('Any N where NOT N has_text "bidüle"')
        self.assertFalse([r[0] for r in rset.rows if r[0] in biduleeids])
        # duh?
        rset = self.qexecute("Any X WHERE X has_text %(text)s", {"text": "ça"})
    ...
    def test_select_multiple_searchable_text(self):
        self.qexecute("INSERT Personne X: X nom 'bidüle'")
    ...
    def test_select_or_relation(self):
        ...
        rset = self.qexecute(
            "DISTINCT Any P WHERE P travaille S1 OR P travaille S2, "
    ...
    def test_select_ordered_distinct_3(self):
        rset = self.qexecute(
            "DISTINCT Any N ORDERBY GROUP_SORT_VALUE(N) WHERE X is CWGroup, X name N"
        )
        self.assertEqual(rset.rows, [["owners"], ["guests"], ["users"], ["managers"]])

    def test_select_or_value(self):
        rset = self.qexecute(
            'Any U WHERE U in_group G, G name "owners" OR G name "users"'
        )
        self.assertEqual(len(rset.rows), 0)
        rset = self.qexecute(
            'Any U WHERE U in_group G, G name "guests" OR G name "managers"'
        )
        self.assertEqual(len(rset.rows), 2)

    def test_select_explicit_eid(self):
        rset = self.qexecute(
            "Any X,E WHERE X owned_by U, X eid E, U eid %(u)s",
    ...
    def test_today_bug(self):
        ...
        rset = self.qexecute(
            "Any D WHERE X name in ('bidule', 'toto') , X creation_date D"
        )
    ...
    def test_sqlite_patch(self):
        ...
        self.qexecute(
            "INSERT Personne X: X nom 'bidule', X datenaiss NOW, X tzdatenaiss NOW"
        )
    ...
    def test_select_boolean(self):
        rset = self.qexecute(
            "Any N WHERE X is CWEType, X name N, X final %(val)s", {"val": True}
        )
        self.assertEqual(
            sorted(r[0] for r in rset.rows),
            [
                "BigInt",
                "Boolean",
                "Bytes",
        ...
        with self.admin_access.cnx() as cnx:
            cnx.create_entity("Personne", nom="louis", test=True)
            self.assertEqual(
                len(cnx.execute("Any X WHERE X test %(val)s", {"val": True})), 1
            )
            self.assertEqual(len(cnx.execute("Any X WHERE X test TRUE")), 1)
            self.assertEqual(
                len(cnx.execute("Any X WHERE X test %(val)s", {"val": False})), 0
            )
    ...
    def test_select_constant(self):
        ...
        self.assertEqual(
            ...
            list(
                zip(
                    ("CWGroup", "CWGroup", "CWGroup", "CWGroup"),
                    (
                        "String",
                        "String",
                        "String",
        ...
        rset = self.qexecute(
            "Any X, %(value)s ORDERBY X WHERE X is CWGroup", {"value": "toto"}
        )
        self.assertEqual(
            rset.rows,
            list(
        ...
        rset = self.qexecute(
            'Any X,GN WHERE X is CWUser, G is CWGroup, X login "syt", '
    ...
    def test_select_union_aggregat(self):
        ...
        # for each sub-query
        self.qexecute(
            "(Any N,COUNT(X) GROUPBY N WHERE X name N, X is State)"
            " UNION "
    ...
    def test_select_union_aggregat_independant_group(self):
        with self.admin_access.cnx() as cnx:
            ...
            cnx.execute('INSERT Transition X: X name "hop"')
            rset = cnx.execute(
                "Any N,NX ORDERBY 2 WITH N,NX BEING "
            ...
            self.assertEqual(rset.rows, [["hop", 2], ["hop", 2]])
    ...
    def test_select_union_description_diff_var(self):
        ...
        rset = self.qexecute(
            "(Any X WHERE X eid %(x)s)" " UNION " "(Any Y WHERE Y eid %(y)s)",
            {"x": eid1, "y": eid2},
        )
    ...
    def test_identity(self):
        ...
        self.assertEqual(eid, 1)
        login = self.qexecute('Any L WHERE X login "admin", X identity Y, Y login L')[
            0
        ][0]
    ...
    def test_select_subquery_aggregat_1(self):
        ...
        rset = self.qexecute(
            "Any GN, COUNT(X)*100/T GROUPBY GN ORDERBY 2,1"
            " WHERE G name GN, X in_group G"
            " WITH T BEING (Any COUNT(U) WHERE U is CWUser)"
        )
        self.assertEqual(rset.rows, [["guests", 50], ["managers", 50], ["users", 100]])
        self.assertEqual(
            rset.description, [("String", "Int"), ("String", "Int"), ("String", "Int")]
        )

    def test_select_subquery_aggregat_2(self):
        expected = self.qexecute(
            "Any X, 0, COUNT(T) GROUPBY X " "WHERE X is Workflow, T transition_of X"
        ).rows
        rset = self.qexecute(
            """
Any P1,B,E WHERE P1 identity P2 WITH
  ...
  P2,E BEING (Any P,COUNT(T) GROUPBY P WHERE P is Workflow, T is Transition,
    ...
    def test_insert_2(self):
        rset = self.qexecute(
            "INSERT Personne X, Personne Y: X nom 'bidule', Y nom 'tutu'"
        )
        self.assertEqual(rset.description, [("Personne", "Personne")])
    ...
    def test_insert_4(self):
        ...
        self.qexecute(
            "INSERT Personne X: X nom 'bidule', X travaille Y WHERE Y nom 'toto'"
        )
        rset = self.qexecute(
            'Any X, Y WHERE X nom "bidule", Y nom "toto", X travaille Y'
        )
        self.assertTrue(rset.rows)
        self.assertEqual(
            rset.description,
    ...
    def test_insert_4bis(self):
        peid = self.qexecute("INSERT Personne X: X nom 'bidule'")[0][0]
        seid = self.qexecute(
            "INSERT Societe Y: Y nom 'toto', X travaille Y WHERE X eid %(x)s",
            {"x": str(peid)},
        )[0][0]
        self.assertEqual(len(self.qexecute("Any X, Y WHERE X travaille Y")), 1)
        self.qexecute(
            "INSERT Personne X: X nom 'chouette', X travaille Y WHERE Y eid %(x)s",
            {"x": str(seid)},
        )
    ...
    def test_insert_5(self):
        self.qexecute("INSERT Personne X: X nom 'bidule'")
        self.qexecute(
            "INSERT Societe Y: Y nom 'toto', X travaille Y WHERE X nom 'bidule'"
        )
        rset = self.qexecute(
            'Any X, Y WHERE X nom "bidule", Y nom "toto", X travaille Y'
        )
    ...
    def test_insert_6(self):
        self.qexecute(
            "INSERT Personne X, Societe Y: X nom 'bidule', Y nom 'toto', X travaille Y"
        )
        rset = self.qexecute(
            'Any X, Y WHERE X nom "bidule", Y nom "toto", X travaille Y'
        )
    ...
    def test_insert_7(self):
        self.qexecute(
            "INSERT Personne X, Societe Y: X nom N, Y nom 'toto', "
            "X travaille Y WHERE U login 'admin', U login N"
        )
        rset = self.qexecute(
            'Any X, Y WHERE X nom "admin", Y nom "toto", X travaille Y'
        )
        self.assertTrue(rset.rows)
        self.assertEqual(
            rset.description,
    ...
    def test_insert_8(self):
        self.qexecute(
            "INSERT Societe Y, Personne X: Y nom N, X nom 'toto', X travaille Y "
            "WHERE U login 'admin', U login N"
        )
        rset = self.qexecute(
            'Any X, Y WHERE X nom "toto", Y nom "admin", X travaille Y'
        )
        self.assertTrue(rset.rows)
        self.assertEqual(
            rset.description,
    ...
    def test_insert_9(self):
        ...
        rset = self.qexecute(
            "INSERT Personne X: X nom N, X travaille Y, X travaille_subdivision Z "
    ...
    def test_insert_query_error(self):
        self.assertRaises(
            Exception, self.qexecute, "INSERT Personne X: X nom 'toto', X is Personne"
        )
        self.assertRaises(
            Exception,
            self.qexecute,
    ...
    def test_insertion_description_with_where(self):
        rset = self.qexecute(
            'INSERT CWUser E, EmailAddress EM: E login "X", E upassword "X", '
    ...
    def test_delete_2(self):
        rset = self.qexecute(
            "INSERT Personne X, Personne Y, Societe Z : "
        ...
        self.assertEqual(
            self.qexecute("Any N WHERE X nom N, X eid %s" % rset[0][0])[0][0], "syt"
        )
    ...
    def test_delete_3(self):
        ...
        self.assertEqual(len(rset.rows), 1)
        self.qexecute(f"DELETE X travaille Y WHERE X eid {peid}, Y eid {seid}")
        rset = self.qexecute("Personne P WHERE P travaille S")
        self.assertEqual(len(rset.rows), 0)
    ...
    def test_delete_symmetric(self):
        ...
        self.qexecute(f"SET X see_also Y WHERE X eid {teid1}, Y eid {teid2}")
        rset = self.qexecute("Any X,Y WHERE X see_also Y")
        self.assertEqual(len(rset), 2, rset.rows)
        self.qexecute(f"DELETE X see_also Y WHERE X eid {teid1}, Y eid {teid2}")
        rset = self.qexecute("Any X,Y WHERE X see_also Y")
        self.assertEqual(len(rset), 0)
        self.qexecute(f"SET X see_also Y WHERE X eid {teid1}, Y eid {teid2}")
        rset = self.qexecute("Any X,Y WHERE X see_also Y")
        self.assertEqual(len(rset), 2)
        self.qexecute(f"DELETE X see_also Y WHERE X eid {teid2}, Y eid {teid1}")
        rset = self.qexecute("Any X,Y WHERE X see_also Y")
        self.assertEqual(len(rset), 0)
    ...
    def test_nonregr_delete_cache(self):
        ...
        (eeid,) = self.qexecute(
            ...
            "X sender Y, X recipients Y WHERE Y is EmailAddress"
        )[0]
    ...
    def test_update_1(self):
        ...
        self.assertEqual(len(rset.rows), 1)
        rset = self.qexecute(
            "SET X nom 'tutu', X prenom 'original' WHERE X is Personne, X nom 'toto'"
        )
    ...
    def test_update_2(self):
        peid, seid = self.qexecute(
            "INSERT Personne X, Societe Y: X nom 'bidule', Y nom 'toto'"
        )[0]
    ...
    def test_update_2bis(self):
        ...
        self.qexecute(
            "SET X travaille Y WHERE X eid %(x)s, Y eid %(y)s",
        ...
        self.assertFalse(
            self.qexecute(
                "SET X travaille Y WHERE X eid %(x)s, Y eid %(y)s," "NOT X travaille Y",
                {"x": str(eid1), "y": str(eid2)},
            )
        )
    ...
    def test_update_multiple2(self):
        with self.admin_access.cnx() as cnx:
            ueid = cnx.execute("INSERT CWUser X: X login 'bob', X upassword 'toto'")[0][
                0
            ]
            peid1 = cnx.execute("INSERT Personne Y: Y nom 'turlu'")[0][0]
            peid2 = cnx.execute("INSERT Personne Y: Y nom 'tutu'")[0][0]
            cnx.execute(
                "SET P1 owned_by U, P2 owned_by U "
                "WHERE P1 eid %s, P2 eid %s, U eid %s" % (peid1, peid2, ueid)
            )
            self.assertTrue(
                cnx.execute(f"Any X WHERE X eid {peid1}, X owned_by U, U eid {ueid}")
            )
            self.assertTrue(
                cnx.execute(f"Any X WHERE X eid {peid2}, X owned_by U, U eid {ueid}")
            )
    ...
    def test_update_math_expr(self):
        orders = [
            r[0]
            for r in self.qexecute(
                'Any O ORDERBY O WHERE ST name "Personne", '
        ...
        for i, v in enumerate(orders):
            if v != orders[0]:
                ...
                break
        self.qexecute(
            'SET X ordernum Y+1 WHERE X from_entity SE, SE name "Personne", '
    ...
    def test_update_string_concat(self):
        ...
        self.qexecute(
            "SET X title XN + %(suffix)s WHERE X is Bookmark, X title XN",
            {"suffix": "-moved"},
        )
        newname = self.qexecute("Any XN WHERE X eid %(x)s, X title XN", {"x": beid})[0][
            0
        ]
    ...
    def test_update_query_error(self):
        ...
        self.assertRaises(
            QueryError,
            self.qexecute,
            "SET X nom 'toto', X has_text 'tutu' " "WHERE X is Personne",
        )
        self.assertRaises(
            QueryError, self.qexecute, "SET X login 'tutu', X eid %s" % cnx.user.eid
        )
    ...
    def test_insert_having(self):
        ...
        self.assertFalse(
            self.qexecute(
                "INSERT Personne Y: Y nom 'hop', Y tel 2 "
    ...
    def test_delete_having(self):
        self.qexecute("INSERT Personne Y: Y nom 'hop', Y tel 1")[0][0]
        self.assertFalse(
            self.qexecute("DELETE Personne Y WHERE X tel XT HAVING XT&2=2")
        )
    ...
    def test_insert_upassword(self):
        rset = self.qexecute(
            "INSERT CWUser X: X login 'bob', X upassword 'toto', "
        ...
        self.assertRaises(
            Unauthorized,
            self.qexecute,
            "Any P WHERE X is CWUser, X login 'bob', X upassword P",
        )
        with self.admin_access.cnx() as cnx:
            cursor = cnx.cnxset.cu
            cursor.execute(
                "SELECT %supassword from %sCWUser WHERE %slogin='bob'"
                % (SQL_PREFIX, SQL_PREFIX, SQL_PREFIX)
            )
            passwd = bytes(cursor.fetchone()[0])
            self.assertEqual(passwd, crypt_password("toto", passwd))
        rset = self.qexecute(
            "Any X WHERE X is CWUser, X login 'bob', X upassword %(pwd)s",
            {"pwd": Binary(passwd)},
        )
        self.assertEqual(len(rset.rows), 1)
        self.assertEqual(rset.description, [("CWUser",)])
    ...
    def test_update_upassword(self):
        with self.admin_access.cnx() as cnx:
            rset = cnx.execute(
                "INSERT CWUser X: X login 'bob', X upassword %(pwd)s", {"pwd": "toto"}
            )
            self.assertEqual(rset.description[0][0], "CWUser")
            rset = cnx.execute(
                "SET X upassword %(pwd)s WHERE X is CWUser, X login 'bob'",
                {"pwd": b"tutu"},
            )
            cursor = cnx.cnxset.cu
            cursor.execute(
    ...
    def test_tz_datetime(self):
        self.qexecute(
            "INSERT Personne X: X nom 'bob', X tzdatenaiss %(date)s",
    ...
    def test_tz_datetime_cache_nonregr(self):
        ...
        self.assertTrue(
            self.qexecute("Any X WHERE X tzdatenaiss %(d)s", {"d": datenaiss})
        )
        self.assertFalse(
            self.qexecute(
                "Any X WHERE X tzdatenaiss %(d)s", {"d": datenaiss - timedelta(1)}
            )
        )
    ...
    def test_nonregr_2(self):
        ...
        rset = self.qexecute("Any X WHERE E eid %(x)s, E tags X", {"x": teid})
    ...
    def test_nonregr_3(self):
        ...
        """
        rset = self.qexecute(
            'Any S,ES,T WHERE S state_of WF, WF workflow_of ET, ET name "CWUser",'
    ...
    def test_nonregr_4(self):
        ...
        rset = self.qexecute(
            'Any ER,SE,OE WHERE SE name "Comment", ER name "comments", OE name "Comment",'
    ...
    def test_nonregr_5(self):
        ...
        neid = self.qexecute(
            "INSERT Note X: X todo_by U, X filed_under T "
            "WHERE U login 'admin', T name 'hop'"
        )[0][0]
        weid = self.qexecute(
            "INSERT Affaire X: X concerne N, X filed_under T "
            "WHERE N is Note, T name 'hip'"
        )[0][0]
        rset1 = self.qexecute(
            "Any N,U WHERE N filed_under T, T eid %s,"
        ...
        rset3 = self.qexecute(
            "Any N,U WHERE N todo_by U, T eid %s,"
    ...
    def test_nonregr_6(self):
        self.qexecute(
            "Any N,COUNT(S) GROUPBY N ORDERBY COUNT(N) WHERE S name N, S is State"
        )
    ...
    def test_sqlite_encoding(self):
        ...
        self.assertEqual(rset.rows, [["\xe9name0"]])
    ...
    def test_nonregr_cache_1(self):
        ...
        self.qexecute("SET X in_basket Y WHERE X is Personne, Y eid %(y)s", {"y": beid})
        rset = self.qexecute("Any X WHERE X in_basket B, B eid %(x)s", {"x": beid})
        self.assertEqual(rset.rows, [[peid]])
        rset = self.qexecute("Any X WHERE X in_basket B, B eid %(x)s", {"x": beid})
        self.assertEqual(rset.rows, [[peid]])
    ...
    def test_nonregr_extra_joins(self):
        ...
        self.qexecute(f"SET X filed_under Y WHERE X eid {neid1}, Y eid {teid1}")
        self.qexecute(f"SET X filed_under Y WHERE X eid {neid2}, Y eid {teid2}")
        self.qexecute("SET X todo_by Y WHERE X is Note, Y eid %s" % ueid)
        rset = self.qexecute(
            "Any N WHERE N todo_by U, N is Note, U eid %s, N filed_under T, T eid %s"
            % (ueid, teid1)
        )
        self.assertEqual(len(rset), 1)
    ...
    def test_nonregr_XXX(self):
        ...
        rset = self.qexecute(
            "Any O WHERE O is State, " "S eid %(x)s, S transition_of ET, O state_of ET",
            {"x": teid},
        )
        self.assertEqual(len(rset), 2)
        rset = self.qexecute(
            "Any O WHERE O is State, NOT S destination_state O, "
            "S eid %(x)s, S transition_of ET, O state_of ET",
            {"x": teid},
        )
        self.assertEqual(len(rset), 1)
    ...
    def test_nonregr_set_datetime(self):
        ...
        self.qexecute(
            "SET X creation_date %(date)s WHERE X eid 1", {"date": date.today()}
        )

    def test_nonregr_u_owned_by_u(self):
        ueid = self.qexecute(
            "INSERT CWUser X: X login 'bob', X upassword 'toto', X in_group G "
            "WHERE G name 'users'"
        )[0][0]
    ...
    def test_nonregr_orderby(self):
        ...
        self.qexecute(
            "Any X,S, MAX(T) GROUPBY X,S ORDERBY S "
            "WHERE X is CWUser, T tags X, S eid IN(%s), X in_state S" % seid
        )

    def test_nonregr_solution_cache(self):
        self.skipTest(
            "XXX should be fixed or documented"
        )  # (doesn't occur if cache key is provided.)
    ...
    def test_nonregr_eid_cmp(self):
        ...
        rset = self.qexecute(
            "Any X,Y WHERE X is Personne, Y is Personne, "
    ...
    def test_nonregr_has_text_ambiguity_1(self):
        peid = self.qexecute(
            "INSERT CWUser X: X login 'bidule', X upassword 'bidule', "
            "X in_group G WHERE G name 'users'"
        )[0][0]
        ...
        self.assertEqual(rset.rows, [[peid]])
        rset = self.qexecute(
            'Any X WHERE X is CWUser, X has_text "bidule", ' "X in_state S, S name SN"
        )
        self.assertEqual(rset.rows, [[peid]])
    ...
    def test_nonregr_sql_cache(self):
        ...
        self.assertFalse(
            self.qexecute("Any X WHERE X is CWEType, X name %(name)s", {"name": None})
        )
        self.assertTrue(
            self.qexecute(
                "Any X WHERE X is CWEType, X name %(name)s", {"name": "CWEType"}
            )
        )
...
class NonRegressionTC(CubicWebTC):
    def test_has_text_security_cache_bug(self):
        with self.admin_access.repo_cnx() as cnx:
            ...
            aff1 = cnx.create_entity("Societe", nom="aff1")
            aff2 = cnx.create_entity("Societe", nom="aff2")
            cnx.commit()
    ...
    def test_set_relations_eid(self):
        with self.admin_access.repo_cnx() as cnx:
            ...
            a1 = cnx.create_entity("EmailAddress", address="a1")
            a2 = cnx.create_entity("EmailAddress", address="a2")
            a3 = cnx.create_entity("EmailAddress", address="a3")
            # SET relations using '>=' operator on eids
            cnx.execute('SET U use_email A WHERE U login "admin", A eid >= %s' % a2.eid)
            self.assertEqual(
                [[a2.eid], [a3.eid]],
                cnx.execute(
                    'Any A ORDERBY A WHERE U use_email A, U login "admin"'
                ).rows,
            )
            # DELETE
            cnx.execute(
                'DELETE U use_email A WHERE U login "admin", A eid > %s' % a2.eid
            )
            self.assertEqual(
                [[a2.eid]],
                cnx.execute(
    ...
    def test_computed_relation_in_write_queries(self):
        ...
        def check(cnx):
            with self.assertRaises(QueryError) as cm:
                yield
            self.assertIn("'user_login' is a computed relation", str(cm.exception))
            cnx.rollback()

        with self.admin_access.cnx() as cnx:
            person = cnx.create_entity("Personne", nom="p")
            cnx.commit()
            # create
            with check(cnx):
                cnx.execute(
                    'INSERT CWUser X: X login "user", X user_login P'
                    ' WHERE P is Personne, P nom "p"'
                )
            # update
            bob = self.create_user(cnx, "bob")
            with check(cnx):
