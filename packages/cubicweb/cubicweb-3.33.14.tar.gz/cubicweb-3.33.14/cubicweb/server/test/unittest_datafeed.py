...
class DataFeedTC(CubicWebTC):
        ...
        with self.admin_access.repo_cnx() as cnx:
            with self.base_parser(cnx):
                cnx.create_entity(
                    ...
                    parser="testparser",
                    url="ignored",
                    config="synchronization-interval=1min",
                )
                cnx.commit()
    ...
    def base_parser(self, session):
        class AParser(datafeed.DataFeedParser):
            ...
            def process(self, url, raise_on_error=False):
                ...
                store.prepare_insert_entity(
                    "Card",
                    cwuri="http://www.cubicweb.org/",
                    title="cubicweb.org",
                    content="the cw web site",
                )
                store.flush()
                store.commit()
        ...
        with self.temporary_appobjects(AParser):
            try:
                source = self.repo.source_by_uri("ô myfeed")
            except ValueError:
                yield
            else:
    ...
    def test(self):
        dfsource = self.repo.source_by_uri("ô myfeed")
        self.assertNotIn("use_cwuri_as_url", dfsource.__dict__)
        self.assertEqual(
            {"type": "datafeed", "uri": "ô myfeed", "use-cwuri-as-url": True},
        ...
        with self.repo.internal_cnx() as cnx:
            with self.base_parser(cnx):
                ...
                self.assertEqual(entity.cw_source[0].name, "ô myfeed")
        ...
        # test_rename_source
        with self.admin_access.repo_cnx() as cnx:
            cnx.entity_from_eid(dfsource.eid).cw_set(name="myrenamedfeed")
            cnx.commit()
            entity = cnx.execute("Card X").get_entity(0, 0)
            self.assertEqual(entity.cwuri, "http://www.cubicweb.org/")
    ...
    def test_update_url(self):
        dfsource = self.repo.source_by_uri("ô myfeed")
        with self.admin_access.repo_cnx() as cnx:
            cnx.entity_from_eid(dfsource.eid).cw_set(
                url="http://pouet.com\nhttp://pouet.org"
            )
            cnx.commit()
        self.assertEqual(dfsource.urls, ["ignored"])
        dfsource = self.repo.source_by_uri("ô myfeed")
        self.assertEqual(dfsource.urls, ["http://pouet.com", "http://pouet.org"])
    ...
    def test_parser_not_found(self):
        ...
        self.assertIn(
            "failed to load parser for source &quot;ô myfeed&quot;", importlog
        )
            ...
            with self.base_parser(cnx):
                with self.assertRaises(ValidationError) as cm:
                    cnx.create_entity(
                        ...
                        parser="testparser",
                        url="ignored",
                        config="synchronization-interval=1s",
                    )
                self.assertIn(
                    "synchronization-interval must be greater than 1 minute",
                    str(cm.exception),
                )
                cnx.rollback()
                ...
                self.assertIn("specifying an URL is mandatory", str(cm.exception))
                cnx.rollback()
                ...
                with self.assertRaises(ValidationError) as cm:
                    cnx.create_entity(
                        ...
                        parser="testparser",
                        url="ignored",
                        config="synch-interval=1min",
                    )
                self.assertIn("unknown options synch-interval", str(cm.exception))
                cnx.rollback()
