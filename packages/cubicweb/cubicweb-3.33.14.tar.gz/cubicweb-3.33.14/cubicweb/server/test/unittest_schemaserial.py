...
class Schema2RQLTC(TestCase):
    ...
    def test_rdef2rql(self):
        self.assertListEqual(
            ...
            list(
                rdef2rql(
                    schema["description_format"].rdefs[("CWRType", "String")],
                    cstrtypemap,
                )
            ),
        )
    ...
    def test_updaterschema2rql2(self):
        ...
        for i, (rql, args) in enumerate(
            updaterschema2rql(schema.rschema("add_permission"), 1)
        ):
        ...
        self.assertListEqual(
            ...
            [
                ...
                for rql, kwargs in erperms2rql(
                    schema.eschema("CWEType"), self.GROUP_MAPPING
                )
            ],
        )
    ...
    def test_rperms2rql2(self):
        self.assertListEqual(
            ...
            [
                (rql, kwargs)
                for rql, kwargs in erperms2rql(
                    schema.rschema("read_permission").rdef("CWEType", "CWGroup"),
                    self.GROUP_MAPPING,
                )
            ],
        )
    ...
    def test_rperms2rql3(self):
        self.assertListEqual(
            ...
            [
                (rql, kwargs)
                for rql, kwargs in erperms2rql(
                    schema.rschema("name").rdef("CWEType", "String"), self.GROUP_MAPPING
                )
            ],
        )
...
class ComputedAttributeAndRelationTC(CubicWebTC):
    ...
    def test(self):
        ...
        self.assertEqual(
            schema["has_employee"].rdef("Company", "Person").permissions["read"],
            ("managers",),
        )
