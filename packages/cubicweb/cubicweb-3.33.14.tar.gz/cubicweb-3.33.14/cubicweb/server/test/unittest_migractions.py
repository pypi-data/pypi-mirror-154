...
from cubicweb import ConfigurationError, ValidationError, ExecutionError, Binary
...
class MigrationCommandsTC(MigrationTC):
    ...
    def test_add_attribute_varchar(self):
        with self.mh() as (cnx, mh):
            ...
            self.assertEqual(
                fields["%sshortpara" % SQL_PREFIX], ("character varying", 11)
            )
            # test default value set on existing entities
    ...
    def test_add_datetime_with_default_value_attribute(self):
        with self.mh() as (cnx, mh):
            ...
            d3 = mh.rqlexec(
                "Any D WHERE X eid %(x)s, X oldstyledefaultdate D", {"x": eid1}
            )[0][0]
            d4 = mh.rqlexec(
    ...
    def test_workflow_actions(self):
        with self.mh() as (cnx, mh):
            ...
            for etype in ("Personne", "Email"):
                s1 = mh.rqlexec(
                    'Any N WHERE WF workflow_of ET, ET name "%s", WF name N' % etype
                )[0][0]
                self.assertEqual(s1, "foo")
                s1 = mh.rqlexec(
                    'Any N WHERE ET default_workflow WF, ET name "%s", WF name N'
                    % etype
                )[0][0]
                self.assertEqual(s1, "foo")
    ...
    def test_add_entity_type(self):
        with self.mh() as (cnx, mh):
            ...
            self.assertEqual(
                sorted(str(rs) for rs in self.schema["Folder2"].subject_relations()),
                [
                    "created_by",
                    "creation_date",
                    "cw_source",
            ...
            self.assertCountEqual(
                [str(rs) for rs in self.schema["Folder2"].object_relations()],
    ...
    def test_add_relation_definition_nortype(self):
        with self.mh() as (cnx, mh):
            ...
            self.assertEqual(
                self.schema["concerne2"].rdef("Personne", "Affaire").cardinality, "1*"
            )
    ...
    def test_sync_schema_props_perms(self):
        with self.mh() as (cnx, mh):
            ...
            self.assertEqual(
                [rdef.rtype.name for rdef in rexpr.reverse_add_permission],
                ["ecrit_par"],
            )
            ...
            self.assertCountEqual(
                self.schema.eschema("Personne")._unique_together[0],
    ...
    def test_add_drop_cube_and_deps(self):
        with self.mh() as (cnx, mh):
            ...
            self.assertEqual(
                sorted((str(s), str(o)) for s, o in schema["see_also"].rdefs),
                sorted(
                    [
    ...
    def test_change_attribute_type(self):
        with self.mh() as (cnx, mh):
            ...
            self.assertEqual(
                self.schema["tel"].rdefs[("Societe", "Float")].object, "Float"
            )
...
class MigrationCommandsComputedTC(MigrationTC):
    ...
    def assert_score_initialized(self, mh):
        self.assertEqual(
            self.schema["score"].rdefs["Company", "Float"].formula,
