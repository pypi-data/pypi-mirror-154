...
def create_user(session, login, pwd, *groups):
    ...
    for group in groups:
        session.execute(
            "SET U in_group G WHERE U eid %(u)s, G name %(group)s",
...
def init_repository(config, interactive=True, drop=False, vreg=None, init_config=None):
    ...
    from cubicweb.server.sqlutils import (
        _SQL_DROP_ALL_USER_TABLES_FILTER_FUNCTION as drop_filter,
    )
    ...
    with repo.internal_cnx() as cnx:
        ...
        handler = config.migration_handler(
            schema, interactive=False, repo=repo, cnx=cnx
        )
    ...
    with repo.internal_cnx() as cnx:
        ...
        ssource = cnx.create_entity("CWSource", type="native", name="system")
    ...
    with connect(repo, login, password=pwd) as cnx:
        with cnx.security_enabled(False, False):
            ...
            handler = config.migration_handler(
                schema, interactive=False, cnx=cnx, repo=repo
            )
...
SOURCE_TYPES = {
    "native": LazyObject("cubicweb.server.sources.native", "NativeSQLSource"),
