...
class ServerMigrationHelper(MigrationHelper):
    ...
    def cube_upgraded(self, cube, version):
        self.cmd_set_property("system.version.%s" % cube.lower(), str(version))
        self.commit()
    ...
    def backup_database(self, backupfile=None, askconfirm=True, format="native"):
        ...
        backupfile = backupfile or osp.join(
            instbkdir, "%s-%s.tar.gz" % (config.appid, timestamp)
        )
        # check backup has to be done
        if osp.exists(backupfile) and not self.confirm(
            "Backup file %s exists, overwrite it?" % backupfile
        ):
    ...
    def restore_database(self, backupfile, drop=True, askconfirm=True, format="native"):
        ...
        if askconfirm and not self.confirm(
            "Restore %s database from %s ?" % (self.config.appid, backupfile)
        ):
            return
        # unpack backup
        tmpdir = tempfile.mkdtemp()
    ...
    def cmd_install_custom_sql_scripts(self, cube=None):
        ...
        for fpath in sql_scripts:
            ...
            failed = sqlexec(
                open(fpath).read(), self.cnx.system_sql, False, delimiter=";;"
            )
            if failed:
    ...
    def _synchronize_permissions(self, erschema, teid):
        ...
        for action in erschema.ACTIONS:
            ...
            for geid, gname in self.rqlexec(
                ...
                if gname not in newgroups:
                    if not confirm or self.confirm(
                        "Remove %s permission of %s to %s?" % (action, erschema, gname)
                    ):
                        self.rqlexec(
                            "DELETE T %s G WHERE G eid %%(x)s, T eid %s" % (perm, teid),
                            {"x": geid},
                            ask_confirm=False,
                        )
                else:
            ...
            for gname in newgroups:
                if not confirm or self.confirm(
                    "Grant %s permission of %s to %s?" % (action, erschema, gname)
                ):
                    try:
                        self.rqlexec(
                            "SET T %s G WHERE G eid %%(x)s, T eid %s" % (perm, teid),
                            {"x": gm[gname]},
                            ask_confirm=False,
                        )
                    except KeyError:
            ...
            for expreid, expression in self.rqlexec(
                "Any E, EX WHERE T %s E, E expression EX, " "T eid %s" % (perm, teid),
                ask_confirm=False,
            ):
                if expression not in newexprs:
                    if not confirm or self.confirm(
                        ...
                        self.rqlexec(
                            "DELETE T %s E WHERE E eid %%(x)s, T eid %s" % (perm, teid),
                            {"x": expreid},
                            ask_confirm=False,
                        )
                else:
            ...
            for expression in newexprs.values():
                ...
                if not confirm or self.confirm(
                    "Add %s expression for %s permission of %s?"
    ...
    def cmd_add_cubes(self, cubes, update_database=True):
        ...
        for rschema in newcubes_schema.relations():
            ...
            for (fromtype, totype) in rschema.rdefs:
                ...
                # don't add it
                if (fromtype, totype) in existingschema.rdefs or rschema.rdefs[
                    (fromtype, totype)
                ].infered:
                    continue
            ...
            if rschema in removedcubes_schema and rschema in reposchema:
                ...
                for fromtype, totype in rschema.rdefs:
                    if (fromtype, totype) not in removedcubes_schema[
                        rschema.type
                    ].rdefs and (fromtype, totype) in reposchema[rschema.type].rdefs:
    ...
    def cmd_drop_attribute(self, etype, attrname, commit=True):
        ...
        try:
            ...
            print(
                "warning: attribute %s %s is not known, skip deletion"
                % (etype, attrname)
            )
        else:
                ...
                for rschema, tschemas, role in spschema.relation_definitions(True):
                    for tschema in tschemas:
                        ...
                        if role == "subject":
                            ...
                            subjschema = tschema
                            objschema = spschema
                        if rschema.rdef(subjschema, objschema).infered or (
                            instschema.has_relation(rschema)
                            and (subjschema, objschema) in instschema[rschema].rdefs
                        ):
                            continue
                        self.cmd_add_relation_definition(
    ...
    def cmd_add_relation_type(self, rtype, addrdef=True, commit=True):
        ...
        if rtype in reposchema:
            print("warning: relation type %s is already known, skip addition" % (rtype))
        elif rschema.rule:
    ...
    def cmd_drop_relation_type(self, rtype, commit=True):
        ...
        """
        self.rqlexec(
            "DELETE CWRType X WHERE X name %r" % rtype, ask_confirm=self.verbosity >= 2
        )
        self.rqlexec(
            "DELETE CWComputedRType X WHERE X name %r" % rtype,
            ask_confirm=self.verbosity >= 2,
        )
        if commit:
            self.commit()
    ...
    def cmd_rename_relation_type(self, oldname, newname, commit=True, force=False):
        ...
        if not self.repo.schema[oldname].rule:
            self.rqlexec(
                "SET X %s Y WHERE X %s Y" % (newname, oldname),
                ask_confirm=self.verbosity >= 2,
            )
    ...
    def cmd_add_relation_definition(self, subjtype, rtype, objtype, commit=True):
        ...
        if rschema.rule:
            raise ExecutionError(
                "Cannot add a relation definition for a "
                "computed relation (%s)" % rschema
            )
    ...
    def cmd_drop_relation_definition(self, subjtype, rtype, objtype, commit=True):
        ...
        if rschema.rule:
            raise ExecutionError(
                "Cannot drop a relation definition for a "
                "computed relation (%s)" % rschema
            )
    ...
    def cmd_change_attribute_type(self, etype, attr, newtype, commit=True):
        ...
        * the actual schema won't be updated until next startup
        ...
        dbhelper.change_col_type(
            cursor, "cw_%s" % etype, "cw_%s" % attr, sqltype, allownull
        )
        if commit:
            self.commit()
    ...
    def cmd_add_entity_type_table(self, etype, commit=True):
        ...
        for sql in eschema2sql(
            dbhelper, self.repo.schema.eschema(etype), prefix=SQL_PREFIX
        ):
            self.sqlexec(sql)
        if commit:
            self.commit()
