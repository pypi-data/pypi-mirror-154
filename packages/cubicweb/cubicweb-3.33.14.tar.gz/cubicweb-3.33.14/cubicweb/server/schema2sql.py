...
from yams.constraints import SizeConstraint, UniqueConstraint, Attribute, NOW, TODAY
...
def sql_create_index(self, table, column, unique=False):
    ...
    if unique:
        return "ALTER TABLE {} ADD CONSTRAINT {} UNIQUE({})".format(table, idx, column)
    else:
        return "CREATE INDEX {} ON {}({})".format(idx, table, column)
...
def build_index_name(table, columns, prefix="idx_"):
    ...
    return "{}{}".format(
        prefix,
        md5((table + "," + ",".join(sorted(columns))).encode("ascii")).hexdigest(),
    )
...
def rschema_has_table(rschema, skip_relations):
    ...
    return not (
        ...
        or rschema.inlined
        or rschema.rule
        or rschema.type in skip_relations
    )
...
def schema2sql(dbhelper, schema, skip_entities=(), skip_relations=(), prefix=""):
    ...
    for etype in sorted(schema.entities()):
        ...
        if eschema.final or eschema.type in skip_entities:
            continue
        yield from eschema2sql(dbhelper, eschema, skip_relations, prefix)
    for rtype in sorted(schema.relations()):
        rschema = schema.rschema(rtype)
        if rschema_has_table(rschema, skip_relations):
            yield from rschema2sql(rschema)
...
def eschema_sql_def(dbhelper, eschema, skip_relations=(), prefix=""):
    ...
    attrs = [
        attrdef
        for attrdef in eschema.attribute_definitions()
    ...
    for i in range(len(attrs)):
        ...
        if attrschema is not None:
            ...
            sqltype = aschema2sql(
                dbhelper, eschema, rschema, attrschema, creating=False
            )
        else:  # inline relation
            sqltype = "integer"
        result.append(("{}{}".format(prefix, rschema.type), sqltype))
    return result
...
def eschema2sql(dbhelper, eschema, skip_relations=(), prefix=""):
    ...
    for i in range(len(attrs)):
        ...
        if i == len(attrs) - 1:
            w(" {}{} {}".format(prefix, rschema.type, sqltype))
        else:
            w(" {}{} {},".format(prefix, rschema.type, sqltype))
    for rschema, aschema in attrs:
        ...
        for constraint in rdef.constraints:
            cstrname, check = check_constraint(
                rdef, constraint, dbhelper, prefix=prefix
            )
            if cstrname is not None:
                w(", CONSTRAINT {} CHECK({})".format(cstrname, check))
    w(")")
    yield "\n".join(output)
    # create indexes
    for i in range(len(attrs)):
    ...
    for attrs, index_name in iter_unique_index_names(eschema):
        columns = ["{}{}".format(prefix, attr) for attr in attrs]
...
def check_constraint(rdef, constraint, dbhelper, prefix=""):
    ...
    if constraint.type() == "BoundaryConstraint":
        ...
        return cstrname, "{}{} {} {}".format(prefix, attr, constraint.operator, value)
        ...
        if constraint.minvalue is not None:
            ...
            condition.append("{}{} >= {}".format(prefix, attr, value))
        ...
        if not isinstance(sample, str):
            ...
            # XXX better quoting?
            values = ", ".join(
                "'%s'" % word.replace("'", "''") for word in constraint.vocabulary()
            )
        return cstrname, "{}{} IN ({})".format(prefix, attr, values)
    return None, None
...
def grant_schema(schema, user, set_owner=True, skip_entities=(), prefix=""):
    ...
    for etype in sorted(schema.entities()):
        ...
        if eschema.final or etype in skip_entities:
            continue
        yield from grant_eschema(eschema, user, set_owner, prefix=prefix)
    for rtype in sorted(schema.relations()):
        rschema = schema.rschema(rtype)
        if rschema_has_table(
            rschema, skip_relations=()
        ):  # XXX skip_relations should be specified
            yield from grant_rschema(rschema, user, set_owner)
...
def grant_eschema(eschema, user, set_owner=True, prefix=""):
    ...
    etype = eschema.type
    if set_owner:
        yield "ALTER TABLE {}{} OWNER TO {}".format(prefix, etype, user)
    yield "GRANT ALL ON {}{} TO {}".format(prefix, etype, user)
...
def grant_rschema(rschema, user, set_owner=True):
    ...
    """
    if set_owner:
        yield "ALTER TABLE {}_relation OWNER TO {}".format(rschema.type, user)
    yield "GRANT ALL ON {}_relation TO {}".format(rschema.type, user)
