...
def sql_or_clauses(sql, clauses):
    ...
    if restrclauses:
        restr = "{} AND ({})".format(" AND ".join(restrclauses), " OR ".join(clauses))
    else:
        restr = "(%s)" % " OR ".join(clauses)
    return "{} WHERE {}".format(select, restr)
...
def rdef_table_column(rdef):
    ...
    the database
    """
    return (SQL_PREFIX + str(rdef.subject), SQL_PREFIX + str(rdef.rtype))
...
def _undo_check_relation_target(tentity, rdef, role):
    ...
    if card in "?1" and tentity.related(rdef.rtype, role):
        ...
        raise _UndoException(
            msg % {"role": neg_role(role), "rtype": rdef.rtype, "eid": tentity.eid}
        )
...
class NativeSQLSource(SQLAdapterMixIn, AbstractSource):
    ...
    options = (
        ...
        (
            ...
            {
                "type": "string",
                "default": CubicWebNoAppConfiguration.mode == "user"
                and getlogin()
                or "cubicweb",
                "help": "database user",
                "group": "native-source",
        ...
        if eids is None:
            ...
            for eid, etype in zip(eids, etypes):
                ...
                if etype is not None:
                    cache.pop("Any X WHERE X eid {}, X is {}".format(eid, etype), None)
    ...
    def map_attribute(self, etype, attr, cb, sourcedb=True):
        self._rql_sqlgen.attr_map["{}.{}".format(etype, attr)] = (cb, sourcedb)

    def unmap_attribute(self, etype, attr):
        self._rql_sqlgen.attr_map.pop("{}.{}".format(etype, attr), None)
    ...
    def storage(self, etype, attr):
        ...
        try:
            ...
            raise Exception("no custom storage set for {}.{}".format(etype, attr))
    ...
    def _delete_relation(self, cnx, subject, rtype, object, inlined=False):
        ...
        if inlined:
            ...
            sql = "UPDATE {} SET {}=NULL WHERE {}eid=%(eid)s".format(
                table,
                column,
                SQL_PREFIX,
            )
    ...
    def undo_transaction(self, cnx, txuuid):
        ...
        self.doexec(cnx, "DELETE FROM transactions WHERE tx_uuid='%s'" % txuuid)
        if errors:
    ...
    def _undo_d(self, cnx, action):
        ...
        try:
            ...
            err(
                "can't restore entity %s of type %s, type no more supported"
                % (eid, etype)
            )
            return errors
...
def grant_schema(user, set_owner=True):
    ...
    for table in (
        ...
        if set_owner:
            yield "ALTER TABLE {} OWNER TO {};".format(table, user)
        yield "GRANT ALL ON {} TO {};".format(table, user)
    ...
    * a list of rows (as tuples with one element per column)
    ...
    def get_tables(self):
        ...
        for etype in self.schema.entities():
            ...
            if eschema.final:
                continue
            etype_tables.append("{}{}".format(prefix, etype))
    ...
    def write_metadata(self, archive):
        ...
        versions_str = "\n".join("{} {}".format(k, v) for k, v in versions)
    ...
    def read_metadata(self, archive, backupfile):
        ...
        if formatinfo.strip() != "1.1":
            ...
            raise ValueError("Unknown format in {}: {}".format(backupfile, formatinfo))
        ...
        if archive_versions != db_versions:
            ...
            if new_cubes:
                self.logger.critical(
                    "In the db:\n%s",
                    "\n".join(
                        "{}: {}".format(cube, ver) for cube, ver in sorted(new_cubes)
                    ),
                )
            ...
            if old_cubes:
                self.logger.critical(
                    "In the archive:\n%s",
                    "\n".join(
                        "{}: {}".format(cube, ver) for cube, ver in sorted(old_cubes)
                    ),
                )
