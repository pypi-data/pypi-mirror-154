...
def rewrite_unstable_outer_join(select, solutions, unstable, schema):
    ...
    for varname in tuple(unstable):
        ...
        # full sub-query
        aliases = [
            VariableRef(select.get_variable(avar.name, i))
...
def sort_term_selection(sorts, rqlst, groups):
    """
    Add missing terms in selection if they are not already in select.
    :param sorts: a list of SortTerm instances
    :param rqlst: a Select object
    :param groups: if we need to group variables
    """
    # XXX beurk
    if isinstance(rqlst, list):
        ...
        selectionidx = {str(term) for term in rqlst}
    else:

        def append(term):
            rqlst.selection.append(term.copy(rqlst))

        selectionidx = {str(term) for term in rqlst.selection}

    for sortterm in sorts:
        term = sortterm.term
        if (
            not isinstance(term, Constant)
            and not str(term) in selectionidx
            and not is_aggregat(term)
        ):
...
def is_aggregat(node):
    return isinstance(node, Function) and node.descr().aggregat
...
def fix_selection_and_group(rqlst, needwrap, selectsortterms, sorts, groups, having):
    ...
    if sorts and groups:
        ...
        for sortterm in sorts:
            ...
            for vref in term.iget_nodes(VariableRef):
                if not (
                    any(vref.is_equivalent(group) for group in groupvrefs)
...
def iter_mapped_var_sels(stmt, variable):
    ...
    # callback
    if not (
        len(variable.stinfo["rhsrelations"]) <= 1
        and variable.stinfo["selected"]  # < 1 on column alias
    ):
        raise QueryError("can't use %s as a restriction variable" % variable.name)
...
def update_source_cb_stack(state, stmt, node, stack):
    while True:
        ...
        if funcd.source_execute is None:
            raise QueryError("%s can not be called on mapped attribute" % node.name)
...
class StateInfo:
    ...
    * `outer_pending` is a dictionary containing some conditions that will have
    ...
    def alias_and_add_table(self, tablename, scope=-1):
        alias = f"{tablename}{self.count}"
        self.count += 1
        self.add_table(f"{tablename} AS {alias}", alias, scope)
        return alias
    ...
    def relation_table(self, relation):
        ...
        rid = f"rel_{relation.r_type}{self.count}"
        ...
        for varref in relation.iget_nodes(VariableRef):
            ...
            scope = -1
        self.count += 1
        self.add_table(f"{relation.r_type}_relation AS {rid}", rid, scope=scope)
    ...
    def replace_tables_by_outer_join(self, leftalias, rightalias, outertype, condition):
        ...
        louter, lconditions, lchain = outer_tables.get(leftalias, (None, None, None))
        router, rconditions, rchain = outer_tables.get(rightalias, (None, None, None))
        if lchain is None and rchain is None:
            ...
            if (
                (outertype == "FULL" and router != "FULL")
...
class SQLGenerator:
    ...
    def union_sql(self, union, needalias=False):  # pylint: disable=E0202
        ...
        sqls = (
            "(%s)" % self.select_sql(select, needalias) for select in union.children
        )
        return "\nUNION ALL\n".join(sqls)
    ...
    def noparen_union_sql(self, union, needalias=False):
        ...
        sqls = (
            self.select_sql(select, needalias)
    ...
    def select_sql(self, select, needalias=False):
        ...
        fix_selection_and_group(
            select, needwrap, selectsortterms, sorts, groups, having
        )
        if needwrap:
        ...
        # limit / offset
        sql = self.dbhelper.sql_add_limit_offset(
            sql, select.limit, select.offset, sqlsortterms
        )
        return sql
    ...
    def _selection_sql(self, selected, distinct, needaliasing=False):
        ...
        for term in selected:
            ...
            if needaliasing:
                ...
                clause.append(f"{sql} AS {colalias}")
    ...
    def _visit_exists(self, exists):
        ...
        if not tables:
            ...
            sql = f"SELECT 1 FROM {tables} WHERE {restriction}"
        return sql
    ...
    def visit_relation(self, relation):
        ...
        if rschema.final:
            ...
            rtype == "is"
            and isinstance(rhs.children[0], Constant)
            and rhs.children[0].eval(self._args) is None
        ):
            lhssql = lhs.accept(self)
            return "{}{}".format(lhssql, rhs.accept(self))
    ...
    def _visit_inlined_relation(self, relation):
        ...
        if isinstance(relation.parent, Not):
            ...
            if rhsvar is not None and rhsvar._q_invariant:
                ...
                sql = "({} IS NULL OR {}!={})".format(
                    lhssql,
                    lhssql,
                    (rhsvar or rhsconst).accept(self),
                )
        elif rhsconst is not None:
            sql = "{}={}".format(lhssql, rhsconst.accept(self))
            ...
            sql = "{}={}".format(lhssql, rhsvar.accept(self))
        if moresql is None:
            return sql
        return f"{sql} AND {moresql}"
    ...
    def _process_relation_term(self, relation, rid, termvar, termconst, relfield):
        if termconst or not termvar._q_invariant:
            ...
            yield f"{rid}.{relfield}={termsql}"
        elif termvar._q_invariant:
            extrajoin = self._extra_join_sql(relation, f"{rid}.{relfield}", termvar)
    ...
    def _visit_relation(self, relation, rschema):
        ...
        if relation.r_type == "identity":
            ...
            lhs, rhs = relation.get_parts()
            return "{}{}".format(lhs.accept(self), rhs.accept(self))
        ...
        if rightconst is None:
            ...
            if rightalias is None:
                if rightvar.stinfo["principal"] is not relation:
                    self._state.replace_tables_by_outer_join(
                        leftalias,
                        rightvar.stinfo["principal"]._q_sqltable,
                        outertype,
                        "{}.{}={}".format(leftalias, restrattr, rightvar.accept(self)),
                    )
            else:
                self._state.replace_tables_by_outer_join(
                    leftalias,
    ...
    def _visit_outer_join_inlined_relation(self, relation, rschema):
        ...
        if lhsalias is None:
            ...
            lhssql = f"{lhsalias}.{SQL_PREFIX}{attr}"
        condition = "{}={}".format(lhssql, (rhsconst or rhsvar).accept(self))
        ...
        if rhsalias is None:
            if rhsconst is not None:
                ...
                if relation.r_type != "identity":
                    condition = f"({condition} OR {lhssql} IS NULL)"
            ...
            if ored or (
                ...
                if principal is relation:
                    ...
                    _rel = [
                        ...
                        if rel is not principal
                    ][0]
                else:
                    _rel = relation
                lhssql = self._inlined_var_sql(_rel.children[0].variable, _rel.r_type)
    ...
    def _visit_attribute_relation(self, rel):
        ...
        if isinstance(lhs.variable, ColumnAlias):
            if rel.r_type != "eid":
                raise BadRQLQuery(
                    "Attribute %s of %s must be selected from subqueries"
            ...
            if table is None:
                ...
                mapkey = "{}.{}".format(self._state.solution[lhs.name], rel.r_type)
                if mapkey in self.attr_map:
                    ...
                    lhssql = f"{table}.{SQL_PREFIX}{rel.r_type}"
        try:
            if rel._q_needcast == "TODAY":
                sql = f"DATE({lhssql}){rhssql}"
    ...
    def _visit_has_text_relation(self, rel):
        ...
        if me_is_principal:
            ...
            if lhsvar.stinfo["typerel"] is not None:
                if not lhsvar._q_invariant or len(lhsvar.stinfo["possibletypes"]) == 1:
                    ...
                    self._state.add_table(
                        "cw_{} AS {}".format(self._state.solution[lhs.name], ealias),
                        ealias,
                    )
                else:
                    subquery = " UNION ".join(
                        "SELECT cw_eid FROM cw_%s" % etype
                        for etype in sorted(lhsvar.stinfo["possibletypes"])
                    )
                    restriction = f" AND {lhsvar._q_sql} IN ({subquery})"
        ...
        return (
            self.dbhelper.fti_restriction_sql(alias, query, jointo, not_) + restriction
        )
    ...
    def visit_comparison(self, cmp):
        ...
        if sql is None:
            if lhs is None:
                sql = "{}{}".format(operator, rhs.accept(self))
            else:
                sql = "{}{}{}".format(lhs.accept(self), operator, rhs.accept(self))
        if optional is None:
            return sql
        ...
        if optional == "right":
            self._state.replace_tables_by_outer_join(leftalias, rightalias, "LEFT", sql)
        elif optional == "left":
            self._state.replace_tables_by_outer_join(rightalias, leftalias, "LEFT", sql)
        else:
            self._state.replace_tables_by_outer_join(leftalias, rightalias, "FULL", sql)
        return ""
    ...
    def visit_mathexpression(self, mexpr):
        ...
        try:
            if (
                ...
                return "(%s)" % self.dbhelper.sql_concat_string(
                    lhs.accept(self), rhs.accept(self)
                )
        ...
        try:
            ...
            if (
                ...
                and operator in "+-"
                and lhs_type in ("date", "datetime", "tzdatetime", "tzdate")
                and rhs_type == "int"
            ):
        ...
        return "({} {} {})".format(lhs.accept(self), operator, rhs.accept(self))

    def visit_unaryexpression(self, uexpr):
        """generate SQL for a unary expression"""
        return "{}{}".format(uexpr.operator, uexpr.children[0].accept(self))
    ...
    def visit_function(self, func):
        ...
        if func.name == "FTIRANK":
            try:
                ...
                raise BadRQLQuery(
                    "can't use FTIRANK on variable not used in an"
    ...
    def visit_variable(self, variable):
        ...
        if variable.stinfo["attrvar"]:
            ...
            if principal is None:
                ...
                sql = "{}.{}".format(
                    self._state.fti_table(principal, self.dbhelper.fti_table),
    ...
    def _var_info(self, var):
        ...
        sql = f"{tablealias}.{SQL_PREFIX}eid"
        self._state.add_table(
            f"{SQL_PREFIX}{etype} AS {tablealias}", tablealias, scope=scope
        )
    ...
    def _inlined_var_sql(self, var, rtype):
        ...
        if rtype == "eid":
            ...
            sql = "{}.{}{}".format(self._var_table(var), SQL_PREFIX, rtype)
        return sql

    def _linked_var_sql(self, variable):
        rel = variable.stinfo.get("principal") or next(
            iter(variable.stinfo["rhsrelations"])
        )
        ...
        if isinstance(linkedvar, ColumnAlias):
            raise BadRQLQuery(
                "variable %s should be selected by the subquery" % variable.name
            )
        mapkey = "{}.{}".format(self._state.solution[linkedvar.name], rel.r_type)
        if mapkey in self.attr_map:
            cb, sourcecb = self.attr_map[mapkey]
            if not sourcecb:
        ...
        return f"{linkedvar._q_sqltable}.{SQL_PREFIX}{rel.r_type}"
