...
def dbg_st_search(uri, union, args, cachekey=None, prefix="rql for"):
    if server.DEBUG & server.DBG_RQL:
        ...
        print(
            ...
            uri,
            "source:",
            highlight_terminal(repr(union.as_string())[1:-1], "RQL"),
        )
        t = time()
...
class AbstractSource(object):
    ...
    def __init__(self, repo, source_config, eid=None):
        ...
        self.config = self._check_config_dict(eid, source_config, raise_on_error=False)

    def __repr__(self):
        return "<%s %s source %s @%#x>" % (
            self.uri,
        ...
        for optname, optdict in self.options:
            ...
            if value is configuration.REQUIRED:
                ...
                raise ValidationError(
                    eid, {role_name("config", "subject"): msg}, msgargs
                )
    ...
    def syntax_tree_search(self, cnx, union, args=None, cachekey=None, debug=0):
