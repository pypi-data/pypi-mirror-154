...
def prefill_entity_caches(entity):
    ...
    for rschema in entity.e_schema.subject_relations():
        ...
        if rschema.final:
            ...
            entity.cw_set_relation_cache(rtype, "subject", cnx.empty_rset())
...
def del_existing_rel_if_needed(cnx, eidfrom, rtype, eidto):
    ...
    if card[1] in "1?":
        with cnx.security_enabled(read=False):
            cnx.execute(
                "DELETE X %s Y WHERE Y eid %%(y)s, " "NOT X eid %%(x)s" % rtype,
                {"x": eidfrom, "y": eidto},
            )
    ...
    for attr in entity.cw_edited:
        ...
        if not rschema.final:  # inlined relation
            ...
            if rdef.cardinality[1] in "1?" and activeintegrity:
                with cnx.security_enabled(read=False):
                    cnx.execute(
                        "DELETE X %s Y WHERE Y eid %%(y)s" % attr,
...
class LifoDeque(queue.LifoQueue):
    def get_oldest(self, block=True, timeout=None):
        ...
        with self.not_empty:
            ...
            item = self.queue.pop(
                0
            )  # <- that's the only code difference from super().get
...
class _CnxSetPool(_BaseCnxSet):
    ...
    def _close_idle_cnxset(self):
        ...
        """
        if (
            abs(time.time() - self.idle) > self.idle_timeout
            and self.size() > self.min_size
        ):
            try:
    ...
    def get(self):
        ...
        """
        self.debug(
            f"[pool] try to get a connection, free queue size is "
        ...
        try:
            ...
            if self.max_size and self.size() >= self.max_size:
                try:
                    ...
                    raise Exception(
                        "no connections set available after 5 secs, probably either a "
                ...
                self.debug(
                    "[pool] we had to open a new connection, connection number: %d/%d"
    ...
    def release(self, cnxset):
        ...
        self.debug(
            f"[pool] release {cnxset} [{id(cnxset)}], free queue size is "
            f"{self.qsize()}/{self.size()} (max {self.max_size})"
        )
        self._close_idle_cnxset()
...
def get_cnxset(config, source, bootstrap=False):
    ...
    return _CnxSetPool(
        source, min_size=min_size, max_size=max_size, idle_timeout=idle_timeout
    )
...
class Repository(object):
    ...
    def __init__(self, config, scheduler=None, vreg=None):
        ...
        self.system_source = self.get_source(
        ...
        if config.quick_start or config.creating or not config.read_instance_schema:
            ...
            if not config.creating:
                ...
                with self.internal_cnx() as cnx:
                    for etype, eid in cnx.execute(
                        "Any XN,X WHERE X is CWEType, X name XN"
                    ):
                        try:
                            ...
                            self.warning(
                                "No %s entity type in the file system schema", etype
                            )
        else:
    ...
    def sources_by_uri(self):
        ...
        mapping.update(
            (sourceent.name, source) for sourceent, source in self._sources()
        )
        return mapping
    ...
    def _sources(self):
        ...
        with self.internal_cnx() as cnx:
            for sourceent in cnx.execute(
                ...
                'S name SN, S type SA, S config SC, S name != "system"'
            ).entities():
    ...
    def _source_from_cwsource(self, sourceent):
        source = self.get_source(
            sourceent.type, sourceent.name, sourceent.host_config, sourceent.eid
        )
    ...
    def looping_task(self, interval, func, *args):
        ...
        if not self.has_scheduler():
            ...
            return
        event = utils.schedule_periodic_task(self._scheduler, interval, func, *args)
        self.info(
            "scheduled periodic task %s (interval: %.2fs)",
            event.action.__name__,
            interval,
        )
    ...
    def shutdown(self):
        ...
        if not (
            self.config.creating or self.config.repairing or self.config.quick_start
        ):
        ...
        self.cnxsets.close()
        hits, misses = (
            self.querier.rql_cache.cache_hit,
            self.querier.rql_cache.cache_miss,
        )
        try:
            self.info(
                "rql st cache hit/miss: %s/%s (%s%% hits)",
                hits,
                misses,
    ...
    def check_auth_info(self, cnx, login, authinfo):
        ...
        for source in self.sources_by_uri.values():
            ...
            raise AuthenticationError(
                f"authentication failed with all sources "
    ...
    def authenticate_user(self, cnx, login, **authinfo):
        ...
        if (
            self.config.consider_user_state
            and not cwuser.cw_adapt_to("IWorkflowable").state
            in cwuser.AUTHENTICABLE_STATES
        ):
                ...
                if checkversions:
                    ...
                    if version < fsversion:
                        ...
                        raise ExecutionError(
                            msg % (cube, version, fsversion, self.config.appid)
                        )
        return vcconf
    ...
    def find_users(self, fetch_attrs, **query_attrs):
        ...
        with self.internal_cnx() as cnx:
            ...
            rset = cnx.execute(
                rql + ",".join("X %s %%(%s)s" % (attr, attr) for attr in query_attrs),
                query_attrs,
            )
            return rset.rows
    ...
    def internal_cnx(self):
        ...
        with Connection(self, InternalManager()) as cnx:
            cnx.user._cw = (
                cnx  # XXX remove when "vreg = user._cw.vreg" hack in entity.py is gone
            )
    ...
    def glob_add_entity(self, cnx, edited):
        ...
        for attr, value in relations:
            self.hm.call_hooks(
                "before_add_relation", cnx, eidfrom=entity.eid, rtype=attr, eidto=value
            )
            self.hm.call_hooks(
                "after_add_relation", cnx, eidfrom=entity.eid, rtype=attr, eidto=value
            )
        return entity.eid
    ...
    def glob_add_relations(self, cnx, relations):
        ...
        for rtype, source_relations in relations_by_rtype.items():
            self.hm.call_hooks(
                "before_add_relation", cnx, rtype=rtype, eids_from_to=source_relations
            )
        for rtype, source_relations in relations_by_rtype.items():
        ...
        for rtype, source_relations in relations_by_rtype.items():
            self.hm.call_hooks(
                "after_add_relation", cnx, rtype=rtype, eids_from_to=source_relations
            )
    ...
    def glob_delete_relation(self, cnx, subject, rtype, object):
        ...
        source = self.system_source
        self.hm.call_hooks(
            "before_delete_relation", cnx, eidfrom=subject, rtype=rtype, eidto=object
        )
        source.delete_relation(cnx, subject, rtype, object)
        rschema = self.schema.rschema(rtype)
        cnx.update_rel_cache_del(subject, rtype, object, rschema.symmetric)
        self.hm.call_hooks(
            "after_delete_relation", cnx, eidfrom=subject, rtype=rtype, eidto=object
        )
