...
from cubicweb.schema import KNOWN_RPROPERTIES, CONSTRAINTS, VIRTUAL_RTYPES
...
def group_mapping(cnx, interactive=True):
    ...
    """
    res = {}
    for eid, name in cnx.execute(
        "Any G, N WHERE G is CWGroup, G name N", build_descr=False
    ):
        res[name] = eid
    if not interactive:
        return res
    ...
    if missing:
        print(
            "some native groups are missing but the following groups have been found:"
        )
...
def deserialize_schema(schema, cnx):
    ...
    if sqlcu.fetchall():
        sql = dbhelper.sql_rename_col(
            "cw_CWRType",
            "cw_symetric",
            "cw_symmetric",
            dbhelper.TYPE_MAPPING["Boolean"],
            True,
        )
        sqlcu.execute(sql)
        sqlcu.execute(
            "UPDATE cw_CWRType SET cw_name='symmetric' WHERE cw_name='symetric'"
        )
        cnx.commit()
    ertidx = {}
    copiedeids = set()
    ...
    for eid, etype, desc in cnx.execute(
        "Any X, N, D WHERE X is CWEType, X name N, X description D", build_descr=False
    ):
        ...
        if etype in ETYPE_NAME_MAP:
            ...
            if sqlexec(
                "SELECT 1 FROM %(p)sCWEType WHERE %(p)sname=%%(n)s"
                ...
                sqlexec(
                    "UPDATE %(p)sCWEType SET %(p)sname=%%(n)s WHERE %(p)seid=%%(x)s"
                    % {"p": sqlutils.SQL_PREFIX},
                    {"x": eid, "n": netype},
                )
                if etype.lower() != netype.lower():
                    alter_table_sql = dbhelper.sql_rename_table(
                        sqlutils.SQL_PREFIX + etype, sqlutils.SQL_PREFIX + netype
                    )
                    sqlexec(alter_table_sql)
            sqlexec(
                "UPDATE entities SET type=%(n)s WHERE type=%(x)s",
                {"x": etype, "n": netype},
            )
            cnx.commit(False)
            tocleanup = [eid]
            tocleanup += (
                eid for eid, cached in repo._type_cache.items() if etype == cached
            )
    ...
    if has_computed_relations:
        ...
        for eid, rule_name, rule, description in rset.rows:
            rtype = ybo.ComputedRelation(
                name=rule_name, rule=rule, eid=eid, description=description
            )
    ...
    for eid, rtype, desc, sym, il, ftc in cnx.execute(
        ...
        "X symmetric S, X inlined I, X fulltext_container FTC",
        build_descr=False,
    ):
        ertidx[eid] = rtype
    ...
    try:
        extra_props = dict(
            cnx.execute("Any X, XTP WHERE X is CWAttribute, " "X extra_props XTP")
        )
    except Exception:
        cnx.critical(
            "Previous CRITICAL notification about extra_props is not "
    ...
    # load attributes
    rql = (
        "Any X,SE,RT,OE,CARD,ORD,DESC,IDX,FTIDX,I18N,DFLT%(fm)s "
    ...
    for values in cnx.execute(rql, build_descr=False):
        ...
        attrs.update(
            json.loads(typeparams.getvalue().decode("ascii")) if typeparams else {}
        )
    ...
    for values in cnx.execute(
        ...
        "X from_entity SE, X to_entity OE, X composite C",
        build_descr=False,
    ):
    ...
    rset = cnx.execute(
        ...
        "X constraint_of E, X relations R",
        build_descr=False,
    )
    for values in rset:
...
def deserialize_ertype_permissions(cnx):
    ...
    for action in ("read", "add", "update", "delete"):
        ...
        rql = (
            "Any E,X,EXPR,V WHERE X is RQLExpression, X expression EXPR, "
        ...
        for eid, expreid, expr, mainvars in cnx.execute(rql, build_descr=False):
            ...
            res.setdefault(eid, {}).setdefault(action, []).append(
                (expr, mainvars, expreid)
            )
    return res
...
def deserialize_rdef_constraints(cnx):
    ...
    if cnx.repo.system_source.dbdriver != "sqlite" or sqlite3.sqlite_version_info >= (
        3,
        7,
        12,
    ):
    ...
    for rdefeid, ceid, ct, val in cnx.execute(
        ...
        "X cstrtype T, T name TN, X value V",
        build_descr=False,
    ):
...
def serialize_schema(cnx, schema):
    ...
    for cstrtype in CONSTRAINTS:
        cstrtypemap[cstrtype] = execute(rql, {"ct": cstrtype}, build_descr=False)[0][0]
        pb.update()
    # serialize relations
    for rschema in schema.relations():
        ...
        if rschema.symmetric:
            rdefs = [
                rdef
                for k, rdef in rschema.rdefs.items()
...
def uniquetogether2rqls(eschema):
    ...
    for columns in eschema._unique_together:
        if columns in columnset:
            print(
                "schemaserial: skipping duplicate unique together %r %r"
...
def _uniquetogether2rql(eschema, unique_together):
    ...
    rql = (
        "INSERT CWUniqueTogetherConstraint C: C name %%(name)s, C constraint_of X, %s "
        "WHERE X eid %%(x)s, %s"
    )
...
def rschema2rql(rschema, cstrtypemap=None, addrdef=True, groupmap=None):
    ...
    if addrdef:
        ...
        for rdef in sorted(rschema.rdefs.values(), key=lambda x: (x.subject, x.object)):
...
def constraints2rql(cstrtypemap, constraints, rdefeid=None):
    for constraint in constraints:
        values = {
            "ct": cstrtypemap[constraint.type()],
            "value": constraint.serialize(),
            "x": rdefeid,
        }  # when not specified, will have to be set by the caller
    ...
    for action in erschema.ACTIONS:
        ...
        for group_or_rqlexpr in grantedto:
            if isinstance(group_or_rqlexpr, str):
                ...
                try:
                    yield (
                        "SET X %s_permission Y WHERE Y eid %%(g)s, X eid %%(x)s"
                        % action,
                        {"g": groupmap[group_or_rqlexpr]},
                    )
                except KeyError:
                    print(
                        "WARNING: group %s used in permissions for %s was ignored because it doesn't exist."
                        " You may want to add it into a precreate.py file"
                        % (group_or_rqlexpr, erschema)
                    )
                    continue
            else:
                # rqlexpr
                ...
                yield (
                    "INSERT RQLExpression E: E expression %%(e)s, E exprtype %%(t)s, "
...
def updaterschema2rql(rschema, eid):
    if rschema.rule:
        yield ("SET X rule %(r)s WHERE X eid %(x)s", {"x": eid, "r": rschema.rule})
    else:
        relations, values = rschema_relations_values(rschema)
        values["x"] = eid
        yield "SET %s WHERE X eid %%(x)s" % ",".join(relations), values
