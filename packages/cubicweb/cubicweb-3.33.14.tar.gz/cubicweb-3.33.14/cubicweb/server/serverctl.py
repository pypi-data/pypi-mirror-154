...
class RepositoryDeleteHandler(CommandHandler):
    ...
    def _cleanup_steps(self, source):
        ...
        if db_namespace:
            yield (
                'Delete database namespace "%s"' % db_namespace,
                self._drop_namespace,
                True,
            )
        # 2/ delete database
        yield ('Delete database "%(db-name)s"' % source, self._drop_database, True)
        # 3/ delete user
        helper = get_db_helper(source["db-driver"])
        if source["db-user"] and helper.users_support:
            # XXX should check we are not connected as user
            yield ('Delete user "%(db-user)s"' % source, self._drop_user, False)
...
class CreateInstanceDBCommand(Command):
    ...
    def run(self, args):
            ...
            try:
                if helper.users_support:
                    ...
                    if not helper.user_exists(cursor, user) and (
                        automatic
                        or ASK.confirm(
                            "Create db user %s ?" % user, default_is_yes=False
                        )
                    ):
        ...
        if namespace and (
            automatic
            or ASK.confirm("Create schema %s in database %s ?" % (namespace, dbname))
        ):
    ...
    def run(self, args):
        ...
        try:
            with cnx:
                ...
                while True:
                    type = input("source type (%s): " % ", ".join(sorted(SOURCE_TYPES)))
                ...
                while True:
                    parser = input(
                        "parser type (%s): " % ", ".join(sorted(repo.vreg["parsers"]))
                    )
...
def _local_restore(appid, backupfile, drop, format="native"):
    ...
    #   anyway, just rewrite vc.conf and warn user he has to upgrade
        ...
        print(
            "** The database of %s is older than the installed software." % config.appid
        )
