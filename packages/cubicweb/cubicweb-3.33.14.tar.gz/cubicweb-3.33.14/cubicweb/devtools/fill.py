...
def decompose_b26(index, ascii=False):
    ...
    return base_decompose_b26(index, "éabcdefghijklmnopqrstuvwxyz")
...
class _ValueGenerator(object):
    ...
    def __generate_value(self, entity, attrname, index, **kwargs):
        ...
        gen_func = getattr(
            self,
            "generate_%s_%s" % (eschema, attrname),
    ...
    def generate_string(self, entity, attrname, index, format=None):
        ...
        if num_len >= attrlength:
            ascii = getattr(self.eschema.rdef(attrname), "internationalizable", False)
        ...
        if format == "text/html":
            value = "<span>é%s<b>%d</b></span>" % (attrprefix, index)
...
""" % (
    ...
    def generate_datetime(self, entity, attrname, index):
        ...
        base = datetime(
            randint(2000, 2004), randint(1, 12), randint(1, 28), 11, index % 60
        )
        return self._constrained_generate(
            entity, attrname, base, timedelta(hours=1), index
        )
    ...
    def generate_date(self, entity, attrname, index):
        ...
        return self._constrained_generate(
            entity, attrname, base, timedelta(days=1), index
        )
    ...
    def generate_CWDataImport_log(self, entity, index, **kwargs):
        ...
        logs = [
            ...
            % (
                logging.ERROR,
                "http://url.com?arg1=hop&arg2=hip",
                1,
                xml_escape('hjoio&oio"'),
            )
        ]
        return "<br/>".join(logs)
    ...
    def __new__(mcs, name, bases, classdict):
        for attrname, attrvalue in classdict.items():
            if callable(attrvalue):
                if (
                    attrname.startswith("generate_")
                    and len(inspect.getfullargspec(attrvalue).args) < 2
                ):
...
def insert_entity_queries(
    etype, schema, vreg, entity_num, choice_func=_default_choice_func
):
    ...
    for index in range(entity_num):
        restrictions = []
        args = {}
        for attrname, value in make_entity(
            etype, schema, vreg, index, choice_func
        ).items():
            restrictions.append("X %s %%(%s)s" % (attrname, attrname))
            args[attrname] = value
        if restrictions:
            queries.append(("INSERT %s X: %s" % (etype, ", ".join(restrictions)), args))
...
def make_entity(
    etype, schema, vreg, index=0, choice_func=_default_choice_func, form=False
):
    ...
    for rschema, attrschema in eschema.attribute_definitions():
        ...
        if attrname == "eid":
            ...
            continue
        if (
            attrname.endswith("_format")
            and attrname[:-7] in eschema.subject_relations()
        ):
    ...
    for attrname, attrschema in attributes:
        if attrname in relatedfields:
            ...
            format = valgen.generate_attribute_value(
                entity, attrname + "_format", index
            )
            # then a value coherent with this format
            value = valgen.generate_attribute_value(
                entity, attrname, index, format=format
            )
        else:
        ...
        if form:  # need to encode values
            if attrschema.type == "Bytes":
                ...
                value = (filename, "text/plain", fakefile)
...
def make_relations_queries(schema, edict, cnx, ignored_relations=(), existingrels=None):
...
class RelationsQueriesGenerator(object):
    ...
    def compute_queries(self, entities_dict, ignored_relations):
        """
        entities_dict: Dict[str(EntityTypeName), Set[Entity]]
        """
        ...
        for rschema in rels:
            sym = set()
            sedict = deepcopy(entities_dict)
            oedict = deepcopy(entities_dict)
            delayed = []
            ...
            for subj, obj in rschema.rdefs:
                ...
                if subjcard in "1+" or objcard in "1+" or composite_relation(rschema):
                    for query, args in self.make_relation_queries(
                        sedict, oedict, rschema, subj, obj
                    ):
                        yield query, args
                else:
    ...
    def make_relation_queries(self, sedict, oedict, rschema, subj, obj):
        ...
        rql = self.rql_tmpl % rschema.type
        constraints = [c for c in rdef.constraints if isinstance(c, RQLConstraint)]
        if constraints:
        ...
        if subjcard in "?1+":
            for subjeid in set(subjeids):
                ...
                if objeid is None or (subjeid, objeid) in used:
                    continue
                yield rql, self.qargs(
                    subjeids, objeids, subjcard, objcard, subjeid, objeid
                )
            ...
            pairs = list(itertools.product(subjeids, objeids))
            take_only = min(len(pairs), 20)  # FIXME: 20 should be read from config
            for subjeid, objeid in sample(pairs, take_only):
                if subjeid != objeid and (subjeid, objeid) not in used:
                    used.add((subjeid, objeid))
                    yield rql, self.qargs(
                        subjeids, objeids, subjcard, objcard, subjeid, objeid
                    )
...
def check_card_satisfied(card, remaining, subj, rschema, obj):
    if card in "1+" and remaining:
        raise Exception(
            "can't satisfy cardinality %s for relation %s %s %s"
