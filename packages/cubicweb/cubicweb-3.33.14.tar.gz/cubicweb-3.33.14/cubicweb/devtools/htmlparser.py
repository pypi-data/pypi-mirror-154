...
_REM_SCRIPT_RGX = re.compile(rb"<script[^>]*>.*?</script>", re.M | re.I | re.S)
...
class Validator:
    ...
    def _parse(self, pdata):
        try:
            ...
            new_exc = AssertionError("invalid document: %s" % exc)
...
class DTDValidator(Validator):
    ...
    def preprocess_data(self, data):
        ...
        expected = (
            "p h1 h2 h3 h4 h5 h6 div ul ol dl pre hr blockquote address "
            "fieldset table form noscript ins del script".split()
        )
        if namespace:
            blockquotes = tree.findall(".//{%s}blockquote" % namespace)
            expected = [f"{{{namespace}}}{tag}" for tag in expected]
        else:
        ...
        return f'<?xml version="1.0" encoding="UTF-8"?>{STRICT_DOCTYPE}\n{data}'
...
class PageInfo:
    ...
    def __init__(self, source, root):
        self.source = source
        self.etree = root
        self.raw_text = "".join(root.xpath("//text()"))
    ...
    def find_tag(self, tag, gettext=True):
        ...
        return ["".join(elt.xpath(".//text()")) for elt in self.etree.iterfind(iterstr)]
