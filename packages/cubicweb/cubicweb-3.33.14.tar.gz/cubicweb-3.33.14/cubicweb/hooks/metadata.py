...
class InitMetaAttrsHook(MetaDataHook):
    ...
    def __call__(self):
        ...
        if not self._cw.transaction_data.get("do-not-insert-cwuri"):
            cwuri = f"{self._cw.base_url()}{self.entity.eid}"
...
class SetCreatorOp(hook.DataOperationMixIn, hook.Operation):
    def precommit_event(self):
        ...
        relations = [
            ...
            # explicitly set
            if not cnx.deleted_in_transaction(eid)
            and not cnx.entity_from_eid(eid).created_by
        ]
    ...
    def precommit_event(self):
        for compositeeid, composedeid in self.get_data():
            ...
            if self.cnx.deleted_in_transaction(composedeid):
                continue
            self.cnx.execute(
                "SET X owned_by U WHERE C owned_by U, C eid %(c)s,"
...
class UpdateFTIHook(MetaDataHook):
    ...
    def __call__(self):
        ...
        if ftcontainer == "subject":
            cnx.repo.system_source.index_entity(cnx, cnx.entity_from_eid(self.eidfrom))
        elif ftcontainer == "object":
            cnx.repo.system_source.index_entity(cnx, cnx.entity_from_eid(self.eidto))
