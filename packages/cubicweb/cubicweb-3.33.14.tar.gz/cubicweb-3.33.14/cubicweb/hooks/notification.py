    ...
    def select_view(self, vid, rset, row=0, col=0):
        try:
            return self._cw.vreg["views"].select_or_none(
                vid, self._cw, rset=rset, row=row, col=col
            )
...
class RelationChangeHook(NotificationHook):
    ...
    def __call__(self):
        ...
        view = self.select_view(f"notif_{self.event}_{self.rtype}", rset=rset, row=0)
        if view is None:
            return
...
class EntityUpdatedNotificationOp(hook.SingleLastOperation):
    ...
    def precommit_event(self):
        ...
        for eid in cnx.transaction_data["changes"]:
            view = cnx.vreg["views"].select(
                "notif_entity_updated", cnx, rset=cnx.eid_rset(eid), row=0
            )
            notify_on_commit(
                self.cnx,
                view,
...
class EntityUpdateHook(NotificationHook):
    ...
    def __call__(self):
        ...
        attrs = [k for k in self.entity.cw_edited if not k in self.skip_attrs]
        if not attrs:
            return
        ...
        for i, attr in enumerate(attrs):
            var = chr(65 + i)
            rqlsel.append(var)
            rqlrestr.append(f"X {attr} {var}")
        rql = "Any {} WHERE {}".format(",".join(rqlsel), ",".join(rqlrestr))
    ...
    def _call(self):
        ...
        if event == "update_entity":
            ...
            if self.entity.e_schema == "CWUser":
                if not (
                    frozenset(self.entity.cw_edited)
                    - frozenset(("eid", "modification_date", "last_login_time"))
                ):
                    # don't record last_login_time update which are done
                    # automatically at login time
                    return False
        self._cw.transaction_data.setdefault("pendingchanges", []).append((event, self))
        return True
