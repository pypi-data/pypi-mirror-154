...
class MigrationToolsTC(TestCase):
    ...
    def test_filter_scripts_base(self):
        self.assertListEqual(
            filter_scripts(self.config, SMIGRDIR, (2, 3, 0), (2, 4, 0)), []
        )
        self.assertListEqual(
            filter_scripts(self.config, SMIGRDIR, (2, 4, 0), (2, 5, 0)),
            [((2, 5, 0), SMIGRDIR + "2.5.0_Any.sql")],
        )
        self.assertListEqual(
            filter_scripts(self.config, SMIGRDIR, (2, 5, 0), (2, 6, 0)),
            [((2, 6, 0), SMIGRDIR + "2.6.0_Any.sql")],
        )
        self.assertListEqual(
            filter_scripts(self.config, SMIGRDIR, (2, 4, 0), (2, 6, 0)),
            [
                ((2, 5, 0), SMIGRDIR + "2.5.0_Any.sql"),
                ((2, 6, 0), SMIGRDIR + "2.6.0_Any.sql"),
            ],
        ...
        self.assertListEqual(
            filter_scripts(self.config, SMIGRDIR, (2, 5, 0), (2, 5, 1)), []
        )
        self.assertListEqual(
            filter_scripts(self.config, SMIGRDIR, (2, 5, 0), (2, 10, 2)),
            [
                ((2, 6, 0), SMIGRDIR + "2.6.0_Any.sql"),
        ...
        self.assertListEqual(
            filter_scripts(self.config, SMIGRDIR, (2, 5, 1), (2, 6, 0)),
            [((2, 6, 0), SMIGRDIR + "2.6.0_Any.sql")],
        )

        self.assertListEqual(
            filter_scripts(self.config, TMIGRDIR, (0, 0, 2), (0, 0, 3)),
            [((0, 0, 3), TMIGRDIR + "0.0.3_Any.py")],
        )
        self.assertListEqual(
            filter_scripts(self.config, TMIGRDIR, (0, 0, 2), (0, 0, 4)),
            [
                ((0, 0, 3), TMIGRDIR + "0.0.3_Any.py"),
...
class BaseCreationTC(TestCase):
    def test_db_creation(self):
        ...
        with cnx:
            self.assertEqual(
                cnx.execute(
                    'Any SN WHERE X is CWUser, X login "admin", X in_state S, S name SN'
                ).rows,
                [["activated"]],
            )
        repo.shutdown()
