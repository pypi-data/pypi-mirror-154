...
for rel in RELS:
    ...
    if _type == "concerne":
        schema.add_relation_def(
            RelationDefinition(_from, _type, _to, __permissions__=CONCERNE_PERMISSIONS)
        )
    else:
...
class CubicWebSchemaTC(TestCase):
    ...
    def test_entity_perms(self):
        self.assertEqual(eperson.get_groups("read"), {"managers", "users", "guests"})
        self.assertEqual(
            eperson.get_groups("update"),
            {
                "managers",
                "owners",
            },
        )
        self.assertEqual(eperson.get_groups("delete"), {"managers", "owners"})
        self.assertEqual(eperson.get_groups("add"), {"managers"})
        self.assertEqual(
            [str(e) for e in eperson.get_rqlexprs("add")],
            ["Any X WHERE X travaille S, S owned_by U, X eid %(x)s, U eid %(u)s"],
        )
        eperson.set_action_permissions("read", ("managers",))
        self.assertEqual(eperson.get_groups("read"), {"managers"})

    def test_relation_perms(self):
        rconcerne = schema.rschema("concerne").rdef("Personne", "Societe")
        self.assertEqual(rconcerne.get_groups("read"), {"managers", "users", "guests"})
        self.assertEqual(rconcerne.get_groups("delete"), {"managers"})
        self.assertEqual(rconcerne.get_groups("add"), {"managers"})
        rconcerne.set_action_permissions("read", ("managers",))
        self.assertEqual(rconcerne.get_groups("read"), {"managers"})
        self.assertEqual(
            [str(e) for e in rconcerne.get_rqlexprs("add")],
...
class SchemaReaderClassTest(TestCase):
    def test_order_eschemas(self):
        ...
        self.assertEqual(
            order_eschemas([schema["Note"], schema["SubNote"]]),
            [schema["Note"], schema["SubNote"]],
        )
        self.assertEqual(
            order_eschemas([schema["SubNote"], schema["Note"]]),
            [schema["Note"], schema["SubNote"]],
        )
    ...
    def test_permission_settings(self):
        ...
        self.assertEqual(aschema.get_groups("read"), {"managers", "users", "guests"})
        self.assertEqual(aschema.get_rqlexprs("read"), ())
        self.assertEqual(aschema.get_groups("update"), {"managers"})
        self.assertEqual(
            [x.expression for x in aschema.get_rqlexprs("update")],
    ...
    def test_relation_perm_overriding(self):
        ...
        self.assertEqual(
            rdef.permissions,
            {"read": ("managers",), "add": ("managers",), "delete": ("managers",)},
        )
    ...
    def test_computed_attribute(self):
        ...
        self.assertEqual(
            rdef.permissions,
            {"add": (), "update": (), "read": ("managers", "users", "guests")},
        )
...
class SchemaReaderComputedRelationAndAttributesTest(TestCase):
    def test_infer_computed_relation(self):
        ...
        self.assertEqual(
            [("Person", "Service")], list(schema["produces_and_buys"].rdefs.keys())
        )
        self.assertEqual(
            [("Person", "Service")], list(schema["produces_and_buys2"].rdefs.keys())
        )
        self.assertCountEqual(
            [("Company", "Service"), ("Person", "Service")],
        ...
        self.assertEqual(
            rdef.permissions,
            {"add": (), "delete": (), "read": ("managers", "users", "guests")},
        )
...
class BadSchemaTC(TestCase):
    ...
    def _test(self, schemafile, msg):
        self.loader.handle_file(join(DATADIR, schemafile), splitext(schemafile)[0])
...
class NormalizeExpressionTC(TestCase):
    def test(self):
        self.assertEqual(
            normalize_expression("X  bla Y,Y blur Z  ,  Z zigoulou   X "),
            "X bla Y, Y blur Z, Z zigoulou X",
        )
        self.assertEqual(
            normalize_expression('X bla Y, Y name "x,y"'), 'X bla Y, Y name "x,y"'
        )
...
class RQLExpressionTC(TestCase):
    def test_comparison(self):
        self.assertEqual(
            ERQLExpression("X is CWUser", "X", 0), ERQLExpression("X is CWUser", "X", 0)
        )
        self.assertNotEqual(
    ...
    def test_has_update_permission(self):
        ...
        self.assertEqual(found, [("update", 1)])
...
class GuessRrqlExprMainVarsTC(TestCase):
    def test_exists(self):
        mainvars = guess_rrqlexpr_mainvars(
            normalize_expression(
                "NOT EXISTS(O team_competition C, C level < 3, C concerns S)"
            )
        )
        self.assertEqual(mainvars, {"S", "O"})
...
class RQLConstraintTC(CubicWebTC):
    def test_user_constraint(self):
        ...
        with self.admin_access.repo_cnx() as cnx:
            ...
            self.assertRaises(ValidationError, cstr.repo_check, cnx, 1, "rel", anoneid)
            self.assertEqual(
                cstr.repo_check(cnx, 1, cnx.user.eid), None
            )  # no validation error, constraint checked
...
class CompositeSchemaTC(CubicWebTC):
    composites = {
        ...
        "CWAttribute": [
            ("add_permission", "CWAttribute", "RQLExpression", "subject"),
        ...
        "CWComputedRType": [
            ("read_permission", "CWComputedRType", "RQLExpression", "subject")
        ],
        "CWSource": [
        ...
        "CWUser": [
            ("for_user", "CWProperty", "CWUser", "object"),
        ...
        "Workflow": [
            ("state_of", "State", "Workflow", "object"),
    ...
    def test_composite_entities(self):
        ...
        for etype in self.composites:
            with self.subTest(etype=etype):
                self.assertEqual(
                    self.composites[etype],
                    sorted(
                        [
                            (r.rtype.type, r.subject.type, r.object.type, role)
