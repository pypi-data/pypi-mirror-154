...
class EntityTC(CubicWebTC):
    ...
    def test_fetch_rql(self):
        ...
        torestore = []
        for rdef, card in [
            (peschema.subjrels["travaille"].rdef(peschema, seschema), "1*"),
        ...
        try:
            with self.admin_access.web_request() as req:
                ...
                peschema.subjrels["travaille"].rdef(
                    peschema, seschema
                ).cardinality = "?*"
                Personne.fetch_attrs = ("nom", "prenom", "travaille")
    ...
    def test_unrelated_rql_s_linkto_o(self):
        with self.admin_access.web_request() as req:
            ...
            self.assertEqual(
                ...
                "O is_instance_of Societe, NOT EXISTS(O eid %(O)s), "
                "A is Personne",
                rql,
            )
    ...
    def test_unrelated_rql_o_linkto_s(self):
        with self.admin_access.web_request() as req:
            ...
            self.assertEqual(
                ...
                "S is_instance_of Personne, NOT EXISTS(S eid %(S)s), "
                "A is Societe",
                rql,
            )
...
du :eid:`1:*ReST*`"""
            ...
            if HAS_TAL:
                ...
                self.assertEqual(e.printable_value("content"), "<h1>zou</h1>")
    ...
    def test_nonregr_relation_cache(self):
        with self.admin_access.web_request() as req:
            ...
            p2 = req.create_entity("Personne", nom="toto")
    ...
    def test_absolute_url_empty_field(self):
        with self.admin_access.web_request() as req:
            ...
            self.assertEqual(
                card.absolute_url(), "http://testing.fr/cubicweb/%s" % card.eid
            )
