...
class ResultSetTC(CubicWebTC):
    ...
    def test_build_url(self):
        with self.admin_access.web_request() as req:
            ...
            self.compare_urls(
                req.build_url("view", vid="foo", rql="yo"),
                "%sview?vid=foo&rql=yo" % baseurl,
            )
            self.compare_urls(
                req.build_url("view", _restpath="task/title/go"),
    ...
    def test_get_entity_simple(self):
        with self.admin_access.web_request() as req:
            ...
            e = req.execute('Any X,T WHERE X login "adim", X surname T').get_entity(
                0, 0
            )
    ...
    def test_get_entity_advanced(self):
        with self.admin_access.web_request() as req:
            req.create_entity("Bookmark", title="zou", path="/view")
            req.drop_entity_cache()
    ...
    def test_get_entity_advanced_prefilled_cache(self):
        with self.admin_access.web_request() as req:
            e = req.create_entity("Bookmark", title="zou", path="path")
            req.cnx.commit()
            rset = req.execute(
    ...
    def test_get_entity_union(self):
        with self.admin_access.web_request() as req:
            req.create_entity("Bookmark", title="manger", path="path")
            req.drop_entity_cache()
            rset = req.execute(
    ...
    def test_one(self):
        with self.admin_access.web_request() as req:
            ...
            e = req.execute('Any X, N WHERE X login "cdevienne", X surname N').one()
            self.assertEqual(e.surname, "de Vienne")

            e = req.execute('Any N, X WHERE X login "cdevienne", X surname N').one(
                col=1
            )
            self.assertEqual(e.surname, "de Vienne")
    ...
    def test_first(self):
        with self.admin_access.web_request() as req:
            ...
            e = req.execute('Any X, N WHERE X login "cdevienne", X surname N').first()
            self.assertEqual(e.surname, "de Vienne")

            e = req.execute('Any N, X WHERE X login "cdevienne", X surname N').first(
                col=1
            )
            self.assertEqual(e.surname, "de Vienne")
    ...
    def test_last(self):
        with self.admin_access.web_request() as req:
            ...
            e = req.execute('Any X, N WHERE X login "cdevienne", X surname N').last()
            self.assertEqual(e.surname, "de Vienne")

            e = req.execute('Any N, X WHERE X login "cdevienne", X surname N').last(
                col=1
            )
            self.assertEqual(e.surname, "de Vienne")
    ...
    def test_related_entity_optional(self):
        with self.admin_access.web_request() as req:
            req.create_entity("Bookmark", title="aaaa", path="path")
    ...
    def test_related_entity_union_subquery_1(self):
        with self.admin_access.web_request() as req:
            e = req.create_entity("Bookmark", title="aaaa", path="path")
            rset = req.execute(
                "Any X,N ORDERBY N WITH X,N BEING "
                "((Any X,N WHERE X is CWGroup, X name N)"
                " UNION "
    ...
    def test_related_entity_trap_subquery(self):
        with self.admin_access.web_request() as req:
            req.create_entity("Bookmark", title="test bookmark", path="")
    ...
    def test_printable_rql(self):
        with self.admin_access.web_request() as req:
            rset = req.execute("CWEType X WHERE X final FALSE")
            self.assertEqual(
    ...
    def test_searched_text(self):
        with self.admin_access.web_request() as req:
            rset = req.execute('Any X WHERE X has_text "foobar"')
            self.assertEqual(rset.searched_text(), "foobar")
            rset = req.execute("Any X WHERE X has_text %(text)s", {"text": "foo"})
