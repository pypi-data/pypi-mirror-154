...
def transcription(audio_path):
	...
	inputs_values = my_processor(speech, sampling_rate=16_000, return_tensors="pt", padding=True).input_values

	with torch.no_grad():
  		logits = my_model(input_values, attention_mask=inputs.attention_mask).logits
