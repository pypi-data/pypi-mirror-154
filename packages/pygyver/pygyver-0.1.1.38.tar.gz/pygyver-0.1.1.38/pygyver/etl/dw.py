...
class BigQueryExecutor:
    ...
    def create_dataset(
        ...
        Parameters:
            dataset_id (string): BigQuery dataset ID.
            project_id (string): BigQuery project ID.
            location (string): BigQuery location - if empty takes the project's default location
        """
        dataset_ref = self.get_dataset_ref(dataset_id, project_id=project_id)
        if self.dataset_exists(dataset_id, project_id=project_id):
            ...
            try:
                ...
                location_name = location if location else "default"
                logging.info(
                    "Created dataset %s in project %s and %s location",
                    dataset_id,
                    project_id,
                    location_name
                )
    ...
    def create_table(self, table_id, dataset_id=bq_default_dataset(),project_id=bq_default_project(), sql=None, file=None,
                     ...
                     location=bq_default_location(), schema_path='',
                     partition=False,
                     partition_field='_PARTITIONTIME',
                     clustering=None,
                     ...
                     **kwargs):
        """ create a bigquery table from a sql query or file
        ...
        Parameters:
            ...
            partition:
            partition_field:
            clustering: A list of field names that the table will be clustered on, e.g. [field1, field2]
            priority:
            description:
            increment_field: The field on which the incremental load for a non-partitioned table is based. Allows for existing entries to be replaced.
                             Requires "FROM_DATE" and "TO_DATE" env vars to be set to operate correctly.

        Returns:
            Configured query job
        """
        ...
        if increment_field:
            logging.info(f"Increment field set to {increment_field}")
            try:
                from_date = os.environ["FROM_DATE"]
                to_date = os.environ["TO_DATE"]
            except KeyError:
                raise KeyError("Env variables FROM_DATE and TO_DATE have not been set")
            if self.check_increment_dates(dataset_id, table_id, increment_field, from_date, to_date):
                logging.info(f'Discovered existing records in {table_id} between {from_date} and {to_date}')
                self.delete_backfill_increments(dataset_id, table_id, increment_field, from_date, to_date)
    ...
    def check_increment_dates(self,
                              dataset_id,
                              table_id,
                              increment_field,
                              from_date,
                              to_date):
        """ Checks whether increment dates to be added are in existing table.

        Parameters:
            table_id (string): BigQuery table ID.
            dataset_id (string): BigQuery dataset ID.
            increment_field: Field on which table will be incrementally loaded
            project_id (string): BigQuery project ID.

        Returns:
            True if any results returned, otherwise False
        """
        sql = f"""SELECT
                    {increment_field}
                  FROM
    ...
    def delete_backfill_increments(self,
                                   dataset_id,
                                   table_id,
                                   increment_field,
                                   from_date,
        ...
        """
        Delete increments from table in backfill between from_date and to_date

        Args:
            table_id (string): table in backfill dataset that the increments will be deleted from
            increment_field (string): the time-unit column that the table is incremented on
            from_date (date): date of the first records to be deleted
            to_date (date): date of the latest records to be deleted
    ...
    def run_backfill(self,
        ...
        # handling the different bq locations
        bq_loc = bq_default_location().lower()
        backfill_dataset = "backfill" if not bq_loc else f"backfill_{bq_loc}"
    ...
    def execute_dml(self, sql=None, file=None, location=bq_default_location(), project=bq_default_project()):
