class Image:
    ...
    def list_image_by_page(self, program_id, kind=None, page=1, per_page=30):
        params = {'page': page, 'per_page': per_page, 'program_id': program_id}
        if kind is not None:
            params['kind'] = kind
    ...
    def delete(self, image_id):
        data = self.client.post(f'/image/{image_id}/delete')
        return data
    ...
    def release(self, image_id, program_id, name, comment=''):
        ...
        data = self.client.post(f'/image/{image_id}/release', data=params)
        return data
