...
class ElasticSearcher:
    ...
    def _parse_doc(self, doc: dict, filter_by_field_data=True):
        """
        ...
        if self.field_data and filter_by_field_data:
            ...
            parsed_doc = {k: v for k, v in parsed_doc.items() if self.field_data.count(k.split(".")[0])}
        else:
            parsed_doc, _, _ = self._flatten_doc(doc)
        return parsed_doc
    ...
    def _flatten_doc(self, doc: dict):
        """
        Flattens a dictionary by changing it's nested structures into a flat one
