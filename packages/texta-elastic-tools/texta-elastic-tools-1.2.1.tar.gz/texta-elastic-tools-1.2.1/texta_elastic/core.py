...
from elasticsearch_dsl import Date, Keyword, Long, Mapping, Nested, Q, Text, Object
...
class ElasticCore:
    ...
    @elastic_connection
    def add_texta_meta_mapping(self, index: str):
        m = Mapping()
        texta_meta = Object(
            properties={
                "document_uuid": Text()
        ...
        # Set the name of the field along with its mapping body
        mapping = m.field("texta_meta", texta_meta).to_dict()
        doc_type = self.get_doc_type_for_index(index)
        self.es.indices.put_mapping(body=mapping, index=index, doc_type=doc_type, include_type_name=True)
    ...
    def get_annotated_annotation_query(self, query: dict, job_pk: int):
        """
        Return a query dictionary for the annotator for documents that are already annotated.
        :param job_pk:
        :param query: Dictionary of an Elasticsearch query as an additional restriction.
        :return:
        """
