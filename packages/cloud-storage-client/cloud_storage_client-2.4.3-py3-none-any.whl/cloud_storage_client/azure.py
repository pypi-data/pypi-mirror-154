...
class AzureClient():
        ...
        for blob in blobs:
            if blob.name.find(src_folder + '/') == 0:
                ...
                with open(f"{dst_folder + '/' + splitted_name[len(splitted_name) - 1]}", "wb") as data:
                    data.write(blob_client.download_blob(validate_content=True, max_concurrency=4).readinto(data))
    ...
    def download_file(self, folder_id, selected_chunk, output_folder):
        ...
        with open(f"{output_folder}/{selected_chunk}", "wb") as data:
            blob_client.download_blob(validate_content=True, max_concurrency=4).readinto(data)
    ...
    def get_file_size(self, filename):
        try:
            print(f'Azure - get_file_size for {filename} - {filename.strip().lstrip("/")}')
            blob_client = self.service.get_blob_client(container=self.bucket_name, blob=filename.strip().lstrip("/"))
            # print(f'Azure - blob_client: {json.dumps(blob_client)}')
