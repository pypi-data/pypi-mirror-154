...
class MainWindow(OWWidget):
    ...
    def setupUi(self):
        ...
        self.setFeedbackUrl(url=self.service + '/security', text=__("user_tip"), attribute=user_tip)
        ...
        self.setFeedbackUrl(url=self.service + '/login', text=__("login"), attribute=self.login)
    ...
    def accept(self):
        url = self.service + "/api/oauth/user/login"
        headers = {
            "Grant-Type": "password",
        }
        data = {
