...
class MarketOnOpenOrdersExecutor(MarketOrdersExecutor):
    def _check_order_validity(self, order):
        assert order.time_in_force == TimeInForce.OPG, \
            "Only TimeInForce.OPG Time in Force is accepted by MarketOnOpenOrdersExecutor"
        assert order.execution_style == MarketOrder(), \
            "Only MarketOrder ExecutionStyle is supported by MarketOnOpenOrdersExecutor"
