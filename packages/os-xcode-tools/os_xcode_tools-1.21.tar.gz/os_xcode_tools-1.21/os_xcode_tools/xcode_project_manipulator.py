...
def add_project(project, other_project_path):
    project.add_proect(other_project_path)
...
def add_framework(project, framework_path, system_framework=False, parent_dir_name='Custom Frameworks'):
    ...
    if system_framework:
        ...
        if not fh.is_dir_exists(framework_dst):
            fh.copy_dir_with_permission_denied(framework_path, framework_dst)

    if '.xcframework' in framework_dst:
        print(f"**************************************************************************************************"
              f"\nWARNING: I currently can't fully automate .xcframework files. "
              f"\nTo embade the framework manualy: "
              f"\n1) Open your project"
              f"\n2) In Target -> General -> Frameworks, Libraries and Embeded content click on the '+' "
              f"\n3) Add other -> Add Files"
              f"\n4) In 'Custom Frameworks' directory -> select '{fh.get_dir_name(framework_dst)}' "
              f"\n**************************************************************************************************")
        return
        ...
        for build_phase_id in target.buildPhases:
            ...
            for build_file_id in build_phase.files:
                ...
                if hasattr(file_ref, 'path'):
                    ...
                    if file_path.endswith('.framework') or file_path.endswith('.xcframework'):
