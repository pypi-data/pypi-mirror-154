...
class HttpRequest:
    ...
    def connect(self, url, headers=None, data=None, proxies=None):
        """ 发出get或post请求, 返回状态码 """
        self.url = url.strip()
...
class httptools():
    ...
    @staticmethod
    def getText(url, headers=None, data=None, proxies=None):
        req = HttpRequest()
    ...
    def downloadM3u8(url, dstfile, headers=None, data=None, proxies=None, maxRetryCount=5, nowRetryCount=0, minSize=1024, theadnum=32, maxFailRate=0.015):
        ...
        if len(os.listdir(m3u8TmpFileDir)) / len(tsFilePaths) < 1 - maxFailRate:
            return 0
