...
class Address(BaseModel):
    ...
    def checkRowCompletion(cls,values):
        all_fields=['po_box','unit','street_number','street_name','city','district','province','country','post_code']
        all_fields_values=[values[field] for field in all_fields]
        
        required_fields=['street_number','street_name','city','country','post_code']
        ...
        if any(all_fields_values) and not all(required_values):
            raise ValueError(f"Please check the row with values ({has_values}) in table-address, some required fileds are missed.")
        return values
