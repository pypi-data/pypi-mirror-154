...
class FeatureName(Enum):
    ...
    @classmethod
    def last_minute(cls, minute, prefix):
        return f'_{prefix}[minute-{minute+1}]'

    @classmethod        
    def last_hour(cls, hour, prefix):
        return f'_{prefix}[hour-{hour+1}]'        

    @classmethod
    def last_day(cls, day, prefix):
        return f'_{prefix}[day-{day+1}]'

    @classmethod
    def last_week(cls, week, prefix):
        return f'_{prefix}[week-{week+1}]'

    @classmethod
    def last_month(cls, month, prefix):
        return f'_{prefix}[month-{month+1}]'
...
def extract_player_events_by_time_periods(player_events, minutes, hours, days, weeks, months, operator, prefix):
    ...
    features.update(extract_player_events_by_time_period(player_events.groupby(elapsed_time_in_minutes), minutes, partial(FeatureName.last_minute, prefix=prefix)))
    features.update(extract_player_events_by_time_period(player_events.groupby(elapsed_time_in_hours), hours, partial(FeatureName.last_hour, prefix=prefix)))
    features.update(extract_player_events_by_time_period(player_events.groupby(elapsed_time_in_days), days, partial(FeatureName.last_day, prefix=prefix)))
    features.update(extract_player_events_by_time_period(player_events.groupby(elapsed_time_in_weeks), weeks, partial(FeatureName.last_week, prefix=prefix)))
    features.update(extract_player_events_by_time_period(player_events.groupby(elapsed_time_in_months), months, partial(FeatureName.last_month, prefix=prefix)))
    ...
    features_extractor = partial(
        ...
        extractors=[
            ...
            partial(extract_player_events,
                    ...
                    prefix='time_of_day_in_sec_mean'),
            partial(extract_player_events,
                    operator=extract_player_events_time_of_day_std,
                    prefix='time_of_day_in_sec_std')
        ],
        counter = counter
    )
...
def generate(filename, events, churn_days, last_minutes, last_hours, 
                last_days, last_weeks, last_months, seed, debug):
    ...
    featues_options = FeaturesOptions(
        churn_days,
        last_minutes,
        last_hours,
        last_days,
    ...
    features_dataframe = generate_player_features(events_dataframe, featues_options)
