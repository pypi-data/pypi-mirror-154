...
import click
from pygdg.common import PlayerEventField, PlayerEventType
...
@main.command(help=f'''
Generate game events of types ({','.join(PlayerEventType.names())}) with fields ({','.join(PlayerEventField.names())}) in a specified csv file name (default={DEFAULT_EVENTS_FILENAME}).
''')
...
@click.option('--plot/--no-plot', default=DEFAULT_PLOT, help='The plot flag (default=False)')
@click.option('--debug/--no-debug', default=DEFAULT_DEBUG, help='The debug flag (default=False)')
...
@main.command(help=f'''
Generate machine learning features ({','.join(f.FeatureName.names())}) with variants ({','.join(f.FeatureVariant.names())}) for each event type ({','.join(f.PlayerEventType.names())}) in a specified csv file name (default={DEFAULT_FEATURES_FILENAME})
''')
