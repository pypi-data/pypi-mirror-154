...
def score(dag, data, score_function='bic', alpha=0.3):
    if type(dag) is str:
    ...
    for var in varnames:
        ...
        if score_function in ['nal', 'nal_g']:
            sco += globals()[score_function](data, np.asarray(cols), alpha)
        else:
            try:
                sco += globals()[score_function](data, np.asarray(cols))
...
def nal(data, cols, alpha=0.3, mul=1):
    arities = np.amax(data, axis=0) + 1
    data_local = np.delete(data, data[:, cols].min(axis=1) < 0, axis=0)
    if data_local.shape[0] == 0:
        return -np.inf
    else:
        return nal_counter(data_local, arities, cols, alpha, mul)
...
@njit(fastmath=True)
def nal_counter(data, arities, cols, alpha=0.3, mul=1):
    strides = np.empty(len(cols), dtype=np.uint32)
    idx = len(cols) - 1
    stride = 1
    while idx > -1:
    ...
    nal /= np.sum(N_ijk)
    if alpha == 1:
        nal -= 0.5 * np.log(data.shape[0]) / data.shape[0] * (arities[cols[0]] - 1) * strides[0]
    else:
        nal -= mul / data.shape[1] * np.power(data.shape[0], -alpha) * (arities[cols[0]] - 1) * strides[0]
    return nal
...
def local_score(data, cols, score_function='default', weights=None, alpha=0.3, mul=1):
    '''
    ...
    :param alpha: alpha parameter for NAL score
    :param mul: multiplicator used for NAL score
    ...
    if len(data) == 0:
        ...
        if score_function in ['nal', 'nal_g']:
            ls = globals()[score_function](data, np.asarray(cols), alpha, mul)
        else:
            try:
