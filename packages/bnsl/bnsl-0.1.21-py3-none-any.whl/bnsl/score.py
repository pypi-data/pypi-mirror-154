...
def nal(data, cols, alpha=0.3):
    ...
    if data_local.shape[0] == 0:
        ...
        return nal_counter(data_local, arities, cols, alpha)
...
@njit(fastmath=True)
def nal_counter(data, arities, cols, alpha=0.3):
    strides = np.empty(len(cols), dtype=np.uint32)
    idx = len(cols) - 1
    stride = 1
    while idx > -1:
    ...
    if alpha == 1:
        ...
        nal -= 1 / data.shape[1] * np.power(data.shape[0], -alpha) * (arities[cols[0]] - 1) * strides[0]
    return nal
