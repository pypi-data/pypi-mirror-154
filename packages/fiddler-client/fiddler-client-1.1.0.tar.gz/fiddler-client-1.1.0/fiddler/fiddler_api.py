...
class FiddlerApi:
    ...
    def initialize_monitoring(  # noqa
        ...
        :param model_id:            The model for which to initialize monitoring.
        :param enable_modify:       [DEPRECATED - only fiddler1] Grant the Fiddler backend permission to
                                    ...
                                    will be read-only.
        :param verbose:             [DEPRECATED - only fiddler1] Bool indicating whether to run in verbose
                                    mode with longer debug messages for errors.
        :param is_sync:             [NEW - only fiddler2] Bool indicating if monitoring must be initialized
        ...
        """
        return 'Initialize monitoring is only supported from v2'

        # @todo: the below method needs to be called if we want init monitoring for v1 client and that needs server side fixing.

        # resp = Monitoring(self.connection, project_id, model_id).initialize_monitoring(
        #     enable_modify, verbose, version
        # )
