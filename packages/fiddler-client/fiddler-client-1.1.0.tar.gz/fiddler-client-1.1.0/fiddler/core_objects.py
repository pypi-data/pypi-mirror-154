...
class WeightingParams:
    ...
    :param sample_weight_column: column from the baseline data that should be used for weighting. Only one of
        class_weight or sample_weight_column may be specified.
    ...
    :param weighted_production_histograms: Flag indicating if production histograms must be weighted or not
        when calculating drift metrics.
    :param weighted_reference_histograms: Flag indicating if weighting scheme should be used when training the
        surrogate model.
    ...
    weighted_production_histograms: bool = True
    weighted_surrogate_training: bool = True

    def __post_init__(self):
        # raise an error if neither or both class and sample weighting is specified
        if self.class_weight is None and self.sample_weight_column is None:
            raise ValueError(
                'Need to specify one of class_weight or sample_weight_column'
            )
        if self.class_weight is not None and self.sample_weight_column is not None:
            raise ValueError(
                (
                    'Both class_weight and sample_weight_column have been specified. '
        ...
        if self.class_weight is not None:
            ...
            try:
                self.class_weight = [float(w) for w in self.class_weight]
            except ValueError:
                raise ValueError('Expected class_weight to be a list of floats')
        if self.sample_weight_column is not None:
            if not isinstance(self.sample_weight_column, str):
                raise ValueError(
                    f'Expected sample_weight_column to be of string type received {type(self.sample_weight_column)}'
                )
        return
...
class ModelInfo:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        available_cols = [column.name for column in self.inputs]
        if self.metadata is not None:
            available_cols += [column.name for column in self.metadata]
        if self.targets is not None:
            available_cols += [column.name for column in self.targets]
