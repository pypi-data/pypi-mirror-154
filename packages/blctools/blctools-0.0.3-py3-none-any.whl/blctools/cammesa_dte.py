...
def nombre(fecha):
    return fecha.strftime('DTE%y%m')

def nombres(fecha_ini, fecha_fin):
    fecha_fin = __fechas.sumar_mes(fecha_fin)
    iterable = __fechas.iterar_mensual(fecha_ini, fecha_fin)    
    
    return [nombre(fechas[0]) for fechas in iterable]

def fecha_archivo(nombre):
    '''Toma un nombre de archivo DTE y devuelve un objeto Datetime con la fecha a la que corresponde'''
    nombre = nombre.split('.')[0]
    
    return __dt.datetime.strptime(nombre,'DTE%y%m')

def disponibles():
    '''Devuelve el nombre de todos los archivos disponibles en la carpeta MDB de los DTE'''
    dir_mdbs = __dir.get_dc_dte() + '\\01 MDB'
...
def descargar(fecha_ini, fecha_fin,exportar_consulta=False,dir_consulta=None):
    ...
    dir_descarga = __dir.get_dc_dte() + '\\00 ZIP'
...
def extraer():
    ...
    dir_zips = __dir.get_dc_dte() + '\\00 ZIP'
    dir_extraccion = __dir.get_dc_dte() + '\\01 MDB'
    __dir.extraer(dir_zips, dir_extraccion,extension='mdb')

def cargar(fecha_ini,fecha_fin,parques,descargar=False):
    '''Función que realiza la consulta en CAMMESA por DTES en un rango de fechas, descarga los archivos .zip,
    extrae los archivos .mdb dentro de los archivos .zip, 
    filtra la tabla DTE seleccionada según el listado de parques provisto y 
...
def a_excel(df,dir_out,parques):
    '''
