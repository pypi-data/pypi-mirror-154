...
class LzyEnv(gym.Env):
    ...
    def _get_obs(self):
        return {"state":np.concatenate([self._get_pos(),self._get_pred()[0],self._get_normal()],0)}
    ...
    def _get_pred(self):
        return np.array(self._pred.iloc[self._day])

    def _get_normal(self):
        return np.array(self._normal_prices.iloc[self._day])
