...
class LzyEnv(gym.Env):
    ...
    def _get_obs(self):
        return {"pos":torch.from_numpy(self._position),
                "pred":torch.from_numpy(self._pr),
                "price":torch.from_numpy(self._price),
                "normal":torch.from_numpy(self._normal)}
    ...
    def _get_normal(self):
        return np.array(torch.tensor([self._normal_prices.iloc[self._day]]))

    def _get_price(self):
        return np.array(torch.tensor([self._real_price[self._day]]))
    ...
    def step(self, action):
        ...
        self._day += 1
        self._asset = self._asset + self._asset*(self._position[0])*(self._get_price()-past_price)/past_price
