...
class LzyEnv(gym.Env):
    ...
    def _get_obs(self):
        return {"pos":torch.tensor([self._position]).numpy(),
                "pred":torch.tensor([self._pred.iloc[self._day]]).numpy(),
                "price":torch.tensor([self._real_price.iloc[self._day]]).numpy(),
                "normal":torch.tensor([self._normal_prices.iloc[self._day]]).numpy()}
    ...
    def _get_normal(self):
        return torch.tensor([self._normal_prices.iloc[self._day]]).numpy()

    def _get_price(self):
        return torch.tensor([self._real_price[self._day]]).numpy()

    def reset(self, return_info=False, options=None):
        self._asset = 100
        self._day = 0
        self._position = torch.tensor([0]).numpy()
    ...
    def step(self, action):
        ...
        self._position = self._action_to_position[action].numpy()
        self._day += 1
