...
class LzyEnv(gym.Env):
    ...
    def __init__(self, pred_df ,normal_prices, real_price):
        ...
        self._action_to_position = {
            ...
            1: torch.tensor([0.25,0]),
            2: torch.tensor([0.50,0]),
            3: torch.tensor([0.75,0]),
            4: torch.tensor([1.0,0]),
        }
    ...
    def _get_obs(self):
        return {"pos":torch.from_numpy(self._position),
                "pred":torch.from_numpy(self._get_pred()),
                "price":torch.from_numpy(self._get_price()),
                "normal":torch.from_numpy(self._get_normal())}
    ...
    def _get_pred(self):
        return np.array(torch.tensor([self._pred.iloc[self._day]]))

    def _get_normal(self):
        return np.array(torch.tensor([self._normal_prices.iloc[self._day],0]))

    def _get_price(self):
        return np.array(torch.tensor([self._real_price[self._day],0]))
    ...
    def step(self, action):
        ...
        self._day += 1
        self._asset = self._asset + self._asset*(self._position[0])*(self._get_price()[0]-past_price[0])/past_price[0]
