...
class TADBoostAug:
    ...
    def load_augmentor(self, arg):
        if isinstance(arg, TADTextClassifier):
            ...
            self.MLM, self.tokenizer = get_mlm_and_tokenizer(self.tad_classifier, self.tad_classifier.opt)
    ...
    def tad_boost_augment(self, config: ConfigManager,
        ...
        for fold_id, b_idx in enumerate(range(len(folds))):
            ...
            self.MLM, self.tokenizer = get_mlm_and_tokenizer(self.tad_classifier, _config)
...
def prepare_dataset_and_clean_env(dataset, task, rewrite_cache=False):
    ...
    #     rename('integrated_datasets', 'integrated_datasets_bak')
    # else:
    #     download_datasets_from_github('integrated_datasets_bak')

    backup_datasets_dir = find_dir('integrated_datasets_bak', key=[dataset, task], disable_alert=True, recursive=True)

    datasets_dir = 'integrated_datasets/'+backup_datasets_dir.strip('integrated_datasets_bak').strip('\\').strip('/')
...
def download_datasets_from_github(save_path='integrated_datasets'):
    if find_files(save_path, key=['__init__.py'], exclude_key='.git'):
        return
