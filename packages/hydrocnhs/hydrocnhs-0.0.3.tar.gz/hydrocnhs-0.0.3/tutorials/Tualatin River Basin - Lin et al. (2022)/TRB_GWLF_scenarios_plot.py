...
prec_M = pd.read_csv(os.path.join(wd,"Data","Pet_cm.csv"), index_col=["Date"],
                     parse_dates=["Date"]).resample("MS").mean()
...
model_urban["ABM"]["Drain_AgtType"] = {
    "DrainAgt1": {"Attributes": {},
                  "Inputs": {"Priority": 1,
                             "Links": {"RCTV": 1},
                             "DMClass": None},
                  "Pars": {}},
    "DrainAgt2": {"Attributes": {},
                  "Inputs": {"Priority": 1,
                             "Links": {"WSLO": 1},
                             "DMClass": None},
                  "Pars": {}}}

HydroCNHS.write_model(model_urban, os.path.join(wd, folder, "Calibrated_TRB_GWLF_urban.yaml"))

# Fixed diversion scenario
model_fixed_div = deepcopy(model)
model_fixed_div["WaterSystem"]["ABM"]["DMClasses"] = ["ReleaseDM", "TransferDM", "FixedDivertDM"]
...
HydroCNHS.write_model(model_fixed_div, os.path.join(wd, folder, "Calibrated_TRB_GWLF_fixedDiv.yaml"))
#%%
##### Urbanization scenario
filename = os.path.join(wd, folder, "Calibrated_TRB_GWLF.yaml")
model = HydroCNHS.load_model(filename)
