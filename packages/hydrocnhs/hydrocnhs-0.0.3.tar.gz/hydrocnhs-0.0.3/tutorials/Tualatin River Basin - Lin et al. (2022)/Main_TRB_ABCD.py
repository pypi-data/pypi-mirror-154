...
# Here we have seven subbasins and we select ABCD as the rainfall-runoff model.
...
mb.set_rainfall_runoff(outlet_list=outlet_list,area_list=area_list,
                       lat_list=lat_list, runoff_model="ABCD")
...
mb.add_agent(agt_type_class="Reservoir_AgtType", agt_name="ResAgt",
             ...
             dm_class="ReleaseDM")
mb.add_agent(agt_type_class="Diversion_AgtType", agt_name="DivAgt",
             api=mb.api.RiverDiv,
             link_dict={"TRGC": -1, "WSLO": ["ReturnFactor", 0, "Plus"]},
             dm_class="DivertDM",
             par_dict={"ReturnFactor": [-99], "a": -99, "b":-99})
mb.add_agent(agt_type_class="Pipe_AgtType", agt_name="PipeAgt",
...
mb.write_model_to_yaml(filename="TRB_ABCD.yaml")
...
# Load model
model_dict = HydroCNHS.load_model(os.path.join(wd, "TRB_ABCD.yaml"))
...
for seed in seeds:
    ...
    HydroCNHS.write_model(model_best,
                          os.path.join(ga.cali_wd, "Calibrated_TRB_ABCD.yaml"))
    summary = ga.summary
#%% ===========================================================================
# Step 5: Run a simulation
# =============================================================================
### Run a simulation.
folder = "Cali_abcd_abm_KGE_10"
...
axes = axes.flatten()
x = sim_Q_M.index
axes[0].plot(x, sim_Q_M["DLLO"], label="$M_{abcd}$")
axes[1].plot(x, sim_Q_M["WSLO"], label="$M_{abcd}$")
axes[2].plot(x, sim_Q_M["ResAgt"], label="$M_{abcd}$")
axes[3].plot(x, sim_Q_M["DivAgt"], label="$M_{abcd}$")
