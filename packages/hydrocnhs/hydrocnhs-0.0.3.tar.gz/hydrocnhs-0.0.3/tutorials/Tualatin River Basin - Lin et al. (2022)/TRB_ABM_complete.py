...
class Reservoir_AgtType(Base):
    ...
    def act(self, outlet):
        ...
        if factor <= 0:
            ...
            res_t = self.dm.make_dm(self.dc.Q_routed["HaggIn"][self.t],
                                    self.current_date)
            action = res_t
        return action
    ...
    def act(self, outlet):
        ...
        if factor < 0:  # Diversion
            ...
            if self.current_date.day == 1:
                div_req = self.dm.make_dm(self.pars["a"], self.pars["b"],
                                          self.current_date)
    ...
    def act(self, outlet):
        ...
        if factor <= 0:
            ...
            if self.current_date.year < 1991:
                ...
                # Historical inputs.
                Res_t = self.obv_data[self.current_date.year-1991,
...
class Drain_AgtType(Base):
    ...
    def act(self, outlet):
        # Get factor
        factor = read_factor(self.config, outlet)
        Qt_change = self.urbanized_ratio[self.current_date.year-1981] * 0.75 \
...
class DivertDM(Base):
    ...
    def make_dm(self, a, b, current_date):
        prec_M_mean = self.db["Prec_M_mean"][current_date.year-1981,
