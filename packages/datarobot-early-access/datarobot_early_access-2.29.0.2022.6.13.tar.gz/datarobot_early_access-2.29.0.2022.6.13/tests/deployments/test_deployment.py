...
def assert_deployment(deployment, deployment_data):
    ...
    assert deployment.status == deployment_data["status"]
...
class TestDeploymentChangeStatus:
    @pytest.fixture()
    def get_deployment_url(self, unittest_endpoint, deployment_data):
        return "{}/deployments/{}/".format(unittest_endpoint, deployment_data["id"])

    @pytest.fixture()
    def active_deployment_get_response(self, get_deployment_url, deployment_data):
    ...
    @pytest.fixture()
    def inactive_deployment_get_response(self, get_deployment_url, deployment_data):
        deployment_data["status"] = "inactive"
        responses.add(
            responses.GET,
            get_deployment_url,
            status=200,
    ...
    @pytest.fixture()
    def deployment_status_patch_response(
        self, unittest_endpoint, deployment_data, get_status_url, get_deployment_url
    ):
        url = "{}/{}/{}/status/".format(unittest_endpoint, "deployments", deployment_data["id"])
        responses.add(
            responses.PATCH,
            url,
    ...
    @responses.activate
    @pytest.mark.usefixtures("deployment_status_patch_response", "active_deployment_get_response")
    def test_activate_deployment(self, deployment_data):
        deployment = Deployment(id=deployment_data["id"])
        deployment.activate()
        assert deployment.status == "active"

    @responses.activate
    @pytest.mark.usefixtures("deployment_status_patch_response", "inactive_deployment_get_response")
    def test_deactivate_deployment(self, deployment_data):
        deployment = Deployment(id=deployment_data["id"])
        deployment.deactivate()
        assert deployment.status == "inactive"
