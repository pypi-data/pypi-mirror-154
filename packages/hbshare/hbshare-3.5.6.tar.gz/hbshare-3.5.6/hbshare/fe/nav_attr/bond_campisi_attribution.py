...
class BondCampisiAttributionOutputs:
    def __init__(self):
        self.params_df = None
        self.p_value_df = None
        self.r_square_series = None
        self.total_return_series = None
        self.attr_return_df = None

    def to_dict(self):
        return {
            "params_df": self.params_df,
            "p_value_df": self.p_value_df,
            "r_square_series": self.r_square_series,
            "total_return_series": self.total_return_series,
            "attr_return_df": self.attr_return_df,
        }
...
class BondCampisiAttribution:
    def __init__(self, fund_id, start_date, end_date, frequency, backup_period='整个季度'):
        ...
        self.backup_period_days = {'整个季度': 200, '近三月': 200, '近六月': 200, '近一年': 400, '近三年': 1200}
    ...
    def _verify_input_param(self):
        ...
        if self.backup_period not in ['近三月', '近六月', '近一年', '近三年', '整个季度']:
    ...
    def _load_data(self):
        ...
        if self.frequency != 'season':
            ...
            self.calendar_df = pd.concat([self.calendar_df, backup_df[['近三月', '近六月', '近一年', '近三年']]], axis=1)
    ...
    def _init_bond_campisi_attribution_data(self):
        ...
        if not (self.fund_nav_series.empty or self.factor_data_df.empty):
            ...
            for date in compute_list:
                ...
                need_length = 60 if self.backup_period == '近三月' else 120 if self.backup_period == '近六月' else 250 if self.backup_period == '近一年' else 750 if self.backup_period == '近三年' else 60
    ...
    def _campisi_analysis(self):
        ...
        params_list, p_value_list, r_square_list, attr_return_list, total_return_list = [], [], [], [], []
        for date, period_data in self.preprocessed_data.items():
            ...
            if period_data['fund_ret_series'].empty or period_data['factor_data_df'].empty:
                ...
                params = pd.DataFrame(index=[date], columns=['CONST'] + factor_list)
                p_value = pd.DataFrame(index=[date], columns=['CONST'] + factor_list)
                r_square = pd.Series(index=[date])
                attr_return = pd.DataFrame(index=[date], columns=['SPECIAL'] + factor_list)
                total_return = pd.Series(index=[date])
            else:
                ...
                r_square = pd.Series(result.rsquared)
                r_square.index = [date]
                ...
                total_return = pd.Series(index=[date], data=[total_return_data])
                ...
                attr_return = params[factor_list] * factor_ret_df[factor_list]
        ...
        r_square_series = pd.concat(r_square_list)
        total_return_series = pd.concat(total_return_list)
        attr_return_df = pd.concat(attr_return_list)
        return params_df, p_value_df, r_square_series, total_return_series, attr_return_df

    def get_all(self):
        output = BondCampisiAttributionOutputs()
        params_df, p_value_df, r_square_series, total_return_series, attr_return_df = self._campisi_analysis()
        output.params_df = params_df
        output.p_value_df = p_value_df
        output.r_square_series = r_square_series
        output.total_return_series = total_return_series
        output.attr_return_df = attr_return_df
        return output.to_dict()


if __name__ == '__main__':
    # # 分析样本：成立满6个月的纯债型、混合型债券基金
    ...
    # bond_fund['FUND_TYPE'] = bond_fund['FUND_TYPE'].replace({'22': '短期纯债型', '23': '混合债券型一级', '24': '混合债券型二级', '27': '中长期纯债型'})
