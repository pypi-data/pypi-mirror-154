#!/usr/bin/python
# coding:utf-8
from hbshare.quant.CChen.load_data import load_funds_data, load_funds_alpha
from hbshare.quant.CChen.fund_perf import ret
import pandas as pd
import numpy as np
...
from hbshare.quant.CChen.load_hsjy import load_fut_fin
...
def gzb_ts_positions(data, width=800, height=600):
    ...
    data_value = data.groupby(['日期'], as_index=False)['绝对占比'].sum().rename(columns={'绝对占比': '总比'})
    data_value_by_sector = data.groupby(['日期', '板块'], as_index=False)['绝对占比'].sum()
    ...
    data_value_net_by_sector = data.groupby(['日期', '板块'], as_index=False)['市值占净值'].sum()

    sectors = data['板块'].drop_duplicates().tolist()
    ...
    web2.set_series_opts(
        ...
        title_opts=opts.TitleOpts(title=data['基金名称'][0] + " 持仓分布"),
        legend_opts=opts.LegendOpts(
            orient="vertical",
            pos_top="5%",
            # pos_left='2%',
...
def gzb_xs_position(data, width=800, height=600):
    ...
    outer_data = data[['科目名称', '绝对占比']].reset_index(drop=True)
    outer_data_pair = [
