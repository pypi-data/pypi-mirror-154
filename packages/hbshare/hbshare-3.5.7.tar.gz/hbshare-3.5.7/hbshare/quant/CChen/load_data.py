...
def load_calendar(start_date=None, end_date=None, freq='', db_path='', table='stocks_index_ts'):
    ...
    try:
        # HB内部网络环境下优先读取内网数据
        # print('Loading trade calendar from HB db')
        ...
        # HB内部网络连接失败情况下读取自建数据
        # print('Failed to connect to HB db, trying loading trade calendar from local db')
...
def load_gzb(name, start_date, end_date, db_path, table='gzb', p_diff=True):
    ...
    data['品种'] = data['科目代码'].apply(lambda x: re.findall(r'[a-zA-Z]+', x[8:])[0].upper())
    if p_diff:
        # 同品种轧差
        # 考虑跨期套利时可能需要设置为False
        data_p = data[['科目名称', '品种']]
        data_p = data_p.drop_duplicates(subset=['品种']).sort_values(by='品种')
        data_p['科目名称'] = data_p['科目名称'].apply(lambda x: x.rstrip('0123456789'))
        data = data.groupby(['日期', '品种'], as_index=False).sum()
        data['基金名称'] = name
        data = data.merge(data_p, on='品种', how='left')
    sec_df = pd.DataFrame()
