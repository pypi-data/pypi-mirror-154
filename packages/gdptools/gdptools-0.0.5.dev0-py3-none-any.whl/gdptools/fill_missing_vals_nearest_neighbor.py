...
def fill_onhm_ncf(
    ...
    Args:
        ...
        mfile (str, optional): _description_. Defaults to None.
        genmap (bool, optional): _description_. Defaults to False.
    """
    odir = Path(output_dir)
    if not odir.exists:
        print(f"Path: {odir} does not exist")
        exit
    data = xarray.open_dataset(nfile, engine="netcdf4")
    ...
    if genmap:
        ...
        df_mask = data_1d.where(data[var] >= netCDF4.default_fillvals["f8"], drop=True)
        ...
        df_filled = data_1d.where(data.tmax < netCDF4.default_fillvals["f8"], drop=True)
