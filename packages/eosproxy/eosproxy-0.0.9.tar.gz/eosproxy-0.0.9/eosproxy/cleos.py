...
from .typeseos import EOSEncoder, Transaction, PackedTransaction, Abi
...
class Cleos:
    ...
    # get methods
    #####
    def get_info(self, timeout=30, proxies=None):
        ''' '''
        return self.get('chain.get_info', timeout=timeout, proxies=proxies)

    def get_chain_lib_info(self, timeout=30, proxies=None):
        ''' '''
        chain_info = self.get('chain.get_info', timeout=timeout, proxies=proxies)
        lib_info = self.get_block(chain_info['last_irreversible_block_num'], timeout=timeout, proxies=proxies)
        return chain_info, lib_info

    def get_block(self, block_num, timeout=30, proxies=None):
        ''' '''
        return self.post('chain.get_block', params=None, json={'block_num_or_id': block_num}, timeout=timeout,
                         proxies=proxies)

    def get_account(self, acct_name, timeout=30, proxies=None):
        ''' '''
        return self.post('chain.get_account', params=None, json={'account_name': acct_name}, timeout=timeout,
                         proxies=proxies)

    def get_code(self, acct_name, code_as_wasm=True, timeout=30, proxies=None):
        ''' '''
        return self.post('chain.get_code', params=None, json={'account_name': acct_name, 'code_as_wasm': code_as_wasm},
                         timeout=timeout, proxies=proxies)

    def get_accounts(self, public_key, timeout=30, proxies=None):
        ''' '''
    ...
    def get_actions(self, acct_name, pos=-1, offset=-20, timeout=30, proxies=None):
        '''
        POST /v1/history/get_actions
        {"account_name":"eosnewyorkio","pos":-1,"offset":-20}
        '''
        json = {'account_name': acct_name, "pos": pos, "offset": offset}
        return self.post('history.get_actions', params=None, json=json, timeout=timeout, proxies=proxies)
    ...
    def get_currency_balance(self, account, code='eosio.token', symbol='EOS', timeout=30, proxies=None):
        '''
        ...
        return self.post('chain.get_currency_balance', params=None, json=json, timeout=timeout, proxies=proxies)
    ...
    def get_servants(self, acct_name, timeout=30):
        ''' '''
        return self.post('account_history.get_controlled_accounts', params=None,
                         json={'controlling_account': acct_name}, timeout=timeout)
    ...
    def get_table(self, code, scope, table, index_position='', key_type='', lower_bound='', upper_bound='', limit=10,
        ...
        '''
        json = {"json": True, "code": code, "scope": scope, "table": table, "key_type": key_type,
                "index_position": index_position, "lower_bound": lower_bound, "upper_bound": upper_bound,
                "limit": limit}
        return self.post('chain.get_table_rows', params=None, json=json, timeout=timeout, proxies=proxies)
    ...
    def get_producers(self, lower_bound='', limit=50, timeout=30):
        ...
        '''
        return self.post('chain.get_producers', params=None,
                         json={'json': True, 'lower_bound': lower_bound, 'limit': limit}, timeout=timeout)
    ...
    def push_transaction(self, transaction, keys, broadcast=True, compression='none', timeout=30, proxies=None):
        ''' parameter keys can be a list of WIF strings or EOSKey objects or a filename to key file'''
        chain_info, lib_info = self.get_chain_lib_info(proxies=proxies)
        ...
        if broadcast:
            return self.post('chain.push_transaction', params=None, data=data, timeout=timeout, proxies=proxies)
        return data
    ...
    def abi_json_to_bin(self, code, action, args, timeout=30, proxies=None):
        ''' '''
        json = {'code': code, 'action': action, 'args': args}
        # print(json)
        return self.post('chain.abi_json_to_bin', params=None, json=json, timeout=timeout, proxies=proxies)
    ...
    def vote_producers(self, voter, proxy, producers):
        return self.get('chain.abi_json_to_bin', params=None,
                        json={"voter": voter, "proxy": proxy, "producers": producers})
    ...
    def create_account(self, creator, creator_privkey, acct_name, owner_key,
        ...
        newaccount_data = self.abi_json_to_bin('eosio', 'newaccount',
                                               {'creator': creator, 'name': acct_name, 'owner': owner_auth,
                                                'active': active_auth})
        ...
        delegate_data = self.abi_json_to_bin('eosio', 'delegatebw',
                                             {'from': creator, 'receiver': acct_name, 'stake_net_quantity': stake_net,
