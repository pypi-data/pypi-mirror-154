...
# Author: Clay Risser
# -----
# Last Modified: 07-06-2022 11:19:49
...
class GTKlara:
    ...
    def __init__(self, children=[], props={}, kind=None):
        self.__cleanup = []
        self._ready = False
        self._active = None
        self._edges = []
    ...
    def _get(self, id):
        ...
        if isinstance(id, GTKlara):
            ...
            widget = self._builder.get_object(id) if type(id) is str else id
        if widget:
            return widget
    ...
    def _set_signal(self, name, value, id=None, z=None):
        if not id:
            id = self._active
        if not id:
            return
        ...
        if prop_name in self._props:
            widget.connect(signal_name, value)
