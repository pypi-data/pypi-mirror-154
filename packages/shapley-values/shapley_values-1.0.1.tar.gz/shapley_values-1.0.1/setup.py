...
NAME = "shapley_values"
URL = "https://github.com/LIADN7/shapley_values"
HERE = pathlib.Path(__file__).parent
print(f"\nHERE = {HERE.absolute()}\n")
README = (HERE / "README.md").read_text()
REQUIRES = (HERE / "requirements.txt").read_text().strip().split("\n")
REQUIRES = [lin.strip() for lin in REQUIRES]
print(f'\nVERSION = {(HERE / "shapley_values" / "VERSION").absolute()}\n')
VERSION = (HERE / "shapley_values" / "VERSION").read_text().strip()
...
setuptools.setup(
    ...
    author='Liad Nagi',
    author_email='liadnagi@gmail.com',
    description='Shapley values package',
