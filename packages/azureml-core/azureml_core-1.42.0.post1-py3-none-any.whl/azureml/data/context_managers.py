...
class DatasetContextManager(_CommonContextManager):
    ...
    def _mount_write(self, name, config):
        ...
        if is_single_file or asset_type:
            ...
            output_uri = self._construct_datastore_uri(destination[0], destination[1])
    ...
    def _upload(self, name, config):
        ...
        def upload_asset(src_path, destination, single_file_name, overwrite):
            from azureml.dataprep.rslex import Copier, PyLocationInfo, PyIfDestinationExists
            from azureml._common.exceptions import AzureMLException
            ...
            try:
                arguments = {
                    'subscription': self._workspace.subscription_id,
                    'resourceGroup': self._workspace.resource_group,
                    'workspaceName': self._workspace.name,
                    'datastoreName': destination[0].name
                }
                dest_si = PyLocationInfo(
                    'AmlDatastore',
                    destination[1],
                    arguments)
        ...
        try:
            ...
            if asset_type is None:
                ...
                upload_asset(src_path, upload_destination, single_file_name, overwrite)
        except Exception as e:
    ...
    def _get_datastore_and_path(self, config):
        ...
        if asset_type:
            ...
            if datastore_uri_match is None:
                raise UserErrorException("Only short form datastore uri is support for asset output")
