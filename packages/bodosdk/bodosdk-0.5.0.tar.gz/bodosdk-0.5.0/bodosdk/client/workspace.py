...
from bodosdk.models import WorkspaceDefinition, WorkspaceCreatedResponse, GetWorkspaceResponse
...
class WorkspaceClient:
    ...
    def create(self, workspace_definition: WorkspaceDefinition) -> WorkspaceCreatedResponse:
        """
        Creates workspace
    ...
    def remove(self, uuid: str, mark_as_terminated: bool = False):
        ...
        :type uuid: str
        :param mark_as_terminated: if True workspace resources won't be removed, workspace will be marked as terminated only
        :type mark_as_terminated: bool
        """
        return self._api.remove_workspace(uuid, mark_as_terminated)

    def list(self) -> List[GetWorkspaceResponse]:
        """
    ...
    def get(self, workspace_uuid: str) -> GetWorkspaceResponse:
        """
        Get workspace for specific uuid
        :param workspace_uuid:
        :return:
        """
        return self._api.get_workspace(workspace_uuid)

    def assign_users(self, workspace_uuid: Union[str, UUID], users: List[UserAssignment]):
        """
