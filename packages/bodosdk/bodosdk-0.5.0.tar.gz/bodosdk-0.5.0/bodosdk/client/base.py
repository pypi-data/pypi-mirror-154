...
class BodoOrganizationClient:
    ...
    def __init__(self, auth_api: AuthApi, workspace_client: WorkspaceClient, cloud_config_client: CloudConfigClient):
        self._auth_api = auth_api
        self.workspace = workspace_client
        self.cloud_config = cloud_config_client


def get_bodo_client(auth: APIKeys = None, api_url="https://api.bodo.ai/api", auth_url='https://prod-auth.bodo.ai',
                    print_logs=False):
    """
...
def get_bodo_organization_client(auth: PersonalKeys, api_url="https://api.bodo.ai/api",
                                 auth_url='https://prod-auth.bodo.ai', print_logs=False):
    """
    :param auth: a set of client_id / seceret_key used for auth token generation
    ...
    if not auth:
        ...
        if client_id and secret_key:
            ...
            raise APIKeysMissing
    auth_api = AuthApi(auth, auth_url, RequestWrapper(print_logs))
    workspace_client = WorkspaceClient(WorkspaceApi(auth_api, api_url, RequestWrapper(print_logs)))
    cloud_config_client = CloudConfigClient(CloudConfigApi(auth_api, api_url, RequestWrapper(print_logs)))
