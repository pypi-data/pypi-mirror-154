...
from bodosdk.models.base import CamelCaseBase, BodoRole
...
class WorkspaceDefinition(CamelCaseBase):
    ...
    infra_config_uuid: Optional[int] = Field(None, alias='infraConfigUUID')
    aws_network_data: Optional[AWSNetworkData] = Field(None, alias='awsNetworkData')
    default_notebook_config: Optional[CreateUpdateNotebookConfig] = Field(None, alias='defaultNotebookConfig')
...
class WorkspaceInfo(CamelCaseBase):
    ...
    infra_config: Optional[Any] = Field(None, alias="infraConfig")
    data: Any
...
class WorkspaceCreatedResponse(WorkspaceInfo):
    ...
    cluster_credits_used_this_month: int = Field(None, alias='clusterCreditsUsedThisMonth')
    clusters: Any
    notebooks: Any
    jobs: Any


class GetWorkspaceResponse(CamelCaseBase):
    name: str
    uuid: str
    status: str
    provider: str
