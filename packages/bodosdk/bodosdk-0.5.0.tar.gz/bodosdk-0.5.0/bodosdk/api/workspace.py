...
from bodosdk.models import GetWorkspaceResponse, WorkspaceDefinition, WorkspaceCreatedResponse
...
class WorkspaceApi(BackendApi):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._resource_url = f"workspace"
    ...
    def create_workspace(self, workspace_definition: WorkspaceDefinition):
        ...
        resp = self._requests.post(f"{self.get_resource_url()}",
                                   data=workspace_definition.json(by_alias=True), headers=headers)
        if str(resp.status_code).startswith('2'):
            return WorkspaceCreatedResponse(**resp.json())
        if resp.status_code == 404:
            raise ResourceNotFound("Check if personal keys or the definition passed is correct.")
    ...
    def get_workspace(self, uuid):
        params = {
            "withTasks": str(False).lower()
        }
        response = self._requests.get(f"{self.get_resource_url()}", params=params, headers=self.get_auth_header())
        if str(response.status_code).startswith('2'):
            result = None
            for entry in response.json():
                if entry['uuid'] == uuid:
                    result = GetWorkspaceResponse(**entry)
                    break
            return result
    ...
    def list_all_workspaces(self, with_tasks: bool = False):
        params = {
            "withTasks": str(with_tasks).lower()
        }
        response = self._requests.get(f"{self.get_resource_url()}", params=params, headers=self.get_auth_header())
        if str(response.status_code).startswith('2'):
            result: List = []
            for entry in response.json():
                result.append(GetWorkspaceResponse(**entry))
            return result
        if response.status_code == 404:
            raise ResourceNotFound
        if response.status_code == 503:
    ...
    def remove_workspace(self, uuid, mark_as_terminated):
        ...
        response = self._requests.delete(f"{self.get_resource_url()}/{uuid}", params=params,
    ...
    def assign_users(self, uuid: Union[str, UUID], users: List[UserAssignment]):
        response = self._requests.post(f"{self.get_resource_url()}/{str(uuid)}/users",
