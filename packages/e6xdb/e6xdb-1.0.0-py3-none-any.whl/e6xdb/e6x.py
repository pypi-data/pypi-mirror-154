...
from e6xdb.datainputstream import DataInputStream, get_query_columns_info, read_rows_from_batch
...
class Cursor(common.DBAPICursor):
    ...
    def fetch_batch(self):
        ...
        self._batch = read_rows_from_batch(self._query_columns_description, dis)

    def fetchall(self):
        _logger.info("fetching all")
        return self._fetch_all()

    # def fetchoneOld(self):
    #     _logger.info("fetching one")
    #     if len(self._batch) == 0:
    #         self.fetch_batch()
    #         if len(self._batch) == 0:
    #             return None
    #     self.rows_count += len(self._batch)
    #     if self.rows_count <= 1000:
    #         row = self._batch.pop()
    #     else:
    #         row = None
    #         # reset row counter for next query execution
    #         self.rows_count = 0
    #     return row

    def fetchone(self):
        _logger.info("fetch returning the batch itself")
        if len(self._batch) == 0:
            self.fetch_batch()
            if len(self._batch) == 0:
                return None
        # in order to stop fetching next batch, we are limiting to one batch - 1000 records
        self.rows_count += len(self._batch)
        if self.rows_count <= 1000:
            row = self._batch
        else:
...
if __name__ == '__main__':
    connection = connect(host="44.203.152.210",
                         port=9000,
                         scheme="e6xdb",
                         username="admin",
                         database="s3a://e6-tpcds/parquet100gb_128/",
                         auth=None,
                         configuration=None,
                         kerberos_service_name=None,
                         password="admin",
    ...
    cursor.execute("SELECT c_first_name FROM store_returns AS sr, customer AS c WHERE c.c_customer_sk = sr.sr_customer_sk AND c.c_first_name = 'Susan' ORDER BY sr_returned_date_sk;")
