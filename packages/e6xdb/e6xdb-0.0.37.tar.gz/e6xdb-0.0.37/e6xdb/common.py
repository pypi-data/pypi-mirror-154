...
class DBAPICursor(with_metaclass(abc.ABCMeta, object)):
    ...
    def execute(self, operation, parameters=None):
        ...
        operation. Variables are specified in a database-specific notation (see the e6xdb's
    ...
    def fetchone(self):
        ...
        if not self._data:
            ...
            if self._rownumber == 1000:
                _logger.info("Commons - Breaking the loop after 1000 records")
                return None
            self._rownumber += 1
    ...
    def fetchmany(self, size=None):
        ...
        :py:meth:`execute` did not produce any result set or no call was issued yet.
        """
        _logger.info("Commons - fetch many.... size - {}".format(size))
        rowsToReturn = self.fetchone()
        if rowsToReturn is None:
            return None
        return list(rowsToReturn)
    ...
    def fetchall(self):
        ...
        :py:meth:`execute` did not produce any result set or no call was issued yet.
        """
        _logger.info("Commons - fetch all")
    ...
    def __next__(self):
        ...
        exhausted.
        """
        _logger.info("Commons - fetching the next row")
        one = self.fetchone()
        if one is None:
            raise StopIteration
        else:
