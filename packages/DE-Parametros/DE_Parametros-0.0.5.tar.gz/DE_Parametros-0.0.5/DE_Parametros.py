...
class PARAMETROS:
    ...
    def _update_row(self, row: dict, commit: bool = True):
        ...
        utl = U.LIB()
        try:
            dml =  f"""UPDATE parametros SET %s = '%s'" %(.join({row.keys()}), .join({row.values()}))"""
            dml =   f"""
                      Update parametros
                         set hash_parent        = :hash_parent          
                            ,num_ordem       	= :num_ordem       
                            ,des_familia        = :des_familia     
                            ,des_grupo          = :des_grupo       
    ...
    def FAMILIA_get(self, des_familia: list = None) -> dict:
        result, where_add = None, None
        try:
            utl = U.LIB()
            now = dt.datetime.now()
            separador_listas = "|"
            if des_familia is None:
                where_add = ""
            else:
                where_add = f"""and p.des_familia in ('{"','".join(des_familia)}')"""
            stmt =  f"""
                    Select *
                      from parametros p
            ...
            for row in rs:
                ...
                elif row["des_datatype"] == "JSON/CRIPTOGRAFADO":
                    token_string = self.VARIAVEL_get(["GERAL_TOKEN_BASE"])[0]
                    json_string = utl.CRYPTOGRAPHY(word=row["val_parametro"], token=token_string)
                    row["val_parametro"] = ast.literal_eval(json_string)
            ...
            rs = f"""Falha na obtenção dos parametros para a FAMILIA de PARAMETROS: {des_familia}.\nErro: {error}"""
        finally:
            return rs
    ...
    def REPLICA_FAMILIA(self, des_familia_origem: str, des_familia_destino: str, commit: bool = True):
        try:
            stmt = f"""Select *
                         from parametros
    ...
    def _oracle_DDL(self):
        try:
            stmt = f"""
                    ...
                    des_familia         VARCHAR (30)   NOT NULL,
