...
def loop_steps(context: Context, model, steps: dict, index: int):
    ...
    logger.info('Loop Start -step_range [{}-{}] -count {}'.format(lp.range.start, lp.range.end, lp.count))
    context.runtime.loop_total_times = lp.count
    for i in range(lp.count):
        logger.info('NO[{}]loop Total[{}]loops'.format(i + 1, lp.count))
        blank = 0
