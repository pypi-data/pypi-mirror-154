...
def handle_encode_frames(context: Context, encode_frame_parse: dict):
    """
    处理组帧
    """
    parses = {}
    ...
    for pos, parse in parses.items():
        frames[pos] = command.encode(context, parse, cache='{}#{}'.format(assist.cache.name, pos))
...
def handle_decode_frames(context: Context, app_return_result: dict):
    ...
    for pos, parse in parses.items():
        framing_frames[pos] = __handle_framing(context, pos, parse)

    if framing_frames is not None and len(framing_frames) > 0:
        handle_decode_frames(context, __send_next(context, framing_frames))
    # 分帧处理结束
