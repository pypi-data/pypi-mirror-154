...
class Client(object):
    ...
    def connect(self):
        self.s.connect((CONFIG.get('EM', 'ip'), CONFIG.get('EM', 'port')))
...
def negotiate(m_sn: str, m_counter: int):
    ...
    return {"em": {"rn": rn, "data": data, "mac": mac}}


def verify_session(rn, m_sn, m_rn, m_mac):
    command = 'UP698002:05;00010008001000300004;00{}{}{}{}'.format(m_sn.upper(), rn.upper(), m_rn.upper(), m_mac.upper())
    result = Client().send(command)
    iv = result.split(';')[3]
