...
python_path=$(python -m site --user-site)
...
if [[ $(which k3s) && $(sudo k3s --version) ]]; then
         ...
         sudo k3s kubectl create -f $python_path/src/scripts/k8s-services/rancher-k3s/dashboard.admin-user.yml -f $python_path/src/scripts/k8s-services/rancher-k3s/dashboard.admin-user-role.yml
