...
class emerge:
  ...
  def __init__(
    ...
    self.environment = 'test.' if environment.lower() == 'test' else ''
    self.base_url = f"https://{self.environment}intelifi.com/{self.environment}usaintel.com/billing_hubspot_webhook"
  ...
  def customers(self, start: int = 0, end: int = 100):
    return self.api_call('get', f"/api/hubspot/customers/{start}/{end}")
