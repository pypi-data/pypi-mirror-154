...
def detect_window_maxima(data, wind):
    span = int(wind / 2)
    return np.array(argrelextrema(data, np.greater_equal, order=span)[0])
...
def detect_maxima(data, tr=None):
    ...
    if tr is None:
        ...
        ret = []
        if i in argrelextrema(data, np.greater_equal)[0]:
            if data[i] > tval:
                ret.append(i)
        return np.array(ret)
...
def detect_qrs(data, srate):
    ...
    """
    y1 = band_pass(data, srate, 10, 20)  # The qrs value must be at 10-20 hz
    ...
    y4 = moving_average(y3, int(srate * 0.15))
