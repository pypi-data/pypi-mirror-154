...
class _ForwardedKubernetesHTTPConnection(HTTPConnection):
    """
    Implement a version of the HTTPConnection python class that takes a k8s portforward socket to send HTTP POST requests.
    """
...
class KubernetesPodExecutor(NodeExecutor):
    ...
    def expose_port(self, expose_port: int, pod_port: int) -> None:
        """
        Expose port from a container/pod as a so-called Kubernetes service. The k8s service has the name of the node.

        :param expose_port: Port to expose the service to. This will be exposed within the k8s cluster, NOT the rettij host system.
        :param pod_port: Port inside the rettij node to be exposed.
        """
        self.logger.info(
            f"Exposing port '{pod_port}' as service / from node '{self.name}' in namespace '{self.namespace_id}' to '{expose_port}' in k8s cluster..."
        )
        ...
        service = kubernetes.client.V1Service(
            api_version="v1",
            kind="Service",
            metadata=kubernetes.client.V1ObjectMeta(name=self.name),  # kubernetes service name
            spec=kubernetes.client.V1ServiceSpec(
                ports=[kubernetes.client.V1ServicePort(port=expose_port, target_port=pod_port)]
            ),
        )
        kubernetes_api.create_namespaced_service(self.namespace_id, service)

    def send_http_post_msg_to_node(self, port: int, msg: str) -> None:
        """
        Send a HTTP POST request to the rettij node. Make sure to expose the port as a service first via the expose_port() method.

        :param port: Port that is exposed in the k8s cluster.
        """
        self.logger.debug(f"Sending HTTP message from rettij host to node '{self.name}' (port {port}): {msg}")
        try:
            kubernetes_api = kubernetes.client.CoreV1Api()
            ...
            if resp.status != 200:
                self.logger.error(
                    f"Error occurred sending data via forwarded kubernetes port to node. Got http status '{resp}'. "
                    f"Expected '200'. "
                )
        except Exception as e:
            self.logger.error(f"Could not send POST request: {e}")
