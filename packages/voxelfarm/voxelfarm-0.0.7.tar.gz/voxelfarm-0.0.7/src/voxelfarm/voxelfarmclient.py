...
# region GetTimeSeriesData
    def get_time_series_data_by_search(self, timespan, projectID, spatialID=None, objectID=None, properties=None, filter=None):
        ...
        if response["instances"]["hits"]:
            ...
            return self.build_dataframe_from_processes(processes)
        else:
            return pd.DataFrame()
                ...
                processes.append(
                    executor.submit(
                        ...
                        [
                            ...
                            id["properties"] if hasattr(id, "properties") else None,
                            id["filter"] if hasattr(id, "filter") else None,
                            timespan,
                            authorizationToken,
                        ],
                    )
    ...
    def get_data_by_id(
        ...
        while "continuationToken" in list(response.keys()):
            ...
            result["properties"].extend(response["properties"])
        return result

    def build_dataframe_from_processes(self, processes):
            ...
            if "timestamps" in result:
                for i, (timestamp) in enumerate(result["timestamps"]):
                    ...
                    for j, (res) in enumerate(lambdaData[timestamp]):
                        if result["timeseries"][0] in res["spatialID"] and result["timeseries"][1] in res["objectID"]:
                            for k, prop in enumerate(properties):
                                if not prop in res["properties"]:
        ...
        for i, (timestamps) in enumerate(lambdaData):
            for j, (event) in enumerate(lambdaData[timestamps]):
                values = [pd.to_datetime(timestamps), event["spatialID"], event["objectID"]]
                ...
                dataframe = pd.concat([dataframe, pd.DataFrame([values], columns=indexes)])
        return dataframe
