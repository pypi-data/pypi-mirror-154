...
class WiringForRunChainCreationForm(autoform.AutomaticEntityForm):
    __select__ = autoform.AutomaticEntityForm.__select__ & has_link_to_runchain()
...
_affk.tag_subject_of(
    ("ParameterValueFile", "value_file", "*"), {"label": value_file_label}
)
...
class RunEditForm(autoform.AutomaticEntityForm):
    ...
    def param_defs(self):
        ...
        return [
            pdef
            for pdef in executable.reverse_parameter_of
        ...
        for pdef in self.param_defs:
            ...
            try:
                if edition:
                    ...
                    formview = next(
                        self.inline_creation_form_view(rschema, ttype, role)
                    )
...
class ParameterValueFileEditForm(autoform.AutomaticEntityForm):
    ...
    __select__ = autoform.AutomaticEntityForm.__select__ & is_instance(
        "ParameterValueFile"
    )
    ...
    def should_display_inline_creation_form(self, rschema, existant, card):
        if rschema == "value_file":
            return not existant
        return super(
            ParameterValueFileEditForm, self
        ).should_display_inline_creation_form(rschema, existant, card)
    ...
    def should_display_add_new_relation_link(self, rschema, existant, card):
        ...
        return super(
            ParameterValueFileEditForm, self
        ).should_display_add_new_relation_link(rschema, existant, card)
...
class ParameterValueFormRenderer(formrenderers.EntityInlinedFormRenderer):
    ...
    fieldset_css_class = table_class = ""
    ...
    def render_help(self, form, field):
        ...
        if descr:
            return '<div class="helper">%s</div>' % descr
        return ""
