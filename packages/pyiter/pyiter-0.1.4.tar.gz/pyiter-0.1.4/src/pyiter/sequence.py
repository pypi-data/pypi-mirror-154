...
class Sequence(Generic[T], Iterable[T]):
    ...
    def parallel_foreach(self, action: Callable[[T], None], max_workers: Optional[int]=None) -> None:
        """
        ...
        c
         ...
         Example 2:
        >>> lst = ['a', 'b', 'c']
        >>> it(lst).parallel_foreach(lambda x: print(x), max_workers=2)
        a
        b
        c
        """
    ...
    def progress_bar(self, bar: Callable[[Iterable[T]], Iterable[T]]) -> Sequence[T]:
        """
        ...
        >>> it(range(10)).progress_bar(lambda x: tqdm(x, total=len(x))).parallel_map(lambda x: sleep(1), max_workers=5).to_list() and None
        """
        return it(bar(self))
