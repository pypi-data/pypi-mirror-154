...
class Sequence(Generic[T], Iterable[T]):
    ...
    def flatten(self) -> Sequence[T]:
        ...
        ['a', 'b', 'c', 'd', 'e']
        """
        return FlatteningSequence(self, lambda x: x)
    ...
    def zip_with_next(self) -> Sequence[Tuple[T, T]]:
        ...
        to an each pair of two adjacent elements in this sequence.
         ...
         Example 1:
        >>> lst = ['a', 'b', 'c']
        >>> it(lst).zip_with_next(lambda x, y: x + '__' + y)
        ['a__b', 'b__c']

         Example 2:
        >>> lst = ['a', 'b', 'c']
        >>> it(lst).zip_with_next()
        [('a', 'b'), ('b', 'c')]
        """
        ...
    @overload
    ...
    def let(self, block: Callable[[Sequence[T]], R]) -> R:
        """
         ...
         Example 1:
        >>> it(['a', 'b', 'c']).let(lambda x: x.map(lambda y: y + '!'))
        ['a!', 'b!', 'c!']
        """
        return block(self)
    ...
    def also(self, block: Callable[[Sequence[T]], R]) -> R:
        """
         ...
         Example 1:
        >>> it(['a', 'b', 'c']).also(lambda x: x.map(lambda y: y + '!'))
        ['a', 'b', 'c']
        """
        block(self)
        return self
