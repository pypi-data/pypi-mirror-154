...
# This code is based on the `keywords` in sqlparse with modification.
...
SQL_REGEX = [
    ...
    (r"(AT|WITH)\s+TIME\s+ZONE\s*'[^']+'", tokens.Keyword.TZCast),
    # Added Timezone Casting keyword used by PostgreSQL
    (r"(AT|WITH)\s+TIME\s+ZONE\,*\s+", tokens.Keyword.TZCast),
...
SQL_REGEX = [(re.compile(rx, FLAGS).match, tt) for rx, tt in SQL_REGEX]
...
KEYWORDS = {
    ...
    'SIZE': tokens.Keyword,
    'SOME': tokens.Keyword,
    # `SOURCE` is not reserved keyword
    # removing
