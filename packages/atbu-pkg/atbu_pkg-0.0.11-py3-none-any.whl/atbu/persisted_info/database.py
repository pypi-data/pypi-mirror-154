...
from ..common.file_info import (
    FileInformationPersistent,
    get_loc_persistent_file_info,
    LocationFileInfoUpdater,
)
from ..common.hasher import GlobalHasherDefinitions, HasherDefinitions
...
def extract_location_info(
    ...
    if len(locations) < min_required:
        raise InvalidCommandLineArgument(
            f"You have only specified '{len(locations)}' which is less than the {min_required} required."
        )
    if max_allowed is not None and len(locations) > max_allowed:
        # Given above checks, this should never occur.
        raise InvalidCommandLineArgument(
            f"You specified {len(locations)} locations which is more than the {max_allowed} allowed."
        )
    return locations
...
def handle_savedb(args):
    ...
    logging.info(
        f"All locations total physical files .... {sum(len(l) for l in all_locations_info.values())}"
    )
