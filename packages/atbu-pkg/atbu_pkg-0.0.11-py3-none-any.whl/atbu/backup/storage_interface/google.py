...
from ...common.exception import RetryLimitReached, ObjectDoesNotExistError
from ..config import *
    ...
    def get_object(self, object_name: str):
        try:
            ...
            raise RetryLimitReached(
                f"Retry limit reached while trying to access blob {object_name} of container {self.container.name}. {exc_to_string(ex)}",
                ex,
            ) from ex
        if blob is None:
    ...
    def upload_stream_to_object(
        ...
        while not upload.finished:
            ...
            try:
                ...
                logging.debug(
                    f"Uploading chunk {chunk_number} at {seconds_running:.3f} seconds into the backup. "
                ...
                logging.debug(
                    f"upload_stream_to_object: chunk upload response={response}"
                )
                logging.debug(
                    f"upload_stream_to_object: chunk bytes uploaded={upload.bytes_uploaded - last_bytes_uploaded}"
                )
                logging.debug(
                    f"upload_stream_to_object: total bytes uploaded={upload.bytes_uploaded}"
                )
                chunk_number += 1
                ...
                logging.error(
                    f"Chunk upload of {chunk_size} bytes via transmit_next_chunk failed. seconds_running={seconds_running:.3f}"
                    f"upload.invalid={upload.invalid} pos_before={pos_before_xmit} pos_after={pos_after_xmit} {exc_to_string(ex)}"
                )
                # Has retry strategy limit been reached?
                if seconds_running >= DEFAULT_RETRY_SECONDS:
                    raise RetryLimitReached(
                        f"Backup file write failed after {seconds_running:.3f} overall retry time. {exc_to_string(ex)}",
                        ex,
                    ) from ex
                # Attempt to recover.
            ...
            try:
                while not download.finished:
                    try:
                        ...
                        # logging.debug(f"finished={download.finished} read={len(r.content)} chunk_size={chunk_size}")
                        ...
                        if download.invalid:
                            raise RetryLimitReached(
                                f"The download object is invalid, cannot retry. Backup file write failed after {seconds_running:.3f} overall retry time. ex={ex}",
                                ex,
                            ) from ex
            finally:
