...
from ..common.util_helpers import is_valid_base64_string
            ...
            if k == "W":
                ...
                if self.work_factor != PBKDF2_WORK_FACTOR:
                    ...
                    raise CredentialInvalid(
                        f"The PBDKF2 work factor {self.work_factor} is invalid. Only {PBKDF2_WORK_FACTOR} is supported."
                    )
            elif k == "S":
                self.salt = c
            elif k == "H":
...
def get_base64_password_from_keyring(
    ...
    if raw_password is None:
        raise CredentialNotFoundError(
            f"The credential was not found in the keyring: service_name={service_name} username={username}"
        )
