...
from ..common.exception import (
    ...
    StorageDefinitionNotSpecifiedError,
    StorageDefinitionNotFoundError,
    BackingFileInformationNotFound,
    InvalidStateError,
)
...
def parse_storage_def_specifiers_patterns(
    ...
    for raw_arg in raw_arguments:
        ...
        if not storage_specifiers:
            if not is_existing_filesystem_storage_path(storage_location=raw_arg):
                raise StorageDefinitionNotFoundError(
                    f"The specifier '{raw_arg}' did not yield any storage definitions or filesystem backups."
                )
        ...
        #
        # Fall through, raw_arg is not a selection pattern, should be storage definition specifier...
        #
    ...
    for sbs in sbs_list:
        if not storage_def_name:
            ...
            raise InvalidStateError(
                f"All SpecificBackupSelection instances in the list must target the same storage definition."
            )
