...
from .hasher import GlobalHasherDefinitions, Hasher, HasherDefinitions
...
class FileInformation:
    ...
    def calculate_hashes(self, hasher: Hasher = None, max_attempts=None) -> Hasher:
        ...
        if max_attempts is not None and max_attempts <= 0:
            raise ValueError(
                f"max_attempts must be None for forever, or a positive number indicating max attempts."
            )
        if hasher is None:
...
class FileInformationPersistent(FileInformation):
    """Like FileInformation, instances of FileInformationPersistent represent information
    about a file, but are instances that represent file information as obtained from one
    of several mechanisms...
        - Originally, the information is likely obtained from a file within a file system.
        - The original information can be stored in per-file .atbu file information files,
          where it can be later retrieved for use in comparing with other files or even with
          its own current state.
        - The original information can also be stored in a file information database (json db)
          where it can be compared with either another json db or a live file system.

    In all the above cases, once information is retrieved, whether from the file system, a json db,
    or a .atbu file information file, the information is stored in instances of FileInformationPersistent
    in a "firewalled" manner from the live OS information.

    Therefore, any testing of FileInformationPersistent should ensure that file information
    read from any other source is never polluted from live OS information until such time as
    someone calls a method on the instance (i.e., refresh_info_from_phys_file) to do so.

    With all that said, keep in mind quite simply that FileInformation and FileInformationPersistent
    are different. FileInformation deals with information just read from the file system (not info
    saved in a db or .atbu file), whereas FileInformationPersistent, aside from its initial sampling
    of information from the file system, never deals with live OS data until commanded to do so.
    """
    ...
    def read_info_data_file(self):
        ...
        if "MAIN" not in cp.sections():
            raise InvalidPersistentFileInfoError(
                f"The persistent info does not have the expected MAIN section: {self.info_data_file_path}"
            )
        ...
        if self.version != "2":
            raise PersistentFileInfoVersionMismatch(
                f"The persistent info file version is mismatched: v={self.version} {self.info_data_file_path}"
            )
    ...
    def update_info_data_file_to_latest_version(self, hashers):
        ...
        if self.version != "0.1":
            # Caller should have attempted read_info_data_file which sets version.
            raise PersistentFileInfoError(
                f"update_info_data_file_to_latest_version cannot be called for anything but version 1.0."
            )
...
class LocationFileInfoUpdater:
    ...
    def _process_file_info(self, file_info: FileInformationPersistent):
        ...
        try:
            ...
            if not self.update_stale:
                raise AtbuException(
                    f"Cannot upgrade persistent file info because option to update not specified. {exc_to_string(ex)}"
        ...
        if is_changed:
            ...
            if file_info.is_modified_date_or_size_changed() and not self.whatif:
                self.skipped_files.append(
                    (
                        file_info,
                        f"The {ATBU_PERSISTENT_INFO_EXTENSION} file info remains stale after update attempt",
                    )
                )
                self.total_files_stale_info_skipped += 1
                logging.warning(
                    f"The {ATBU_PERSISTENT_INFO_EXTENSION} file info remains stale after update attempt, skipping {file_info.path}."
                )
                return
            self.total_file_info_updated += 1
        ...
        if some_updating_occurred:
            ...
            # logging.debug(f"The {ATBU_EXTENSION} info data file was {added_changed_str}: {file_info}")
            if self.per_file_persistence:
                logging.info(
                ...
                logging.debug(
                    f"The {ATBU_PERSISTENT_INFO_EXTENSION} file info was {added_changed_str}: {file_info}"
                )
            else:
                logging.info(
