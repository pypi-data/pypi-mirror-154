...
from ..common.util_helpers import is_platform_path_case_sensitive
...
class MoveFileInformationCommand(FileInformationCommandBase):
    ...
    def _move_file_to_dest_path(
        ...
        """Move a file and its FileInformationPersistent from a source path to a destination path, retaining the
        same directory structure for non-root portions of the paths.
        """
        file_moved = False
        ...
        logging.info(
            f"Moving{self.whatif_str} {file_info.path} ---to--> {destination_file_path} digest={file_info.get_digest()}"
        )
        if not whatif:
            ...
            if not file_info.is_loaded_from_db:
                logging.info(
                    f"Moving{self.whatif_str} {file_info.info_data_file_path} ---to--> {destination_config_file_path}"
                )
                try:
                    os.makedirs(
                    ...
                    try:
                        ...
                        pass
                    logging.error(
                        f"Failure moving config file: path={file_info.info_data_file_path} {exc_to_string(ex)}"
                    )
                    raise
    ...
    def perform_command(self):
        """Move relevant files and associated config files from a source tree to a destination tree."""
...
class RemoveFileInformationCommand(FileInformationCommandBase):
    ...
    def _log_remove_error(
        ...
        """The a removal error, returns True if error should be raised, else do not raise."""
        ...
        if isinstance(exception, OSError):
            logging.error(
                f"Ensure the file is read/write, is not held/locked, and that you have permission to remove the file."
            )
    ...
    def _remove_file(
        ...
        if not is_remove_config:
            logging.info(
                f"Removing{self.whatif_str} {path_to_remove} digest={file_info_to_remove.primary_digest}"
            )
        else:
        ...
        try:
            ...
            if raise_exception:
                raise
            # TODO: Add switch for selection to try to remove read only (but not make that default)
            # logging.warning(f"    OSError: {err}")
            # logging.warning(f"    Trying to remove read-only attribute: chmod(""{file_info_to_remove.path}"",stat.S_IWRITE)")
    ...
    def _remove_file_and_file_info(
        ...
        if not is_loaded_from_db and not os.path.isfile(
            ...
            raise InvalidStateError(
                f"Unexpected state: {ATBU_ACRONUM_U} configuration file note found: {file_info_to_remove.info_data_file_path}"
            )
        if not whatif:
            ...
            if os.path.isfile(file_info_to_remove.path):
                logging.info(
                    f"Removing{self.whatif_str} {file_info_to_remove.path} digest={file_info_to_remove.get_digest()}"
                )
                if not is_loaded_from_db:
                    logging.info(
...
def is_file_info_list_bad_state(
    ...
    for file_info in file_info_list[1:]:
        ...
        if primary_digest != other_primary_digest:
            logging.error(
                f"Unexpected list state error, digest mismatch '{primary_digest}' != '{other_primary_digest}'"
            )
        ...
        if size_in_bytes != file_info.size_in_bytes:
            logging.error(
                "Unexpected size mismatch encountered, potential collision, skipping the following file:"
            )
            for file_info2 in file_info_list:
...
def diff_locations(
    ...
    for digest, file_info_list_locA in locationA_info.items():
        ...
        if file_info_list_locB is None:
            logging.debug(
                f"Location A file_info not in Location B: {primary_hasher_name} digest={primary_digest_locA} file_info[0]={file_info_list_locA[0].path}"
            )
            continue
        if is_file_info_list_bad_state(primary_hasher_name, file_info_list_locB):
            logging.warning(
                f"Location B file_info list bad state causes location A file_info to be considered not in B."
            )
            continue
        # Sanity check
        ...
        if primary_digest_locA != primary_digest_locB:
            logging.error(
                f"Location A and B primary digests do not match when expected: {primary_hasher_name}_locA={primary_digest_locA} {primary_hasher_name}_locB={primary_digest_locB} pathA={file_info_list_locA[0].path} pathB={file_info_list_locB[0].path}"
            )
            continue
        logging.debug(
            f"Location A and B digests match: {primary_hasher_name}_locA={primary_digest_locA} {primary_hasher_name}_locB={primary_digest_locB} pathA={file_info_list_locA[0].path} pathB={file_info_list_locB[0].path}"
        )
        logging.info(
            f"Location A and B digests match: {primary_hasher_name}={primary_digest_locA} {file_info_list_locA[0].basename}"
        )
    ...
    try:
        ...
        if len(updaterA.sneaky_corruption_potentials) > 0:
            ...
            for scp in updaterA.sneaky_corruption_potentials:
                ...
                logging.info(f"-" * 65)
            logging.info(
                f"    Total potential sneaky corruption Location A: {len(updaterA.sneaky_corruption_potentials)}"
            )
            logging.info(f"=" * 65)
        if len(updaterB.sneaky_corruption_potentials) > 0:
            ...
            for scp in updaterB.sneaky_corruption_potentials:
                ...
                logging.info(f"-" * 65)
            logging.info(
                f"{'Total potential sneaky corruption Location B:':.<65} {len(updaterB.sneaky_corruption_potentials)} (see details above)"
            )
            logging.info(f"=" * 65)
        ...
        logging.info(
            f"{'Total Location A unique files also in Location B ':.<65} {len(locA_in_locB)}"
        )
        logging.info(
            f"{'Total Location A unique files not found in Location B ':.<65} {len(locA_not_in_locB)}"
        )
        if post_diff_command is not None:
            ...
            logging.info(
                f"{'Total Location A unique files ' + past_tense_verb_whatif:.<65} {post_diff_command.unique_files_affected}"
            )
            logging.info(
                f"{'Total Location A physical files ' + past_tense_verb_whatif:.<65} {post_diff_command.physical_files_affected}"
            )
            if locationA_DBs.has_per_file_persistence:
                logging.info(
                    f"{'Total Location A config files ' + past_tense_verb_whatif:.<65} {post_diff_command.config_files_affected}"
                )
                logging.info(
                    f"{'Total Location A config files not ' + past_tense_verb_whatif:.<65} {post_diff_command.config_files_total - post_diff_command.config_files_affected}"
                )
            logging.info(
                f"{'Total Location A affected directories ':.<65} {len(post_diff_command.directories_affected)}"
            )
            logging.info(
                f"{'Total Location A affected directories emptied/removed ':.<65} {post_diff_command.directories_removed}"
            )
