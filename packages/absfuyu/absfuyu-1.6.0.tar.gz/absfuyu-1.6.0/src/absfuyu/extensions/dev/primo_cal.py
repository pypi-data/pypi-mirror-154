...
# Core
########################################
def div_down(big_num: int, small_num: int):
    """Round down divided number
    """
    return math.floor(big_num / small_num)
...
def fate_result(
    ...
    # Abyss
    _ab_time = div_down(_primo_day.days, ABYSS_INTERVAL)
    _ab_gem = div_down(abyss_star, ABYSS_CHAMBER_PER_FLOOR) * ABYSS_GEM *_ab_time
    ...
    # Intertwined Fate
    _if1 = div_down(_total_primo, PRIMO_PER_FATE) # Primogem
    _if2 = div_down(masterless_starglitter, GLITTER_PER_FATE) # Glitter
    _if3 = 0 # Extra fates from monthly shop
    if _primo_day.days >= 30:
        _if3 = div_down(_primo_day.days, 30) * IFATE_MONTHLY_SHOP
    ...
    _ex_gl = div_down(_base_if, 10) * 2
    _ex_if = div_down(_ex_gl, GLITTER_PER_FATE)
    ...
    output = {
        ...
        "5-star character (worst case)": div_down(_total_if, 90),
        "5-star weapon (worst case)": div_down(_total_if, 80),
    }
    # Output debug
    output_d = {
        "Primogems": {
            ...
            "Abyss": {
                ...
                "Abyss stars to Primogem": div_down(abyss_star, ABYSS_CHAMBER_PER_FLOOR) * ABYSS_GEM,
        ...
        "Intertwined Fates": {
            ...
            "Base": {
                ...
                "From Primogems (base)": div_down(_base_primo, PRIMO_PER_FATE),
                "From Glitters": _if2,
                "Extra from monthly shop": _if3,
                "Total Intertwined Fates (no shop)": intertwined_fate + div_down(_base_primo, PRIMO_PER_FATE) + _if2,
                "Total Intertwined Fates": intertwined_fate + div_down(_base_primo, PRIMO_PER_FATE) + _if2 + _if3,
            },
