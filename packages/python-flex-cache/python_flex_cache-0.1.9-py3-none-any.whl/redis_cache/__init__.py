...
class RedisCacheDecorator(BaseCacheDecorator):
    ...
    def cache_output(self, key, serialized):
        get_cache_lua_fn(self._cache)(keys=[key, self.keys_key], args=[serialized, self.ttl, self.limit])
    ...
    def invalidate_all(self, *args, **kwargs):
        chunks_gen = chunks(self._cache.scan_iter(f'{self.prefix}:{self.namespace}:*'), 500)
