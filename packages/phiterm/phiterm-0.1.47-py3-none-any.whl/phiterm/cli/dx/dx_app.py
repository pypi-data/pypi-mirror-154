...
def run(
    ...
    if target_env_str is None:
        target_env_str = (
            active_ws_data.ws_config.default_env
            if active_ws_data.ws_config and active_ws_data.ws_config.default_env
            else None
        )
    if target_env_str is None:
        target_env_str = "dev"
    ...
    try:
        target_env = cast(WorkspaceEnv, WorkspaceEnv.from_str(target_env_str))
