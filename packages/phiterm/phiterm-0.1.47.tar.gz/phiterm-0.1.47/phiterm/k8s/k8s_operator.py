...
from phidata.infra.k8s.resource.types import K8sResourceType, K8sResource
...
def save_k8s_resources(
    ...
    # Step 1: Filter K8sConfigs
    k8s_configs: List[InfraConfig] = []
    if target_env is None or target_env == WorkspaceEnv.all:
        if ws_config.k8s is not None:
            k8s_configs.extend(ws_config.k8s)
    else:
        for config in ws_config.k8s:
            ...
            if config_env == target_env:
                k8s_configs.append(config)

    # Step 2. Prepare configs
    configs_to_build: List[K8sConfig] = []
    for config in k8s_configs:
        ...
        if not isinstance(_config, K8sConfig):
            print_error(f"{_config} is not an instance of K8sConfig")
            continue
        configs_to_build.append(_config)
    ...
    for config in configs_to_build:
        ...
        for rg_name, resource_group in k8s_resources.items():
            ...
            if target_app is not None:
                if target_app.lower() not in rg_name.lower():
                    logger.debug(f"Skipping {rg_name}")
                    continue
