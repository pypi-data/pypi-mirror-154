...
@request.request
def read_fs_stats(conninfo: Connection, credentials: Optional[Credentials]) -> request.RestResponse:
    method = 'GET'
    uri = '/v1/file-system'
...
def write_file(
    ...
    @param {str}  id        File id of the file. If None, path must not be None
...
def read_directory(
    ...
    @param {str} id_         Directory to read, by ID
    @param {str} snapshot    Snapshot ID of directory to read
...
def set_permissions_settings(
    conninfo: Connection, credentials: Optional[Credentials], mode: str
) -> request.RestResponse:
    """
...
def get_atime_settings(
    ...
    return request.rest_request(conninfo, credentials, 'GET', '/v1/file-system/settings/atime')
...
def set_atime_settings(
    ...
    return request.rest_request(
        conninfo, credentials, 'PATCH', '/v1/file-system/settings/atime', body=payload
    )
