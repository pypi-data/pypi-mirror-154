...
class BitFieldMember(Member):
    ...
    def typ(self) -> type:
        ...
        if self._typ is None:  # pragma: no cover
            raise RuntimeError("internal error")
    ...
    def argrepr(self) -> str:
        ...
        if isinstance(self._argrepr, str):
            ...
            repr_format = f"{self.name}={{str(self.{self.name})}}"
    ...
    def add_name_index(
        ...
        cls_name = self.typ.__name__
        try:
            is_same = code_injector.evaluate(cls_name) is self.typ
        except NameError:  # pragma: no cover
            is_same = False
        cls_name = cls_name if is_same else f"cls.{self.name}.typ"

        dump_name = f"{aux.group_name}_dump"
        dump_group = [f"{dump_name} = dump.add_record()"]

        if issubclass(self.typ, BitFields):
            dump_member = [
                # line 1
        ...
        if self.fset is None and not self.readonly:
            ...
            if issubclass(self.typ, IntEnum):
                store.setter = (
                    []
            ...
            if self.typ is int:
                ...
                store.setter.append(f"self[{self.index}] = {cls_name}(value)")
