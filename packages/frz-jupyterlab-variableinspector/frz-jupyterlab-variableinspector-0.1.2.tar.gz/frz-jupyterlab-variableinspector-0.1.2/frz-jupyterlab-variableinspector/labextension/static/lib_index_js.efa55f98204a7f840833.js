...
Languages.r_script = `library(repr)
      ...
      n = 5) 
  {
      ...
      if (!'jlvi_brief' %in% names) {
          obj.content[!wtf] <- napply(names[!wtf], function(x) {a=capture.output(str(x,max.level=1,list.len=3)); b=lapply(a[1:min(length(a),4)], function(x) {paste(substring(x, 1, 30), ifelse(nchar(x) > 30, "...", ""))}); paste(b,collapse=rawToChar(as.raw(c(92,110))))})
      }
          
      is_function <- (obj.type == "function")
      if (!'jlvi_brief' %in% names) {
          obj.content[is_function] <- napply(names[is_function], function(x) { a = strsplit(repr_text(f), rawToChar(as.raw(c(92, 110))))[[1]]; if (length(a) >= 4) a[[4]] = "..."; b = lapply(a[1:min(length(a), 4)], function(x) { paste(substring(x, 1, 30), ifelse(nchar(x) > 30, "...", "")) }); paste(b, collapse = rawToChar(as.raw(c(92, 110)))) })
      }
...
//# sourceMappingURL=lib_index_js.efa55f98204a7f840833.js.map