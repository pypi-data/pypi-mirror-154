...
def skip_with_macos_pluggable_device(reason, func=None):
  def decorator(f):
    if tf_inspect.isclass(f):
      ...
      return f
        ...
        def decorated(self, *args, **kwargs):
          pluggable_devices = pywrap_tfe.TF_ListPluggablePhysicalDevices()
          if (platform.system() == 'Darwin' and len(pluggable_devices) >= 1) :
            self.skipTest("Test is not supported on MacOS pluggable devices")
