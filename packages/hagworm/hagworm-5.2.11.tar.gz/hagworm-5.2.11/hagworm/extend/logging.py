...
from elasticsearch import AsyncElasticsearch, Elasticsearch, helpers as es_helpers
...
class AsyncElasticsearchLogReader:
    """Elasticsearch异步日志读取
    """

    def __init__(self, hosts, index, *, buffer_maxsize=0xffff, **kwargs):

        self._elasticsearch = AsyncElasticsearch(hosts, **kwargs)
        self._index = index

        self._buffer = asyncio.Queue(buffer_maxsize)
        self._closed = False

        self._task = Utils.create_task(self._do_task())
    ...
    def read(self):
        ...
        try:
            while not self._buffer.empty():
                result.append(self._buffer.get_nowait())
        except Exception as err:
            sys.stderr.write(str(err))
    ...
    async def _do_task(self):
        ...
        sort = [{r'datetime': {r'order': r'asc'}}, r'_doc']
        ...
        while not self._closed:
            ...
            try:
                ...
                resp = await self._elasticsearch.search(
                    index=self._index,
                    query=query, sort=sort, search_after=search_after,
                    from_=0, size=5000
                )

                _data_list = resp.body[r'hits'][r'hits']

                if _data_list:

                    last_time = _data_list[-1][r'sort'][0]
                    search_after = _data_list[-1][r'sort']

                    for _data in _data_list:
                        await self._buffer.put(_data[r'_source'])
