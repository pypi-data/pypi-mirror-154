...
from flatspin.plotting import montage_fig, vector_montage, save_animation, gridless_crop
...
def main_vectors(dataset, args):
    ...
    if args.grid == (0, 0):
        # Grid disabled
        if not args.arrows:
            raise ValueError("Image-based visualization not possible without a grid, try --arrows?")
        #if args.crop:
