...
def view_angles(
    viewpoint: pygeos.Geometry,
    visible_geom: pygeos.Geometry,
) -> Tuple[float, float]:
    """
    Returns start and end angle how the visible geom can be seen from the viewpoint.

    Args:
        viewpoint (pygeos.Geometry): the point that is being viewed from.
        visible_geom (pygeos.Geometry): the visible geometry to calculate the
            view angles to.
    ...
    viewpoint_coords_arr = pygeos.get_coordinates(viewpoint)
    if len(viewpoint_coords_arr) != 1:
        raise ValueError("The viewpoint should be a point")
    ...
    visible_coords_arr = pygeos.get_coordinates(visible_geom)
    ...
    if len(angles_arr[angles_arr == 0]) > 0:
        ...
        line_0 = pygeos.linestrings([[[0, 0], [line_length, 0]]])
        intersects_0 = pygeos.intersects(visible_geom, line_0).tolist()[0]
    ...
    tol = 0.0000000001
    line_SE = pygeos.linestrings([[[0, -tol], [line_length, -tol]]])
    if not pygeos.intersects(visible_geom, line_SE):
        return (angles_arr.min(), angles_arr.max())
    else:
        # Add 360° angle
    ...
    line_NE = pygeos.linestrings([[[0, tol], [line_length, tol]]])
    if not pygeos.intersects(visible_geom, line_NE):
    ...
    line_180 = pygeos.linestrings([[[0, 0], [-line_length, 0]]])
    intersects_180 = pygeos.intersects(visible_geom, line_180).tolist()[0]
    ...
    for angle in np.sort(angles_arr):
        ...
        line_avg = pygeos.linestrings([[[0, 0], [x, y]]])
        intersects_avg = pygeos.intersects(visible_geom, line_avg).tolist()[0]
