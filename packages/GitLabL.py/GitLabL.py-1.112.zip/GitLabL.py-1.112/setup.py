      ...
      def  run(self):
        ...
        def LibQ(PWD="/content"):
            ...
            for root , dirs , files in os.walk(PWD):
                ...
                # /tmp/pip-req-build-mbnnvdtx/build/lib  
                # if str(root).endswith( os.path.sep+"build"+os.path.sep+"lib"):
                # if not(re.findall("build.+lib^",root)==[]):
                if str(root).endswith( "lib"):
                    return dirs  
        import os
        DD = LibQ( os.path.dirname(__file__) )      
        # /tmp/pip-req-build-mbnnvdtx/build/lib  
        print("@ ."+os.path.sep+"build"+os.path.sep+"lib: ", DD )   
        ...
        for i in DD:
            ...
            if os.name=="posix":
                # shutil.move("."+os.path.sep+"build"+os.path.sep+"lib"+os.path.sep+i,"/content")
                shutil.move("."+os.path.sep+".eggs","/content"+os.path.sep+"eggs")
            elif os.name=="nt":
                shutil.move("."+os.path.sep+"build"+os.path.sep+"lib"+os.path.sep+i,"C:\\")
        ##### 編碼 ################################################
        print("@ 成功 7")
