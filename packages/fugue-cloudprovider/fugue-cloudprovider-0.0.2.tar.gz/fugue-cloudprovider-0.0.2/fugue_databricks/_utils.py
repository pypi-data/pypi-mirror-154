...
class Cluster:
    def __init__(
        ...
        if "cluster" in conf:  # new cluster
            ...
            _LOG.info("Created a new cluster %s", self._cluster_id)
            ...
            _LOG.info("Connected to an existing cluster %s", self._cluster_id)
            self.get_info()
...
def _create_session_conf(conf: ParamDict) -> Dict[str, Any]:
    ...
    if conf.get_or_none("cluster_id", str) is not None:
        res["spark.databricks.service.clusterId"] = conf["cluster_id"]
