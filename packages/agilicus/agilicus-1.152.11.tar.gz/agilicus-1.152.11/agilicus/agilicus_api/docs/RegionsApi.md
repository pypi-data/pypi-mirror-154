...
with agilicus_api.ApiClient(configuration) as api_client:
    ...
    try:
        # List all regions
        api_response = api_instance.list_point_of_presences(limit=limit, name=name, page_at_name=page_at_name, include_tag=include_tag, exclude_tag=exclude_tag)
        pprint(api_response)
    except agilicus_api.ApiException as e:
 ...
 **include_tag** | **[str]**| A list of case-sensitive tags to include in the search. Each provided tag must match in order for the item to be returned.  | [optional]
 **exclude_tag** | **[str]**| A list of case-sensitive tags to exclude in the search. If any provided tag matches, then the item is not returned.  | [optional]
