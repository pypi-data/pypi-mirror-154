...
with agilicus_api.ApiClient(configuration) as api_client:
    ...
    try:
        # List all regions
        api_response = api_instance.list_point_of_presences(limit=limit, name=name, page_at_name=page_at_name, includes_all_tag=includes_all_tag, excludes_any_tag=excludes_any_tag)
        pprint(api_response)
    except agilicus_api.ApiException as e:
