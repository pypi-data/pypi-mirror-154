...
def list_point_of_presences(
    ...
    excludes_all_tags = _tag_list_to_tag_names(excludes_all_tag)
    excludes_any_tags = _tag_list_to_tag_names(excludes_any_tag)
    includes_all_tags = _tag_list_to_tag_names(includes_all_tag)
    includes_any_tags = _tag_list_to_tag_names(includes_any_tag)
...
def add_point_of_presence(ctx, name, tag: List[str], domain=None, **kwargs):
    ...
    tags = []
    if tag:
        tags = _tag_list_to_tag_names(tag)
...
def update_point_of_presence(
    ...
    **kwargs,
    ...
    if not overwrite_tags:
        ...
        tags = _tag_list_to_tag_names(list(set(str(tag) for tag in tags)))
...
def _tag_list_to_tag_names(tags: List[str]) -> List[agilicus.FeatureTagName]:
