...
def get_ffme_returns():
    ...
    rets.index = pd.to_datetime(rets.index, format="%Y%m").to_period(
        "M")  # .to_period("M") forces the index to be monthly period...
    return rets
...
def efficient_frontier(n_portfolios, rets, covmat, periods_per_year, risk_free_rate=0.0,
    ...
    portfolio_spr = [sharpe_ratio(r, risk_free_rate, periods_per_year, v=v) for r, v in
                     zip(portfolio_ret, portfolio_vol)]
    ...
    if iplot:
        ax = df.plot.line(x="volatility", y="return", style="--", color="coral", grid=True, label="Efficient frontier",
                          figsize=(8, 4))
        if hsr or cml:
...
def summary_stats_terminal(rets, floor=0.8, periods_per_year=2, name="Stats", target=np.inf):
    ...
    stats = pd.DataFrame.from_dict({
        "Mean ann. ret.": annualize_rets(rets, periods_per_year=periods_per_year).mean(),
        # mean annualized returns over scenarios
        ...
        "Exp shortfall": (floor - terminal_wlt[floor_breach]).mean() if floor_breach.sum() > 0 else 0
        # expected shortfall if floor is reached
...
def simulate_gbm_from_prices(n_years=10, n_scenarios=20, mu=0.07, sigma=0.15, periods_per_year=12, start=100.0):
    ...
    prices_dt = np.exp(
        np.random.normal(loc=(mu - 0.5 * sigma ** 2) * dt, scale=sigma * (dt ** (0.5)), size=(n_steps, n_scenarios)))
...
def show_cppi(n_years=10, n_scenarios=50, m=3, floor=0, mu=0.04, sigma=0.15,
    ...
    # Plot parameters
    fig, (wealth_ax, hist_ax) = plt.subplots(figsize=(20, 7), nrows=1, ncols=2, sharey=True,
    ...
    # Plot the histogram
    violations_per_scenarios = (
                cppiw < start * floor).sum()  # number of CPPI wealth violations of the floor per each scenario
    ...
    if floor > 0.0:
        hist_ax.annotate("Violations (overall): {}".format(total_violations), xy=(0.5, 0.75), xycoords="axes fraction",
                         fontsize=15)
        hist_ax.annotate("Violations (end period): {} ({:.1f}%)".format(n_failures, p_fail * 100), xy=(0.5, 0.7),
                         xycoords="axes fraction", fontsize=15)
        hist_ax.annotate("E(shortfall) (end period): ${:.2f}".format(e_shorfall), xy=(0.5, 0.65),
                         xycoords="axes fraction", fontsize=15)
...
def simulate_cir(n_years=10, n_scenarios=10, a=0.05, b=0.03, sigma=0.05, periods_per_year=12, r0=None):
    ...
    def zcbprice(ttm, r, h):
        A = ((2 * h * np.exp(0.5 * (a + h) * ttm)) / (2 * h + (a + h) * (np.exp(h * ttm) - 1))) ** (
                    2 * a * b / (sigma ** 2))
...
def bond_price(principal=100, maturity=10, coupon_rate=0.02, coupons_per_year=2, ytm=0.03, cf=None):
    ...
    # single bond price
    def single_price_bond(principal=principal, maturity=maturity, coupon_rate=coupon_rate,
                          coupons_per_year=coupons_per_year, ytm=ytm, cf=cf):
        if cf is None:
            # compute the bond cash flow on the fly
            cf = bond_cash_flows(maturity=maturity, principal=principal, coupon_rate=coupon_rate,
    ...
    if isinstance(ytm, pd.DataFrame):
        ...
        for i in range(n_scenarios):
            ...
            prices = [single_price_bond(principal=principal, maturity=maturity - t / coupons_per_year,
                                        coupon_rate=coupon_rate,
                                        coupons_per_year=coupons_per_year, ytm=y, cf=cf) for t, y in
                      zip(ytm.index[:-1], ytm.iloc[:-1, i])]
        ...
        bond_price = pd.concat([bond_price,
                                pd.DataFrame([[principal + principal * coupon_rate / coupons_per_year] * n_scenarios],
                                             index=[ytm.index[-1]])],
                               axis=0)
        return bond_price
    else:
...
def bond_returns(principal, bond_prices, coupon_rate, coupons_per_year, periods_per_year, maturity=None):
    ...
    if isinstance(bond_prices, pd.DataFrame):
        ...
        pay_date = np.linspace(periods_per_year / coupons_per_year, last_date,
                               int(coupons_per_year * last_date / periods_per_year), dtype=int)
        coupons.iloc[pay_date] = principal * coupon_rate / coupons_per_year
        tot_return = (bond_prices + coupons) / bond_prices.shift(1) - 1
        return tot_return.dropna()
    else:
        cf = bond_cash_flows(principal=principal, maturity=maturity, coupon_rate=coupon_rate,
                             coupons_per_year=coupons_per_year)
...
def ldi_floor_allocator(psp_rets, lhp_rets, zcb_price, floor, m=3):
    ...
    for date in range(dates):
        ...
        lhp_w = 1 - psp_w
        # update
        account_value = psp_w * account_value * (1 + psp_rets.iloc[date]) + lhp_w * account_value * (
                    1 + lhp_rets.iloc[date])
...
def black_litterman(w_prior, Sigma_prior, P, Q, Omega=None, delta=2.5, tau=0.02):
    ...
    sigma_bl = Sigma_prior + (tau * Sigma_prior) - (tau * Sigma_prior).dot(P.T).dot(invmat).dot(P).dot(
        tau * Sigma_prior)
...
def run_cppi(risky_r = None, safe_rate = 0., m = 3, start = 100, floor = 0.8, drawdown = None):
    ...
    for step in range(0, n_steps):
        if drawdown is not None :
            ...
            floor_value = peak * (1-drawdown)
        cushion = np.maximum((account_value - floor_value) / account_value,0.)
        risky_w = m * cushion
        risky_w = np.minimum(risky_w, 1.)
        risky_w = np.maximum(risky_w, 0.)
        safe_w = 1 - risky_w
        ...
        account_value = risky_alloc * (1 + risky_r.iloc[step,0]) + safe_alloc * (1 + safe_r.iloc[step,0])
        ## save the values
        cushion_history.iloc[step] = cushion
        risky_w_history.iloc[step] = risky_w
        account_history.iloc[step] = account_value
