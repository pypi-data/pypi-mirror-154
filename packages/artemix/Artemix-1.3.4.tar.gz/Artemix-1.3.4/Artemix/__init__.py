...
def E(expr,color = "red"):
  ...
  for element in DatosList:
    if (element[1] != None) :
      if isinstance(element[1],sy.Float) or isinstance(element[1],float) or isinstance(element[1],int) or isinstance(element[1],sy.core.numbers.Integer):
...
def G(c_componente):#Guardar
  ...
  if dentro == False:
    global DatosList
    DatosList = DatosList.append(c_componente)#Si el emento no estaba adentro, simplemente lo agregamos.
