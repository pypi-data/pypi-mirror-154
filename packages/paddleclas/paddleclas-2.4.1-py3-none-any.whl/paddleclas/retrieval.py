...
def retrieval_eval(engine, epoch_id=0):
    ...
    if engine.gallery_query_dataloader is not None:
        ...
        query_feas, query_img_id, query_query_id = cal_feature(
            engine, name='query')
    print(f"query.shape: {query_feas.shape}")
    print(f"gallery.shape: {gallery_feas.shape}")
    np.save(f"query_vector.npy", query_feas.numpy())
    ...
    if engine.eval_loss_func is None:
        ...
        for block_idx, block_fea in enumerate(fea_blocks):
            ...
            np.save(f"similarity_matrix_{block_idx}.npy", similarity_matrix_np)
            if query_query_id is not None:
...
def cal_feature(engine, name='gallery'):
    ...
    total_samples = len(dataloader.dataset) if not engine.use_dali else dataloader.size
    all_feas = all_feas[:total_samples]
    all_image_id = all_image_id[:total_samples]
    if has_unique_id:
        all_unique_id = all_unique_id[:total_samples]

    #print(all_unique_id.shape)
    #print(all_unique_id)
