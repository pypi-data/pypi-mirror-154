...
def get_convergence(search_dir,with_surrogate=False,werr=True,niters=None,nruns=None,debug=False,median=False,**kwargs):
    ...
    for dir_name in dir_list:
        ...
        for loss,std in evaluation:
            if len(loss_surr)==0 or loss<loss_surr[-1]:
...
def extract(path,target_loss='inner',target_unc='Uncertainty',debug=False,raw=False,**kwargs):
    hps = []
    data = []
    nevals = 0
    for fname in sorted(glob.glob(path)):
        ...
        for i,line in enumerate(content):
            ...
            if target_unc in line:
                ...
                data.append([params,loss,std]+samples+[nevals,gen_params,dis_params,time])
                hps.append(samples)
    ...
    if len(data)>0:
        data = pd.DataFrame(data=data,columns=['params','loss','stdev']+names+['nevals','gen_params','dis_params','time']).sort_values(by=['nevals'])
    if debug:
        print(data)
    return data
    ...
    for fname in sorted(glob.glob(path)):
        ...
        for i,line in enumerate(content):
            ...
            if 'Test Loss' in line:
                ...
                data.append([params,loss,trial]+samples+[nevals,gen_params,dis_params,time])
    if len(data)>0:
        data = pd.DataFrame(data=data,columns=['params','loss','trial']+['hp%i'%(i+1) for i in range(len(samples))]+['nevals','gen_params','dis_params','time']).sort_values(by=['nevals'])
    if debug:
        print(data)
    return data
