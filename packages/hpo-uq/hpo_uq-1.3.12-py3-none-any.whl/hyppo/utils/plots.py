...
def plot_convergence(log_path,isurr=None,ylim=[],xlim=[],save=None,**kwargs):
    loss, stdev = get_convergence(log_path,**kwargs)
...
def static_solutions(input_data,metric='params',ms=20,cmap='plasma',**kwargs):
    """
...
def dynamic_solutions(results,ms=20,edgewidth=2,log=True,metric='params',height=500,**kwargs):
...
def static_sensitivity(results,ms=20,edgewidth=0.1,ncols=3,log=False,**kwargs):
...
def dynamic_sensitivity(results,ms=20,edgewidth=2,ncols=3,log=False,height=200,**kwargs):
    """
    Parameters
    ----------
