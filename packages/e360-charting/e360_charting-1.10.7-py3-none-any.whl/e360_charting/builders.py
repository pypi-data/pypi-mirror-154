...
@dataclass
class BaseBoxVisualisation(BaseBuilder, BaseVisualisation):

    q1: Optional[list] = field(default=None)
    median: Optional[list] = field(default=None)
    q3: Optional[list] = field(default=None)
    lowerfence: Optional[list] = field(default=None)
    upperfence: Optional[list] = field(default=None)
    mean: Optional[list] = field(default=None)
    sd: Optional[list] = field(default=None)
    notchspan: Optional[list] = field(default=None)
    ...
    def setup_chart(self, style_class=None):
        style_class = style_class or self
        box_fields = style_class.chart_dict()
        for k, v in self.__dataclass_fields__.items():
            if k in ChartModel.__dataclass_fields__.keys():
                box_fields[k] = getattr(self, k)
        chart = ChartModel(**box_fields, id=f'chart{len(self.report.charts) + 1}')
