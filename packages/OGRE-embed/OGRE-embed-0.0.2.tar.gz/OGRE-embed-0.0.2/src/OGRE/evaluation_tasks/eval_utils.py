...
def calculate_static_embeddings(datasets_path, embeddings_path, dict_dataset, methods, initial_methods, params_dict,
    ...
    :param embeddings_path: Path to where the embeddings need to be saved. Notice state-of-the-art methods are saved in
                            a different path.
...
def export_time(z, name, save):
    ...
    csv_file = os.path.join(save, f"{name} times_1.csv")
    keys = list(z.keys())
    for key in keys:
        if " + " in key:
            se = z[key]
...
def export_results_lp_nc_all(n, save, dict_all_embeddings, dict_mission, our_initial, name, mission):
    ...
    csv_file = os.path.join(save, f"{name} {mission}.csv")
    try:
        with open(csv_file, 'w') as csvfile:
            writer = csv.DictWriter(csvfile, fieldnames=csv_columns)
            writer.writeheader()
