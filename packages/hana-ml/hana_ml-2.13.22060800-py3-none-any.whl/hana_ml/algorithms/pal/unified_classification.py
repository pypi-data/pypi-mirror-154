...
class UnifiedClassification(PALBase, _UnifiedClassificationReportBuilder, AMDPHelper):#pylint: disable=too-many-instance-attributes
    ...
    **kwargs : keyword arguments
    ...
    __fit_add_dict = {
        ...
        'SVM' : {}}
    __fit_param_dict = {
        'partition_method' : ('PARTITION_METHOD', int, {'no' : 0, 'user_defined' : 1, 'stratified' : 2}),
    ...
    def __init__(self,
        ...
        self.pal_funcname = 'PAL_UNIFIED_CLASSIFICATION'
    ...
    def enable_mlflow_autologging(self, schema=None, meta=None):
        """
        ...
        ----------
        schema : str, optional
            Define the model stroage schema for mlflow autologging.
    ...
    @mlflow_autologging(logtype='unified_api_fit')
    @trace_sql
    def fit(self,#pylint: disable=too-many-branches, too-many-statements
        ...
        strategy_by_col : ListOfTuples, optional
            ...
            .. note::
                ...
                The following parameters all have pre-fix 'als\_',
                and are invoked only when 'als' is selected as a `valid` imputation strategy
                in either ``strategy`` or ``strategy_by_col``.
        ...
        **kwargs : keyword arguments
        ...
        if self.massive is True:
            ...
            try:
                if check_pal_function_exist(conn, '%UNIFIED_MASSIVE%', like=True):
    ...
    def predict(self,
        ...
        group_params : dict, optional
            If massive mode is activated (``massive`` is True), input data for classification shall be divided into different
            groups with different classification parameters applied. This parameter specifies the parameter
            values of the chosen classification algorithm ``func`` in predict() w.r.t. different groups in a dict format,
            where keys corresponding to group_key while values should be a dict for classification algorithm
            parameter value assignments.
        ...
        if self.massive is True:
            ...
            if not err_msg.collect().empty:
                ...
                for i in range(1, row+1):
                    ...
                    warnings.warn(message=warn_msg)
            return conn.table(result_tbl), conn.table(errmsg_tbl)
        ...
        if self.massive is True:
            ...
            if not err_msg.collect().empty:
                ...
                for i in range(1, row+1):
                    ...
                    warnings.warn(message=warn_msg)
            return conn.table(result_tbl), conn.table(stats_tbl), conn.table(cm_tbl), conn.table(metrics_tbl), conn.table(errmsg_tbl)
