...
from typing import (Any, Generic, Iterable, Literal, Mapping, NoReturn,
                    Sequence, Set, SupportsIndex, TextIO, Type, TypeVar, Union,
                    overload)
...
class Schema(Generic[_RowSpec]):
    ...
    def from_rows(
        ...
        **kwargs,
        ...
        """
        df = pd.DataFrame.from_records([asdict(row) for row in rows], columns = list(self.dtypes.keys()))
