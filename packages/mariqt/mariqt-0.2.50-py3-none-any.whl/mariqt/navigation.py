...
def runAutomatedCuration(nav_data,params:dict,setNone=False,splineTo1sec=True):
	...
	if "dep_same_file_latlon" in params and params["dep_same_file_latlon"] == False:
		depthSeparate = True
	num_duplicates = removeDuplicatePositions(nav_data_curated,depthSeparate,setNone=setNone)
...
def removeDuplicatePositions(positions:miqtg.Positions,depthSeparate=False,setNone=False):
	""" 
	Deletes positions where the lat, lon (and dep if depthSeparate==False) values for consecutive time points are identical.
	...
	for utc in time_points:
		added = False
		if positions[utc].lat == pre_lat and positions[utc].lon == pre_lon and not depthSeparate and positions[utc].dep == pre_dep:
			if not setNone:
		...
		if setNone and positions[utc].hgt == pre_hgt:
...
def removeDuplicateAttitudes(attitudes:miqtg.Attitudes):
	""" 
	Deletes attitudes where the yaw, pitch and roll values for consecutive time points are identical.	
	"""
	...
	for utc in time_points:
		if attitudes[utc].yaw == pre_yaw and attitudes[utc].pitch == pre_pitch and attitudes[utc].roll == pre_roll:
...
def removeSpeedOutliers(positions:miqtg.Positions,max_lateral_speed:float=2.0,max_vertical_speed:float=3.0,max_time_gap:int=300,setNone=False):
	...
	for utc in time_points:
		...
		if lateral_speed > max_lateral_speed or vertical_speed > max_vertical_speed: # m/s
			if not setNone:
				positions.remUTC(utc)
			elif lateral_speed > max_lateral_speed:
				positions.positions[utc].lat = None
				positions.positions[utc].lon = None
			elif vertical_speed > max_vertical_speed:
				positions.positions[utc].dep = None
			time_removals += 1
			continue
...
def smoothPositionsSliding2dMedianLatLonGaussianDepth(positions:miqtg.Positions,median_half_width:int=60,gauss_half_width:int=60):
	...
	<..>_half_width paramaters in seconds. The whole windows are of size 2*gauss_half_width+1
	"""
	...
	for utc in time_points:
		...
		if not new_dep is None:
			...
			sd_dep = past_sd_dep['sd'] + (utc - past_sd_dep['utc']) / 1000 * 0.05 # uncert increases with 0.1 m/s
		else: 
			sd_dep = None
		if not new_lat is None and not new_lat is None:
			...
			sd_latLon = past_sd_latLon['sd'] + (utc - past_sd_latLon['utc']) / 1000 * 0.05 # uncert increases with 0.1 m/s
		else:
			sd_latLon = None
...
def getSmoothedPositionAtMaxDepth(positions,windowHalfSeconds:int,nrPointsBeforeAfter=[]):
		""" returns the smoothed position at max depth"""
		# find max depth
		maxDepth = 0
		maxDepthTime = 0
		for utc in positions:
			if positions[utc].dep > maxDepth:
				...
				maxDepthTime = utc
		print("max depth time: ",datetime.datetime.fromtimestamp(maxDepthTime / 1000,tz=datetime.timezone.utc))
