...
class Positions(NumDataTimeStamped):
	...
	def interpolateAtTime(self,utc:int,sorted_time_points:list = [] ,startIndex:int = 0):
		...
		if alpha == 0.0:
			new_lat = self.positions[pre_utc].lat
		else:
		...
		if alpha == 0.0:
			new_lon = self.positions[pre_utc].lon
		else:	
		...
		alpha,pre_utc,post_utc,past_nearest_index = self.getAlpha(sorted_time_points_none_cleanded,utc,past_nearest_index)
		if alpha == 0.0:
			new_dep = self.positions[pre_utc].dep
		else:
		...
		alpha,pre_utc,post_utc,past_nearest_index = self.getAlpha(sorted_time_points_none_cleanded,utc,past_nearest_index)
		if alpha == 0.0:
			new_hgt = self.positions[pre_utc].hgt
		else:
		...
		alpha,pre_utc,post_utc,past_nearest_index = self.getAlpha(sorted_time_points_none_cleanded,utc,past_nearest_index)
		if alpha == 0.0:
			new_uncert = self.positions[pre_utc].uncert
		else:
...
def findNearestNeighbors(values_sorted:list,value, startIndex):
	...
	if value < values_sorted[0] or value > values_sorted[-1]:
		raise IndexError("value " + str(value) + " out of range")
