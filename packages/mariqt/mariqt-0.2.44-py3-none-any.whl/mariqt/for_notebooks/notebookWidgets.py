...
class DoubleMinMaxConverter(ConverterBase):
    """ Set values outside min max range to default value """
    def __init__(self):
        ...
        self.defaultwidget = widgets.FloatText(description='Default')
        self._widget = widgets.VBox([self.minWidget,self.maxWidget,self.defaultwidget])

    def convert(self, value: str) -> str:
        if float(value) < self.maxWidget.value and float(value) > self.minWidget.value:
            return str(float(value))
        else:
            return None#str(self.defaultwidget.value)

    def setParams(self,params):
        self.minWidget.value = params[0]
        self.maxWidget.value = params[1]
        self.defaultwidget.value = params[2]

    def getParams(self):
        return [self.minWidget.value,self.maxWidget.value,self.defaultwidget.value]
...
class DoubleConstOffsetConverter(ConverterBase):
    ...
    def convert(self, value: str) -> str:
        return str(float(value) + self._widget.value)
