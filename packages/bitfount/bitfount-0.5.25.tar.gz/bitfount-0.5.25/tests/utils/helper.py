...
# Segmentation image dataset functions
...
def generate_random_data(
    height: int, width: int, count: int
...
def generate_img_and_mask(height: int, width: int) -> Tuple[np.ndarray, np.ndarray]:
    ...
    circle_location2 = get_random_location(*shape, zoom=1.5)
...
def get_random_location(
    height: int, width: int, zoom: float = 1.0
) -> Tuple[int, int, int]:
...
def add_circle(
    arr: np.ndarray, x: int, y: int, size: int, fill: bool = False
) -> np.ndarray:
...
def create_segmentation_dataset(
    seg_dir: Path, height: int = 100, weight: int = 100, count: int = 25
) -> pd.DataFrame:
    """Create a segmentation dataset."""
    input_images, target_masks = generate_random_data(height, weight, count=count)
    input_images_rgb = [x.astype(np.uint8) for x in input_images]
    target_masks_rgb = [masks_to_colorimg(x.astype(np.uint8)) for x in target_masks]
    img_names_list = []
    masks_names_list = []
    # Save images
