...
from bitfount.federated.task_requests import _TaskRequestMessage
...
class Pod:
    ...
    def __init__(
        ...
        self._pod_config = PodConfig(
            ...
            pod_details=pod_details_config,
            pod_name=name,
            schema=schema if isinstance(schema, Path) else None,
            access_manager=AccessManagerConfig(),
            message_service=ms_config if ms_config else MessageServiceConfig(),
            other_pods=approved_pods,
        )
        # Create datasource
    ...
    def to_yaml(self, output_file: Optional[str]) -> Optional[Dict]:
    ...
    async def _create_and_run_worker(self, message: _BitfountMessage) -> None:
        ...
        if _is_secure_share_task_request(task_request) and not unapproved_workers:
            ...
            worker_mailbox = _WorkerMailbox(**mailbox_kwargs)
