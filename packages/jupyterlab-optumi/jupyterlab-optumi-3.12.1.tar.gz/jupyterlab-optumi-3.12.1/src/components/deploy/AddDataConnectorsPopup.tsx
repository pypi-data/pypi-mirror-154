...
interface IProps {
    ...
    onOpen?: () => void
    onClose?: () => void
    onDataConnectorsAdded: (paths: IntegrationMetadata[]) => void
...
export class AddDataConnectorsPopup extends React.Component<IProps, IState> {
    ...
    private getSelectedDataConnectors: () => IntegrationMetadata[] = () => []
        ...
        return (
            <DIV sx={Object.assign({display: 'inline-flex', width: '50%'}, this.props.sx)} >
				...
				>
                    ...
                    <ShadowedDivider />
                    <IntegrationBrowser sx={{flexGrow: 1, height: '100%', maxHeight: 'calc(100% - 60px - 60px - 2px)'}} type={IntegrationType.DATA_CONNECTOR} onAdd={this.handleAdd} getSelected={(getSelectedDataConnectors: () => IntegrationMetadata[]) => this.getSelectedDataConnectors = getSelectedDataConnectors} />
                    <Divider />
                    <DIV sx={{display: 'inline-flex', margin: '6px 100px', height: '60px'}}>
                        ...
                        <Button
                            ...
                            color='primary'
                            onClick={() => {
                                Global.followLink(Global.Target.SettingsPopup.ConnectorsTab)
                                this.safeSetState({open: false})
                            }}
                            endIcon={<OpenInNew />}
                        >
