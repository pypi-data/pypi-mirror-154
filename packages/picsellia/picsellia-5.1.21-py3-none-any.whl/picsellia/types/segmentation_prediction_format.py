...
class SegmentationPredictionFormat(PredictionFormat):
    ...
    def __init__(self, label_ids : List[int], boxes : List[List[int]], masks: List[List[int]], detection_scores : List[float]):
    ...
    def check_validity(self) -> bool:
        ...
        assert isinstance(self.masks, List[List[int]]), "Scores shall be a List of float values."
        ...
        for mask in self.masks:
            assert len(mask) > 0, "One mask is empty"
